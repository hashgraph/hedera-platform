...
public class PrometheusEndpoint implements AutoCloseableNonThrowing {
    ...
    private static final String TIME_METRIC_KEY =
            DefaultMetrics.calculateMetricKey(Metrics.INFO_CATEGORY, "time");
    ...
    public PrometheusEndpoint(final HttpServer httpServer, final CollectorRegistry registry)
            throws IOException {
     ...
     *
     * @param notification the {@link MetricsEvent}
     */
    public void handleMetricsChange(final MetricsEvent notification) {
        ...
        if (TIME_METRIC_KEY.equals(metricKey)) {
            // filter out the time metric, because Prometheus has its own mechanism to store the
            // timestamp,
    ...
    /**
     * This method handles new snapshots. It should only be called by the {@link
     * com.swirlds.common.notification.NotificationEngine}.
     *
     * @param notification the {@link SnapshotEvent}
     */
