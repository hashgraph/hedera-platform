 ...
 *
 * @deprecated This class will be removed. Use one of the specialized {@link Metric}-implementations
 *     instead.
 */
...
public interface StatEntry extends Metric {
    ...
    /**
     * Getter for {@code resetStatsStringSupplier}, a lambda that returns the statistic string and
     * resets it at the same time
     *
     * @return the lambda
     */
    ...
    final class Config<T> extends MetricConfig<StatEntry, Config<T>> {
         ...
         * @return a reference to {@code this}
         */
        public StatEntry.Config<T> withResetStatsStringSupplier(
                final Supplier<T> resetStatsStringSupplier) {
            return new StatEntry.Config<>(
                    getCategory(),
                    getName(),
