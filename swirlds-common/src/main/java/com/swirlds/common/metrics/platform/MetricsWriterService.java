 ...
 *
 * <p>This class contains only the general functionality, writing the actual files is left to the
 * different implementations of {@link MetricWriter}.
 *
 * <p>The writers are created and configured according to their settings, which are described in the
 * javadocs of each class respectively.
 *
 * <p>This class creates an {@link java.util.concurrent.ExecutorService} that triggers all writers
 * in regular intervals. The frequency of these write operations can be configured with {@link
...
public class MetricsWriterService {
     ...
     * @param selfId the {@link NodeId} of the current node
     */
    public MetricsWriterService(
            final Metrics metrics, final NodeId selfId, final ScheduledExecutorService executor) {
    ...
    public void start() {
        if (future != null) {
            throw new IllegalStateException(
                    "The service was already " + (isRunning() ? "started" : "shutdown"));
        }
     ...
     *
     * <p>This method waits at most the time configured in {@link SettingsCommon#csvWriteFrequency},
     * which should be the maximum all writes require.
     *
     * <p>Calling this method after the service was already shutdown is possible and should have no
     * effect.
     *
     * @return {@code true}, if the service was shutdown successfully, {@code false} if it ran into
     *     a timeout
     * @throws IllegalStateException if the service was not started
     * @throws InterruptedException if the method was interrupted while waiting for the shutdown
     */
