...
public class PlatformStatEntry extends PlatformMetric implements StatEntry {
    ...
    public PlatformStatEntry(final StatEntry.Config<?> config) {
        super(config);
    ...
    public List<SnapshotValue> takeSnapshot() {
        if (buffered == null) {
            return List.of(new SnapshotValue(VALUE, resetStatsStringSupplier.get()));
        }
        ...
        return List.of(
                new SnapshotValue(VALUE, value),
                new SnapshotValue(MAX, max),
                new SnapshotValue(MIN, min),
                new SnapshotValue(STD_DEV, stdDev));
    }
