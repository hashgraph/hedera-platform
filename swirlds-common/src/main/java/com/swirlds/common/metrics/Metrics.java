 ...
 * <p>
 * The {@code Metrics} class provides functionality to add and delete metrics. There are also several
...
public class Metrics {
	 ...
	 *
	 * @param executor
	 * 		the {@link ScheduledExecutorService} that will be used to run updaters once per second
	 * @throws IllegalArgumentException
	 * 		if one of the parameters is {@code null}
	 *        {@code executor} may be {@code null}, if regular updates are disabled
	 */
	public Metrics(final ScheduledExecutorService executor, final MetricsFactory factory) {
		...
		this.executor = SettingsCommon.metricsUpdatePeriodMillis > 0 ?
				CommonUtils.throwArgNull(executor, "executor") : null;
	}
	 ...
	 * @param name
	 * 		the name of the wanten category
	...
	public <T extends Metric> T getOrCreate(final MetricConfig<T, ?> config) {
		...
		final Metric metric = metricMap.computeIfAbsent(calculateMetricKey(config), key -> config.create(factory));
	...
	private void runUpdaters() {
		try {
			...
			exceptionRateLimiter.handle(e,
					error -> LOG.error(EXCEPTION.getMarker(), "Exception while updating metrics.", error));
		}
	}
