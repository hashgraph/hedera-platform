 ...
 * <p>
 * This class is abstract and contains only common parameters. If you want to define the configuration for a
 * specific {@code Metric}, there are special purpose configuration objects (e.g. {@link Counter.Config}).
 * <p>
 ...
 *
 * @param <T>
 * 		the {@code Class} for which the configuration is
 */
@SuppressWarnings("removal")
public abstract sealed class MetricConfig<T extends Metric, C extends MetricConfig<T, C>>
		permits Counter.Config, DoubleAccumulator.Config, DoubleGauge.Config, DurationGauge.Config, FunctionGauge.Config,
		IntegerAccumulator.Config, IntegerGauge.Config, IntegerPairAccumulator.Config, LongAccumulator.Config,
		LongGauge.Config, RunningAverageMetric.Config, SpeedometerMetric.Config, StatEntry.Config {
	...
	MetricConfig(
		...
		if (description.length() > MAX_DESCRIPTION_LENGTH) {
			throw new IllegalArgumentException("Description must not be longer than " + MAX_DESCRIPTION_LENGTH
