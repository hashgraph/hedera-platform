...
/**
 * Service that creates new snapshots in regular intervals and sends a {@link SnapshotEvent}. This
 * is in particular used to write metrics-data to different file formats.
 *
 ...
 *
 * <p>This class uses a provided {@link java.util.concurrent.ExecutorService} that triggers the
 * snapshot creation. The frequency of these write operations can be configured with {@link
...
public class SnapshotService implements Startable {
    ...
    private final Queue<Consumer<? super SnapshotEvent>> subscribers =
     ...
     * @throws IllegalArgumentException if any of the arguments is {@code null} or {@code
     *     globalMetrics} is not global
     */
    ...
    public SnapshotService(
        ...
        if (!globalMetrics.isGlobalMetrics()) {
            throw new IllegalArgumentException(
                    "Trying to create SnapshotService with non-global Metrics");
        }
    ...
    /**
     * Add a platform-specific {@link com.swirlds.common.metrics.Metrics} to the {@code
     * SnapshotService}
     *
     * @param platformMetrics the {@link DefaultMetric} to add
     * @throws IllegalArgumentException if {@code platformMetrics} is {@code null} or not
     *     platform-specific
     */
    ...
    /**
     * Remove a platform-specific {@link com.swirlds.common.metrics.Metrics} from the {@code
     * SnapshotService}
     *
    ...
    private void mainLoop() {
        ...
        final Map<String, Snapshot> globalSnapshots =
                globalMetrics.getAll().stream()
                        .map(DefaultMetric.class::cast)
                        .map(Snapshot::of)
                        .collect(
                                Collectors.toMap(
                                        snapshot -> calculateMetricKey(snapshot.metric()),
                                        Function.identity()));
        ...
        for (final DefaultMetrics platformMetrics : platformMetricsList) {
            final List<Snapshot> platformSnapshots =
                    platformMetrics.getAll().stream()
                            .map(
                                    metric ->
                                            globalSnapshots.getOrDefault(
                                                    calculateMetricKey(metric),
                                                    Snapshot.of((DefaultMetric) metric)))
                            .toList();

            logger.trace(
                    () ->
                            String.format(
                                    "Created %d snapshots for node %s",
                                    platformSnapshots.size(), platformMetrics.getNodeId()));

            final SnapshotEvent platformEvent =
                    new SnapshotEvent(platformMetrics.getNodeId(), platformSnapshots);
        ...
        try {
            executor.schedule(
                    this::mainLoop, Math.max(0L, delayNanos - delta), TimeUnit.NANOSECONDS);
