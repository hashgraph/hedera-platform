...
public class DefaultMetrics implements Metrics {
    ...
    // A map of all global metrics in the system (only used if this instance maintains platform
    // metrics
    ...
    // Helper class that maintains a list of all metrics, which need to be updated in regular
    // intervals
     ...
     *
     * @param selfId the {@link NodeId} of the platform, {@code null} if these are the global
     *     metrics
     * @param metricKeyRegistry the {@link MetricKeyRegistry} that ensures no conflicting metrics
     *     are registered
     ...
     *
     * <p>A new subscriber will immediately receive ADD-events for all metrics, that are already
     * registered.
     *
     * <p>If the list of metrics is modified while a new subscriber is added, it may happen, that
     * the new subscriber gets two ADD-events for the same {@code Metric} or a REMOVE-event for a
     * {@code Metric} that was not added before.
     *
    ...
    public Runnable subscribe(final Consumer<? super MetricsEvent> subscriber) {
        final Supplier<Stream<MetricsEvent>> previousEventsSupplier =
                () ->
                        metricMap.values().stream()
                                .map(metric -> new MetricsEvent(ADDED, selfId, metric));
        ...
        if (metric == null) {
            ...
            if (!metricKeyRegistry.register(selfId, key, config.getResultClass())) {
                throw new IllegalStateException(
                        String.format(
                                "A different metric with the category '%s' and name '%s' was"
                                        + " already registered",
    ...
    private void throwIfGlobal(final String metricKey) {
        if (globalMetricKeys.containsKey(metricKey)) {
            throw new IllegalArgumentException(
                    String.format(
                            "Not possible to remove the global Metric (%s) from a non-global"
                                    + " Metrics",
                            metricKey));
        }
    }
