...
public class ThrottleStreamExtension implements InputStreamExtension, OutputStreamExtension {
     ...
     *
     * @param bytesPerSecond the maximum allowable throughput
     * @param timeIncrement the time increment. Smaller time increments result in smoother
     *     throttling at the cost of additional overhead.
     */
    public ThrottleStreamExtension(final long bytesPerSecond, final Duration timeIncrement) {
        this.timeIncrement = timeIncrement;
        this.bytesPerIncrement =
                (long) (bytesPerSecond * (timeIncrement.toNanos() * NANOSECONDS_TO_SECONDS));
        this.currentIncrementStart = Instant.now();
        this.bytesInCurrentIncrement = bytesPerIncrement;
    }

    /**
     * Check the current capacity of the stream, and return the number of bytes that are permitted
     * to pass at this moment. If the current capacity is 0 then this method will block until more
