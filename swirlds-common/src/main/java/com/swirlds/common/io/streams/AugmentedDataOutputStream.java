...
/**
 * This data output stream provides additional functionality for serializing various basic data
 * structures.
 */
public class AugmentedDataOutputStream extends DataOutputStream {
     ...
     * @param writeChecksum whether to read the checksum or not
    ...
    /**
     * Writes a byte array to the stream. Can be null. Same as {@link #writeByteArray(byte[],
     * boolean)} with {@code writeChecksum} set to false
     *
     * @param data the array to write
     * @throws IOException thrown if any IO problems occur
     */
     ...
     * writes it to the output stream encoded in the Swirlds default charset (UTF8). This is
     * important for having a consistent method of converting Strings to bytes that will guarantee
     * that two identical strings will have an identical byte representation
     *
