...
/**
 * This stream extension causes the stream to expire after a certain duration. If bytes are read
 * from the stream after the expiration period then the stream throws an IOException. Note that this
 * extension will not interrupt extremely long read/write operations if the operation starts before
 * the stream expires and ends after the stream expires.
 */
public class ExpiringStreamExtension extends AbstractStreamExtension {
     ...
     *
     * @param period the length of time until the stream expires
     */
     ...
     *
     * @param period the length of time until the stream expires
     * @param bytesPerSample check the time after reading this many bytes. A large number may cause
     *     the stream to be usable beyond its expiration period, while a small number will have
     *     higher overhead.
     */
     ...
     *
     * @param length the number of bytes passing through the stream
     * @throws IOException if the stream has expired
     */
