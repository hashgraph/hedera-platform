...
public class MerkleDataInputStream extends SerializableDataInputStream {
     ...
     *
     * @param directory the directory from which data is being read
     * @param node the leaf node to be read
     * @param version version of this leaf
     */
    private void finishReadingLeaf(final Path directory, final MerkleLeaf node, final int version)
            throws IOException {
     ...
     *
     * @param directory the directory from which data is being read
     * @param node the internal node to be read
     * @param version version of this internal node
     */
    private void finishReadingInternal(
            final Path directory, final MerkleInternal node, final int version) throws IOException {
        ...
        if (node instanceof ExternalSelfSerializable) {
            ...
            if (childCount < node.getMinimumChildCount()
                    || childCount > node.getMaximumChildCount()) {
            ...
            if (childCount > 0) {
                internalNodes.add(
                        new PartiallyConstructedMerkleInternal(node, version, childCount));
            }
        }
    }
     ...
     *
     * @param directory the directory from which data is being read
     * @param deserializedVersions versions of deserialized nodes
     */
    ...
    private static void validateDirectory(final Path directory) {
        ...
        if (!Files.isReadable(directory)) {
            throw new IllegalArgumentException(
                    "invalid read permissions for directory " + directory);
        }
    }
     ...
     *
     * @param directory the directory from which data is being read
     * @param maxNumberOfNodes maximum number of nodes to read
     * @param <T> Type of the node
     * @return the merkle tree read from the stream
     * @throws IOException thrown when version or the options or nodes count are invalid
     */
    public <T extends MerkleNode> T readMerkleTree(final Path directory, final int maxNumberOfNodes)
        ...
        if (merkleVersion == MerkleSerializationProtocol.VERSION_1_ORIGINAL) {
            throw new MerkleSerializationException(
                    "Unhandled merkle serialization version " + merkleVersion);
