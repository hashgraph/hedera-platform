...
/**
 * A stream that performs the same role as a {@link MerkleDataInputStream} but with extra debug
 * functionality. This debuggability adds overhead, so use of this stream should be limited to test
 * environments or production environments where there is a known serialization problem (heaven
 * forbid).
 */
public class DebuggableMerkleDataInputStream extends MerkleDataInputStream {
     ...
     *
     * <ul>
     *   <li>index 0: a frame from inside the java thread API
     *   <li>index 1: a frame inside {@link #startOperation(SerializationOperation)}
     *   <li>index 2: the frame where {@link #startOperation(SerializationOperation)} is called
     *   <li>index 3: the original caller (this is the one we are interested in)
     * </ul>
     */
    ...
    /** {@inheritDoc} */
    @Override
    public byte[] readByteArray(final int maxLength, final boolean readChecksum)
            throws IOException {
    ...
    /** {@inheritDoc} */
    @Override
    public String[] readStringArray(final int maxLength, final int maxStringLength)
            throws IOException {
    ...
    /** {@inheritDoc} */
    @Override
    public List<String> readStringList(final int maxLength, final int maxStringLength)
            throws IOException {
        startOperation(SerializationOperation.READ_STRING_LIST);
        try {
    ...
    protected void readNextNode(
            ...
            final Map<Long /* class ID */, Integer /* version */> deserializedVersions)
            throws IOException {
    ...
    public <T extends SelfSerializable> void readSerializableIterableWithSize(
        ...
        try {
            super.readSerializableIterableWithSize(
                    size, readClassId, serializableConstructor, callback);
        } finally {
            finishOperation();
        }
    }
    ...
    /** {@inheritDoc} */
    @Override
    public <T extends SelfSerializable> List<T> readSerializableList(final int maxListSize)
            throws IOException {
        startOperation(SerializationOperation.READ_SERIALIZABLE_LIST);
        try {
    ...
    public <T extends SelfSerializable> T[] readSerializableArray(
        ...
        try {
            return super.readSerializableArray(
                    arrayConstructor, maxListSize, readClassId, serializableConstructor);
        } finally {
            finishOperation();
        }
    }
