...
/**
 * A drop-in replacement for {@link DataInputStream}, which handles SerializableDet classes
 * specially. It is designed for use with the SerializableDet interface, and its use is described
 * there.
 */
public class SerializableDataInputStream extends AugmentedDataInputStream {
    ...
    /**
     * Reads the protocol version written by {@link
     * SerializableDataOutputStream#writeProtocolVersion()} and saves it internally. From this point
    ...
    /**
     * Reads a {@link SerializableDet} from a stream and returns it. The instance will be created
     * using the {@link ConstructableRegistry}. The instance must have previously been written using
     * {@link SerializableDataOutputStream#writeSerializable(SelfSerializable, boolean)}
     * (SerializableDet, boolean)} with {@code writeClassId} set to true, otherwise we cannot know
     * what the class written is.
     *
    ...
    public <T extends SelfSerializable> T readSerializable(
            final boolean readClassId, final Supplier<T> serializableConstructor)
            throws IOException {
    ...
    /**
     * Called when the class ID of an object becomes known. This method is a hook for the debug
     * stream.
     *
    ...
    /**
     * Same as {@link #readSerializable(boolean, Supplier)} except that the constructor takes a
     * class ID
     */
    private <T extends SelfSerializable> T readSerializable(
            final boolean readClassId, final Function<Long, T> serializableConstructor)
            throws IOException {
    ...
    public <T extends SelfSerializable> void readSerializableIterableWithSize(
        ...
        checkLengthLimit(size, maxSize);
        readSerializableIterableWithSizeInternal(
                size, readClassId, id -> serializableConstructor.get(), callback);
    }
    ...
    /**
     * Helper method for {@link #readSerializableIterableWithSize(int, Consumer)}. Protected instead
     * of private to allow debug framework to intercept this method.
     *
     * @param allSameClass true if the elements all have the same class
     * @param readClassId if true then the class ID needs to be read, ignored if allSameClass is
     *     true
     ...
     * @throws IOException thrown if any IO problems occur
     */
    public <T extends SelfSerializable> List<T> readSerializableList(int maxListSize)
            throws IOException {
        return readSerializableList(
                maxListSize, true, SerializableDataInputStream::registryConstructor);
    }
    ...
    private <T extends SelfSerializable> List<T> readSerializableList(
        ...
        readSerializableIterableWithSizeInternal(
                length, readClassId, serializableConstructor, list::add);
        return list;
    }
    ...
    public <T extends SelfSerializable> T[] readSerializableArray(
        ...
        final List<T> list =
                readSerializableList(
                        maxListSize, readClassId, SerializableDataInputStream::registryConstructor);
        if (list == null) {
            return null;
        }
    ...
    public <T extends SelfSerializable> T[] readSerializableArray(
        ...
        final List<T> list =
                readSerializableList(maxListSize, readClassId, id -> serializableConstructor.get());
        if (list == null) {
            return null;
        }
        return list.toArray(arrayConstructor.apply(list.size()));
