...
public class InvalidVersionException extends IllegalArgumentException {
    public InvalidVersionException(final int expectedVersion, final int version) {
        super(
                String.format(
                        "Illegal version %d was read from the stream. Expected %d",
                        version, expectedVersion));
    }

    public InvalidVersionException(
            final int minimumVersion, final int maximumVersion, final int version) {
        super(
                String.format(
                        "Illegal version %d was read from the stream. Expected version in the range"
                                + " %d - %d",
    ...
    public InvalidVersionException(final int version, SerializableDet object) {
        super(
                String.format(
                        "Illegal version %d was read from the stream for %s (class ID %d(0x%08X))."
                                + " Expected version in the range %d - %d",
                        version,
                        object.getClass(),
                        object.getClassId(),
                        object.getClassId(),
