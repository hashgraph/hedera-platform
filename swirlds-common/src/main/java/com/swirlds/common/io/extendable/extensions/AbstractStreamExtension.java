...
/**
 * Implements boilerplate that allows a simple extension definition be used for both input and
 * output streams. Although compatible with multiple stream types, each instance of this class
 * should only be used with a single stream.
 */
public abstract class AbstractStreamExtension
        implements InputStreamExtension, OutputStreamExtension {
     ...
     *
     * @param aByte the byte that has passed through the stream
     */
     ...
     * @param offset the start offset in array bytes at which the data is
