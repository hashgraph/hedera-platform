...
public final class TemporaryFileBuilder {
    ...
    public static synchronized Path getTemporaryFileLocation() throws IOException {
        if (temporaryFileLocation == null) {
            final TemporaryFileConfig config =
                    ConfigurationHolder.getConfigData(TemporaryFileConfig.class);
            final StateConfig stateConfig = ConfigurationHolder.getConfigData(StateConfig.class);
            overrideTemporaryFileLocation(
                    getAbsolutePath(config.getTemporaryFilePath(stateConfig)));
        }
    ...
    /**
     * Update the location where temporary files are written. This method may be useful when writing
     * unit tests.
     *
     * <p>WARNING! Calling this method will cause any files in the original location to be deleted.
     * It's ok to set the temporary file location to something like "/myTemporaryFiles" or
     * "~/myTemporaryFiles". If it's set to something like "/" or "~/", EVERYTHING IN THOSE
     * DIRECTORIES WILL BE DELETED!
     *
     * @param newTemporaryFileLocation the new location where temporary files will be written
     */
    public static synchronized void overrideTemporaryFileLocation(
            final Path newTemporaryFileLocation) throws IOException {
    ...
    /**
     * Return a temporary file. File will not exist when this method returns. File is guaranteed to
     * have a unique name. File will not be automatically deleted until this JVM is restarted.
     *
     ...
     * have a unique name. File will not be automatically deleted until this JVM is restarted.
     *
     * @param postfix an optional postfix, helps to make temporary file directory easier to
     *     understand if a human ever looks at it directly. Ignored if null.
     * @return a new temporary file
     */
    ...
    /**
     * Return a temporary directory. Directory will exist when this method returns. Directory is
     * guaranteed to have a unique name. Directory will not be automatically deleted until this JVM
     * is restarted.
     *
     ...
     * @return a new temporary directory
     */
    public static synchronized Path buildTemporaryDirectory(final String postfix)
            throws IOException {
