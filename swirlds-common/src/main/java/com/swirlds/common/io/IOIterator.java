...
public interface IOIterator<T> extends AutoCloseableNonThrowing {
    ...
    /**
     * Check if there is another object available. If this method returns true then {@link #next()}
     * will return a valid object without throwing. If this method returns false then {@link
     * #next()} will throw a {@link NoSuchElementException}.
     *
     * @return true if there is another object available
     * @throws IOException if there is a problem reading the next object
     */
     ...
     * @throws IOException if there is a problem reading the next object
     * @throws NoSuchElementException if {@link #hasNext()} returns false and this method is called
     */
     ...
     *
     * @param action the action that will operate on each object still in the iterator
     * @throws IOException if there is a problem reading an object
     */
    ...
    /**
     * Transform this iterator into an iterator that returns a different type. Iterating over the
     * returned iterator causes the parent iterator to also advance.
     *
     * @param converter a function that converts from the old type into the new type.
     * @param <U> the type returned by the new iterator
