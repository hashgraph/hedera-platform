...
/**
 * This extension will trigger an exception on a timeout if a single operation takes too long on a
 * stream. If a timeout is triggered then the stream is closed.
 *
 * <p>This extension will not trigger an exception if no data is passing through the stream, and no
 * data has been requested to pass through the stream.
 *
 * <p>Timing is approximate in the positive direction. That is, things make take longer than the
 * configured duration to time out, but will never time out in less than the configured duration.
 */
public class TimeoutStreamExtension implements InputStreamExtension, OutputStreamExtension {
    ...
    /**
     * Construct and register a timeout stream extension. This is handled by a static method due to
     * registration requirements with the timeout stream manager.
     *
     * @param timeoutPeriod the time required
     ...
     *
     * @param timeoutPeriod the maximum period of time permitted for a single operation
     */
    ...
    /**
     * Called periodically to check if the timeout has expired. Closes the stream once the timeout
     * has occurred.
     *
    ...
    /** {@inheritDoc} */
    @Override
    public int readNBytes(final byte[] bytes, final int offset, final int length)
            throws IOException {
        final long op = operationStartNumber.incrementAndGet();
