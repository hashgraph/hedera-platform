...
/**
 * A drop-in replacement for {@link DataOutputStream}, which handles SerializableDet classes
...
public class SerializableDataOutputStream extends AugmentedDataOutputStream {
    ...
    /**
     * Write the serialization protocol version number to the stream. Should be used when
     * serializing to a file that can be read by future versions.
     *
    ...
    /**
     * Writes a {@link SelfSerializable} object to a stream. If the class is known at the time of
     * deserialization, the the {@code writeClassId} param can be set to false. If the class might
     * be unknown when deserializing, then the {@code writeClassId} must be written.
     *
     * @param serializable the object to serialize
     * @param writeClassId whether to write the class ID or not
     * @throws IOException thrown if any IO problems occur
     */
    public void writeSerializable(SelfSerializable serializable, boolean writeClassId)
            throws IOException {
        writeSerializable(serializable, writeClassId, serializable);
    }

    public <E extends Enum<E>> void writeOptionalSerializable(
            OptionalSelfSerializable<E> serializable, boolean writeClassId, E option)
            throws IOException {
        writeSerializable(serializable, writeClassId, out -> serializable.serialize(out, option));
    }

    /**
     * Writes a list of objects returned by an {@link Iterator} when the size in known ahead of
     * time. If the class is known at the time of deserialization, the {@code writeClassId} param
     * can be set to false. If the class might be unknown when deserializing, then the {@code
     * writeClassId} must be written.
     *
    ...
    public <T extends SelfSerializable> void writeSerializableIterableWithSize(
            Iterator<T> iterator, int size, boolean writeClassId, boolean allSameClass)
            throws IOException {
        this.writeInt(size);
        if (size == 0) {
            return;
     ...
     * @param writeClassId set to true if the classID should be written. This can be false if the
     *     class is known when de-serializing
     * @param allSameClass should be set to true if all the objects in the list are the same class
    ...
    protected void writeClassIdVersion(
            final SerializableDet serializable, final boolean writeClassId) throws IOException {
