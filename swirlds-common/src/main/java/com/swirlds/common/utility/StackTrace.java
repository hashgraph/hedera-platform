...
public record StackTrace(StackTraceElement[] frames) {
     ...
     * @param ignoredFrames the number of frames to remove from the beginning
     * @return a new array that does not contain the ignored frames
     */
    private static StackTraceElement[] ignoreFrames(
            final StackTraceElement[] frames, final int ignoredFrames) {
        if (frames.length <= ignoredFrames) {
            return new StackTraceElement[0];
        }
        final StackTraceElement[] reducedFrames =
                new StackTraceElement[frames.length - ignoredFrames];
