 ...
 *
 * <p>All constants are in the form "UNIT1_TO_UNIT2" where you multiply by the constant to go from
 * UNIT1 to UNIT2, or divide by the constant to go from UNIT2 to UNIT1.
 */
public final class Units {
    ...
    /**
     * Multiply by these values for converting KB to bytes ("kilo" meaning 10^3, "kibi" meaning
     * 2^10).
     */
    ...
    /**
     * Multiply by these values for converting MB to bytes ("mega" meaning 10^6, "mebi" meaning
     * 2^20).
     */
    ...
    /**
     * Multiply by these values for converting GB to bytes ("giga" meaning 10^9, "gibi" meaning
     * 2^30). Unlike KIBI/KILO and MEBI/MEGA, these are defined as longs, because Integer.MAX_VALUE
     * is one less than 2*GIBIBYTES_TO_BYTES, so multiplying by anything larger than 1 is very
     * likely to require a long to hold the result. (2 GIGABYTES can fit in an int, just not 2
     * GIBIBYES, but that's entirely a different matter.)
     */
