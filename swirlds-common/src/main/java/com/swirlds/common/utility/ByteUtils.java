...
public final class ByteUtils {
    ...
    /**
     * Return a long derived from the 8 bytes data[position]...data[position+7], big endian. If the
     * byte array is not long enough, zeros are substituted for the missing bytes.
     *
    ...
    public static long byteArrayToLong(final byte[] data, final int position) {
        if (data.length > position + 8) {
            // Hard coded constants are used instead of a for loop to reduce the arithmetic required
            // at runtime
    ...
    public static void longToByteArray(final long value, final byte[] data, final int position) {
        // Hard coded constants are used instead of a for loop to reduce the arithmetic required at
        // runtime
    ...
    /**
     * Write the short value into 2 bytes of the array, starting at a given position pos, big
     * endian.
     *
     * @param value the value to write
     * @param data the array to write to
