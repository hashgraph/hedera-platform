...
public class CommonUtils {
    ...
    /**
     * Normalizes the string in accordance with the Swirlds default normalization method (NFD) and
     * returns the bytes of that normalized String encoded in the Swirlds default charset (UTF8).
     * This is important for having a consistent method of converting Strings to bytes that will
     * guarantee that two identical strings will have an identical byte representation
     *
     ...
     * @param velocity the "velocity" (volume, or speed with which the note is played). 0 is silent,
     *     127 is max.
    ...
    /**
     * This is equivalent to System.out.println(), but is not used for debugging; it is used for
     * production code for communicating to the user. Centralizing it here makes it easier to search
     * for debug prints that might have slipped through before a release.
     *
    ...
    /**
     * This is equivalent to sending text to doing both Utilities.tellUserConsole() and writing to a
     * popup window. It is not used for debugging; it is used for production code for communicating
     * to the user.
     *
    ...
    public static <T> T throwArgNull(final T arg, final String argName) {
        if (arg == null) {
            throw new IllegalArgumentException(
                    String.format("The supplied argument '%s' cannot be null!", argName));
        }
        return arg;
    }
    ...
    public static String throwArgBlank(final String arg, final String argName) {
        ...
        if (StringUtils.isBlank(arg)) {
            throw new IllegalArgumentException(
                    String.format("The supplied argument '%s' cannot be blank!", argName));
        }
        return arg;
    }
    ...
    public static void throwRangeInvalid(String name, int value, int minValue, int maxValue) {
        if (value < minValue || value > maxValue) {
            throw new IllegalArgumentException(
                    String.format(
                            "The argument '%s' should have a value between %d and %d! Value"
                                    + " provided is %d",
    ...
    /**
     * Given a name from the address book, return the corresponding alias to associate with
     * certificates in the trust store. This is found by lowercasing all the letters, removing
     * accents, and deleting every character other than letters and digits. A "letter" is anything
     * in the Unicode category "letter", which includes most alphabets, as well as ideographs such
     * as Chinese.
     *
     * <p>WARNING: Some versions of Java 8 have a terrible bug where even a single capital letter in
     * an alias will prevent SSL or TLS connections from working (even though those protocols don't
     * use the aliases). Although this ought to work fine with Chinese/Greek/Cyrillic characters, it
     * is safer to stick with only the 26 English letters.
     *
     * @param name a name from the address book
     * @return the corresponding alias
     */
    public static String nameToAlias(String name) {
        // Convert to lowercase. The ROOT locale should work with most non-english characters.
        // Though there
        // can be surprises. For example, in Turkey, the capital I would convert in a
        // Turkey-specific way to
        // a "lowercase I without a dot". But ROOT would simply convert it to a lowercase I.
        String alias = name.toLowerCase(Locale.ROOT);

        // Now find each character that is a single Unicode codepoint for an accented character, and
        // convert
        // it to an expanded form consisting of the unmodified letter followed
        // by all its modifiers. So if "à" was encoded as U+00E0, it will be converted to U+0061
        // U++U0300.
        // This is necessary because Unicode normally allows that character to be encoded either
        // way, and
        // they are normally treated as equivalent.
        alias = Normalizer.normalize(alias, Normalizer.Form.NFD);

        // Finally, delete the modifiers. So the expanded "à" (U+0061 U++U0300) will be converted to
        // "a"
        // (U+0061). Also delete all spaces, punctuation, special characters, etc. Leave only digits
        // and
        // unaccented letters. Specifically, leave only the 10 digits 0-9 and the characters that
        // have a
