...
public final class BinarySearch {
     ...
     *
     * @param minimumIndex the minimum index to search (inclusive)
     * @param maximumIndex the maximum index to search (exclusive)
     * @param compareToDesired the result of {@link Comparable#compareTo(Object)} of the object at
     *     the given index to the desired object
     * @return an index with a value that satisfies the comparison function, or if no such index can
     *     be found the highest index that causes the comparison function to return a negative value
     *     (i.e. the largest value that does exceed the requested value)
     * @throws java.util.NoSuchElementException if no index that causes the comparison function to
     *     return a negative value can be found (i.e. all of the values in the provided range are
     *     too large)
     */
    public static long search(
            final long minimumIndex,
            final long maximumIndex,
            final LongToIntFunction compareToDesired) {
     ...
     *     return a negative value can be found (i.e. all of the values in the provided range are
     *     too large)
     * @throws E if compareToDesired throws E
     */
    public static <E extends Exception> long throwingSearch(
            final long minimumIndex,
            final long maximumIndex,
            final ThrowingFunction<Long, Integer, E> compareToDesired)
            throws E {

        if (minimumIndex >= maximumIndex) {
            throw new IllegalArgumentException(
                    "maximum index must be strictly greater than the minimum index");
        }
        ...
        if (compareToDesired.apply(leftBoundary) > 0) {
            // There exists no index in the specified range that causes the comparison function to
            // return
            // a negative value.
