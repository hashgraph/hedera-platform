 ...
 *
 * @param <T> the type of constructor used by this registry
 */
public class GenericConstructorRegistry<T> implements ConstructorRegistry<T> {
    ...
    private final Map<Long, GenericClassConstructorPair<T>> constructors =
            new ConcurrentHashMap<>();

    /**
     * @param constructorType the type of constructor for this registry to use
     */
    public GenericConstructorRegistry(final Class<T> constructorType) {
        ...
        if (!constructorType.isInterface()) {
            throw new IllegalArgumentException(
                    String.format(
                            "The constructor type needs to be an interface, '%s' is not an"
                                    + " interface",
        ...
        if (!RuntimeConstructable.class.isAssignableFrom(constructorSignature.getReturnType())) {
            throw new IllegalArgumentException(
                    "The constructor return type must extend RuntimeConstructable");
        }
    }
     ...
     *
     * @param constructableClasses the classes to register
     * @param additionalClassloader a non-default classloader used
     * @throws ConstructableRegistryException if any class has an issue being registered
     */
    public void registerConstructables(
            final ConstructableClasses<?> constructableClasses,
        ...
        for (final Class<? extends RuntimeConstructable> constructable :
                constructableClasses.getClasses()) {
            registerConstructable(
                    constructable, createConstructorLambda(constructable, additionalClassloader));
        }
    }
    ...
    @Override
    public void registerConstructable(
            final Class<? extends RuntimeConstructable> aClass, final T constructor)
            throws ConstructableRegistryException {
        final GenericClassConstructorPair<T> pair =
                new GenericClassConstructorPair<>(aClass, constructor);
    ...
    private long getClassId(final GenericClassConstructorPair<T> pair)
            throws ConstructableRegistryException {
        // before the ConstructableClass annotation was introduced, the only to get the class ID was
        // to create an
        ...
        if (pair.constructor() instanceof NoArgsConstructor nac) {
            ...
            final ConstructableClass classIdAnnotation =
                    pair.constructable().getAnnotation(ConstructableClass.class);
    ...
    private T createConstructorLambda(
        ...
        if (!hasRequiredConstructor(constructable)) {
            throw new ConstructableRegistryException(
                    String.format(
                            "Cannot find appropriate constructor for class: %s",
                            constructable.getCanonicalName()));
        }
        final T constructor;
        try {
            ...
            final MethodHandle mh =
                    lookup.findConstructor(
                            constructable, MethodType.methodType(void.class, params));
    ...
    private boolean hasRequiredConstructor(
            final Class<? extends RuntimeConstructable> constructable) {
    ...
    private static Method getMethod(final Class<?> constructorType) {
        ...
        final Method[] methods =
                Arrays.stream(constructorType.getDeclaredMethods())
