...
public final class MerkleCopy {
    ...
    /**
     * Makes a fast copy if the provided node is a leaf, otherwise constructs a pseudo fast copy
     * using the constructable registry. This method will no longer be necessary once internal
     * merkle nodes become fast copyable.
     *
     * <p>This method does not initialize the copy of the node, external callers are responsible for
     * this process.
     */
    ...
    /**
     * Helper function for copyTreeToLocation. Adds all children of a merkle node to the queue. If
     * the parent is actually null or a leaf then take no action.
     *
     * @param oldParent the children of this node (if it has children) are added to the queue
     * @param copiedParent this is a copy of oldParent. All children will eventually be copied and
     *     added as children to this node.
     * @param queue the queue that holds nodes that still need to be copied
     */
     ...
     *
     * <p>This algorithm does not work on data structures that do not support being copied node-by
     * node (e.g. virtual data structures).
     *
     * <p>Warning: This method may leave behind a partially immutable tree in the original location.
     * It is the responsibility of the caller to appropriately clean up the original tree.
     *
     * @param parent the destination parent where that will hold the subree
     * @param index the destination index where the subtree will sit
     * @param child the subtree being moved
    ...
    public static <T extends MerkleNode> T copyTreeToLocation(
        ...
        // Hold reference to tree being copied. This prevents it from being released if we are
        // copying to
        ...
        // Since we have obtained new instances of internal nodes without calling copy on them we
    ...
    /**
     * Take the children from an internal node and make them also the children of another. Does not
     * fast copy anything, each child will have an additional direct parent after this method
     * returns. Assumes that two nodes are in the exact same position within the tree.
     *
     * <p>This method is safe to use on both mutable and immutable children.
     *
     * @param originalParent the node that is providing children
     * @param newParent the node that is getting the children
     */
    public static void adoptChildren(
            final MerkleInternal originalParent, final MerkleInternal newParent) {
