...
/**
 * Describes methods that implement a queue of nodes to be handled by the teacher during reconnect.
 * Nodes are placed into this queue when they are sent in a query, and handled when it is time to
 * send the lesson for the node.
 *
 * @param <T> a type that represents a merkle node in the view
 */
public interface TeacherHandleQueue<T> {
     ...
     *
     * <p>May not be called on all nodes. For the nodes it is called on, the order will be
     * consistent with a breadth first ordering.
     *
