 ...
 * <p>Data format:
 *
 * <p>Routes are encoded as an array of integers.
 *
 * <p>- If an integer in the array is non-zero positive then it represents a step in the route that
 * is greater or equal to 2. - If an integer in the array is negative then it contains one or more
 * binary steps (i.e. 0 or 1). - If an integer in the array is 0 then it contains no data and is not
 * permitted in a route.
 *
 * <p>The binary format of a negative integer is as follows:
 *
 * <p>1XXXXXX...10000 | | | | | | | |-- 0 or more 0s (does not encode any steps) | | | | | |-- A
 * single 1 (does not encode a step) | | | |-- A sequence of 0s or 1s representing binary steps |
...
public class BinaryMerkleRoute extends AbstractMerkleRoute {
     ...
     *
     * @param baseRoute the route to copy
     ...
     * @param index the index of the bit in concern
    ...
    /**
     * Get a new int by setting the bit at a particular index within an integer to be a particular
     * value
     *
     * @param data an int value
     * @param index the index of the bit in concern
    ...
    /**
     * Deep copy route data, expanding the copy if necessary to add the specified step. More
     * efficient than first copying and then expanding.
     *
     * @param routeData the route data to be deep copied
     * @param step the binary step that the resulting route must have the capacity to store
    ...
    /**
     * Count the number of integers that need to be added to the array in order to hold all of the
     * steps.
     *
     * @param routeData the route that will need to hold the steps
     * @param steps the steps that will be added
     * @return the number of integers that need to be added to the array to hold the required list
     *     of steps
     */
     ...
     *     of steps
     */
    private static int getExpansionRequiredToHoldSteps(
            final int[] routeData, final List<Integer> steps) {
     ...
     *
     * @param routeData the route that will need to hold the steps
     * @param steps an iterator for the steps that will be added
     * @return the number of integers that need to be added to the array to hold the required list
     *     of steps
     */
    private static int getExpansionRequiredToHoldSteps(
            final int[] routeData, final Iterator<Integer> steps) {
        ...
        while (steps.hasNext()) {
            ...
            if (availableBinaryCapacity == 0
                    || (step > 1 && availableBinaryCapacity < CAPACITY_PER_INT)) {
                expansionCount++;
    ...
    private static int getRemainingCapacityInInt(int intData) {
        if (intData == 0 || intData == NEW_BINARY_DATA_INT) {
            // The first bit is reserved for the negative sign, the last bit is used to show the
            // number of steps
    ...
    /**
     * Check if route data has capacity for a given step. More efficient than {@link
     * #getRemainingCapacityInInt(int)}.
     *
     * @param routeData the route data to be checked
     * @param step the step to add to the route.
     */
     ...
     *
     * @param routeData the route data to add a step to. Will be modified by this function.
     * @param index the index within the route array where the step will be placed
     * @param step the binary step to add to the route data.
     */
    private static void addBinaryStepToRouteData(
            final int[] routeData, final int index, final int step) {
     ...
     * @param routeData the route data to add a step to. Will be modified by this function.
     * @param index the index within the route array where the step will be placed
     * @param step the n-ary step to add to the route data.
     */
    private static void addNaryStepToRouteData(
            final int[] routeData, final int index, final int step) {
     ...
     *
     * @param route the route to add a step to. Will be modified by this function.
     * @param index the index within the route array where the step will be placed
     * @param step the step to add to the route.
     */
     ...
     *
     * @param route the route to add a step to. Will be modified by this function.
     * @param initialIndex the index within the route array where the first step will be placed
     * @param steps an iterator for the steps that will be added
     */
    private static void addStepsToRouteData(
            final int[] route, final int initialIndex, final Iterator<Integer> steps) {
