 ...
 *
 * @param <T> an object that represents a node in the tree
 */
public interface TreeView<T> extends AutoCloseable {
     ...
     * @param isOriginal true if the node is from the original tree. This will always be {@code
     *     true} when called for the teacher.
     ...
     * @throws MerkleSynchronizationException if the node in question is a leaf node
     */
     ...
     *
     * @param node the node in question, guaranteed to only be called on the roots of trees that
     *     define custom reconnect views
