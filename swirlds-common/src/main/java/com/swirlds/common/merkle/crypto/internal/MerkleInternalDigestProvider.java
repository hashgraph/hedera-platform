...
/**
 * A {@link CachingOperationProvider} implementation that is capable of computing a hash for a
 * supplied {@link MerkleInternal} instance.
 */
public class MerkleInternalDigestProvider
        extends CachingOperationProvider<
                MerkleInternal, List<Hash>, Hash, HashBuilder, DigestType> {
    ...
    protected Hash handleItem(
        ...
        for (int index = 0; index < childHashes.size(); index++) {
            ...
            if (childHash == null) {
                ...
                final String msg =
                        String.format(
                                "Child has an unexpected null hash [ parentClass = '%s', childClass"
                                        + " = '%s', childRoute = %s ]",
    ...
    @Override
    public Hash compute(
            final MerkleInternal node, final List<Hash> childHashes, final DigestType algorithmType)
            throws NoSuchAlgorithmException {
