...
/**
 * This class implements boilerplate functionality for a binary {@link MerkleInternal} (i.e. an
 * internal node with 2 or fewer children). Classes that implement {@link MerkleInternal} are not
 * required to extend a class such as this or {@link PartialNaryMerkleInternal}, but absent a reason
 * it is recommended to do so in order to avoid re-implementation of this code.
 */
public non-sealed class PartialBinaryMerkleInternal extends AbstractMerkleInternal
     ...
     * {@inheritDoc}
     *
     * <p>In the binary case this is always BINARY_CHILD_COUNT (2).
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * <p>If the index is either LEFT or RIGHT, then return the correct child otherwise return an
     * IllegalChildIndexException.
     *
     ...
     * {@inheritDoc}
     *
     * <p>Select either the {@link ChildIndices#LEFT} (0) or {@link ChildIndices#RIGHT} (1) using an
     * index number. This will throw an error if a different value is used.*
     *
     ...
     * Set the left child.
     *
     * @param left a merkle node that will become this node's left child
     * @param <T> the type of the child
     */
     ...
     * Get the left child.
     *
     * @param <T> the type of the left child
     ...
     * Set the right child.
     *
     * @param right a merkle node that will become this node's right child
     * @param <T> the type of the child
     */
     ...
     * {@inheritDoc}
     *
     * <p>In the N-Ary case, this will increase the number of children to the value provided. In
     * this binary case, there are always two children, so this is a NOP.
     *
     * @param index unused
     */
    @Override
    protected void allocateSpaceForChild(final int index) {
        // in the binary case, these children are members of the object and don't need to be
     ...
     * {@inheritDoc}
     *
     * <p>In the N-Ary case, this verifies that the index is between 0 and the number of actual
     * children (including null children) of the node. However, in the binary case, there are always
     * two: ChildIndices.LEFT (0) and ChildIndices.RIGHT (1), and the subsequent call is to
     * getChild(index), above, which also tests the legality of the index. As a result, there is no
     * need to perform an extra bounds test here. This is effectively a NOP.
     *
     * @param index
     */
    @Override
