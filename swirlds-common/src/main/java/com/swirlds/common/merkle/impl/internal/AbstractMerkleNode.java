...
public abstract sealed class AbstractMerkleNode extends AbstractReservable
    ...
    public void setRoute(final MerkleRoute route) {
        if (!(getRoute() == route || getRoute().equals(route)) && getReservationCount() > 1) {
            // If you see this exception, the most likely culprit is that you are attempting to
            // "move" a merkle
            // node from one position to another but the merkle node has multiple parents. If this
            // operation was
            // allowed to proceed, a node in multiple trees would have the incorrect route in at
            // least one of those
            // trees. Instead of moving the node, make a copy and move the copy.
            throw new MerkleRouteException(
                    "Routes can not be set unless the reservation count is 0 or 1. (type = "
