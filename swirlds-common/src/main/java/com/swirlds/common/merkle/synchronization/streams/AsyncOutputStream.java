 ...
 *
 * <p>Only one type of message is allowed to be sent using an instance of this class. Originally
 ...
 *
 * <p>This object is not thread safe. Only one thread should attempt to send data over this stream
...
public class AsyncOutputStream<T extends SelfSerializable> implements AutoCloseable {
    ...
    public AsyncOutputStream(
            final SerializableDataOutputStream outputStream, final StandardWorkGroup workGroup) {
    ...
    public void sendAsync(final T message) throws InterruptedException {
        if (!isAlive()) {
            throw new MerkleSynchronizationException(
                    "Messages can not be sent after close has been called.");
        }
    ...
    /**
     * Close this buffer and release resources. If there are still messages awaiting transmission
     * then resources will not be immediately freed.
     */
    @Override
    public void close() {
        alive = false;
