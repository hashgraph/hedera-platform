...
public class MerkleCryptoEngine implements MerkleCryptography {
    ...
    public Hash digestSync(
            final MerkleInternal node, final List<Hash> childHashes, final boolean setHash) {
        try {
            final Hash hash =
                    merkleInternalDigestProvider.compute(node, childHashes, MERKLE_DIGEST_TYPE);
            if (setHash) {
                node.setHash(hash);
            }
            return hash;
    ...
    @Override
    public Hash digestSync(
            final MerkleInternal node, final DigestType digestType, boolean setHash) {
