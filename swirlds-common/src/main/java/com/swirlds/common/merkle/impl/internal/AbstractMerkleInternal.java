 ...
 * are not required to extend an abstract class such as this or {@link PartialNaryMerkleInternal},
...
public abstract sealed class AbstractMerkleInternal extends AbstractMerkleNode
        implements MerkleParent permits PartialBinaryMerkleInternal, PartialNaryMerkleInternal {
    ...
    /**
     * Copy constructor. Initializes internal variables and copies the route. Does not copy children
     * or other metadata.
     */
     ...
     * @param child new node to attach
     */
     ...
     *
     * @param index in the N-Ary case, expand the array to accommodate this many children in the
     *     Binary case, this is a NOP
     */
    protected abstract void allocateSpaceForChild(final int index);

    /**
     * Check whether the requested index is in valid range [0, maximum child count). In the Binary
     * case, this is a NOP as the error testing happens in getChild(index).
     *
     * @param index - child position to verify is legal
     */
     ...
     * @param childMayBeImmutable if true then the child is permitted to be immutable, if false then
     *     it is not permitted
     */
    private void throwIfInvalidChild(
            final int index, final MerkleNode child, final boolean childMayBeImmutable) {
        ...
        if (child != null) {
            ...
            if (!childMayBeImmutable && child.isImmutable()) {
                throw new MutabilityException(
                        "Immutable child can not be added to parent. parent = "
     ...
     * @param childClassId The class id of the child. May be NULL_CLASS_ID if the child is null.
    ...
    /**
     * Compute and construct a new route for a child at a given position. Recycles the route of the
     * existing child if possible.
     *
     * @param index the index of the child
     */
     ...
     * {@inheritDoc}
     *
     * <p>Deserialize from a list of children
     *
     * @param children A list of children.
     * @param version Version (e.g. format) of the deserialized data.
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * <p>WARNING: setting the route on an internal node with children requires a full iteration of
     * the subtree. For large trees this may be very expensive.
     */
    @Override
     ...
     *
     * @param route this node's new route
     */
