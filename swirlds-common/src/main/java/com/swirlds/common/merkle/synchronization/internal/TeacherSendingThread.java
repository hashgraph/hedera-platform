...
public class TeacherSendingThread<T> {
    ...
    /**
     * The lesson used to describe an up to date node is always eactly the same. No need to create a
     ...
     * @param out the output stream, this object is responsible for closing this object when
     *     finished
     * @param subtrees a queue containing roots of subtrees to send, may have more roots added by
     *     this class
    ...
    /**
     * When a {@link Lesson} for in an internal node is sent, that lesson contains embedded queries.
     * This method prepares for the responses to those queries.
     */
    ...
    private Lesson<T> buildCustomReconnectRootLesson(final T node) {
        final Lesson<T> lesson =
                new Lesson<>(CUSTOM_VIEW_ROOT, new CustomViewRootLesson(view.getClassId(node)));
        final CustomReconnectRoot<?, ?> subtreeRoot =
                (CustomReconnectRoot<?, ?>) view.getMerkleRoot(node);
    ...
    /**
     * Send a lesson about a node. Each query sent to the learner is always followed by a lesson
     * (eventually). Some lessons are just confirmations that the learner has the data. Others
     * actually contain the data required by the learner to reconstruct the node.
     *
     * <p>Lessons containing data about an internal node may also contain queries. The queries will
     * be for the children of the internal node.
     */
    ...
    private void run() {
        try (out) {
            ...
            throw new MerkleSynchronizationException(
                    "exception in the teacher's receiving thread", ex);
        } finally {
