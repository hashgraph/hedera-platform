...
public class StandardTeacherTreeView implements TeacherTreeView<NodeToSend> {
    ...
    /** {@inheritDoc} */
    @Override
    public NodeToSend getChildAndPrepareForQueryResponse(
            final NodeToSend parent, final int childIndex) {
    ...
    public int getNumberOfChildren(final NodeToSend node) {
        ...
        if (merkleNode == null || merkleNode.isLeaf()) {
            throw new MerkleSynchronizationException(
                    "can not get number of children from node that is not internal");
        }
    ...
    /** {@inheritDoc} */
    @Override
    public void serializeLeaf(final SerializableDataOutputStream out, final NodeToSend leaf)
            throws IOException {
        ...
        if (!merkleNode.isLeaf()) {
            throw new MerkleSynchronizationException(
                    "this method can not serialize an internal node");
        }
