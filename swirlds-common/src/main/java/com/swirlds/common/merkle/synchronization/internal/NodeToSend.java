...
/**
 * A wrapper object for a node that the sending synchronizer intends to send to the receiver. These
 * objects form a shadow tree that contains nodes that 1) have not yet received learner responses
 * and 2) have been or are about to be sent by the teacher.
 */
public class NodeToSend {

    /**
     * A node that may be sent. Will not be sent if the learner tells the teacher that it has the
     * node already.
     */
    ...
    /**
     * If it comes time to send this node, but a response has not yet been heard from the learner,
     * make sure that this amount of time has passed (since the query was sent) before sending the
     * node.
     */
    private final long unconditionalSendTimeMilliseconds;

    /**
     * Wrappers around the children of this node. This list is only populated once the children have
     * had a lesson sent for them.
     */
    ...
    /**
     * Whenever the teacher sends a query to the learner about a node, it registers that node to its
     * parent in this shadow tree. This allows a positive response corresponding to an ancestor to
     * propagate information that can prevent the node from needing to be sent.
     *
     * @param child the node that is now eligible for sending. It is called a child because it is
     *     the child of a node that was just sent.
     */
    ...
    /**
     * Return true if the learner has confirmed that it has the node in question (i.e. the node
    ...
    /**
     * Wait for a resonse from the learner. Will return immediately if a response has already been
     * received or if an ancestor has received a positive response. May sleep a short period if
     * neither are true. There is no guarantee that a response will have been received when this
     * method returns.
     */
