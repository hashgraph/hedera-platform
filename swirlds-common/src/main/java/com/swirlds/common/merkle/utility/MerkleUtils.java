...
public final class MerkleUtils {
    ...
    /**
     * Invalidate the hashes of an entire tree (or subtree) rooted at the given node. Does not
     * perform invalidation for parts of the merkle tree that are self hashing.
     *
    ...
    /**
     * Rehash the entire tree, discarding any hashes that are already computed. Does not rehash
     * virtual parts of the tree that are self hashing.
     *
            ...
            if (node.isLeaf()) {
                ...
                if (maxDepth > 0 && depth + 1 > maxDepth) {
                    ...
                    for (int childIndex = 0;
                            childIndex < internal.getNumberOfChildren();
                            childIndex++) {
                        buildMerkleString(
                                sb,
    ...
    public static String getMerkleString(
            final MerkleNode root, final boolean printNodeDescription) {
     ...
     *
     * @param root root of the merke tree
     * @param maxDepth max depth of nodes to be included in this string
     * @param printNodeDescription whether print node description or not
     * @return a string which represents this merkle tree
     */
    public static String getMerkleString(
    ...
    /**
     * Find the index of a given child within its parent. O(n) time in the number of children of the
     * parent. Sufficiently fast for tiny nodes (i.e. binary nodes), too slow for large nary nodes.
     *
     * @param parent the parent in question
     * @param child the child in question
     * @return the index of the child within the parent
     */
    public static int findChildPositionInParent(
            final MerkleInternal parent, final MerkleNode child) {
