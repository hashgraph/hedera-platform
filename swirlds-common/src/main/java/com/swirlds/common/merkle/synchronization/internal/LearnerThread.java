...
public class LearnerThread<T> {
     ...
     * @param root a reference which will eventually hold the root of this subtree
     * @param view a view used to interface with the subtree
     * @param nodeCount an object used to keep track of the number of nodes sent during the
     *     reconnect
     */
    ...
    /**
     * Handle a lesson for the root of a tree with a custom view. When such a node is encountered,
     * instead of iterating over its children, the node is put into a queue for later handling.
     * Eventually that node and the subtree below it are synchronized using the specified view.
     */
    private T handleCustomRootInitialLesson(
        ...
        // Sometimes the original node is null if the original tree does not have any node in this
        // position.
        ...
        if (customRoot == null) {
            throw new MerkleSynchronizationException(
                    "unable to construct object with class ID "
                            + classIdString(lesson.getCustomViewClassId()));
        }

        if (originalNode != null
                && view.getClassId(originalNode) == lesson.getCustomViewClassId()) {
    ...
    private void handleQueries(
        ...
        for (int childIndex = 0; childIndex < childCount; childIndex++) {
            ...
            if (view.isInternal(originalParent, true)
                    && view.getNumberOfChildren(originalParent) > childIndex) {
            ...
            final boolean nodeAlreadyPresent =
                    originalHash != null && originalHash.equals(teacherHash);
    ...
    private void addToNodeCount(
            final ExpectedLesson<T> expectedLesson, final Lesson<T> lesson, final T newChild) {
        if (lesson.isCurrentNodeUpToDate()) {
            return;
        }
        ...
        try (in;
            ...
            while (view.hasNextExpectedLesson()) {
                ...
                if (lesson.hasQueries()) {
                    ...
                    handleQueries(
                            view, in, out, queries, expectedLesson.getOriginalNode(), newChild);
                }
            }
