...
public class LearningSynchronizer implements ReconnectNodeCount {
     ...
     * @param breakConnection a method that breaks the connection. Used iff an exception is
     *     encountered. Prevents deadlock if there is a thread stuck on a blocking IO operation that
        ...
        logger.info(
                ...
                () ->
                        new SynchronizationCompletePayload("Finished synchronization")
                                .setTimeInSeconds(
                                        synchronizationTimeMilliseconds * MILLISECONDS_TO_SECONDS)
                                .setHashTimeInSeconds(
                                        hashTimeMilliseconds * MILLISECONDS_TO_SECONDS)
                                .setInitializationTimeInSeconds(
                                        initializationTimeMilliseconds * MILLISECONDS_TO_SECONDS)
    ...
    /**
     * Get the root of the resulting tree. May return an incomplete tree if called before
    ...
    private <T> MerkleNode receiveTree(final MerkleNode root) throws InterruptedException {
        ...
        final AsyncInputStream<Lesson<T>> in =
                new AsyncInputStream<>(inputStream, workGroup, () -> new Lesson<>(view));
        ...
        if (interruptException != null || workGroup.hasExceptions()) {
            ...
            // Depending on where the failure occurred, there may be deserialized objects still
            // sitting in
    ...
    /**
     * Build the output stream. Exposed to allow unit tests to override implementation to simulate
     * latency.
     */
