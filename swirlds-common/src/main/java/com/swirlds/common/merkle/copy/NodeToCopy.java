...
class NodeToCopy {
    ...
    /**
     * @param newParent the parent into which the node should be inserted when copied
     * @param indexInParent the index at which the copy should be inserted
     * @param nodeToCopy the node that should be copied
     * @param originalNodeInSamePosition The node that was previously replaced by new parent, may be
    ...
    /**
     * Get the route where the node is being placed if it is known. Otherwise return null. Route is
     * recycled from node that originally occupied this location in the tree.
     */
    ...
    /**
     * For a given child index within nodeToCopy, get the node that was originally in the same
     * position as that child in the original tree. Useful for recycling routes.
     */
