...
public final class MerkleHashChecker {
    ...
    /**
     * Traverses the merkle tree and checks if there are any hashes that are not valid. Recalculates
     * all hashes that have been calculated externally and check them against the getHash value.
     *
    ...
    public static void findInvalidHashes(
            final MerkleNode root, final Consumer<MerkleNode> mismatchCallback) {
        if (root == null) {
            return;
        }
    ...
    private static void findInvalidHash(
            final MerkleNode node, final Consumer<MerkleNode> mismatchCallback) {
        final MerkleCryptography cryptography = MerkleCryptoFactory.getInstance();

        // some nodes calculate their own hash, we could potentially check these if we serialize and
        // deserialize
        ...
        if (node.isLeaf()) {
            ...
            recalculated =
                    cryptography.digestSync(internal, Cryptography.DEFAULT_DIGEST_TYPE, false);
        }
    ...
    /**
     * Get a list of all nodes in a tree that have an invalid hash. Returns an empty list if the
     * entire tree has valid hashes.
     *
    ...
    /**
     * Check if all of the hashes within a tree are valid. Write a detailed message to the log if
     * invalid hashes are detected in the tree.
     *
     * @param root the root of the tree to check
     * @param context the context that the check is being done in. This is written to the log if a
     *     problem is detected.
     * @param limit the maximum number of invalid nodes to log. If the entire tree is invalid then
     *     the log could be massively spammed. A sane limit reduces the amount logged in that
     *     situation.
     * @return true if the tree is valid, false if it is not valid
     */
    public static boolean checkHashAndLog(
            final MerkleNode root, final String context, final int limit) {
    ...
    /**
     * Generate a human-readable debug string that contains hash information about nodes near the
     * top of a tree. No hash information is displayed for nodes below a certain depth, or for nodes
     * below other nodes annotated with {@link
     * com.swirlds.common.merkle.utility.DebugIterationEndpoint DebugIterationEndpoint}.
     *
     * @param root the root of a tree (or subtree) to iterate
     * @param maxDepth the maximum depth of the tree to iterate
