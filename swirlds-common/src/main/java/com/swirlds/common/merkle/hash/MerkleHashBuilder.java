...
public class MerkleHashBuilder {
    ...
    public Future<Hash> digestTreeAsync(MerkleNode root) {
        if (root == null) {
            ...
            for (int threadIndex = 0; threadIndex < cpuThreadCount; threadIndex++) {
                threadPool.execute(
                        createHashingRunnable(threadIndex, activeThreadCount, result, root));
            }
            return result;
        }
    }
     ...
     *
     * @param threadId Used to generate a randomized iteration order
     */
    private Runnable createHashingRunnable(
        ...
        return () -> {
            final MerkleIterator<MerkleNode> it =
                    root.treeIterator()
                            .setFilter(MerkleHashBuilder::filter)
                            .setDescendantFilter(MerkleHashBuilder::descendantFilter);
            if (threadId > 0) {
     ...
     *
     * @param it An iterator that walks through the tree.
     * @param activeThreadCount If single threaded then this is null, otherwise contains the number
     *     of threads that are actively hashing. Once the active thread count dips below the
     *     maximum, this means that one thread has either finished or exploded.
     */
    private void hashSubtree(final Iterator<MerkleNode> it, final AtomicInteger activeThreadCount) {
        while (it.hasNext()) {
            final MerkleNode node = it.next();
            // Potential optimization: if this node is currently locked, do not wait here. Skip it
            // and continue.
