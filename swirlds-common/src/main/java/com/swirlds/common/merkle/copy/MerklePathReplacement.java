...
public final class MerklePathReplacement {
    ...
    /**
     * The position of the child (i.e. the last node) within a path generated by {@link
     * #replacePath(MerkleNode, MerkleRoute)} relative to the length of the path. To get the index
     * of the child, add this value to the length of the path.
     */
    private static final int CHILD_PATH_OFFSET = -1;

    /**
     * The position of the parent within a path generated by {@link #replacePath(MerkleNode,
     * MerkleRoute)} relative to the length of the path. To get the index of the parent, add this
     * value to the length of the path.
     */
    private static final int PARENT_PATH_OFFSET = -2;

    /**
     * The position of the grandparent within a path generated by {@link #replacePath(MerkleNode,
     * MerkleRoute)} relative to the length of the path. To get the index of the grandparent, add
     * this value to the length of the path.
     */
    ...
    /**
     * Make a copy of a node in the path, if needed. Does not copy if the child has a reference
     * count that does not exceed 1.
     *
     * @param child the node to possibly copy
     * @param pathIndex the position of the child within the path
     * @param pathLength the total length of the path
     * @param skipAtEnd the number of nodes at the end of the path that should not be copied
     * @param artificialChildReferences the number of artificial references to the child. Only
     *     values of 0 or 1 are expected.
     * @param parentInPath the parent that will hold the node after the copy
     * @param indexToCopy the index within parentInPath
     * @param previousNodeCopied true if a previous node in the path has already been copied
     ...
     *
     * @param iterator an iterator over the steps in a route
     * @param stepsToSkip the number of steps to skip
     */
        ...
        for (int i = 0; i < stepsToSkip; i++) {
            if (!iterator.hasNext()) {
                throw new IllegalStateException(
                        "can not skip more steps there exist within the iterator");
            }
            iterator.next();
        }
    }
     ...
     *
     * @param path an array where a path is being constructed
     * @param nodesRequiringInitialization an array containing data about which nodes require
     *     initialization
     * @param pathIndex the index of the node within the path
     * @param node the node to add to the path
     * @param nodeWasCopied true if this node was copied
     */
    ...
    /**
     * Utility method. Take an artificial reference to nodes to prevent them from being prematurely
     * deleted.
     *
     * @param isLastNode true if the child is the last node in the path
     * @param root the root of the path
     * @param parent the parent of the current child
     * @param child the child of the current parent
    ...
    private static int handleArtificialReferences(
        ...
        if (parent != root) {
            // Now that the child holds a reference count, release the artificial reference count on
            // the parent
            parent.release();
        }
     ...
     *
     * <p>Note: this step will not be necessary once internal nodes have a uniform and well behaving
     * copy().
     *
     * @param path nodes in the path
     * @param nodeRequiringInitialization a list of boolean values, true if the node needs
     *     initialization
     */
    private static void initializePath(
            final MerkleNode[] path, final boolean[] nodeRequiringInitialization) {
    ...
    /**
     * Replace a path from the root down to a given node. It is assumed that the root never needs to
     * be replaced.
     *
     * <p>WARNING: until merkle nodes become naturally fast copyable, it is assumed that each node
     * in the path that needs to be replaced is fast copyable and that each fast copy operation does
     * not attempt to copy its children.
     *
     * @param root the root of the tree (or subtree). Is not replaced.
     * @param route the route to replace
    ...
    /**
     * Replace a path from one node down to another node, skipping the last several steps in the
     * route if configured. It is assumed that the first node in the path never needs to be
     * replaced.
     *
     * <p>Nodes in the newly created path are initialized via the {@link MerkleInternal#rebuild()}
     * method.
     *
     * <p>All nodes along the given path are returned with null hashes, with the exception of the
     * skipped nodes at the end of the path (if there are any) which are returned without the hashes
     * being modified.
     *
     * @param firstNodeInPath the first node in the path. Is not replaced.
     * @param route the route to replace
     * @param skipAtEnd the number of steps at the end of the path to NOT copy. If 0 then the entire
     *     path is replaced. If 1 then the entire path with the exception of the last node is
     *     replaced, and so on. Nodes not replaced are still included in the returned path.
     * @return all nodes in the replaced path, followed by nodes along the route that were not
     *     replaced
     */
    public static MerkleNode[] replacePath(
        ...
        if (firstNodeInPath.isLeaf()) {
            if (pathLength != 1) {
                throw new MerkleRouteException(
                        "First node is a leaf but path has length " + pathLength);
            }
            return path;
        }
        ...
        while (iterator.hasNext()) {
            ...
            addNodeToPath(
                    path, nodesRequiringInitialization, pathIndex, childInPath, nodeWasCopied);
            pathIndex++;
        ...
        if (pathIndex != pathLength) {
            throw new MerkleRouteException(
                    "Path expected to be of length "
                            + pathLength
                            + " but is actually of length "
                            + pathIndex);
        }
    ...
    /**
     * Get the child in a given path generated by {@link #replacePath(MerkleNode, MerkleRoute)}. The
     * child is considered to be the last node in the path.
     *
     * @param path a path
    ...
    /**
     * Get the parent in a given path generated by {@link #replacePath(MerkleNode, MerkleRoute)}.
     * The parent is considered to be the second to last node in the path.
     *
     * @param path a path
     * @param <T> the type of data returned
    ...
    /**
     * Get the grandparent in a given path generated by {@link #replacePath(MerkleNode,
     * MerkleRoute)}. The grandparent is considered to be the third to last node in the path.
     *
     * @param path a path
     * @param <T> the type of data returned
