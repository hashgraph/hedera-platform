 ...
 *
 * @param <T> The type of the node returned by this iterator.
 */
public class MerkleIterator<T extends MerkleNode> implements Iterator<T> {
    ...
    /**
     * If not null, do not return any nodes below any internal nodes that causes this method to
     * return false.
     */
    private Predicate<MerkleInternal> descendantFilter;

    /**
     * If true then don't return null nodes, and don't pass any null nodes to the {@link #filter} or
     * {@link #descendantFilter}.
     */
    ...
    /**
     * Provide an optional filter. If not null, do not return any node that causes the filter to
     * return false.
     *
     * @param filter a predicate that is applied to every node before it can be returned. If {@link
     *     #ignoreNull(boolean)} has been called and set to true then this method will never be
     *     passed a null node.
     * @return this object
     * @throws IllegalStateException if called after {@link #hasNext()} or {@link #next()}
     */
     ...
     * @param filter a predicate that is applied to every node before it can be returned. If {@link
     *     #ignoreNull(boolean)} has been called and set to true then this method will never be
     *     passed a null node. The merkle route corresponds the merkle route of the node, which is
     *     useful if null valuess are being filtered.
     * @return this object
     * @throws IllegalStateException if called after {@link #hasNext()} or {@link #next()}
     */
    public MerkleIterator<T> setFilter(final BiPredicate<MerkleNode, MerkleRoute> filter) {
        if (algorithm != null) {
            throw new IllegalStateException(
                    "iterator can not be configured after iteration has started");
        }
        this.filter = filter;
        return this;
    }
    ...
    /**
     * Provide an optional filter that can exclude descendants of a node. If this method returns
     * false for an internal node, no node descendant from that internal node will be returned. An
     * internal node that causes the descendant filter to return false can still itself be returned
     * if the filter specified by {@link #setFilter(Predicate)} returns true for that internal node.
     *
     * @param descendantFilter only nodes that cause this filter to return true will have their
     *     descendants iterated over
     * @return this object
     * @throws IllegalStateException if called after {@link #hasNext()} or {@link #next()}
     */
    ...
    /**
     * Specify if null nodes should be returned. If true then null nodes will never be returned, and
     * null nodes will never be passed to the predicate set by {@link #setFilter(Predicate)}.
     * Default value is true.
     *
     * @param ignoreNullNodes if true then ignore null nodes (default), if false then return null
     *     nodes
     * @return this object
     * @throws IllegalStateException if called after {@link #hasNext()} or {@link #next()}
     */
    ...
    /**
     * Specify the iteration order. If unset, default order is {@link
     * MerkleIterationOrder#POST_ORDERED_DEPTH_FIRST}.
     *
     * @param order an iteration order
     * @return this object
     * @throws IllegalStateException if called after {@link #hasNext()} or {@link #next()}
     */
    ...
    /**
     * A filter that is applied to each internal node. If false then none of the nodes descended
     * from the internal node will be returned.
     *
    ...
    /**
     * A filter that is applied to each node. If false then this node will not be returned by the
     * iterator. Does not stop the iterator from possibly returning descendant nodes.
     */
    ...
    private boolean hasChildrenToHandle(MerkleNode node) {
        // If an internal node has children that need to be handled it will have been added to the
        // stack twice in a row
        return node != null
                && node.isInternal()
                && algorithm.size() > 0
                && algorithm.peek() == node;
     ...
     *
     * @param parent the parent of the node to push
     * @param childIndex the index within the parent of the node to push
     */
    ...
    private void setup() {
        if (algorithm == null) {
            switch (order) {
                ...
                case POST_ORDERED_DEPTH_FIRST_RANDOM -> algorithm =
                        new PostOrderedDepthFirstRandomAlgorithm();
                ...
                default -> throw new UnsupportedOperationException(
                        "unhandled iteration algorithm " + order);
            }
            if (root != null) {
                pushNode(root);
            }
    ...
    private void findNext() {
        ...
        while (algorithm.size() > 0) {
            ...
            final MerkleNode target =
                    candidate.getClassId() == NullNode.CLASS_ID ? null : candidate;
    ...
    /**
     * Get the merkle route of the most recently returned node. Useful for obtaining the route when
     * iterating over nodes that may be null.
     *
     * @return the merkle route of the most recently returned node, or null if no node has been
     *     returned
     */
    ...
    /**
     * Convert this iterator into an iterator that walks over the same data but returns a different
     * type. Calling {@link #next()} on this iterator will update the transformed iterator, and vice
     * versa.
     *
     * @param converter a method that converts from the original type of the iterator to the new
     *     iterator type
     * @param <K> the new type that the iterator will return
    ...
    /**
     * Similar to {@link Iterator#forEachRemaining(Consumer)}, except that the action is allowed to
     * throw an {@link IOException}.
     *
     * @param action the action to perform
     * @throws IOException if the action throws an IO exception
     */
     ...
     *
     * @param action the action to perform
     * @throws InterruptedException if the action throws an interrupted exception
     */
