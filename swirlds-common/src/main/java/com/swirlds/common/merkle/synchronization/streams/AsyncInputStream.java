 ...
 *
 * <p>Only one type of message is allowed to be read using an instance of this class. Originally
 * this class was capable of supporting arbitrary message types, but there was a significant memory
 * footprint optimization that was made possible by switching to single message type.
 *
 * <p>This object is not thread safe. Only one thread should attempt to read data from stream at any
 * point in time.
 */
public class AsyncInputStream<T extends SelfSerializable> implements AutoCloseable {
     ...
     * @param messageFactory this function constructs new message objects. These messages objects
     *     are then used to read data via {@link
     *     SelfSerializable#deserialize(SerializableDataInputStream, int)}.
     */
    ...
    /**
     * Returns true if the message pump is still running or false if the message pump has terminated
     * or will terminate.
     *
     * @return true if the message pump is still running; false if the message pump has terminated
     *     or will terminate
     */
    ...
    private void run() {
        ...
        try {
            while (isAlive() && !Thread.currentThread().isInterrupted()) {
                final int previous =
                        anticipatedMessages.getAndUpdate(
    ...
    /**
     * Get an anticipated message. Blocks until the message is ready. Object returned will be the
     * same object passed into addAnticipatedMessage, but deserialized from the stream.
     */
    ...
    /**
     * This method should be called when the reader decides to stop reading from the stream (for
     * example, if the reader encounters an exception). This method ensures that any resources used
     * by the buffered messages are released.
     */
    public void abort() {
        close();
    ...
    /**
     * Read a message. Will throw an exception if time equal to {@link #pollTimeoutMs} passes
    ...
    private T asyncRead() throws InterruptedException {
        ...
        if (data == null) {
            try {
                // An interrupt may not stop the thread if the thread is blocked on a stream read
                // operation.
