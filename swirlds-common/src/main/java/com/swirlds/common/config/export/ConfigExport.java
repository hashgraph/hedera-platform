...
public final class ConfigExport {
    ...
    /**
     * Provides information about the config with 1 line per config property to the consumer. The
     * format of one line looks like this:
     *
     * <p><code>name   -> value    [(NOT )USED IN RECORD]</code>
     *
     * @param configuration the configuration
     * @param lineConsumer the line consumer
     */
    public static void printConfig(
            final Configuration configuration, final Consumer<String> lineConsumer) {
        ...
        final Set<String> allConfigNames =
                combine(recordProperties.keySet(), nonRecordProperties.keySet());
        final Set<Object> allConfigValues =
                combine(recordProperties.values(), nonRecordProperties.values());
        ...
        recordProperties.keySet().stream()
                .sorted()
                .forEach(
                        name -> {
        ...
        nonRecordProperties.keySet().stream()
                .sorted()
                .forEach(
                        name -> {
    ...
    private static String buildLine(
            ...
            final String suffix) {
        return name
                + createSpaces(name, maxNameLength)
                + " -> "
                + value
    ...
    /**
     * Writes information about the config with 1 line per config property to the given stream. The
     * format of one line looks like this:
     *
     * <p><code>name   -> value    [(NOT )USED IN RECORD]</code>
     *
     ...
     */
    public static void printConfig(
            final Configuration configuration, final OutputStream printStream) throws IOException {
        CommonUtils.throwArgNull(configuration, "configuration");
    ...
    public static void addConfigContents(
            final Configuration configuration, final StringBuilder builder) {
        CommonUtils.throwArgNull(configuration, "configuration");
        CommonUtils.throwArgNull(builder, "builder");
        printConfig(configuration, line -> builder.append(line).append(System.lineSeparator()));
    }

    private static Map<String, Object> getPropertiesForConfigDataRecords(
        ...
        configuration
                ...
                .forEach(
                        configDataType ->
                                putValuesInMap(configuration.getConfigData(configDataType), map));
        return map;
    }
    ...
    private static void putValuesInMap(final Record configData, final Map<String, Object> map) {
        ...
        Arrays.stream(configDataType.getRecordComponents())
                .forEach(
                        component -> {
                            final String name =
                                    ConfigReflectionUtils.getPropertyNameForConfigDataProperty(
    ...
    private static Object getComponentValue(
            final Record configData, final RecordComponent component) {
        try {
            return component.getAccessor().invoke(configData);
        } catch (final Exception e) {
            throw new IllegalStateException(
                    "Can not access config value for record type '"
                            + component.getClass()
                            + "."
    ...
    private static String createSpaces(final String value, final int maxLength) {
        return IntStream.range(value.length(), maxLength)
                .mapToObj(i -> " ")
                .reduce("", (a, b) -> a + b);
    }
