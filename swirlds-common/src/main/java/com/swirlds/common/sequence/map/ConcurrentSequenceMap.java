...
public class ConcurrentSequenceMap<K, V> extends AbstractSequenceMap<K, V> {
    ...
    /**
     * When inserting data into this data structure, it is critical that data is not inserted after
     * the data's sequence number has been purged (as this would lead to a memory leak). Whenever
     * new data is inserted, acquire a lock that prevents concurrent purging of that sequence
     * number.
     */
