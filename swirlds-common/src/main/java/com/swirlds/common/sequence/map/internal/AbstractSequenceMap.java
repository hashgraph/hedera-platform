...
public abstract class AbstractSequenceMap<K, V> implements SequenceMap<K, V> {
     ...
     *
     * @param initialFirstSequenceNumber the lowest allowed sequence number when this object is
     *     constructed, or after it is cleared
     * @param sequenceNumberCapacity the number of sequence numbers permitted to exist in this data
     *     structure. E.g. if the lowest allowed sequence number is 100 and the capacity is 10, then
     *     values with a sequence number between 100 and 109 (inclusive) will be allowed, and any
     *     value with a sequence number outside that range will be rejected.
    ...
    protected AbstractSequenceMap(
        ...
        keySets =
                (SequenceKeySet<K>[])
                        Array.newInstance(SequenceKeySet.class, sequenceNumberCapacity);
    ...
    /**
     * Acquire a lock on window management. Held during purge/expand calls, and during clear. No-op
     * for implementations that do not require thread safety.
     */
    ...
    /**
     * Release a lock on window management. Held during purge/expand calls, and during clear. No-op
     * for implementations that do not require thread safety.
     */
    ...
    /**
     * Acquire an exclusive lock on a sequence number. No-op for implementations that do not require
     * thread safety.
     *
    ...
    /**
     * Release an exclusive lock on a sequence number. No-op for implementations that do not require
     * thread safety.
     *
    ...
    /**
     * Acquire an exclusive lock on all sequence numbers. No-op for implementations that do not
    ...
    /**
     * Release an exclusive lock on all sequence numbers. No-op for implementations that do not
     * require thread safety.
     */
    ...
    /**
     * When the window is shifted significantly, it can be more efficient to grab all locks at the
     * start, as compared to locking on each sequence number one at a time. This method describes
     * the size of the shift required to trigger a full lock.
     *
    ...
    private int getSequenceKeyIndex(final long sequenceNumber) {
        ...
        return (int)
                (((sequenceNumber % sequenceNumberCapacity) + sequenceNumberCapacity)
    ...
    public void shiftWindow(
            final long firstSequenceNumberInWindow, final BiConsumer<K, V> removedValueHandler) {
        windowLock();
        ...
        try {
            ...
            if (firstSequenceNumberInWindow < previousFirstSequenceNumber) {
                throw new IllegalStateException(
                        "Window can only be shifted towards larger value. "
                                + "Current lowest sequence number = "
                                + previousFirstSequenceNumber
                                + ", requested lowest sequence number = "
            ...
            for (int offset = 0; offset < sequenceNumberCapacity; offset++) {
                ...
                try {
                    ...
                    keys.setSequenceNumber(
                            mapToNewSequenceNumber(
                                    firstSequenceNumberInWindow, sequenceNumberToReplace));
                    keys.getKeys().clear();
                } finally {
                    if (!largeShift) {
    ...
    /**
     * When the window is shifted, it causes some key sets in the circular buffer increase their
     * sequence number. This method computes the new sequence number that the key set is required to
     * have.
     */
    private long mapToNewSequenceNumber(
            final long firstSequenceNumberInWindow, final long sequenceNumberToReplace) {
        ...
        final long wrapFactor =
                difference / getSequenceNumberCapacity()
    ...
    /** {@inheritDoc} */
    @Override
    public void removeValuesWithSequenceNumber(
            final long sequenceNumber, final BiConsumer<K, V> removedValueHandler) {
        windowLock();
        try {
            final SequenceKeySet<K> keys = getSequenceKeySet(sequenceNumber);
            lockSequenceNumber(sequenceNumber);
