...
/**
 * An implementation of a bloom filter. A bloom filter is a data structure that has similarities to
 * a set. Elements can be added to the bloom filter, but not removed. When checked to see if the
 * bloom filter contains an element, it can respond "no, this element is not present" or "this
 * element may be present". The probability of false positives can be tuned depending on input
 * configuration. A bloom filter can be advantageous when there is a need for a set too large to fit
 * into memory, as its memory footprint is SIGNIFICANTLY smaller than a standard set of the same
 * size.
 *
 * <p>This data structure is not thread safe. External synchronization required if used in a
...
public class BloomFilter<T> implements SelfSerializable {
    ...
    /**
     * The maximum array size is JVM dependant, and there is no good way to find out what that limit
     * is programmatically. This constant is chosen because it seems to work in tested environments.
     */
    ...
    /**
     * Contains the bloom filter's data. Split into an array of arrays to accommodate very large
     * bloom filters that do not fit into a single array.
     */
    private int[][] filter;
     ...
     */
    public BloomFilter(
            final int hashCount, final BloomHasher<T> hashProvider, final long filterSizeInBits) {
        ...
        this.hashProvider =
                Objects.requireNonNull(hashProvider, "null hash provider not supported");
     ...
     * @throws NullPointerException if the hasher does not support null values but a null element is
     *     provided
     */
    ...
    /**
     * Add an element using precomputed hashes. This method is useful for when an element's hashes
     * are already known (for example, if a contains operation was already performed).
     *
     ...
     *
     * <p>This method uses the internal buffer, and so it is not thread safe.
     *
     ...
     *
     * <p>Even though this is a read only operation (from the perspective of the caller), internally
     * this method uses a shared buffer to store hash data. If multi-threaded read access is needed,
     * each thread must provide it's own buffer and use ({@link #hash(Object)} or {@link
     * #hash(Object, long[])}) and {@link #contains(long[])}.
     *
     * @param element the element in question
     * @return if false, then the element is guaranteed not to be contained within the bloom filter.
     *     If true, then the element may or may not be contained by the bloom filter.
     * @throws NullPointerException if the hasher does not support null values but a null element is
     *     provided
     */
    ...
    /**
     * Check if an element is contained by the bloom filter using the hashes of the element. This
     * method is useful for when an element's hashes are already known.
     *
     * <p>This method is thread safe as long as not called concurrently with additions to the bloom
     * filter.
     *
     ...
     * @throws NullPointerException if a null array of hashes is provided, or if the hasher does not
     *     support null values but a null element is provided
     */
    ...
    /**
     * Given a bit index with respect to the entire bloom filter, return the index of the array that
     * contains the bit
     *
    ...
    /**
     * Given a bit index with respect to the entire bloom filter, return the index of the integer
    ...
    /**
     * Given a bit index with respect to the entire bloom filter, return the index of the bit within
     * the integer
     *
     * @param index the bit index with respect to the entire bloom filter
