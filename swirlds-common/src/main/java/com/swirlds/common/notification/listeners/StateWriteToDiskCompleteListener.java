...
/**
 * The interface that must be implemented by all notification listeners {@link Listener} listening
 * for notification when state is written to disk.
 */
@DispatchModel(mode = DispatchMode.SYNC, order = DispatchOrder.ORDERED)
public interface StateWriteToDiskCompleteListener
        extends Listener<StateWriteToDiskCompleteNotification> {}
