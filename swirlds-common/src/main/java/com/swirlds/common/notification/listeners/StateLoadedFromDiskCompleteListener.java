...
/**
 * An interface that must be implemented by all notification listeners {@link Listener} listening
 * for notification of state loaded from disk.
 */
@DispatchModel(mode = DispatchMode.SYNC, order = DispatchOrder.ORDERED)
public interface StateLoadedFromDiskCompleteListener
        extends Listener<StateLoadedFromDiskNotification> {}
