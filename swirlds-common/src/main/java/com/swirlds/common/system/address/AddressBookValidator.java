...
/**
 * This class provides methods for validating new address books. These methods do not throw
 * exceptions if validation fails, but they are intentionally very noisy in the logs when they fail.
 */
public final class AddressBookValidator {
    ...
    // FUTURE WORK: this class is only partially completed. Additional validation will be added
    ...
    /**
     * Sanity check, make sure that the next address book has a next ID greater or equal to the
     * previous one.
     *
    ...
    /**
     * No address that is removed may be re-added to the address book. If address N is skipped and
     * address N+1 is later added, then address N can never be added (as this is difficult to
     * distinguish from N being added and then removed).
     *
     * @param previousAddressBook the previous address book
     * @param addressBook the address book to validate
     * @return if the address book passes this validation
    ...
    /**
     * Check if a genesis address book is valid. In the case of an invalid address book, this method
     * will write an error message to the log.
     *
    ...
    /**
     * Check if a new address book transition is valid. In the case of an invalid address book, this
     * method will write an error message to the log.
     *
