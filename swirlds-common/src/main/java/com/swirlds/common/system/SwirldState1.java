...
public non-sealed interface SwirldState1 extends SwirldState {
	 ...
	 *
	 * <ul>
	 *     <li>{@link #init(Platform, AddressBook, SwirldDualState, InitTrigger, SoftwareVersion)}</li>
	 *     <li>{@link #copy()}</li>
	 ...
	 * @see #handleConsensusRound(Round, SwirldDualState)
	 */
	void handleTransaction(final long creatorId, final Instant timeCreated, final Instant estimatedTimestamp,
			final Transaction trans, final SwirldDualState swirldDualState);
