...
public interface SwirldMain extends Runnable {
     ...
     *
     * <p>SwirldApp can use {@link Platform#getState()} to access {@link SwirldState}, but must not
     * make any changes to {@link SwirldState} returned from {@link Platform#getState()}.
     *
    ...
    /**
     * Close any windows not instantiated via Platform.create*. This method may be called on a
     * thread that is not the Swing event thread.
     *
     * @param platform the Platform that instantiated this SwirldMain
     * @param selfId the ID number for this member (myself)
     */
    ...
    /**
     * The platform calls this method on the SwirldMain just before creating an event. The
     * SwirldMain can optionally create transactions here, knowing they will be sent out almost
     * immediately.
     *
     * <p>Note: The platform will stop generating new events if there are no transactions in the
     * network waiting to reach consensus. That means that an application, that creates transactions
     * only in the preEvent() method, might come to a standstill since this method will never be
     * called in this situation.
     *
     * <p>The run() method is given its own thread, which belongs to the SwirldMain for a particular
     * app running a particular swirld. The preEvent() method runs in a special thread created by
     * the platform each time it is called, and is limited to 0.1 seconds. If preEvent() does not
     * return in 0.1 seconds, then the thread will be interrupted. So when writing preEvent(), make
     * sure that it can always finish within 0.1 seconds, and that the separate threads of run() and
     * preEvent() don't conflict in accessing non-threadsafe objects.
     */
    void preEvent();
    ...
    /**
     * A method called by the Platform each time the status of the Platform changes. It is also
     * called once when the app first starts up, after init() is called.
     *
