 ...
 * to create a hash of an event that is signed, and some data is additional and does not affect that
 * hash. This data is split into 2 classes: {@link BaseEventHashedData} and {@link
...
public class BaseEventUnhashedData implements SelfSerializable {
    ...
    // --------------- NOTE
    // -------------------------------------------------------------------------------------------
    // Sequence number fields are no longer in use, so when an object is constructed, they are set
    // to SEQUENCE_UNUSED
    // These fields are still kept because there are a lot of downstream consequences of changing
    // the event stream
    // format, so they will be removed along with other fields that should not be part of the
    // stream.
    // otherId is also probably not needed anymore, so at some point this class can be replaced with
    // just a Signature
    // ----------------------------------------------------------------------------------------------------------------
