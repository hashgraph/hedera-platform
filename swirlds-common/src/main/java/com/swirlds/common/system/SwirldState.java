...
public sealed interface SwirldState extends Archivable, MerkleNode
	 ...
	 * consensus order by the {@link Round#eventIterator()}. Transactions in each event are iterated in consensus order
	 * by the {@link ConsensusEvent#consensusTransactionIterator()}. Transactions in a single event occur after all the
	 ...
	 * threads that change those variables, then wait until all those threads have ended, and then return. It is even
	 * OK for it to create a pool of threads that continue to exist after it returns, as long as it ensures that those
	 ...
	 *
	 * @param round
	 * 		the round of consensus ordered events to update the state with
	 * @param swirldDualState
