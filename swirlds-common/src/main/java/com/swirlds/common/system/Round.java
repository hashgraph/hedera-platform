...
/**
 * A collection of unique events that reached consensus at the same time. The consensus data for
 * every event in the round will never change, and no more events will ever be added to the round. A
 * round with a lower round number will always reach consensus before a round with a higher round
 * number.
 *
 * <p>IMPORTANT: Although this interface is not sealed, it should only be implemented by internal
...
public interface Round extends Iterable<ConsensusEvent> {
     ...
     * over the same events. This method is thread safe.
     *
     ...
     * numbers. This method is thread safe.
     *
    ...
    /**
     * A convenience method that supplies every transaction in this round to a consumer, along with
     * the transaction's event.
     *
