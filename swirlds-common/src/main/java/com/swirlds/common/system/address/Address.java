...
/**
 * One address in an address book, including all the info about a member. It is immutable. Each
 * getter for a byte array returns a clone of that array. The constructor clones all the arrays
 * passed to it. Each "copySet" simply returns a new deep copy with one variable different. So it
 * isn't a setter, but is a variant of the builder design pattern without a separate build() method.
 */
public class Address implements SelfSerializable {
    ...
    /**
     * Same as {@link #Address(long, String, String, long, boolean, byte[], int, byte[], int,
     * byte[], int, byte[], int, SerializablePublicKey, SerializablePublicKey,
     * SerializablePublicKey, String)} but with different key types. Deprecated, should use the
     ...
     * @param ownHost is that member running on the same machine as the member creating this
     *     address?
    ...
    /**
     * Write the Address to the given stream. It should later be read from the stream with
     * readAddress().
     *
    ...
    /**
     * Return a new Address object read from the given stream. It should have been written to the
     * stream with writeAddress().
     *
    ...
    public static Address readAddress(SerializableDataInputStream inStream, long version)
            throws IOException {
    ...
    public void deserialize(SerializableDataInputStream inStream, int version) throws IOException {
        ...
        switch (version) {
            case 1:
                ...
                break;
            case 2:
                // in version 2 the key type was written as a string and the key version was not
                // written
                sigPublicKey = new SerializablePublicKey();
                encPublicKey = new SerializablePublicKey();
                agreePublicKey = new SerializablePublicKey();
    ...
    public String toString() {
        return "Address{"
                ...
                + stake
                + ", addressInternalIpv4="
                + Arrays.toString(addressInternalIpv4)
                + ", portInternalIpv4="
                + portInternalIpv4
                + ", addressExternalIpv4="
                + Arrays.toString(addressExternalIpv4)
                + ", portExternalIpv4="
                + portExternalIpv4
                + ", addressInternalIpv6="
                + Arrays.toString(addressInternalIpv6)
                + ", portInternalIpv6="
                + portInternalIpv6
                + ", portExternalIpv6="
                + portExternalIpv6
                + ", addressExternalIpv6="
                + Arrays.toString(addressExternalIpv6)
                + ", sigPublicKey="
                + sigPublicKey
                + ", encPublicKey="
                + encPublicKey
