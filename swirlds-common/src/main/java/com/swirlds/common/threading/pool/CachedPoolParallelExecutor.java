...
public class CachedPoolParallelExecutor implements ParallelExecutor {
    ...
    /**
     * Run two tasks in parallel, the first one in the current thread, and the second in a
     * background thread.
     *
     * <p>This method returns only after both have finished.
     *
     * @param foregroundTask a task to execute in parallel
     * @param backgroundTask a task to execute in parallel
     * @param onThrow a cleanup task to be executed if an exception gets thrown. if the foreground
     *     task throws an exception, this could be used to stop the background task, but not vice
     *     versa
     * @throws ParallelExecutionException if either of the invoked tasks throws an exception. if
     *     both throw an exception, then the foregroundTask exception will be the cause and the
    ...
    public <T> T doParallel(
        ...
        try {
            result = foregroundTask.call();
                ...
                final Throwable e) { // NOSONAR: Any exceptions & errors that occur needs to trigger
            // onThrow.
        ...
        try {
            ...
            if (toThrow == null) {
                ...
                // if foregroundTask already threw an exception, we add this one as a suppressed
                // exception
