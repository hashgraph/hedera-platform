...
/**
 * This utility object allows a caller to get a {@link Future} on an object in a sequential stream
 * of objects. If two callers request a future on the same sequence element, then each caller
 * receives the same instance of the Future.
 *
 * @param <T> the type of the object held by the future
 */
public class SequentialFutures<T> {
    ...
    /**
     * The total number of values to store. Old values are removed from {@link #futures} to maintain
     * this limit.
     */
     ...
     *
     * @param nextIndex the next index of the sequence that will be completed
     * @param numberOfValuesToStore the number of previously received values to store
     * @param initialValues a method that is used to pre-populate values that come before the next
     *     index
     * @param gapHandler a method that is used to fill gaps in the sequence
     */
     ...
     * @throws IllegalStateException if an event in the past is requested
     */
    public Future<T> get(final long index) {
        final Future<T> future = getIfAvailable(index);
        if (future == null) {
            throw new IllegalStateException(
                    "requested index " + index + " is in the past and is unavailable");
        }
        return future;
    }

    /**
     * Get a {@link Future} for an event at a given index if it is available. Returns null if the
     * future is not available because the requested index is too far in the past.
     *
     * @param index the requested sequence index
    ...
    /**
     * Wait on a value. Syntactic sugar for SequentialFutures.{@link #get(long)}.{@link Future#get()
     * get()}.
     *
     * @param index the index of the event
     * @return an object returned by a future
     * @throws ExecutionException if there is an exception thrown while computing the value
     * @throws InterruptedException if this thread is interrupted before the value is returned
     * @throws IllegalStateException if an event in the past is requested
     */
    ...
    /**
     * Wait on a value and get it if it is available, or return null if the requested index is too
     * far in the past.
     *
     * @param index the index of the event
     * @return an object returned by a future, or null if the future is not available or if it has
     *     been cancelled
     * @throws ExecutionException if there is an exception thrown while computing the value
     * @throws InterruptedException if this thread is interrupted before the value is returned
     */
     ...
     *
     * @param index the index to fill until
     */
    ...
    /**
     * Make sure the next completion/cancellation index is valid (that is, it is greater or equal to
     * the lowest uncompleted index).
     *
     * @param index the index that is about to be completed/cancelled
     */
    private void assertCompletionIndexIsValid(final long index) {
        if (index < lowestUncompletedIndex) {
            throw new IllegalStateException(
                    "futures must be completed in increasing order, expected index >= "
                            + lowestUncompletedIndex
                            + ", provided index = "
                            + index);
        }
     ...
     * @param buildNewFuture a method called to build the future if it doesn't exist
     * @param updateExistingFuture a method called to update the future if it does exist
     */
     ...
     * completing the work. This method will have no effect if the future is already completed or
     * cancelled.
     *
     * <p>Futures must be handled in a strictly increasing order. Skipping indices causes the gap
     * handler to be called.
     *
     * @param index the index of the future to complete
     * @param value the value of the future
     * @throws IllegalStateException if the futures are completed in an incorrect order
     */
     ...
     * cancelled.
     *
     * <p>Futures must be handled in order without skipping any indices.
     *
     * @param index the index of the future to cancel
     * @throws IllegalStateException if the futures are completed in an incorrect order
     */
    ...
    /**
     * Signals the cancellation of the future with an associated exception. Intended for internal
     * use by the background process completing the work. This method will have no effect if the
     * future is already completed or cancelled.
     *
     * <p>Futures must be handled in order without skipping any indices.
     *
     * @param index the index of the future to cancel
     * @param error an exception that caused computation of the future to fail
     * @throws IllegalStateException if the futures are completed in an incorrect order
     */
     ...
     *
     * @param nextIndex the next index expected to be completed after the reset.
     * @param initialValues a method that is used to generate values that come before the next index
     */
    public synchronized void fastForwardIndex(
            final long nextIndex, final LongFunction<T> initialValues) {
        // Cancel and purge really old futures
        final List<Long> futuresToRemove = new LinkedList<>();
        futures.forEach(
                (final Long round, final StandardFuture<T> future) -> {
        ...
        // Generate futures for values that come before nextIndex, skipping indices that already
        // have values.
