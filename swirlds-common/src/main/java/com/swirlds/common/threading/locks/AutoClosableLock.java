...
/**
 * Similar to {@link java.util.concurrent.locks.Lock} but intended to be used by the
 * try-with-resources statement. Returns an object that will release the lock automatically at the
...
public interface AutoClosableLock {
     ...
     *
     * @return an instance that tells the caller if the lock has been acquired and provides a method
     *     to unlock it if it has
     */
    MaybeLocked tryLock();
     ...
     *
     * <p>Before waiting on the condition the lock must be held by the current thread. A call to
     * {@link Condition#await()} will atomically release the lock before waiting and re-acquire the
     * lock before the wait returns.
     *
