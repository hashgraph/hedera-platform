...
public class QueueThreadPoolImpl<T> extends AbstractBlockingQueue<T> implements QueueThreadPool<T> {
     ...
     *
     * @param configuration configuration for the thread pool
     */
    ...
    /**
     * Attempts to gracefully stop each thread in the pool
     *
     * <p>Should not be called before the threads have been started, or in the case of a seed before
     ...
     * the seed has been built.
     *
     * @param behavior the type of {@link StopBehavior} that should be used to stop each thread
     * @return
     */
    @Override
