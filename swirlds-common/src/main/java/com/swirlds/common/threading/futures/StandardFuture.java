 ...
 *
 * @param <T> the object type returned by the Future
 */
public class StandardFuture<T> implements Future<T> {
    ...
    public interface CancellationCallback {
         ...
         *
         * @param interrupt if there is a thread computing the value of the future, this specifies
         *     if that thread should be interrupted
         * @param exception the exception that caused this future to be cancelled, or null if this
         *     future was cancelled without an exception.
         */
    ...
    /**
     * Create a new future that is instantly completed. More efficient than creating a future and
     * then calling complete if the completion value is already known.
     *
     ...
     * Create a new {@link Future}.
     *
     * @param cancellationCallback if not null, this method will be called if the future is
     *     cancelled
     */
     ...
     * Create a new {@link Future}.
     *
     * @param completionCallback if not null, this method will be called if the future is completed
     */
     ...
     * @throws CancellationException if the computation was cancelled prior to completion
     * @throws InterruptedException if the current thread was interrupted while waiting
     * @throws ExecutionException if there was an exception on the thread computing the value
     */
    @Override
    ...
    /**
     * Same functionality as {@link #get(long, TimeUnit)}, but wraps any thrown {@link
     * ExecutionException}s in a {@link RuntimeException}.
     *
     * @param timeout the maximum time to wait
     * @param unit the time unit of the timeout argument
     * @return the computed result
     ...
     * @throws InterruptedException if the current thread was interrupted while waiting
     */
    public T getAndRethrow(final long timeout, final TimeUnit unit)
    ...
    /**
     * Helper method for signaling the completion of the future. Intended for internal use by the
     * background process completing the work. This method will have no effect if the future is
    ...
    /**
     * Helper method for signaling the cancellation of the future. Intended for internal use by the
     * background process cancelling the work. This method will have no effect if the future is
     * already completed or cancelled.
     */
    public void cancel() {
        cancel(false);
     ...
     * already completed or cancelled.
     *
     * @param error the error that caused the future to be cancelled
     */
    ...
    /**
     * Helper method for signaling the cancellation of the future with an associated exception.
     * Intended for internal use by the background process cancelling the work. This method will
     * have no effect if the future is already completed or cancelled.
     *
     * @param mayInterruptIfRunning if the thread that will complete this future should be
     *     interrupted
     * @param error the error that caused the future to be cancelled
     */
    public synchronized void cancelWithError(
