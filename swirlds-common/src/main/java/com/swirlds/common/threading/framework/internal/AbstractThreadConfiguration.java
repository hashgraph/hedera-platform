...
public abstract class AbstractThreadConfiguration<C extends AbstractThreadConfiguration<C>>
    ...
    /**
     * The thread's fully formatted name. This will be used for the thread name if not null.
     * Otherwise, the thread's name will be derived based on its configuration.
     */
    ...
    /**
     * Extracts the thread configuration from a given thread and loads it into this configuration
     * object.
     *
     ...
     * thread via {@link ThreadConfiguration#build()}. When work is finished, the calling thread is
     * restored back to its original configuration.
     *
    ...
    protected ThreadSeed buildThreadSeed() {
        ...
        return () -> {
            ...
            try {
                ...
                Objects.requireNonNull(getRunnable(), "runnable must not be null").run();
            } finally {
     ...
     * </pre>
     *
     * <p>If the fully formatted thread name has been set, then use that thread name instead of the
     * standard format.
     */
    ...
    /**
     * Configure thread properties. This method is able to set all properties for an unstarted
     * thread except for thread group. If the thread has already been started, then this method will
     * also not configure daemon status.
     *
    ...
    /**
     * Get the fully formatted thread name, or null if the fully formatted name has not been
     * specified.
     *
     ...
     *
     * @param fullyFormattedThreadName the exact thread name as it will appear, or null if the
     *     thread should use an algorithmically generated name
     * @return this object
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Get the node ID of the other node (if created threads will be dealing with a task related to
     * a specific node).
     */
    ...
    /**
     * Set the node ID of the other node (if created threads will be dealing with a task related to
     * a specific node). Ignored if null.
     *
     * @return this object
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Check if this configuration is immutable. A configuration becomes immutable once it is used
     * to create a thread, a factory, or a seed.
     *
