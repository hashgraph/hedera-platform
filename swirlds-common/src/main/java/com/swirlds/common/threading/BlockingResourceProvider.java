...
/**
 * A class that ensures thread safety where multiple providers compete to provide a resource to a
 * single consumer
 *
 * <ul>
 *   <li>The intended use case is multiple providers and a single consumer
 *   <li>All providers should call {@link #acquireProvidePermit()} before providing the resource
 *   <li>The permit will be unavailable until the consumer is blocked waiting for the resource
 *   <li>Once the consumer is waiting, one provider will acquire the permit and can call {@link
 *       #provide(Object)}
 *   <li>If the provider cannot provide the resource, it can release the permit for another provider
 *       to acquire
 *   <li>Once the provider provides the resource, it will be blocked until the consumer is finished
 *       and releases it
 *   <li>All other providers will not be able to acquire the permit until the process is finished
 *       and the consumer starts waiting again
 * </ul>
 *
...
public class BlockingResourceProvider<T> {
    ...
    /**
     * Provide the resource to the consumer. This method should only be called once the provide
     * permit has been acquired. This method will block until the consumer releases the resource.
     *
     * @param resource the resource to provide
     * @throws InterruptedException if the thread is interrupted while providing the resource
     */
     ...
     * @throws InterruptedException if interrupted while waiting for the resource
     */
