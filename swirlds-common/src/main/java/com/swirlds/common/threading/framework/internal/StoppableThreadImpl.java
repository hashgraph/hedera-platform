...
/**
 * Implements the concept of a thread that can be gracefully stopped. Once stopped this instance can
 * no longer be used and must be recreated.
 *
...
public class StoppableThreadImpl<T extends InterruptableRunnable>
    ...
    /**
     * The amount of time in milliseconds to wait after setting the thread status to {@link
     * Status#DYING} before interrupting the thread if {@link #stopBehavior} is {@link
    ...
    private final AtomicReference<CountDownLatch> pauseCompletedLatch =
            new AtomicReference<>(new CountDownLatch(1));

    /**
     * The minimum amount of time that a cycle is permitted to take. If a cycle completes in less
     * time, then sleep until the minimum period has been met.
     */
    ...
    /**
     * This latch is when joining a thread that was injected as a seed. When a seed is injected, the
     * thread may live after the stoppable thread finishes, and so join shouldn't wait for the
     * thread to actually die. Ignored if this thread was not injected.
     */
    ...
    /**
     * Get the current thread. Blocks until the thread has been started. This method is not
     * interruptable, so calling this method before the thread is started is a big commitment.
     *
     * @return the thread
     */
    ...
    public void join() throws InterruptedException {
        ...
        if (injected) {
            // This stoppable thread was injected into a pre-existing thread. Wait for the stoppable
            // thread to finish,
            // but don't worry about the underlying thread being stopped.
            finished.await();
        } else {
            // This stoppable thread was run on top of its own thread. Wait for that thread to be
            // closed.
            thread.get().join();
        }
    }
    ...
    public void join(final long millis, final int nanos) throws InterruptedException {
        ...
        if (injected) {
            ...
            // but don't worry about the underlying thread being stopped.
            finished.await(
                    (long) (millis + nanos * NANOSECONDS_TO_MILLISECONDS), TimeUnit.MILLISECONDS);
        } else {
            // This stoppable thread was run on top of its own thread. Wait for that thread to be
            // closed.
    ...
    /**
     * An implementation of close that will interrupt the work thread if it doesn't terminate
     * quickly enough.
     */
    ...
    /**
     * Perform the last cycle of work. Only called if {@link #stopBehavior} is {@link
     * com.swirlds.common.threading.framework.Stoppable.StopBehavior#BLOCKING BLOCKING}.
     *
     * @throws InterruptedException if the thread running the work is interrupted
     */
