...
public abstract class AbstractStoppableThreadConfiguration<
                C extends AbstractStoppableThreadConfiguration<C, T>,
                T extends InterruptableRunnable>
        extends AbstractThreadConfiguration<C> {

    public static final Stoppable.StopBehavior DEFAULT_STOP_BEHAVIOR =
    ...
    /**
     * If this thread can be interrupted, how long should be waited after a close request before
     * interrupting?
     */
    ...
    /**
     * If a thread is requested to stop but does not it is considered to be a hanging thread after
     * this period. If 0 then thread is never considered to be a hanging thread.
     */
    ...
    protected AbstractStoppableThreadConfiguration(
            final AbstractStoppableThreadConfiguration<C, T> that) {
        super(that);
    ...
    /**
     * Get the method that will be run after the thread is stopped. Ignored if {@link #stopBehavior}
     * is {@link com.swirlds.common.threading.framework.Stoppable.StopBehavior#INTERRUPTABLE
     * INTERRUPTABLE}.
     */
    ...
    /**
     * Set the method that will be run after the thread is stopped. Ignored if {@link #stopBehavior}
     * is {@link com.swirlds.common.threading.framework.Stoppable.StopBehavior#INTERRUPTABLE
     * INTERRUPTABLE}.
     *
     * @return this object
    ...
    /**
     * Get the minimum amount of time that a single cycle is allowed to take, or null if no such
     * minimum is defined.
     *
    ...
    /**
     * Set the minimum amount of time that a single cycle is allowed to take, or null if no such
     * minimum is defined.
     *
    ...
    /**
     * Get the period of time that must elapse after a stop request before this thread is considered
     * to be a hanging thread. A value of 0 means that the thread will never be considered to be a
     * hanging thread.
     */
    ...
    /**
     * Set the period of time that must elapse after a stop request before this thread is considered
     * to be a hanging thread. A value of 0 means that the thread will never be considered to be a
     * hanging thread.
     *
     * @return this object
