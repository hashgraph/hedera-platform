...
public interface ThreadManager extends Lifecycle {
     ...
     * @return a new Thread
     * @throws com.swirlds.common.utility.LifecycleException if called before the thread manager has
     *     been started
     */
    Thread createThread(ThreadGroup threadGroup, Runnable runnable);

    /**
     * Create a new thread factory. Thread factory will throw {@link
     * com.swirlds.common.exceptions.MutabilityException MutabilityException} if it is used to
     * create a thread before the thread manager is started.
     *
     * @param component the component name
     * @param threadName the thread name (each thread created by the factory will also have a unique
     *     number appended to the thread name)
