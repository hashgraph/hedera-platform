...
/**
 * This class is used for generating event stream files when enableEventStreaming is true, and for
 * calculating runningHash for consensus Events.
 */
public class EventStreamManager<
     ...
     * hashQueueThread and writeQueueThread
     */
    ...
    /**
     * receives consensus events from writeQueueThread, serializes consensus events to event stream
     * files
     */
    ...
    /**
     * When we freeze the platform, the last event to be written to EventStream file is the last
     * event in the freeze round. The freeze round is defined as the first round with a consensus
     * timestamp after the start of the freeze period.
     */
     ...
     * @param eventStreamQueueCapacity capacity of the blockingQueue from which we take events and
    ...
    public EventStreamManager(
        ...
        if (enableEventStreaming) {
            ...
            streamFileWriter =
                    new TimestampStreamFileWriter<>(
                            ...
                            signer,
                            /**
                             * when event streaming is started after reconnect, or at state
                             * recovering, startWriteAtCompleteWindow should be set to be true; when
                             * event streaming is started after restart, it should be set to be
                             * false
                             */
                            false,
        ...
        // receives consensus events from hashCalculator, calculates and set runningHash for this
        // event
        final RunningHashCalculatorForStream<T> runningHashCalculator =
    ...
    /**
     * @param multiStream the instance which receives consensus events from ConsensusRoundHandler,
     *     then passes to nextStreams
     * @param isLastEventInFreezeCheck a predicate which checks whether this event is the last event
     *     before restart
     */
    public EventStreamManager(
            final MultiStream<T> multiStream, final Predicate<T> isLastEventInFreezeCheck) {
    ...
    /**
     * receives a consensus event from ConsensusRoundHandler each time, sends it to multiStream
     * which then sends to two queueThread for calculating runningHash and writing to file
     *
    ...
    /**
     * sets startWriteAtCompleteWindow: it should be set to be true after reconnect, or at state
     * recovering; it should be set to be false at restart
     *
