...
public final class LinkedObjectStreamValidateUtils {
    ...
    /**
     * 1. validates if the stream file is valid, i.e., saved endRunningHash matches calculated
     * endRunningHash; 2. if the stream file is valid, then validates if the entireHash saved in the
     * signature file matches the hash calculated from the stream file; 3. if yes, then validates if
     * the signature file contains valid signatures of the entireHash and metaHash
     *
    ...
    public static StreamValidationResult validateFileAndSignature(
        ...
        try {
            Pair<StreamValidationResult, Hash> objectResult =
                    validateDirOrFile(streamFile, streamType);
            ...
            logger.error(
                    ...
                    () ->
                            new StreamParseErrorPayload(
                                    String.format(
                                            "validateFileAndSignature : failed to validate file %s",
                                            streamFile.getName())),
                    ex);
            ...
            logger.error(
                    ...
                    () ->
                            new StreamParseErrorPayload(
                                    String.format(
                                            "validateFileAndSignature : failed to calculate"
                                                    + " entireHash for %s",
                                            streamFile.getName())),
                    ex);
    ...
    /**
     * validates if the sigFile contains given entireHash, and contains valid signatures of the
     * entireHash and metaHash
     *
        ...
        try {
            ...
            logger.error(
                    ...
                    () ->
                            new StreamParseErrorPayload(
                                    String.format(
                                            "parseSigFile : fail to read signature from File %s",
                                            sigFile.getName())),
                    ex);
    ...
    /**
     * for a single object stream file, validates if it is valid, i.e., saved endRunningHash matches
     * calculated endRunningHash; for a directory of object stream files, validates if all files in
     * it are valid and chained
     *
    ...
    public static Pair<StreamValidationResult, Hash> validateDirOrFile(
            final File objectDirOrFile, final StreamType streamType)
            throws InvalidStreamFileException {
        return validateIterator(
                LinkedObjectStreamUtilities.parseStreamDirOrFile(objectDirOrFile, streamType));
    }
    ...
    public static Pair<StreamValidationResult, Hash> validateFileList(
        ...
        return validateIterator(
                LinkedObjectStreamUtilities.parseStreamFileList(fileList, streamType));
    }

    /**
     * Calculates a runningHash for given startRunningHash and objects in the iterator Verifies if
     * the endRunningHash in the Iterator matches the calculated RunningHash
     *
