...
public final class LinkedObjectStreamValidateUtils {
	 ...
	 */
	public static StreamValidationResult validateFileAndSignature(final File streamFile, final File sigFile,
			final PublicKey publicKey, final StreamType streamType) {
		...
		try {
			...
			LOGGER.error(EXCEPTION.getMarker(),
					() -> new StreamParseErrorPayload(
							String.format(
									"validateFileAndSignature : failed to calculate entireHash for %s",
									streamFile.getName())),
					ex);
	 ...
	 * @return validation result
	 */
	public static StreamValidationResult validateSignature(final Hash entireHash, final File sigFile,
			final PublicKey publicKey, final StreamType streamType) {
	...
	public static <T extends SelfSerializable> Pair<StreamValidationResult, Hash> validateIterator(
		...
		while (iterator.hasNext()) {
			...
			runningHash = CryptoFactory.getInstance().calcRunningHash(runningHash, objectHash, DigestType.SHA_384);
			LOGGER.info(OBJECT_STREAM.getMarker(),
					"validateIterator :: after consuming object {}," +
							"hash: {}, updated runningHash: {}",
					selfSerializable, objectHash, runningHash);
		}
		if (objectsCount == 0) {
		...
		if (!runningHash.equals(endHash)) {
			LOGGER.info(EXCEPTION.getMarker(), "calculated: {}, read: {}",
					runningHash, endHash);
