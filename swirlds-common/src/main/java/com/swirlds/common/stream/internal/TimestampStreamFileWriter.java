 ...
 * it may be written to an existing file, or a new file may be created to hold the object. This
 * object also generates signature files.
 *
 * <p>An important property of this stream is determinism. If two similarly configured instances of
 * this stream are fed the same sequence of objects, then both instances will produce the exact same
 * files.
 *
 * @param <T> the type of the object being streamed
 */
public class TimestampStreamFileWriter<
                T extends StreamAligned & RunningHashable & SerializableHashable & Timestamped>
    ...
    /**
     * Metadata should be written to this stream. Any data written to this stream is used to
     * generate a running metadata hash.
     */
    ...
    /**
     * If true, then only start writing when it can be guaranteed only complete files are written.
     * (This may cause several of the first objects that pass through the stream to be ignored.) If
     * false then immediately start writing when the first object is received.
     */
     ...
     * @param windowSizeMs the desired time window for a single file
     * @param signer an object that can sign things
     * @param startWriteAtCompleteWindow if true, then only start writing files when it can be
     *     guaranteed that partial files will not be written
     * @param streamType describes the type of object being passed through this stream
     */
    ...
    /**
     * Generate a signature file for the current object stream file. The signature bytes are
     * generated by signing the endRunningHash.
     *
     * @param entireHash a Hash calculated with all bytes in the entire stream file
     * @param entireSignature a Signature which is generated by signing the value of entireHash
     * @param metaHash a Hash calculated with metadata bytes in the stream file
     * @param metaSignature a Signature which is generated by signing the value of metaHash
     * @param sigFilePath path of the signature file to be written
     ...
     *
     * @param object the first object that will eventually be written to the new file
     */
                ...
                out =
                        new SerializableDataOutputStream(
                                new BufferedOutputStream(
                                        new HashingOutputStream(streamDigest, fileStream)));
                metadataOut =
                        new SerializableDataOutputStream(
    ...
    /** write the beginning part of the file: File Version ID, and initial runningHash */
    private void begin() {
        try {
            // write file header
    ...
    /**
     * Write the running hash to the current file and close it, and generate a signature file. Does
     * nothing if there is no file currently open.
     */
     ...
     * StreamAligned#NO_ALIGNMENT NO_ALIGNMENT}.
     *
     * <p>This method is public to allow direct access for unit testing. This method has side
     * effects, and should not be called without an understanding of those side effects.
     *
    ...
    private String generateStreamFilePath(final T object) {
        return directory
                + File.separator
                + generateStreamFileNameFromInstant(object.getTimestamp(), streamType);
    }
    ...
    public void addObject(final T object) {
        if (shouldStartNewFile(object)) {
            ...
            // if the file already exists, it will not be opened, so we don't write anything to it.
            // begin() would
        ...
        // if stream is null, it means startWriteAtCompleteWindow is true and we are still in the
        // first
        // incomplete
    ...
    /**
     * This method is called when the node falls behind. This method will delete any partially
     * written files.
     */
    @Override
    public void clear() {
    ...
    /**
     * Set if files should only be written in their entirety (as opposed to allowing files to be
     * partially created if the initial objects are not present). Should be set to be true after
     * reconnect, or at state recovering, and should be set to be false at restart
     */
