...
public final class LinkedObjectStreamUtilities {
	 ...
	 */
	public static <T extends SelfSerializable> Iterator<T> parseStreamDirOrFile(final File objectStreamDirOrFile,
			final StreamType streamType) throws InvalidStreamFileException {
		if (objectStreamDirOrFile.isDirectory()) {
			...
			throw new InvalidStreamFileException(
					String.format("the file %s doesn't match streamType %s",
							objectStreamDirOrFile.getName(),
							streamType));
		} else {
	 ...
	 * 		all stream objects contained in the directory, and the endRunningHash in the last stream file
	 */
	public static <T extends SelfSerializable> Iterator<T> parseStreamFileList(final Collection<File> files,
	 ...
	 */
	public static Pair<Pair<Hash, Signature>, Pair<Hash, Signature>> parseSigFile(final File file,
			final StreamType streamType) throws IOException, InvalidStreamFileException {
	 ...
	 */
	public static Hash computeMetaHash(final File file, final StreamType streamType) throws IOException,
			NoSuchAlgorithmException, InvalidStreamFileException {
		MessageDigest md = MessageDigest.getInstance(DigestType.SHA_384.algorithmName());
