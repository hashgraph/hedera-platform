...
/**
 * Utilities methods for: parsing stream files and stream signature files; generating fileName from
 * Instant; calculating period; reading start and end runningHash from a stream file; calculating
 * metaHash and entireHash for a stream file;
 */
public final class LinkedObjectStreamUtilities {
     ...
     */
    public static String generateStreamFileNameFromInstant(
            final Instant timestamp, final StreamType streamType) {
        return convertInstantToStringWithPadding(timestamp) + "." + streamType.getExtension();
    }

    /**
     * A string representation of the Instant using ISO-8601 representation, with colons converted
     * to underscores for Windows compatibility. The nano-of-second always outputs nine digits with
     * padding when necessary, to ensure same length filenames and proper sorting. examples: input:
     * 2020-10-19T21:35:39Z output: "2020-10-19T21_35_39.000000000Z"
     *
     * <p>input: 2020-10-19T21:35:39.454265Z output: "2020-10-19T21:35:39.454265000Z"
     *
    ...
    /**
     * get period number with given consensusTimestamp and logPeriodMs Object with different period
     * number should not be written in the same stream file
     *
    ...
    public static long getPeriod(final Instant consensusTimestamp, final long logPeriodMs) {
        final long nanos =
                consensusTimestamp.getEpochSecond() * SECONDS_TO_NANOSECONDS
                        + consensusTimestamp.getNano();
        return nanos / MILLISECONDS_TO_NANOSECONDS / logPeriodMs;
    }

    /**
     * Extracts the timestamp from event file name and converts it to {@code Instant}
     * representation.
     *
    ...
    /**
     * Parse a single stream file, return an Iterator from which we can get all SelfSerializable
     * objects in the file the first object is startRunningHash; the last object is endRunningHash;
    ...
    public static <T extends SelfSerializable> SingleStreamIterator<T> parseStreamFile(
        ...
        if (!streamType.isStreamFile(file)) {
            String msg =
                    String.format(
                            "Fail to parse File %s, its extension doesn't match %s",
                            file.getName(), streamType.getExtension());
    ...
    /**
     * if it is a single stream file of the given type, parse this file, and return an Iterator
     * which contains all SelfSerializables contained in the file.
     *
     * <p>if it is a directory, parse stream files of the given type in this directory in increasing
     * order by fileName, also validate if each file's startRunningHash matches its previous file's
     * endRunningHash. return an Iterator which contains the startRunningHash in the first stream
     * file, all stream objects contained in the directory, and the endRunningHash in the last
     * stream file
     *
    ...
    public static <T extends SelfSerializable> Iterator<T> parseStreamDirOrFile(
            final File objectStreamDirOrFile, final StreamType streamType)
            throws InvalidStreamFileException {
        if (objectStreamDirOrFile.isDirectory()) {
            return new StreamFilesIterator<>(
                    objectStreamDirOrFile.listFiles(streamType::isStreamFile), streamType);
        } else if (!streamType.isStreamFile(objectStreamDirOrFile)) {
            // throw an exception if it is not a stream file of given type
            throw new InvalidStreamFileException(
                    String.format(
                            "the file %s doesn't match streamType %s",
                            objectStreamDirOrFile.getName(), streamType));
        } else {
     ...
     * @param streamType type of stream files to be parsed
     * @param <T> type of the SelfSerializable objects written in the stream file
     * @return an Iterator which contains the startRunningHash in the first stream file, all stream
     *     objects contained in the directory, and the endRunningHash in the last stream file
     */
     ...
     */
    public static Hash readStartRunningHashFromStreamFile(
            final File file, final StreamType streamType) {
        SingleStreamIterator<SelfSerializable> singleStreamIterator =
                parseStreamFile(file, streamType);
     ...
     * @param streamType type of this stream file
     * @return a pair of two pairs: the first pair contains entireHash and entireSignature; the
     *     second pair contains metaHash and metaSignature
    ...
    public static Pair<Pair<Hash, Signature>, Pair<Hash, Signature>> parseSigFile(
            final File file, final StreamType streamType)
            throws IOException, InvalidStreamFileException {
        if (!streamType.isStreamSigFile(file.getName())) {
            throw new InvalidStreamFileException(
                    String.format(
                            "parseSigFile : fail to read signature from File %s, its extension"
                                    + " doesn't match %s",
            ...
            try (SerializableDataInputStream inputStream =
                ...
                return Pair.of(
                        Pair.of(entireHash, entireSignature), Pair.of(metaHash, metaSignature));
            }
        }
    }
     ...
     *     located or loaded
     */
    public static Hash computeEntireHash(final File file)
    ...
    /**
     * Computes the {@code Hash} representation of metadata of the stream file using the file
     * headers, file version, starting running hash, and ending running hash.
     *
     * @param file a file to be hashed
     * @param streamType type of this stream file
     * @return a {@code Hash} calculated from the file headers, file version, starting running hash,
     *     and ending running hash
