...
/**
 * an Iterator from which we can get all SelfSerializable objects contained in given files; the
 * files are parsed in increasing order by file names; the next file's startRunningHash is validated
 * against the endRunningHash of previous file; the first item is the startRunningHash in the first
 * stream file, then all stream objects contained in the directory, the last item is the
 * endRunningHash in the last stream file. Instances of this class are not thread-safe.
 */
public class StreamFilesIterator<T extends SelfSerializable> implements Iterator<T> {
    ...
    public StreamFilesIterator(final File[] files, final StreamType streamType) {
        ...
        currentFileIterator =
                idx < files.length
                        ? LinkedObjectStreamUtilities.parseStreamFile(files[idx], streamType)
                        : null;
    }
    ...
    private boolean fetchNext() {
        if (currentFileIterator == null
                || !currentFileIterator.hasNext() && idx == files.length - 1) {
            return false;
        }
    ...
    /**
     * open the next file, read startRunningHash and validate it against endRunningHash; if not
     * match, log an error and return false; else read and set next object
     *
     * @return
     */
        ...
        if (!(hash instanceof Hash)) {
            LOGGER.error(
                    ...
                    () ->
                            new StreamParseErrorPayload(
                                    String.format(
                                            "The first item %s in %s is not Hash",
                                            hash, files[idx].getName())));
            return false;
        }
        // update startRunningHash
        ...
        if (!startRunningHash.equals(endRunningHash)) {
            LOGGER.error(
                    ...
                    () ->
                            new StreamParseErrorPayload(
                                    String.format(
                                            "startRunningHash %s in %s doesn't match endRunningHash"
                                                    + " %s in %s",
                                            startRunningHash,
                                            files[idx].getName(),
                                            endRunningHash,
            ...
            LOGGER.error(
                    ...
                    () ->
                            new StreamParseErrorPayload(
                                    String.format(
                                            "file %s only contains startRunningHash",
                                            files[idx].getName())));
            return false;
        } else {
