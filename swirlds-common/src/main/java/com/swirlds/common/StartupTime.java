...
public final class StartupTime {
    ...
    /**
     * This method must be called when a node originally starts. Must be called prior to spawning of
     * threads that may read startup time. If called multiple times (for example, if there are
     * multiple local nodes) then only the first call will assign the startup time.
     */
    ...
    /**
     * Get the time that this node was started. Exact instant is captured at some time when the JVM
     * is starting.
     *
    ...
    /**
     * Get the time since this node was started. Exact instant is captured at some time when the JVM
     * is starting.
     *
