...
/**
 * Keep a running history of a double value vs. time. History is divided into at most maxbins
 * different bins. It records the min and max value, and the min and max time, for each bin.
 *
 * <p>There are many class variables. Other classes should only read, not write, those variables and
 * the elements of their arrays. This is not thread safe. There must never be one thread reading
 * these variables while another thread is calling any of the methods.
 */
public class StatsBuffer {

    /** if the value is less than max, return value + 1; else return this value */
    private static final IntBinaryOperator INC_BY_ONE_WITHIN_MAX =
            (value, max) -> {
                if (value < max) {
                    return value + 1;
                } else {
    ...
    /**
     * record() will ignore all inputs for startDelay seconds, starting from the first time it's
     * called
     */
    private final double startDelay;

    /**
     * 0 if storing all of history. Else store only recent history where each bin covers binSeconds
     * seconds.
     */
    ...
    /**
     * Store a history of samples combined into at most maxBins bins. If recentSeconds is zero, then
     * all of history is stored, with an equal number of samples in each bin. Otherwise, only the
     * last recentSeconds seconds of history is stored, with maxBins different bins each covering an
     * equal fraction of that period.
     *
     * <p>If recentSeconds &gt; 0, then empty bins are not stored, so some of the older bins (more
     * than recentSeconds old) can continue to exist until enough newer bins are collected to
     * discard them.
     *
     * <p>The maxBins must be even. If it's odd, it will be incremented, so passing in 99 is the
     * same as passing in 100.
     *
     * @param maxBins the maximum number of bins to store (must be even)
     * @param recentSeconds the max period of time covered by all the stored data, in seconds (or 0
     *     if covering all of history)
     * @param startDelay record() will ignore all inputs for the first startDelay seconds, starting
     *     from the first time it's called
     */
    ...
    /**
     * return the average of all x values in bin i, where i=0 is the oldest and i=numBins-1 is the
     * newest.
     *
    ...
    /**
     * return the average of all y values in bin i, where i=0 is the oldest and i=numBins-1 is the
     * newest.
     *
     * @param i the index for the bin
     * @return the average
    ...
    /**
     * return the min of all x values in bin i, where i=0 is the oldest and i=numBins-1 is the
     * newest.
     *
     * @param i the index for the bin
     * @return the average
    ...
    /**
     * return the min of all y values in bin i, where i=0 is the oldest and i=numBins-1 is the
     * newest.
     *
     * @param i the index for the bin
     * @return the average
    ...
    /**
     * return the max of all x values in bin i, where i=0 is the oldest and i=numBins-1 is the
     * newest.
     *
     * @param i the index for the bin
     * @return the average
    ...
    /**
     * return the max of all y values in bin i, where i=0 is the oldest and i=numBins-1 is the
     * newest.
     *
     * @param i the index for the bin
     * @return the average
    ...
    /**
     * return the standard deviation of all y values in bin i, where i=0 is the oldest and
     * i=numBins-1 is the newest.
     *
     * @param i the index for the bin
     * @return the average
     */
