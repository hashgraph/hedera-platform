...
/**
 * This class maintains a running average of some numeric value. It is exponentially weighted in
 * time, with a given half life. If it is always given the same value, then that value will be the
 * average, regardless of the timing.
 *
...
public class StatsRunningAverage implements StatsBuffered {
    ...
    /**
     * get the entire history of values of means of this RunningAverage. The caller should not
     * modify it.
     */
    @Override
    ...
    /**
     * get the recent history of values of means of this RunningAverage. The caller should not
     * modify it.
     */
    @Override
    ...
    /**
     * Instantiate a RunningAverage with the given halfLife and start the measurements right now.
     * This will calculate exponentially weighted averages of the values passed to recordValue(),
     * where the exponential weighting has a half life of halfLife seconds.
     *
    ...
    /**
     * Start over on the measurements and counts, to get an exponentially-weighted average of the
     * values passed to recordValue(), with the weighting having a half life of halfLife seconds.
     * This is equivalent to instantiating a new RunningAverage.
     *
     * @param halfLife half of the exponential weighting comes from the last halfLife seconds
     */
    @SuppressWarnings("removal")
    ...
    /**
     * Incorporate "value" into the running average. If it is the same on every call, then the
     * average will equal it, no matter how those calls are timed. If it has various values on
     * various calls, then the running average will weight the more recent ones more heavily, with a
     * half life of halfLife seconds, where halfLife was passed in when this object was
     * instantiated.
     *
    ...
    public void recordValue(final double value) {
        ...
        // StatsRunningAverage is not thread safe, despite this, it is accessed by many threads
        // throughout the platform
        // Until we do a full statistics refactor, this try catch is a safeguard against any issues
        // that might occur
        // from this issue
        try {
    ...
    /**
     * Get the average of recent calls to recordValue(). This is an exponentially-weighted average
     * of recent calls, with the weighting by time, not by number of calls to recordValue().
     *
