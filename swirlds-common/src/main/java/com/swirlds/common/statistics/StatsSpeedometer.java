 ...
 * every period, where its period is 0.1 seconds by default. If instantiated with gamma=0.9, then
 * half the weighting comes from the last 7 periods. If 0.99 it's 70 periods, 0.999 is 700, etc.
 *
 * <p>The timer starts at instantiation, and can be reset with the reset() method.
 *
...
public class StatsSpeedometer implements StatsBuffered {
    ...
    /**
     * Instantiate a Speedometer with the given halfLife and start the measurements right now. This
     * will calculate exponentially weighted averages of the number of times update() is called per
     * second. Where the exponential weighting has a half life of halfLife seconds. This will record
     * the history, so it is the same as using the constructor new StatsSpeedometer(halfLife, true).
     *
     ...
     * Instantiate a Speedometer with the given halfLife and start the measurements right now. This
     * will calculate exponentially weighted averages of the number of times update() is called per
     * second. Where the exponential weighting has a half life of halfLife seconds.
     *
     * @param halfLife half of the exponential weighting comes from the last halfLife seconds
    ...
    /**
     * Start over on the measurements and counts, to get an exponentially-weighted average number of
     * calls to cycle() per second, with the weighting having a half life of halfLife seconds. This
     * is equivalent to instantiating a new Speedometer. This will also record a history of values,
     * so calling this is the same as calling reset(halfLife, true).
     *
     * @param halfLife half of the exponential weighting comes from the last halfLife seconds
     */
    @Override
     ...
     * Start over on the measurements and counts, to get an exponentially-weighted average number of
     * calls to cycle() per second, with the weighting having a half life of halfLife seconds. This
     * is equivalent to instantiating a new Speedometer. If halfLife < 0.01 then 0.01 will be used.
     *
     * @param halfLife half of the exponential weighting comes from the last halfLife seconds
     * @param saveHistory true if a StatsBuffer of recent and all history should be created and used
     */
    ...
    /**
     * calling update(N) is equivalent to calling cycle() N times almost simultaneously. Calling
     * cycle() is equivalent to calling update(1). Calling update(0) will update the estimate of the
     * cycles per second by looking at the current time (to update the seconds) without incrementing
     * the count of the cycles. So calling update(0) repeatedly with no calls to cycle() will cause
     * the cycles per second estimate to go asymptotic to zero.
     *
     * <p>The speedometer initially keeps a simple, uniformly-weighted average of the number of
     * calls to cycle() per second since the start of the run. Over time, that makes each new call
     * to cycle() have less weight (because there are more of them). Eventually, the weight of a new
     * call drops below the weight it would have under the exponentially-weighted average. At that
     * point, it switches to the exponentially-weighted average.
     *
    ...
    /**
     * The same as update(numCycles), except this will only record a new data point if
     * recordData==true
     *
     * @param numCycles number of cycles to record
     * @return estimated number of calls to cycle() per second
