...
/**
 * Implementation of a reusable background thread that dispatches asynchronous work items to the
 * provided {@link AsyncOperationHandler} by removing the work items from the provided {@link
 * Queue}.
 */
public class IntakeDispatcher<
    ...
    /**
     * A {@link BiFunction} that accepts an {@link OperationProvider} and a list of work items then
     * returns an {@link AsyncOperationHandler} instance.
     */
    private final BiFunction<Provider, List<Element>, Handler> handlerSupplier;

    /**
     * The {@link ExecutorService} that provides the thread pool on which the {@link
     * OperationProvider} does its work.
     */
    ...
    /**
     * Constructor that initializes all internal variables and launches the background thread. All
     * background threads are launched with a {@link java.lang.Thread.UncaughtExceptionHandler} to
     * handle and log all exceptions thrown by the thread.
     *
     * <p>All threads constructed by this class are launched with the {@link
    ...
    public IntakeDispatcher(
        ...
        this.worker =
                new ThreadConfiguration()
                        ...
                        .setComponent(THREAD_COMPONENT_NAME)
                        .setThreadName(
                                String.format("%s intake dispatcher", elementType.getSimpleName()))
                        .setExceptionHandler(this::handleThreadException)
    ...
    /**
     * An {@link java.lang.Thread.UncaughtExceptionHandler} implementation to ensure that all
