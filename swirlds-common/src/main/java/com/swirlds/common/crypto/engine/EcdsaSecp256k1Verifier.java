...
public class EcdsaSecp256k1Verifier {
    ...
    public EcdsaSecp256k1Verifier() {
        try {
            ...
            final AlgorithmParameters params =
                    AlgorithmParameters.getInstance(ECDSA_SECP256K1.keyAlgorithm());
            ...
            algorithm =
                    Signature.getInstance(
                            ECDSA_SECP256K1.signingAlgorithm(), ECDSA_SECP256K1.provider());
    ...
    /**
     * Given a "raw" ECDSA signature as the concatenation of its {@code (r, s)} points as unsigned
     * big-endians, returns the ASN.1 DER encoding of the same signature.
     *
     * @param rawEcdsaSig the raw ECDSA signature
     * @return the ASN.1 DER encoding of the signature
     */
    public static byte[] asn1DerEncode(final byte[] rawEcdsaSig) {
        final byte[] r =
                minifiedPositiveBigEndian(Arrays.copyOfRange(rawEcdsaSig, 0, EC_COORD_SIZE));
        final byte[] s =
                minifiedPositiveBigEndian(
                        Arrays.copyOfRange(rawEcdsaSig, EC_COORD_SIZE, rawEcdsaSig.length));
    ...
    /**
     * Given a positive integer value as an <i>unsigned</i> big-endian byte array, returns the
     * minimal-sized <i>signed</i> big-endian byte array for that value.
     *
     * @param v a positive integer encoded in unsigned big-endian
     ...
     *
     * <p>The public key must be 64 bytes where the first 32 bytes are the x-coordinate of the
     * public key, and the second 32 bytes are the y-coordinate of the public key.
     *
     * <p>The signature must be 64 bytes where the first 32 bytes are the {code r} value of the
     * signature and the second 32 bytes are the {@code s} value of the signature.
     *
     * <p>All encodings are to be unsigned big-endian.
     *
     * @param rawSig the (r, s) signature to be verified
    ...
    private ECPublicKey asCryptographic(final byte[] pubKey) throws InvalidKeySpecException {
        ...
        final BigInteger yCoord =
                new BigInteger(1, Arrays.copyOfRange(pubKey, EC_COORD_SIZE, pubKey.length));
