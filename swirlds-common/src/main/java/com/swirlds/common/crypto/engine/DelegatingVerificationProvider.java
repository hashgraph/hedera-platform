...
/**
 * For Internal Use Only. This class will be deprecated and removed once the Platform transitions to
 * a minimum version requirement of JDK 15 or higher. This is short term stop gap solution to
 * address the lack of ED25519 support in older version of the JDK.
 */
public class DelegatingVerificationProvider
    ...
    /** {@inheritDoc} */
    @Override
    protected Object loadAlgorithm(final SignatureType algorithmType)
    ...
    protected Boolean handleItem(
        ...
        switch (algorithmType) {
            case ECDSA_SECP256K1:
                final EcdsaSecp256k1Verifier ecdsaSecp256k1Algo =
                        (EcdsaSecp256k1Verifier) algorithm;
                return ecdsaSecp256k1VerificationProvider.handleItem(
                        ecdsaSecp256k1Algo, algorithmType, item, unused);
            case ED25519:
                final Sign.Native ed25519Algo = (Sign.Native) algorithm;
                return ed25519VerificationProvider.handleItem(
                        ed25519Algo, algorithmType, item, unused);
            default:
                throw new NoSuchAlgorithmException();
        }
    }
