...
public class CryptoEngine implements Cryptography {
    ...
    /**
     * The digest provider instance that is used to compute hashes of {@link Message} instances and
     * byte arrays.
     */
    private final DigestProvider digestProvider;

    /**
     * The verification provider used to perform signature verification of {@link
     * TransactionSignature} instances.
     */
    ...
    /**
     * The verification provider used to delegate signature verification of {@link
     * TransactionSignature} instances to either the {@code ed25519VerificationProvider} or {@code
     * ecdsaSecp256k1VerificationProvider} as apropos.
     */
    ...
    private volatile IntakeDispatcher<
                    TransactionSignature, DelegatingVerificationProvider, AsyncVerificationHandler>
    ...
    /**
     * the {@link ConcurrentLinkedQueue} instance of {@link TransactionSignature} waiting for
     * verification
     */
    private volatile BlockingQueue<List<TransactionSignature>> verificationQueue;

    /**
     * the {@link ConcurrentLinkedQueue} instance of {@link Message} pending message digest
     * computation
     */
    ...
    public CryptoEngine(final CryptographySettings settings) {
        ...
        this.delegatingVerificationProvider =
                new DelegatingVerificationProvider(
     ...
     * @param workItems the {@link List} of items to be processed by the created {@link
     *     AsyncOperationHandler} implementation
    ...
    private static TransactionSignature wrap(
        ...
        final ByteBuffer buffer =
                ByteBuffer.allocate(data.length + signature.length + publicKey.length);
    ...
    private static boolean verifySyncInternal(
        ...
        try {
            ...
            signature.setSignatureStatus(
                    isValid ? VerificationStatus.VALID : VerificationStatus.INVALID);
    ...
    public Future<Boolean> verifyAsync(
            ...
            final byte[] publicKey,
            final SignatureType signatureType) {
        final TransactionSignature wrappedSignature =
                wrap(data, signature, publicKey, signatureType);
        try {
    ...
    public boolean verifySync(
        ...
        if (signatureType == SignatureType.ECDSA_SECP256K1) {
            return ecdsaSecp256k1VerificationProvider.compute(
                    data, signature, publicKey, signatureType);
        } else {
    ...
    /**
     * Applies any changes in the {@link CryptoEngine} settings by stopping the {@link
     * IntakeDispatcher} threads, applying the changes, and relaunching the {@link IntakeDispatcher}
     * threads.
     */
     ...
     * @return an {@link AsyncOperationHandler} implementation
     */
    private AsyncDigestHandler digestHandler(
            final DigestProvider provider, final List<Message> workItems) {
     ...
     * @return the cryptographic hash for the given message contents
     */
    private Hash digestSyncInternal(
