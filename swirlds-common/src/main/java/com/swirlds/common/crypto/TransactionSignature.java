...
/**
 * Encapsulates a cryptographic signature along with the public key to use during verification. In
 * order to maintain the overall throughput and latency profiles of the hashgraph implementation,
 * this class is an immutable representation of a cryptographic signature. Multiple overloaded
 * constructors have been provided to facilitate ease of use when copying an existing signature.
 */
public class TransactionSignature implements Comparable<TransactionSignature> {
    ...
    /**
     * Constructs an immutable Ed25519 signature using the provided signature pointer, public key
     * pointer, and original message pointer.
     *
     ...
     * @throws IllegalArgumentException if any of the offsets or lengths fall outside the bounds of
    ...
    /**
     * Constructs an immutable signature of the given cryptographic algorithm using the provided
     * signature pointer, public key pointer, and original message pointer.
     *
     * @param contents a pointer to a byte buffer containing the message, signature, and public key
     * @param signatureOffset the index where the signature begins in the contents array
     * @param signatureLength the length of the signature (in bytes)
    ...
    public TransactionSignature(
        ...
        if (messageLength < 0
                ...
                || messageLength + messageOffset > contents.length) {
    ...
    /**
     * Constructs a shallow copy of an existing signature replacing the public key indices and
    ...
    /**
     * Constructs a shallow copy of an existing signature replacing the public key source, public
     * key indices and original message indices with the provided values.
     *
     * @param other the Signature to be copied
     * @param expandedPublicKey an optional byte array from which retrieve the public key
     * @param publicKeyOffset an updated public key offset
    ...
    public TransactionSignature(
        ...
        final byte[] publicKeySource =
                (expandedPublicKey != null) ? expandedPublicKey : other.contents;
    ...
    /**
     * Writes a binary representation of this signature, public key, and message to a {@link
     * DataOutputStream}.
     *
     * <p>Maximum number of bytes written is represented by the formula: signature.length +
     * publicKey.length + message.length + (7 * Integer.BYTES)
     *
     * <p>The minimum number of bytes written is represented by the formula: signature.length + (7 *
     * Integer.BYTES)
     *
     ...
     * @param byteCount returns the number of bytes written as the first element in the array or
     *     increments the existing value by the number of bytes written
    ...
    private static void serialize(
        ...
        // Write Public Key
        dos.write(
                (signature.expandedPublicKey != null)
                        ? signature.expandedPublicKey
                        : signature.contents,
     ...
     */
    public static TransactionSignature deserialize(
        ...
        if (sigLen < 0 || sigChecksum != (439 - sigLen)) {
            throw new BadIOException(
                    "Signature.deserialize tried to create signature array of length "
                            + sigLen
                            + " with wrong checksum.");
        }
        ...
        if (pkLen < 0 || pkChecksum != (541 - pkLen)) {
            throw new BadIOException(
                    "Signature.deserialize tried to create public key array of length "
                            + pkLen
                            + " with wrong checksum.");
        }
        ...
        if (msgLen < 0 || msgChecksum != (647 - msgLen)) {
            throw new BadIOException(
                    "Signature.deserialize tried to create message array of length "
                            + pkLen
                            + " with wrong checksum.");
        }
     ...
     *
     * <p>This method is thread-safe and guaranteed to be atomic in nature.
     *
    ...
    /**
     * Internal use accessor that returns a direct (mutable) reference to the transaction
     * contents/payload. Care must be taken to never modify the array returned by this accessor.
     * Modifying the array will result in undefined behaviors and will result in a violation of the
     * immutability contract provided by the {@link TransactionSignature} object.
     *
     * <p>This method exists solely to allow direct access by the platform for performance reasons.
     *
    ...
    /**
     * Internal use accessor that returns a direct (mutable) reference to the expanded public key.
     * Care must be taken to never modify the array returned by this accessor. Modifying the array
     * will result in undefined behaviors and will result in a violation of the immutability
     * contract provided by the {@link TransactionSignature} object.
     *
     * <p>This method exists solely to allow direct access by the platform for performance reasons.
     *
     * @return a direct reference to the transaction content/payload
    ...
    /**
     * Returns the offset where the public key begins. By default this is an index into the {@link
     * #getContents()} array. If the {@link #getExpandedPublicKey()} is provided, then this is an
     * index in the {@link #getExpandedPublicKey()} array.
     *
    ...
    /**
     * Internal use only setter for assigning or updating the {@link Future} attached to this
     * signature.
     *
    ...
    /**
     * Returns the status of the signature verification. If the transaction does not yet have
     * consensus then the value may be {@link VerificationStatus#UNKNOWN}; however, once the
     * transaction reaches consensus then the value must not be {@link VerificationStatus#UNKNOWN}.
     *
     ...
     * Returns a {@link Future} which will be completed once this signature has been verified.
     *
     * <p>NOTE: This method will wait indefinitely for the future to become available
     *
     * @return a future linked to the signature verification state
     ...
     * @throws NullPointerException if the {@code digest} parameter is null
     */
    public void computeDigest(final MessageDigest digest) {
        ...
        HashUtils.update(
                digest,
                (expandedPublicKey != null) ? expandedPublicKey : contents,
                publicKeyOffset,
                publicKeyLength);
     ...
     * does return distinct integers for distinct objects. (The hashCode may or may not be
     * implemented as some function of an object's memory address at some point in time.)
     *
    ...
    public String toString() {
        return "TransactionSignature{"
                ...
                + ", expandedPublicKey="
                + Arrays.toString(expandedPublicKey)
                + ", messageOffset="
                + messageOffset
                + ", messageLength="
                + messageLength
