...
public class SerializablePublicKey implements SelfSerializable {
    ...
    /**
     * Converts an encoded public key representation from the given {@code bytes} argument to a
     * {@link PublicKey} instance.
     *
     * @param bytes the encoded public key
     * @param keyType the JCE key algorithm identifier
     * @return the public key
     * @throws CryptographyException if the algorithm is not available or the encoded key is invalid
     */
     ...
     * @param algorithm the algorithm of the key, this was not stored in the stream before
     * @throws IOException thrown if an IO error happens
     */
    public void deserializeVersion0(SerializableDataInputStream in, String algorithm)
            throws IOException {
        keyType = KeyType.valueOf(algorithm);
        byte[] keyBytes = in.readByteArray(MAX_KEY_LENGTH);
    ...
    public String toString() {
        return "SerializablePublicKey{"
                + "publicKey="
                + (publicKey == null ? null : Arrays.toString(publicKey.getEncoded()))
                + ", keyType="
                + keyType
                + '}';
    }
