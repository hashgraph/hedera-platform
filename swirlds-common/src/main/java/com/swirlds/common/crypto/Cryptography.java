...
public interface Cryptography {
    ...
    /**
     * Computes a cryptographic hash (message digest) for the given message. The resulting hash
     * value will be returned by the {@link Message#getHash()} method once the future (accessible
     * from {@link Message#getFuture()}) has been completed.
     *
     * <p>Note: This implementation is non-blocking and returns almost immediately.
     *
    ...
    /**
     * Computes a cryptographic hash (message digest) for the given list of messages. The resulting
     * hash value will be returned by the {@link Message#getHash()} method once the future
     * (accessible from {@link Message#getFuture()}) has been completed.
     *
     * <p>Note: This implementation is non-blocking and returns almost immediately.
     *
     * @param messages a list of messages to be hashed
     */
    void digestAsync(final List<Message> messages);

    /**
     * Computes a cryptographic hash (message digest) for the given message. Convenience method that
     * defaults to {@link DigestType#SHA_384} message digests.
     *
     * <p>Note: This implementation is non-blocking and returns almost immediately.
     *
     * @param message the message contents to be hashed
     * @return a {@link Future} containing the cryptographic hash for the given message contents
     *     when resolved
     */
    ...
    /**
     * Computes a cryptographic hash for the {@link SelfSerializable} instance by serializing it and
     * hashing the bytes. The hash is then returned by this method
     *
     * @param serializable the object to be hashed
     * @param digestType the type of digest used to compute the hash
     * @return the cryptographic hash for the {@link SelfSerializable} object
    ...
    /**
     * Same as {@link #digestSync(SerializableHashable, DigestType, boolean)} with setHash set to
     * true
     */
    default Hash digestSync(
            final SerializableHashable serializableHashable, final DigestType digestType) {
        return digestSync(serializableHashable, digestType, DEFAULT_SET_HASH);
    }

    /**
     * Computes a cryptographic hash for the {@link SerializableHashable} instance by serializing it
     * and hashing the bytes. The hash is then passed to the object by calling {@link
     * Hashable#setHash(Hash)} if setHash is true.
     *
     * @param serializableHashable the object to be hashed
     * @param digestType the type of digest used to compute the hash
     ...
     */
    Hash digestSync(
            final SerializableHashable serializableHashable,
    ...
    /**
     * Verifies the given digital signature for authenticity. The result of the verification will be
     * returned by the {@link TransactionSignature#getSignatureStatus()} method once the future
     * (available via {@link TransactionSignature#getFuture()}) has been completed.
     *
     * <p>Note: This implementation is non-blocking and returns almost immediately.
     *
    ...
    /**
     * Verifies the given digital signatures for authenticity. The result of the verification will
     * be returned by the {@link TransactionSignature#getSignatureStatus()} method once the future
     * (available via {@link TransactionSignature#getFuture()}) has been completed.
     *
     * <p>Note: This implementation is non-blocking and returns almost immediately.
     *
     ...
     * @return a {@link Future} that will contain the true if the signature is valid; otherwise
     *     false when resolved
     */
    default Future<Boolean> verifyAsync(
            final byte[] data, final byte[] signature, final byte[] publicKey) {
    ...
    /**
     * Computes a cryptographic hash for the concatenation of current running Hash and the given
     * newHashToAdd. return the calculated running Hash
     *
     ...
     */
    Hash calcRunningHash(
            final Hash runningHash, final Hash newHashToAdd, final DigestType digestType);
}
