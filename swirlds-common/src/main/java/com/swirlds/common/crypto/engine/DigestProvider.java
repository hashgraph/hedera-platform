...
/**
 * Implementation of a message digest provider. This implementation depends on the JCE {@link
 * MessageDigest} providers and supports all algorithms supported by the JVM.
 */
public class DigestProvider
     ...
     *
     * @param msg the message for which to compute a message digest
     ...
     *
     * @param msg the message for which to compute a message digest
     * @param algorithmType the required algorithm to be used when computing the message digest
     * @return the message digest as an array of the raw bytes
     * @throws NoSuchAlgorithmException if an implementation of the required algorithm cannot be
     *     located or loaded
     */
    protected Hash compute(final byte[] msg, final DigestType algorithmType)
    ...
    /**
     * Computes the result of the cryptographic transformation using the given subset of bytes from
     * the provided message. This implementation defaults to an SHA-384 message digest and is
     * provided for convenience.
     *
     * @param msg the message for which to compute a message digest
     * @param offset the starting offset to begin reading
     ...
     * @param offset the starting offset to begin reading
     * @param length the total number of bytes to read
     * @param algorithmType the message digest algorithm to be used
     * @return the message digest as an array of the raw bytes
     * @throws NoSuchAlgorithmException if an implementation of the required algorithm cannot be
     *     located or loaded
     */
    private Hash compute(
            final byte[] msg, final int offset, final int length, final DigestType algorithmType)
            throws NoSuchAlgorithmException {
    ...
    /** {@inheritDoc} */
    @Override
    protected MessageDigest handleAlgorithmRequired(final DigestType algorithmType)
            throws NoSuchAlgorithmException {
    ...
    protected Hash handleItem(
        ...
        return new Hash(
                compute(algorithm, item.getPayloadDirect(), item.getOffset(), item.getLength()),
                algorithmType);
    }
     ...
     * provided for convenience.
     *
     * @param algorithm the required algorithm implemented to be used
     * @param msg the message for which to compute a message digest
     * @param offset the starting offset to begin reading
     * @param length the total number of bytes to read
     * @return the message digest as an array of the raw bytes
    ...
    private byte[] compute(
            final MessageDigest algorithm, final byte[] msg, final int offset, final int length) {
        algorithm.reset();
