...
public enum FCQHashAlgorithm {
    ...
    ROLLING_HASH(
            FCQHashAlgorithm::rollingHashCalculator,
            FCQHashAlgorithm::removalRollingHashCalculator),
    MERKLE_HASH(
            FCQHashAlgorithm::merkleHashCalculator, FCQHashAlgorithm::removalMerkleHashCalculator);
     ...
     * hash is the sum of the hashes of the elements, modulo 2^384.
     *
     * <p>Note, for applications like Hedera, for the queue of receipts or queue of records, each
     ...
     *
     * @param localHash current hash
     * @param elementHash hash of element to add
     */
    private static void sumHashCalculator(
            final byte[] localHash, final byte[] elementHash, final int exponent) {
     ...
     * @param elementHash hash of element to remove
     */
    private static void removalSumHashCalculator(
            final byte[] hash, final byte[] elementHash, final int exponent) {
    ...
    /**
     * This is a rolling hash, so it takes into account the order. if the queue contains {a,b,c,d},
     * where "a" is the head and "d" is the tail, then define:
     *
     * <p>hash64({a,b,c,d}) = a * 3^^3 + b * 3^^2 + c * 3^^1 + d * 3^^0 mod 2^^64 hash64({a,b,c}) =
     * a * 3^^2 + b * 3^^1 + c * 3^^0 mod 2^^64 hash64({b,c,d}) = b * 3^^2 + c * 3^^1 + d * 3^^0 mod
     * 2^^64
     *
     * <p>Which implies these:
     *
     * <p>hash64({a,b,c,d}) = hash64({a,b,c}) * 3 + d mod 2^^64 //add(d) hash64({b,c,d}) =
     * hash64({a,b,c,d}) - a * 3^^3 mod 2^^64 //remove() deletes a d a * 3^^2 + b * 3^^1 + c*3^^0
     * rollingHashSize = 3 currentSize = 4 diff = currentSize - rollingHashSize = 1 currenthHash *=
     * power(3, diff) a * 3^^3 + b * 3^^2 + c*3^^1
     *
     * <p>so we add an element by multiplying by 3 and adding the new element's hash, and we remove
     * an element by subtracting that element times 3 to the power of the resulting size.
     *
     * <p>This is all easy to do for a 64-bit hash by keeping track of 3^^size modulo 2^^64, and
     * multiplying it by 3 every time the size increments, and multiplying by the inverse of 3 each
     * time it decrements. The multiplicative inverse of 3 modulo 2^^64 is 0xaaaaaaaaaaaaaaab
     * (that's 15 a digits then a b).
     *
     ...
     * fast
     */
    private static void rollingHashCalculator(
            final byte[] localHash, final byte[] elementHash, final int exponent) {
    ...
    private static void removalRollingHashCalculator(
            final byte[] hash, final byte[] elementHash, final int exponent) {
        final long power = power(HASH_RADIX, exponent);
    ...
    private static void merkleHashCalculator(
            final byte[] hash, final byte[] elementHash, final int exponent) {
        throw new UnsupportedOperationException("Hash algorithm Merkle is not supported");
    }

    private static void removalMerkleHashCalculator(
            final byte[] hash, final byte[] elementHash, final int exponent) {
        throw new UnsupportedOperationException("Hash algorithm Merkle is not supported");
    }
