...
public enum FCQHashAlgorithm {
	...
	FCQHashAlgorithm(final TriConsumer<byte[], byte[], Integer> hashCalculator,
	 ...
	 */
	private static void sumHashCalculator(final byte[] localHash,
	 ...
	 */
	private static void removalSumHashCalculator(final byte[] hash,
			final byte[] elementHash,
			final int exponent) {
	 ...
	 */
	private static void rollingHashCalculator(final byte[] localHash,
			final byte[] elementHash,
			final int exponent) {
		//    hash64({a,b,c,d}) = a * 3^^3 + b * 3^^2 + c * 3^^1 + d * 3^^0 mod 2^^64
	...
	private static void removalRollingHashCalculator(final byte[] hash,
			final byte[] elementHash,
			final int exponent) {
		final long power = power(HASH_RADIX, exponent);
	...
	private static void merkleHashCalculator(final byte[] hash,
			final byte[] elementHash,
			final int exponent) {
		throw new UnsupportedOperationException("Hash algorithm Merkle is not supported");
	}

	private static void removalMerkleHashCalculator(final byte[] hash,
			final byte[] elementHash,
			final int exponent) {
		throw new UnsupportedOperationException("Hash algorithm Merkle is not supported");
	}
