...
/**
 * Singleton factory for loading and registering {@link FCQueue} statistics. This is the primary
 * entry point for all {@link SwirldMain} implementations that wish to track {@link FCQueue}
 * statistics.
 */
public class FCQueueStatistics {
    ...
    private static final RunningAverageMetric.Config FCQ_ADD_EXECUTION_MICROS_CONFIG =
            new RunningAverageMetric.Config(FCQUEUE_CATEGORY, "fcqAddExecMicroSec")
                    .withDescription(
                            "avg time taken to execute the FCQueue add method, not including locks"
                                    + " (in microseconds)")
    ...
    private static final RunningAverageMetric.Config FCQ_REMOVE_EXECUTION_MICROS_CONFIG =
            new RunningAverageMetric.Config(FCQUEUE_CATEGORY, "fcqRemoveExecMicroSec")
                    .withDescription(
                            "avg time taken to execute the FCQueue remove method, not including"
                                    + " locks (in microseconds)")
                    .withFormat(FORMAT_9_6);
