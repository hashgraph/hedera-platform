...
public class SlowMockFCQueue<E extends FastCopyable & SerializableHashable> extends FCQueue<E> {
    ...
    /**
     * the original FCQueue created with "new". Shared by the whole queue group, so every method
     * synchronizes on it
     */
    ...
    public Hash getHash() {
        ...
        for (Iterator<FCQueueNode<E>> it = nodeIterator(); it.hasNext(); ) {
            ...
            if (HASH_ALGORITHM == FCQHashAlgorithm.SUM_HASH) {
                ...
                //	 but we're lazy and skipped the carry - also means this could be done as a
                // vector[6]
