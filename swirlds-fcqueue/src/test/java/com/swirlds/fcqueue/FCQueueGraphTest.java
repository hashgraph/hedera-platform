...
class FCQueueGraphTest {
    ...
    void printHeader() {
        System.out.println(
                "%%{init: { 'theme': 'neutral', 'fontFamily': 'arial', 'fontSize': '14px' } }%%\n");
    ...
    void refCountForInsertions(final int limit) {
        ...
        for (int index = 0; index < limit; index++) {
            ...
            printMermaidGraph(
                    String.format("After adding N%d", index),
                    limit,
    ...
    void refCountForInsertionsAndDeletions(final int limit) {
        ...
        printMermaidGraph(
                String.format("After adding N%d", limit - 1),
                limit,
    ...
    void refCountForInsertionsCopyingAndReleasing(final int limit) {
        ...
        for (int index = 0; index < limit; index++) {
            ...
            printMermaidGraph(
                    String.format("After releasing FCQ%d", index),
                    limit,
                    String.format("re%d", index),
                    copies,
                    links);
        }
    ...
    void refCountForInsertionsCopyingInsertionsAndRemovals(final int limit) {
        ...
        printMermaidGraph(
                "After copying FCQ0", limit, String.format("icirc%d", limit - 1), copies, links);
        ...
        printMermaidGraph(
                "After inserting N13",
                mutableQueue.size(),
                String.format("icirci%d", 2 * limit - 1),
                copies,
                links);
        ...
        printMermaidGraph(
                "After removing N6",
                mutableQueue.size(),
                String.format("icircir%d", 2 * limit - 1),
                copies,
                links);
