...
public class Throttle {
	 ...
	 */
	public synchronized void setTps(final double tps) {
	 ...
	 */
	public synchronized void setBurstPeriod(final double burstPeriod) {
	 ...
	 */
	public Throttle(final double tps) {
		this(tps, 1);
	}
	 ...
	 * 		bursts can allow at most this many seconds' worth of transactions at once (negative values treated as 0)
	 */
	public Throttle(final double tps, final double burstPeriod) {
		this.tps = Math.max(0, tps);
		this.burstPeriod = Math.max(0, burstPeriod);
		this.capacity = this.tps * this.burstPeriod;
		this.traffic = 0;
