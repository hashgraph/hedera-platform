...
public class StartupThrottle
     ...
     *
     * @param addressBook the {@code AddressBook} to use
     * @param selfId the ID this node
     * @param statusManager a {@code PlatformStatusManager} implementor
     * @param enableBetaMirror true iff mirror nodes are enabled. See {@code
    ...
    public StartupThrottle(
        ...
        if (enableBetaMirror) {
            // only count nodes with non-zero stake, excluding self from the count if the local node
            // has stake
    ...
    public EventCreationRuleResponse shouldCreateEvent() {
        ...
        // if this node has created an event, but someone else hasn't, this node should not create
        // anymore events
        // else if this node should wait for node 0 to create an event first, this node should not
        // create any events
        if (numNotStarted.get() > 0
                && (hasNodeStarted(selfId.getIdAsInt()) || shouldWaitForNode0())) {
            return DONT_CREATE;
        }
        return PASS;
    }
    ...
    public void nodeStarted(long nodeId) {
        ...
        if (numNotStarted.get() > 0) {
            ...
            if (wasStarted == null) {
                // if this is the first event received for that member in all of history, or since
                // the restart
            ...
            log.info(
                    STARTUP.getMarker(),
                    "Node {} started with wasStarted {} and numNotStarted {}",
                    nodeId,
                    wasStarted,
                    numNotStarted);
    ...
    /**
     * Returns whether a node has been started or not, by looking at whether we have received any
    ...
    /**
     * An implementation that does not throttle. This is a temporary implementation, at some point
     * we should not depend on this class directly so we can just not instantiate it if its not
     * needed
     */
