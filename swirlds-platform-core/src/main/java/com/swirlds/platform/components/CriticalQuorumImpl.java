...
public class CriticalQuorumImpl implements CriticalQuorum {
    ...
    /**
     * A map from possible thresholds to stakes. The given stake is the stake of all nodes that do
     * not exceed the threshold.
     */
    private final Map<Integer, Long> stakeNotExceedingThreshold;

    /**
     * Any nodes with an event count that does not exceed this threshold are considered to be part
     * of the critical quorum.
     */
    private final AtomicInteger threshold;

    /**
     * The current round. Observing an event from a higher round will increase this value and reset
     * event counts.
     */
    private long round;
    ...
    public void eventAdded(final EventImpl event) {
        ...
        final long originalStakeAtThreshold =
                stakeNotExceedingThreshold.getOrDefault(originalEventCount, totalState);
