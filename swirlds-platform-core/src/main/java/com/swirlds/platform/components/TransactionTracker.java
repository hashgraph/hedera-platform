...
/**
 * Tracks user transactions that have not reached consensus, as well as the round at which the last
 * one reached consensus.
 */
public class TransactionTracker
        implements EventAddedObserver, ConsensusRoundObserver, StaleEventObserver {
    /**
     * the number of non-consensus, non-stale events with user transactions currently in the
     * hashgraph
     */
    ...
    /**
     * the last round received after which there were no non consensus user transactions in the
     * hashgraph, but before this round there were
     */
    private volatile long lastRoundReceivedAllTransCons;

    /**
     * Default-construct a {@code TransactionTracker} instance. The resulting instance contains no
     * transaction information, and returns a round number of -1.
     */
    ...
    public void consensusRound(final ConsensusRound consensusRound) {
        for (EventImpl event : consensusRound.getConsensusEvents()) {
            ...
            if (event.hasUserTransactions()) {
                ...
                // we decrement the numUserTransEvents for every event that has user transactions.
                // If the counter
    ...
    /**
     * @return the number of events with user transactions currently in the hashgraph that have not
     *     reached consensus
     */
    ...
    /**
     * @return the last round received after which there were no non consensus user transactions in
     *     the hashgraph
     */
     ...
     *
     * @param lastRRwithUserTransaction the new value of the last such round
     */
