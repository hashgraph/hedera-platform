...
public class EventCreator {
     ...
     * @param transactionSupplier this method supplies transactions that should be inserted into
    ...
    public boolean createEvent(final long otherId) {
        ...
        //
        // We don't want to create an event if there are no user transactions ready to be put in an
        // event.
        //
        // We still want to create an event if there are state signature transactions when we are
        // frozen.
        ...
        if (eventCreationRules.shouldCreateEvent(selfParent, otherParent)
    ...
    protected boolean hasOtherParentAlreadyBeenUsed(final long otherId) {
        return !selfId.equalsMain(otherId)
                && eventMapper.hasMostRecentEventBeenUsedAsOtherParent(otherId);
    }

    /**
     * Check if there are signature transactions waiting to be inserted into an event during a
     * freeze
     */
     ...
     *
     * <p>If there are no user transactions waiting to be included in an event, there is no reason
     * to create an event for the purposes of user transactions.
     *
     * <p>If there are user transactions waiting to be included in an event but there are user
     * transactions in the hashgraph that have not yet reached consensus, we should not create an
     * event in order to slow event creation. We must receive more events from peers to help the
     * existing user transactions in the hashgraph to reach consensus. We should not overwhelm the
     * graph with our events.
     *
     * @return true if there are no user transactions ready to be put into an event
     */
    protected boolean hasNoUserTransactionsReady() {
        return transactionPool.numTransForEvent() == 0
                || transactionTracker.getNumUserTransEvents() > 0;
    }
