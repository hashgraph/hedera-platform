...
/**
 * This class is used for checking whether should create an event or not. It contains a list of
 * {@link EventCreationRule}s, which are checked one by one. Once a rule has a firm answer such as
 * CREATE or DONT_CREATE, the answer is returned; else we continue checking the next rule.
 */
public class EventCreationRules implements EventCreationRule, ParentBasedCreationRule {
    ...
    public EventCreationRuleResponse shouldCreateEvent(
        ...
        for (final ParentBasedCreationRule rule : parentRules) {
            final EventCreationRuleResponse response =
                    rule.shouldCreateEvent(selfParent, otherParent);
            if (response != PASS) {
                return response;
            }
        }
