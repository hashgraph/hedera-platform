...
/**
 * This class is used for checking whether should initiate a sync and create an event for that sync
 * or not. It contains a list of {@link TransThrottleSyncAndCreateRule}s, which are checked one by
 * one. Once a rule has a firm answer such as THROTTLE or DONT_THROTTLE, the answer is returned;
 * else we continue checking the next rule.
 */
public class TransThrottleSyncAndCreateRules {

    /**
     * a list of rules based on which we check whether should initiate a sync and create an event
     * for that sync or not
     */
