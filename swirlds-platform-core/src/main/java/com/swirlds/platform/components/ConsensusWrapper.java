...
/**
 * A wrapper for the consensus algorithm that returns {@link ConsensusRound} objects instead of
 * {@link EventImpl} objects. This class can be removed if/when the consensus interface is modified
 * to return rounds.
 */
public class ConsensusWrapper {
    ...
    public List<ConsensusRound> addEvent(final EventImpl event, final AddressBook addressBook) {
        final List<EventImpl> consensusEvents =
                consensusSupplier.get().addEvent(event, addressBook);
        ...
        for (final Map.Entry<Long, List<EventImpl>> entry : roundEvents.entrySet()) {
            rounds.add(
                    new ConsensusRound(entry.getValue(), new Generations(consensusSupplier.get())));
        }
