...
/**
 * Responsible for creating and enqueuing event tasks. Event tasks can either be {@link GossipEvent}
 * or {@link CreateEventTask}
 */
public class EventTaskCreator {
    ...
    /**
     * constructor that is given the platform using the hashgraph, and the initial addressBook
     * (which can change)
     *
     ...
     * @param settings provides access to settings
    ...
    /**
     * Creates the appropriate events after a sync has finished successfully. If an event should be
     * created according to the rules in {@link SyncManager#shouldCreateEvent(SyncResult)}, a self
     * event with an other-parent of the node we just synced with is added to the intake queue. An
     * additional event may be created with a different other-parent.
     *
    ...
    /**
     * If an event on this node has no children, then generate a new child event for it, based on a
     * probability value defined by {@code Settings.rescueChildlessInverseProbability}.
     *
     * <p>This functionality may be deprecated in future.
     */
    public void rescueChildlessEvents() {
        ...
        for (int i = 0; i < addressBook.getSize(); i++) {
            if (selfId.equalsMain(i)) {
                // we don't rescue our own event, this might have been the cause of a reconnect
                // issue
                continue;
            }
            ...
            // Decide, with probability = 1 / Settings.rescueChildlessInverseProbability, to create
            // an other-child
    ...
    /**
     * Insert an event task to create a self-event into the hashgraph intake queue. The created
     * event will have other-parent specified by the given node ID
     *
    ...
    /**
     * Add an event task to the queue to be instantiated by other threads in parallel. The
     * instantiated event will eventually be added to the hashgraph by the pollIntakeQueue method.
     *
