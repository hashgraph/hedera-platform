...
/**
 * This data structure is used to track the most recent events from each node. This data structure
 * will track the most recent event added and will not check the ordering of these events. If a fork
 * exists, it will track whichever fork is added last.
 */
public class EventMapper implements EventAddedObserver, SelfEventStorage, Clearable {
    ...
    public synchronized void eventAdded(final EventImpl event) {
        ...
        if (parentMapping == null
                || !parentMapping.getEvent().getBaseHash().equals(otherParent.getBaseHash())) {
    ...
    /**
     * Get the generation number of the most recent event from a given node, or {@link
     * EventConstants#GENERATION_UNDEFINED} if there is no event from that node.
     *
     * @param nodeId the ID of the node in question
     */
    ...
    /**
     * Check if the most recent event created by a given node has been used as an other parent by an
     * event created by this node.
     *
     * @param nodeId the ID of the node in question
     */
     ...
     * @return true if the most recent event has descendants, otherwise false. False if there are no
     *     events for the given node ID.
     */
