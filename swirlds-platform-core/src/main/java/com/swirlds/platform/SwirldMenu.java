...
/**
 * A Swirlds menu icon that goes in the upper-right corner of a window, to allow the user to access
 * the browser and other platform functions.
 */
class SwirldMenu extends JPanel {
     ...
     * of it will be size pixels. The logo will be the standard blue-on-white colors. If the given
     * window already has a Swirlds menu icon, then that old menu will be removed.
     *
    ...
    /**
     * Add a Swirlds menu icon to the upper-right corner of the given window. The height and width
     * of it will be size pixels. If the given window already has a Swirlds menu icon, then that old
     * menu will be removed. The logo will be blue, black, or white, according to foreColor. If
     * there is a background, it will be white, white, or black (respectively). If backColor is
     * false, then the background is transparent.
     *
     * <p>NOTE: SwirldsMenu is a lightweight Component. So the transparent background will work if
     * the window contains only lightweight Components, such as JPanel. If the window contains a
     * heavyweight Component, such as a Canvas, then it is better to set backColor to true.
     *
     * @param platform the Platform running the app that owns this window
    ...
    public static void addTo(
            SwirldsPlatform platform, JFrame window, int size, Color foreColor, boolean backColor) {
        Color fore = null;
        Color back = null;
    ...
    /**
     * Delete the Swirlds menu icon from the given window. If there is no such menu, then nothing
     * happens.
     *
    ...
    /**
     * Recursively search through all components in the container tree, and delete any SwirldsMenu
     * objects found. The search starts at root. The root should be immediately inside the given
     * parent Container.
     *
    ...
    /**
     * Constructor for a menu in the upper-right of the window with the Swirlds logo. The parent is
     * the window to add this to. It should already be set to the right size.
     *
     * @param platform the Platform running the app that owns this window
     ...
     */
    SwirldMenu(
            SwirldsPlatform platform, JFrame parent, int size, Paint foreColor, Paint backColor) {
        ...
        int x = parent.getWidth() - size - Browser.insets.left - Browser.insets.right - size / 16;
        int y = size / 16;
                ...
                case "About":
                    int choice =
                            JOptionPane.showOptionDialog(
                                    ...
                                    JOptionPane
                                            .PLAIN_MESSAGE /* INFORMATION_MESSAGE */, // messageType
                                    null, // icon
    ...
    /**
     * Popup a dialog box with an OK button, giving a scrollable view of the given HTML file within
     * the .jar file.
     *
     * <p>For example, if the path is "/docs/license.html", then the license.html file miust be in
     * the docs directory inside the swirlds.jar file.
     *
     * <p>That will work from the command line. To also work in Eclipse, the file must also be
     * copied to platform/target/classes/docs/license.html
     *
     * @param path the path such as "/docs/license.html" for the file license.html located in both
     *     locations.
     */
    ...
    /**
     * Draw the Swirlds logo into the given Graphics2D context, with upper-left corner at pixel
     * (x,y), with width and height both equal to scale pixels, wtih the given foreground color. If
     * the background color is not null, then the background is filled with the given color.
     *
    ...
    static void drawLogo(
            Graphics2D g, float x, float y, float size, Paint foreColor, Paint backColor) {
     ...
     * <a href="http://englishjavadrinker.blogspot.com/search/label/SVGRoundTrip">SVGRoundTrip</a>
     * The original bounding box in the SVG was at (x,y)=(1,1) and size (300,300)
     *
     * @param path
     */
