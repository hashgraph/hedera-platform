...
/**
 * Implements the reconnect protocol over a bidirectional network. This reconnect implementation is
 * used when the chatter protocol is enabled.
 */
public class ReconnectProtocol implements Protocol {
    ...
    public boolean shouldAccept() {
        ...
        // released by the ReconnectTeacher (or by this class if we fail first).
        ...
        if (teacherState == null) {
            logger.info(
                    ...
                    "Rejecting reconnect request from node {} "
                            + "due to lack of a fully signed state",
                    peerId.getId());
            return false;
        }

        if (!teacherState.isComplete()) {
            // this is only possible if signed state manager violates its contractual obligations
            teacherState.weakReleaseState();
            teacherState = null;
            logger.error(
                    RECONNECT.getMarker(),
        ...
        final boolean reconnectPermittedByThrottle =
                teacherThrottle.initiateReconnect(peerId.getId());
