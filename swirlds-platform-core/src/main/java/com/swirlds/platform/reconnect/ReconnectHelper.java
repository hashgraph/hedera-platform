...
public class ReconnectHelper {
	 ...
	 */
	public SignedState receiveSignedState(final Connection conn) throws ReconnectException {
		final SignedState signedState;
		try {
			signedState = reconnectLearner(conn);
	...
	private SignedState reconnectLearner(final Connection conn) throws ReconnectException {
		final SignedState signedState;
		...
		LOG.info(RECONNECT.getMarker(), () -> new ReconnectStartPayload(
				...
				lastCompleteRoundSupplier.getAsLong()).toString());
