...
public class ReconnectController implements Runnable {
    ...
    public void run() {
        try {
            // the ReconnectHelper uses a ReconnectLearnerThrottle to exit if there are too many
            // failed attempts
    ...
    /**
     * Provides a connection over which a reconnect learner has been already negotiated. This method
     * should only be called if {@link #acquireLearnerPermit()} has returned true previously. This
     * method blocks until the reconnect is done.
     *
     * @param connection the connection to use to execute the reconnect learner protocol
     * @throws InterruptedException if the calling thread is interrupted while the connection is
     *     being used
     */
    ...
    /**
     * Sets the validator used to determine if the state received in reconnect has sufficient valid
     * signatures.
     *
