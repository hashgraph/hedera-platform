...
public class PlatformMetrics {
    ...
    private static final SpeedometerMetric.Config INTERRUPTED_CALL_SYNCS_PER_SECOND_CONFIG =
            new SpeedometerMetric.Config(PLATFORM_CATEGORY, "icSync/sec")
                    .withDescription(
                            "(interrupted call syncs) syncs interrupted per second initiated by"
                                    + " this member")
    ...
    private static final SpeedometerMetric.Config INTERRUPTED_REC_SYNCS_PER_SECOND_CONFIG =
            new SpeedometerMetric.Config(PLATFORM_CATEGORY, "irSync/sec")
                    .withDescription(
                            "(interrupted receive syncs) syncs interrupted per second initiated by"
                                    + " other member")
                    .withFormat(FORMAT_14_7);
    private final SpeedometerMetric interruptedRecSyncsPerSecond;

    private static final RunningAverageMetric.Config AVG_SELF_ID_CONFIG =
            new RunningAverageMetric.Config(INFO_CATEGORY, "memberID")
                    .withDescription("ID number of this member")
                    .withFormat(FORMAT_3_0);
    private final RunningAverageMetric avgSelfId;

    private static final RunningAverageMetric.Config AVG_NUM_MEMBERS_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "members")
    ...
    private static final RunningAverageMetric.Config AVG_WRITE_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "write")
                    .withDescription(
    ...
    private static final RunningAverageMetric.Config AVG_SIM_CALL_SYNCS_MAX_CONFIG =
            new RunningAverageMetric.Config(INTERNAL_CATEGORY, "simCallSyncsMax")
    ...
    private static final RunningAverageMetric.Config AVG_Q_SIGNED_STATE_EVENTS_CONFIG =
            new RunningAverageMetric.Config(INTERNAL_CATEGORY, "queueSignedStateEvents")
                    .withDescription(
                            "number of handled consensus events that will be part of the next"
                                    + " signed state")
                    .withFormat(FORMAT_10_1);
    private final RunningAverageMetric avgQSignedStateEvents;

    private static final RunningAverageMetric.Config AVG_Q_4_FOR_HASH_CONFIG =
            new RunningAverageMetric.Config(INTERNAL_CATEGORY, "q4")
                    .withDescription(
                            "number events in receiving queue waiting to be processed or created")
                    .withFormat(FORMAT_10_1);
    private final RunningAverageMetric avgQ4forHash;

    private static final RunningAverageMetric.Config AVG_SIM_SYNCS_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "simSyncs")
    ...
    private static final RunningAverageMetric.Config AVG_SIM_LISTEN_SYNCS_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "simListenSyncs")
                    .withDescription(
                            "avg number of simultaneous listening syncs happening at any given"
                                    + " time")
                    .withFormat(FORMAT_9_6);
    private final RunningAverageMetric avgSimListenSyncs;

    private static final RunningAverageMetric.Config EVENT_STREAM_QUEUE_SIZE_CONFIG =
            new RunningAverageMetric.Config(INFO_CATEGORY, "eventStreamQueueSize")
                    .withDescription(
                            "size of the queue from which we take events and write to EventStream"
                                    + " file")
                    .withFormat(FORMAT_13_0);
    private final RunningAverageMetric eventStreamQueueSize;

    private static final RunningAverageMetric.Config HASH_QUEUE_SIZE_CONFIG =
            new RunningAverageMetric.Config(INFO_CATEGORY, "hashQueueSize")
                    .withDescription(
                            "size of the queue from which we take events, calculate Hash and"
                                    + " RunningHash")
                    .withFormat(FORMAT_13_0);
    ...
    private static final RunningAverageMetric.Config AVG_ROUND_SUPERMAJORITY_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "roundSup")
    ...
    private static final RunningAverageMetric.Config AVG_EVENTS_IN_MEM_CONFIG =
            new RunningAverageMetric.Config(INTERNAL_CATEGORY, "eventsInMem")
                    .withDescription(
                            "total number of events in memory, for all members on the local machine"
                                    + " together")
                    .withFormat(FORMAT_16_2);
    private final RunningAverageMetric avgEventsInMem;

    private static final FunctionGauge.Config<Integer> TLS_CONFIG =
    ...
    private static final SpeedometerMetric.Config SLEEP_1_PER_SECOND_CONFIG =
            new SpeedometerMetric.Config(INTERNAL_CATEGORY, "sleep1/sec")
                    .withDescription(
     ...
     *
     * @param platform a reference to the {@link SwirldsPlatform}
     */
    public PlatformMetrics(final SwirldsPlatform platform) {
        ...
        interruptedCallSyncsPerSecond =
                metrics.getOrCreate(INTERRUPTED_CALL_SYNCS_PER_SECOND_CONFIG);
        ...
        avgQ1PreConsEvents =
                new AverageAndMax(
                        ...
                        "average number of events in the pre-consensus queue (q1) waiting to be"
                                + " handled",
                        FORMAT_10_3,
        ...
        avgQ2ConsEvents =
                new AverageAndMax(
                        ...
                        "average number of events in the consensus queue (q2) waiting to be"
                                + " handled",
                        FORMAT_10_3,
                        AverageStat.WEIGHT_VOLATILE);
        avgQSignedStateEvents = metrics.getOrCreate(AVG_Q_SIGNED_STATE_EVENTS_CONFIG);
        avgQ4forHash = metrics.getOrCreate(AVG_Q_4_FOR_HASH_CONFIG);
    ...
    private void addFunctionGauges(final Metrics metrics) {
        metrics.getOrCreate(
                new FunctionGauge.Config<>(INFO_CATEGORY, "name", String.class, this::getMemberName)
        ...
        metrics.getOrCreate(
                new FunctionGauge.Config<>(
                                ...
                                "lastGen",
                                Long.class,
                                this::getLastEventGenerationNumber)
                        .withDescription("last event generation number by me")
                        .withFormat("%d"));
        metrics.getOrCreate(
                new FunctionGauge.Config<>(
                                INFO_CATEGORY, "transEvent", Integer.class, this::getTransEventSize)
        ...
        metrics.getOrCreate(
                new FunctionGauge.Config<>(
                                ...
                                "priorityTransEvent",
                                Integer.class,
                                this::getPriorityTransEventSize)
                        .withDescription("priorityTransEvent queue size")
                        .withFormat("%d"));
        metrics.getOrCreate(
                new FunctionGauge.Config<>(
                                INFO_CATEGORY, "transCons", Long.class, this::getTransConsSize)
                ...
                new FunctionGauge.Config<>(
                                ...
                                () ->
                                        platform.getFreezeManager().isEventCreationFrozen()
                                                || platform.getStartUpEventFrozenManager()
                                                        .isEventCreationPausedAfterStartUp())
    ...
    private String getMemberName() {
        ...
        return platform.getAddressBook().getAddress(platform.getSelfId().getId()).getSelfName();
    }
    ...
    void update() {
        ...
        avgQ4forHash.update(platform.getIntakeQueue().size());
