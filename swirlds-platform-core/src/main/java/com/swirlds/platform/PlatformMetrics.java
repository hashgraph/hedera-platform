...
public class PlatformMetrics {
	...
	private static final SpeedometerMetric.Config INTERRUPTED_REC_SYNCS_PER_SECOND_CONFIG =
			new SpeedometerMetric.Config(PLATFORM_CATEGORY, "irSync/sec")
					.withDescription("(interrupted receive syncs) syncs interrupted per second initiated by other " +
							"member")
					.withFormat(FORMAT_14_7);
	...
	private static final RunningAverageMetric.Config AVG_Q_4_FOR_HASH_CONFIG =
			new RunningAverageMetric.Config(INTERNAL_CATEGORY, "q4")
					.withDescription("number events in receiving queue waiting to be processed or created")
					.withFormat(FORMAT_10_1);
	...
	private static final FunctionGauge.Config<Integer> TLS_CONFIG =
			new FunctionGauge.Config<>(INFO_CATEGORY, "TLS", () -> Settings.getInstance().isUseTLS() ? 1 : 0)
	 ...
	 *
	 * @param platform
	 * 		a reference to the {@link AbstractPlatform}
	 */
	public PlatformMetrics(final AbstractPlatform platform) {
	...
	private void addFunctionGauges(final Metrics metrics) {
		metrics.getOrCreate(
				new FunctionGauge.Config<>(INFO_CATEGORY, "name", this::getMemberName)
						.withDescription("name of this member")
						.withFormat("%8s")
		);
		metrics.getOrCreate(
				new FunctionGauge.Config<>(INFO_CATEGORY, "lastGen", this::getLastEventGenerationNumber)
						.withDescription("last event generation number by me")
						.withFormat("%d")
		);
		metrics.getOrCreate(
				new FunctionGauge.Config<>(INFO_CATEGORY, "transEvent", this::getTransEventSize)
						.withDescription("transEvent queue size")
						.withFormat("%d")
		);
		metrics.getOrCreate(
				new FunctionGauge.Config<>(INFO_CATEGORY, "priorityTransEvent", this::getPriorityTransEventSize)
						.withDescription("priorityTransEvent queue size")
						.withFormat("%d")
		);
		metrics.getOrCreate(
				new FunctionGauge.Config<>(INFO_CATEGORY, "transCons", this::getTransConsSize)
	...
	void update() {
		...
		eventStreamQueueSize.update(platform.getEventStreamManager() != null ?
				platform.getEventStreamManager().getEventStreamingQueueSize() : 0);
		hashQueueSize.update(platform.getEventStreamManager() != null ?
