...
/**
 * This class provides all properties from {@link LegacyConfigProperties} (the config.txt file) to
 * the new config API. In the old implementation some values from the config.txt have overwritten
 * settings. By this {@link ConfigSource} this behavior is recreated for the new config api.
 *
 * @deprecated Once we migrated the config.txt to the new config api and property files the class
 *     can be removed.
 */
...
public class ConfigPropertiesSource implements ConfigSource {
    ...
    public ConfigPropertiesSource(final LegacyConfigProperties configurationProperties) {
        ...
        configurationProperties
                .tls()
                .ifPresent(value -> properties.setProperty("useTLS", Boolean.toString(value)));
        configurationProperties
                .maxSyncs()
                .ifPresent(
                        value ->
                                properties.setProperty(
                                        "maxOutgoingSyncs", Integer.toString(value)));
        configurationProperties
                .transactionMaxBytes()
                .ifPresent(
                        value ->
                                properties.setProperty(
                                        "transactionMaxBytes", Integer.toString(value)));
        configurationProperties
                .ipTos()
