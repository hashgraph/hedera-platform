...
public class EventStreamRoundIterator implements IOIterator<Round> {
     ...
     * @param startingRound the round to start iterating at, or {@link
     *     EventStreamPathIterator#FIRST_ROUND_AVAILABLE} if all available rounds should be
     *     returned.
     * @param allowPartialRound if true then allow the last round to contain just some of the events
     *     from that round. If false then do not return a round that does not have all of its
     *     events.
     */
    public EventStreamRoundIterator(
            final Path eventStreamDirectory, final long startingRound, boolean allowPartialRound)
            throws IOException {
        this(
     ...
     */
    public EventStreamRoundIterator(
            final IOIterator<EventImpl> eventIterator, boolean allowPartialRound) {
    ...
    public boolean hasNext() throws IOException {
        ...
        if (ended) {
            // Once we decide that we have finished (e.g. if a round is missing events or a file is
            // corrupted),
