 ...
 *
 * <p>This iterator validates the hashes of the events as it goes. It there is a gap in event stream
 * files, or if an event stream file contains corrupted data, this object will throw an IO exception
 * when that data is reached.
 *
 * <p>The exception to this rule is the final event stream file. The final file is permitted to
 * abruptly terminate very end (if, for example, a node crashed while writing it). All complete
 * events at the beginning of the file are returned by this iterator.
 */
public class EventStreamMultiFileIterator implements IOIterator<DetailedConsensusEvent> {
     ...
     * @param startingRound do not return events prior to this round, or {@link
     *     EventStreamPathIterator#FIRST_ROUND_AVAILABLE} if all events should be walked by this
     *     iterator
     * @throws IOException if there is a problem reading the event stream
     * @throws NoSuchElementException if the starting round can't be found
     */
    ...
    /**
     * Create an iterator that walks over all events in a given directory starting with a given
     * round.
     *
     * @param eventStreamDirectory the directory in question
     * @param startingRound the round to start with, or {@link
     *     EventStreamPathIterator#FIRST_ROUND_AVAILABLE} if all events should be walked by this
     *     iterator
     * @throws IOException if there is a problem reading the event stream
     * @throws NoSuchElementException if the starting round can't be found
     */
    public EventStreamMultiFileIterator(final Path eventStreamDirectory, final long startingRound)
            throws IOException {
    ...
    /**
     * @return events that have been read from a file but are not returned by {@link #next()}
     *     because they have an earlier round then the one requested
     */
