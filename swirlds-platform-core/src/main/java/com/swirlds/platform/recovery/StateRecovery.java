...
/**
 * Performs state recovery in the event of a node crashing. The last signed state saved on disk is loaded and events
 * from the event stream are replayed to get the latest signed state. Once the signed state for the last complete round
 * is written to disk, the node shuts down.
 */
public class StateRecovery {
	 ...
	 * @param addressMemo
	 * 		this node's address memo
	 * @param signedState
	 * 		the latest signed state on disk
	 * @param eventStreamManager
	 * 		the event stream manager
	 * @param signedStateFileManager
	 * 		the signed state file manager
	 * @param consensusRoundHandler
	 * 		the consensus round handler
	 */
	...
	/**
	 * Recover events from the event stream starting from the last saved state on disk and apply them to the state. Save
	 * the last saved state to disk before shutting down.
	 */
	public void execute() {
		...
		final RecoveredEventAggregator aggregator = new RecoveredEventAggregator(
				consensusRoundHandler::addConsensusRound,
				consensusRoundHandler::addMinGenInfo
		);

		final BinaryRouter<EventImpl> eventRouter = new BinaryRouter<>(
		...
		// stream or event aggregator. Continue polling and routing until all events have been routed.
		// Parsing events and applying those events to the state are done simultaneously to avoid running out of memory
		// if a large number of events must be recovered.
		eventRouter.route();

		// No more events will be routed, so we can now determine the last complete round that will be sent to the
		// consensus round handler. Store the last complete round number so that we know to save it to disk regardless
		...
		LOG.info(EVENT_PARSER.getMarker(), "Sent {} events to be aggregated into rounds", eventRouter.getNumPassed());
		LOG.info(EVENT_PARSER.getMarker(), "Sent {} events directly to the event stream", eventRouter.getNumFailed());
		LOG.info(EVENT_PARSER.getMarker(), "Inserted {} events ({} rounds) to consensus round queue",
				aggregator.getTotalEvents(),
				aggregator.getTotalRounds());
		LOG.info(EVENT_PARSER.getMarker(), "roundOfLastRecoveredEvent {}", aggregator.getLastCompleteRound());
	 ...
	 *
	 * @param signedState
	 * 		a signed state that was just constructed, may not be fully signed
	 */
