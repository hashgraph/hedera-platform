...
/**
 * Collects events one by one and aggregates them into rounds. Completed rounds are forwarded to a
 * consumer. A single complete round is buffered in this class before being sent to the consumer.
 *
 * <p>This class is used by the recovery process to aggregate events read from the event stream that
 * need to be handled. An entire round is buffered so that once all events are aggregated, the last
 * complete round number is known. Recovery needs this number so that it knows to shut down when a
 * signed state is written to disk for this round.
 */
public class RecoveredEventAggregator {
    ...
    /**
     * @param roundConsumer consumer of the completed rounds
     * @param minGenConsumer consumer of the min gen for the complete round
     */
     ...
     *
     * <p>Once an event from the third round is received, the first round will be sent to the {@code
     * roundConsumer}. Calling {@link #sendLastRound()} sends the remaining rounds.
     *
    ...
    public void addEvent(final EventImpl event) {
        ...
        if (workingData == null) {
            ...
            LOG.error(
                    ...
                    "Received event {} for invalid round {}. Current working round is {}, last"
                            + " complete round is {}",
    ...
    /**
     * Provides the round number of the last complete round (the last round in which the last event
     * has {@link EventImpl#isLastInRoundReceived()} set to {@code true}).
     *
    ...
    /**
     * Provides the total number of rounds events were aggregated into, whether complete or
     * incomplete.
     *
    ...
    /**
     * Indicates no more events will be added for aggregation. Determines the last complete rounds
     * of events.
     *
    ...
    public long noMoreEvents() {
        ...
        if (lastEvent == null) {
            LOG.warn(
                    EVENT_PARSER.getMarker(),
                    "No recovered events were received by the event aggregator.");
            return 0L;
        }
        ...
        if (!lastEvent.isLastInRoundReceived()) {
            LOG.info(
                    EVENT_PARSER.getMarker(),
                    "Last recovered event is not last event of its round ({})",
                    lastEvent.getRoundReceived());
            if (lastCompleteRound == null) {
                LOG.error(
                        EVENT_PARSER.getMarker(),
                        "Fewer than a round of events were recovered. "
