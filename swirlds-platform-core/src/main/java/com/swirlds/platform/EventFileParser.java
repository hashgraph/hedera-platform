...
public class EventFileParser {
	 ...
	 */
	public static boolean parseEventStreamFile(final Path path, final EventConsumer eventConsumer,
		...
		try {
			...
			if (fileVersion == EVENT_STREAM_FILE_VERSION) {
				...
				LOG.info(EVENT_PARSER.getMarker(), "failed to parse file {} whose version is {}",
	...
	private static boolean parseEventStreamV5(final Path path, final EventConsumer eventConsumer) {
		final Iterator<SelfSerializable> iterator = parseStreamFile(path.toFile(), EVENT);
		...
		while (iterator.hasNext()) {
			...
			if (isStartRunningHash) {
				LOG.info(EVENT_PARSER.getMarker(), "From file {} read startRunningHash = {}",
						path.getFileName().toString(),
						object);
				isStartRunningHash = false;
			} else if (object instanceof Hash) {
				LOG.info(EVENT_PARSER.getMarker(), "From file {} read endRunningHash = {}",
						path.getFileName().toString(),
						object);
			} else {
