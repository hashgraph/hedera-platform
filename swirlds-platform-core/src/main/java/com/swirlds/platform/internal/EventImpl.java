...
/**
 * An internal platform event. It holds all the event data relevant to the platform. It implements
 * the Event interface which is a public-facing form of an event.
 */
...
public class EventImpl extends AbstractSerializableHashable
    ...
    /**
     * An unmodifiable ordered set of system transaction indices in the array of all transactions,
    ...
    public EventImpl(
            ...
            final EventImpl otherParent) {
        this(
                new GossipEvent(baseEventHashedData, baseEventUnhashedData),
                consensusData,
                selfParent,
                otherParent);
    }
    ...
    @Deprecated(
            forRemoval =
                    true) // there is no need to store the events generation inside consensusData
    ...
    /**
     * Returns the timestamp of the last transaction in this event. If this event has no
     * transaction, then the timestamp of the event will be returned
     *
    ...
    public Instant getLastTransTime() {
        ...
        // this is a special case. if an event has 0 or 1 transactions, the timestamp of the last
        // transaction can be
    ...
    public Instant getTransactionTime(final int transactionIndex) {
        ...
        if (transactionIndex >= getTransactions().length) {
            throw new IllegalArgumentException(
                    "Event does not have a transaction with index:" + transactionIndex);
        }
    ...
    @Override
    public int hashCode() {
        return new HashCodeBuilder(17, 37).append(baseEvent).append(consensusData).toHashCode();
    }

    /**
     * Events compare by generation. So sorting is always a topological sort. Returns -1 if
     * this.generation is less than other.generation, 1 if greater, 0 if equal.
     *
    ...
    /**
     * Propagates consensus data to all transactions. Invoked when this event has reached consensus
     * and all consensus data is set.
     */
    public void consensusReached() {
        ...
        for (int i = 0; i < transactions.length; i++) {
            final Instant transConsTime =
                    getConsensusTimestamp().plusNanos(i * MIN_TRANS_TIMESTAMP_INCR_NANOS);
    ...
    /**
     * @return the Election associated with the earliest round involved in the election for this
     *     event's fame
     */
    ...
    /**
     * @param firstElection the Election associated with the earliest round involved in the election
     *     for this event's fame
     */
    ...
    /**
     * @return is roundCreated frozen (won't change with address book changes)? True if an ancestor
     *     of a famous witness
     */
    ...
    /**
     * @param frozen is roundCreated frozen (won't change with address book changes)? True if an
     *     ancestor of a famous witness
     */
    ...
    /**
     * @param isLastEventBeforeShutdown whether this event is the last event to be written to event
     *     stream before shut down
     */
