...
/**
 * This class is used for pausing event creation for a while, when the node starts from a saved
 * state and {@link Settings#freezeSecondsAfterStartup} is positive
 */
public class StartUpEventFrozenManager implements TransThrottleSyncRule, EventCreationRule {
    ...
    boolean isEventCreationPausedAfterStartUp() {
        ...
        // multiple events with the same sequence number. When a node does a restart or a reconnect,
        // it must first
        // check if any other node has events that it created but does not have in memory at that
        // moment.
