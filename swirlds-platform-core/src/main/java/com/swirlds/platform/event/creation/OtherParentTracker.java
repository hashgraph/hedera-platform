...
/**
 * Tracks which events were used as other parents and prevents us from using the same other parent
 * twice. It assumes the creator will only use newer other parents and only tracks the most recent
 * event by creator ID.
 */
public class OtherParentTracker implements ParentBasedCreationRule {
    ...
    public EventCreationRuleResponse shouldCreateEvent(
        ...
        final Hash lastParentUsed = otherParentUsed.get(otherParent.getHashedData().getCreatorId());
