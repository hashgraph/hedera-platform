...
/**
 * An event linker which buffers out-of-order events until their parents are provided, or they
 * become ancient. An event received out of order is called an orphan. Once an event is no longer an
...
public class OrphanBufferingLinker extends AbstractEventLinker {
    ...
    public OrphanBufferingLinker(
        ...
        this.orphanMap =
                new StandardSequenceMap<>(
                        0, futureGenerationLimit, ChatterEventDescriptor::getGeneration);
        this.missingParents =
                new StandardSequenceMap<>(0, futureGenerationLimit, ParentDescriptor::generation);
    }

    private static void parentNoLongerMissing(
            final ChildEvent child, final Hash parentHash, final EventImpl parent) {
        try {
    ...
    private static void orphanPurged(final ChatterEventDescriptor key, final ChildEvent orphan) {
        // this should never happen. an events parents should become ancient and at that point it
    ...
    private void addMissingParent(final ChildEvent childEvent, final boolean missingSelfParent) {
        final ParentDescriptor parentDescriptor =
                missingSelfParent
                        ? childEvent.buildSelfParentDescriptor()
        ...
        if (childSet == null) {
            logger.error(
                    ...
                    () ->
                            missingParents.getFirstSequenceNumberInWindow()
                                    + missingParents.getSequenceNumberCapacity());
        } else {
    ...
    private void processNewlyLinkedEvents() {
        while (!newlyLinkedEvents.isEmpty()) {
            ...
            final Set<ChildEvent> orphans =
                    missingParents.remove(
                            new ParentDescriptor(
                                    newlyLinked.getGeneration(), newlyLinked.getBaseHash()));
            if (orphans == null || orphans.isEmpty()) {
                continue;
            }
            for (final Iterator<ChildEvent> orphanIterator = orphans.iterator();
                    orphanIterator.hasNext(); ) {
    ...
    public void updateGenerations(final GraphGenerations generations) {
        ...
        orphanMap.shiftWindow(
                generations.getMinGenerationNonAncient(), OrphanBufferingLinker::orphanPurged);
        processNewlyLinkedEvents();
    }
