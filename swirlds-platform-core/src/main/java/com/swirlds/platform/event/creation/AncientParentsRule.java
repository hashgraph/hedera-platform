...
/**
 * Prevents us from creating events with ancient parents which will almost certainly be ancient
 * themselves
 */
public class AncientParentsRule implements ParentBasedCreationRule {
     ...
     *
     * @param selfParent the self-parent event
     * @param otherParent the other-parent event
    ...
    public boolean areBothParentsAncient(final BaseEvent selfParent, final BaseEvent otherParent) {
        ...
        // crash on 3 June 2019. Then, 3 out of 10 nodes went offline (due to hardware problems) and
        // had only
        // old events in the state. When the nodes started back up,
        // nodes that previously crashed synced with other nodes that crashed. This created events
        // where both
        // parents are old, and these events could not be entered into the hashgraph on the nodes
        // that created
        // them, and they were not gossipped out.
        // Update 15 November 2021: The code has since changed and creating ancient events no longer
        // breaks the system.
        ...
        if (!generations.areAnyEventsAncient()) {
            // if there are no ancient event yet, we return immediately. This is to account for
            // genesis, where both
