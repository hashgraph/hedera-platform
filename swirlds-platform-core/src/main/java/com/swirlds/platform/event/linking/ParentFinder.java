...
public class ParentFinder {
    ...
    private static boolean requiredParent(
        ...
        final long parentGeneration =
                selfParent
                        ? event.getHashedData().getSelfParentGen()
                        : event.getHashedData().getOtherParentGen();
        // if an event does not have a parent, its generation will be
     ...
     *
     * @param event the event whose parents are looked for
     * @param minGenerationNonAncient the generation below which all events are ancient
