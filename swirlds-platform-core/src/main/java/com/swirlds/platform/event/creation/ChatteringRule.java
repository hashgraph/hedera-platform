...
/**
 * Prevents us from creating events unless we are chattering with a certain percentage of our
 * neighbors
 */
public class ChatteringRule implements EventCreationRule {
    ...
    /**
     * @param chatteringThreshold the fraction of neighbors we should be chattering with in order to
     *     create events
     * @param chatterCommunicationStates all neighbors states of communication
     */
    public ChatteringRule(
            final double chatteringThreshold,
            final Collection<CommunicationState> chatterCommunicationStates) {
        if (chatteringThreshold <= 0 || chatteringThreshold > 1) {
            throw new IllegalStateException(
                    "chatteringThreshold should be: '0 <= cht < 1', supplied value: "
                            + chatteringThreshold);
        }
    ...
    public EventCreationRuleResponse shouldCreateEvent() {
        ...
        final long numChattering =
                chatterCommunicationStates.stream()
