...
public final class LoggingEventCreationRules extends EventCreationRules {
	...
	private LoggingEventCreationRules(
			final List<RuleWrapper> wrappedBasic,
			final List<RuleWrapper> wrappedParent) {
	...
	private void maybeLog() {
		if (nextLog.isBefore(Instant.now())) {
			LOG.info(LogMarker.EVENT_CREATION_THROTTLE.getMarker(),
	...
	private String logThrottles() {
		...
		wrappedRules.forEach(
				r -> sb.append(String.format("%5d", r.getResetDontCreateCount()))
