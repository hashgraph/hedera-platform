...
/**
 * Wraps {@link EventCreationRules}s and adds log messages that summarise which throttles prevent
...
public final class LoggingEventCreationRules extends EventCreationRules {
    ...
    private LoggingEventCreationRules(
            final List<RuleWrapper> wrappedBasic, final List<RuleWrapper> wrappedParent) {
        super(wrappedBasic, wrappedParent);
        this.wrappedRules = Stream.concat(wrappedBasic.stream(), wrappedParent.stream()).toList();
    }
    ...
    public String throttlesToString() {
        ...
        wrappedRules.forEach(
                r ->
                        sb.append(String.format("%5d", r.getResetDontCreateCount()))
                                .append(" - ")
                                .append(r.getName())
                                .append('\n'));
        return sb.toString();
    ...
    private static class RuleWrapper implements EventCreationRule, ParentBasedCreationRule {
        ...
        private RuleWrapper(
                final EventCreationRule rule, final ParentBasedCreationRule parentRule) {
            if (rule != null) {
