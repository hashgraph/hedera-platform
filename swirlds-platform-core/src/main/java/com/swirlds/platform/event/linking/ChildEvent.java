...
/**
 * A {@link GossipEvent} whose parents may or may not be missing. A parent is considered missing if
 * it is non-ancient AND it has not been received and validated. A parent might have been received
 * but is an orphan itself, in that case it is still considered missing. If any parent is missing,
 * then this child is considered an orphan.
 */
public final class ChildEvent {
    ...
    /**
     * @param child the event to wrap
     * @param missingSelfParent is the event's self-parent missing?
     * @param missingOtherParent is the event's other-parent missing?
     * @param selfParent the event's self-parent, may be null if there is no parent, or it is not
     *     found
     * @param otherParent the event's other-parent, may be null if there is no parent, or it is not
     *     found
     */
    public ChildEvent(
    ...
    /**
     * Updates the state of this child with a parent. The parent mey not have been found, it might
     * have become ancient, in which case it is no longer missing and don't even need it.
     *
     * @param hash the hash of the previously missing parent
     * @param parent the parent, null if it has become ancient and was never received
     * @throws IllegalArgumentException in case the supplied parent is not missing, or not a parent
    ...
    public ParentDescriptor buildSelfParentDescriptor() {
        return new ParentDescriptor(
                child.getHashedData().getSelfParentGen(),
                child.getHashedData().getSelfParentHash());
    }
    ...
    public ParentDescriptor buildOtherParentDescriptor() {
        return new ParentDescriptor(
                child.getHashedData().getOtherParentGen(),
                child.getHashedData().getOtherParentHash());
    }
