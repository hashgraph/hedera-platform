...
public final class EventStringBuilder {
    ...
    public EventStringBuilder appendEvent() {
        ...
        appendShortEvent(
                hashedData.getCreatorId(), hashedData.getGeneration(), hashedData.getHash());
        return this;
    }
     ...
     *
     * @param creatorId creator ID of the event
     * @param generation generation of the event
    ...
    private void appendShortEvent(final long creatorId, final long generation, final Hash hash) {
        ...
        if (creatorId == EventConstants.CREATOR_ID_UNDEFINED
                || generation == EventConstants.GENERATION_UNDEFINED) {
            sb.append("none)");
            return;
        }
