...
public class ZeroStakeValidator implements GossipEventValidator {
    ...
    public boolean isEventValid(final GossipEvent event) {
        // If beta mirror node logic is enabled and the event originated from a node known to have a
        // zero stake,
        // then we should discard this event and not even worry about validating the signature in
        // order
