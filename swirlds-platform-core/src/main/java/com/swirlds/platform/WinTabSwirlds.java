...
class WinTabSwirlds extends WinBrowser.PrePaintableJPanel {
        ...
        instructions.setText(
                Utilities.wrap(
                        ...
                        "\n\n" //
                                ...
                                + " Under each app is all of the known swirlds (i.e., shared"
                                + " worlds, shared ledgers, shared databases). Under each swirld is"
                                + " all of the known members using this local machine. Each one"
                                + " that is currently active is marked as \"running\". Click on a"
                                + " member to show their information in all the tabs here. Minimize"
                                + " this window to hide it. Close it to quit the entire program."));
        freeze(instructions);
    }
    ...
    void addEntity(
            ...
            StateHierarchy.InfoEntity entity,
        ...
        row.setColor(
                (WinBrowser.memberDisplayed == entity)
                        ? WinBrowser.MEMBER_HIGHLIGHT_COLOR
                        : Color.WHITE);
        add(row, c);
    }
    ...
    class EntityRow extends JPanel {
         ...
         * @param level number of levels to indent (0 to not indent)
         * @param suffix a string to add at the end of the panel (on the right)
         * @param selectable true if this row can be selected as the member displayed
         */
        EntityRow(StateHierarchy.InfoEntity entity, int level, String suffix, boolean selectable) {
            this.member = entity;
            this.level = level;
            indent = new JPanel();
            name = new JTextPane();
