...
public class SwirldAppLoader {
     ...
     * @throws AppLoaderException if any issue occurs while instantiating the object
     */
    public SwirldMain instantiateSwirldMain() throws AppLoaderException {
        try {
            ...
            throw new AppLoaderException(
                    "ERROR: Couldn't instantiate the class " + mainClassName, e);
    ...
    /**
     * @param mainClassName the name of the app class inheriting from {@link SwirldMain}
     * @param appJarPath the location of the JAR where the apps classes are located
     * @return the SwirldAppLoader object
     * @throws AppLoaderException if any problems occur while loading the app
     */
    public static SwirldAppLoader loadSwirldApp(final String mainClassName, final Path appJarPath)
        ...
        try {
            ...
            if (constructor == null) {
                throw new AppLoaderException(
                        "The class '" + mainClassName + "' must have a default constructor");
            }

            return new SwirldAppLoader(mainClassName, appJarPath, constructor, classLoader);
        } catch (ClassNotFoundException e) {
            throw new AppLoaderException(
                    "ERROR: Couldn't find the class \"" + mainClassName + "\"", e);
        } catch (Exception e) {
            throw new AppLoaderException(
                    "ERROR: There are problems loading the class " + mainClassName, e);
        }
    }
