...
/**
 * A metrics object to track an average number, without history. This class uses an {@link
 * AtomicAverage} so it is both thread safe and performant.
 */
public class AverageStat {
     ...
     * @param format a string that can be passed to String.format() to format the statistic for the
     *     average number
    ...
    public AverageStat(
        ...
        metrics.getOrCreate(
                new StatEntry.Config<>(category, name, Double.class, average::get)
     ...
     *
     * @param value the value used to update the average
     */
