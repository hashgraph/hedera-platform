...
/**
 * A stat designed to track the amount of time spent in various parts of a cycle that is repeated
 * over and over.
 */
public class CycleTimingStat {
    ...
    /**
     * Creates a new instance whose number of time points is equal to the number of time intervals
     * to be recorded. For example, a cycle that requires 3 intervals to be recorded (interval 1,
     * interval 2, and interval 3) should be initialized with {@code numIntervals} equals to 3. Each
     * part of the cycle is measured using the called shown below:
     *
     * <p>Interval 1: from {@code startCycle()} to {@code timePoint(1)}
     *
     * <p>Interval 2: from {@code timePoint(1)} to {@code timePoint(2)}
     *
     * <p>Interval 3: from {@code timePoint(2)} to {@code stopCycle()}
     *
     * @param unit the unit of time to measure all time intervals in this cycle
     * @param definition the definition of the cycle state
     */
    public CycleTimingStat(
            final Metrics metrics, final ChronoUnit unit, final CycleDefinition definition) {
     ...
     *
     * @param i the ith time point to record
     */
    public void setTimePoint(final int i) {
        if (i <= 0) {
            throw new IllegalArgumentException(
                    "Time point must be greater than 0. Use startCycle() to mark the beginning of"
                            + " the cycle.");
        } else if (i >= numIntervals) {
            throw new IllegalArgumentException(
                    String.format(
                            "Time point must be less than %s. Use stopCycle() to mark the end of"
                                    + " the cycle.",
                            numIntervals - 1));
        } else {
            t[i] = now();
