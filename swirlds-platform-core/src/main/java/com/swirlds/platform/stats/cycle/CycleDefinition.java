...
public class CycleDefinition {
    ...
    /**
     * @param category the category of this stat
     * @param name a name that describes the cycle in its entirety
     * @param intervalNames a list of names and describe each time interval. The size must match
     *     {@code intervalDescriptions}
     * @param intervalDescriptions a list of descriptions for each time interval. The size must
     *     match {@code intervalNames}
     */
    public CycleDefinition(
        ...
        if (intervalNames.size() != intervalDescriptions.size()) {
            throw new IllegalArgumentException(
                    String.format(
                            "The number of descriptions for %s (%d) does not match the number of"
                                    + " names (%d).",
     ...
     * @param category the category of this stat
     * @param name a name that describes the cycle in its entirety
     * @param namesAndDescriptions a list of names and descriptions for each time interval
     */
    public CycleDefinition(
            final String category,
            final String name,
            final List<Pair<String, String>> namesAndDescriptions) {
        this(
                category,
                name,
    ...
    /**
     * @param index the index of the cycle interval, fist index is 0
