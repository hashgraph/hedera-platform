...
public class StatConstructor {
    ...
    /**
     * Used to create a StatEntry whose value is limited to a single enum. This stat displays the
     * enums unique ID instead of the name so that it can be tracked on a graph.
     *
     * @param name the name of the stat
     * @param category the category of the stat
     * @param enumValues all possible values for the enum
     * @param enumSupplier supplier of the current value
     * @param <T> The Enum type to track
    ...
    public static <T extends Enum<T> & UniqueId> StatEntry.Config<Integer> createEnumStat(
        ...
        Arrays.stream(enumValues)
                .forEach((v) -> desc.append(v.getId()).append('=').append(v.name()).append(' '));
