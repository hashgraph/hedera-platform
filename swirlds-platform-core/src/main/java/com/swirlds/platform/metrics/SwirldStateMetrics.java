...
public class SwirldStateMetrics {
    ...
    private static final RunningAverageMetric.Config AVG_SEC_TRANS_HANDLED_CONFIG =
            new RunningAverageMetric.Config(INTERNAL_CATEGORY, "secTransH")
                    .withDescription(
                            "avg time to handle a consensus transaction in"
                                    + " SwirldState.handleTransaction (in seconds)")
                    .withFormat(FORMAT_10_6);
    private final RunningAverageMetric avgSecTransHandled;

    private static final RunningAverageMetric.Config AVG_CONS_HANDLE_TIME_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "SecC2H")
                    .withDescription(
                            "time from knowing consensus for a transaction to handling it (in"
                                    + " seconds)")
                    .withFormat(FORMAT_10_3);
    private final RunningAverageMetric avgConsHandleTime;

    private static final SpeedometerMetric.Config TRANS_HANDLED_PER_SECOND_CONFIG =
            new SpeedometerMetric.Config(
                            INTERNAL_CATEGORY, PlatformStatNames.TRANSACTIONS_HANDLED_PER_SECOND)
                    .withDescription(
                            "number of consensus transactions per second handled "
                                    + "by SwirldState.handleTransaction()")
                    .withFormat(FORMAT_9_6);
    private final SpeedometerMetric transHandledPerSecond;

    private static final RunningAverageMetric.Config AVG_STATE_COPY_MICROS_CONFIG =
            new RunningAverageMetric.Config(INTERNAL_CATEGORY, "stateCopyMicros")
                    .withDescription(
                            "average time it takes the SwirldState.copy() method in SwirldState to"
                                    + " finish (in microseconds)")
                    .withFormat(FORMAT_16_2);
    private final RunningAverageMetric avgStateCopyMicros;

    private static final RunningAverageMetric.Config AVG_SHUFFLE_MICROS_CONFIG =
            new RunningAverageMetric.Config(INTERNAL_CATEGORY, "shuffleMicros")
                    .withDescription(
                            "average time spent in SwirldStateManagerSingle.Shuffler#shuffle()"
                                    + " method (in microseconds)")
                    .withFormat(FORMAT_16_2);
    private final RunningAverageMetric avgShuffleMicros;

    /**
     * average time spent in {@code SwirldStateManager#prehandle} by the {@code intake} thread (in
     * microseconds)
     */
    ...
    public SwirldStateMetrics(final Metrics metrics) {
        ...
        preConsHandleTime =
                new AverageTimeStat(
                        ...
                        INTERNAL_CATEGORY,
                        "preConsHandleMicros",
                        "average time it takes to handle a pre-consensus event from q4 (in"
                                + " microseconds)");
        preHandleTime =
                new AverageTimeStat(
                        metrics,
    ...
    /**
     * Records the amount of time between a transaction reaching consensus and being handled in
     * {@link SwirldState}.
     *
     * @param seconds the amount of time in seconds
     */
    ...
    /**
     * Records the time spent performing a shuffle in {@link
     * com.swirlds.platform.state.SwirldStateManagerSingle} (in microseconds).
     *
     * @param micros the amount of time in microseconds
     */
    ...
    /**
     * The amount of time it takes to apply an event or a transaction, depending on which {@link
     * SwirldState} the application implements.
     */
