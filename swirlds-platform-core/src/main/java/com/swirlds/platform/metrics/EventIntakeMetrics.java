...
public class EventIntakeMetrics implements StaleEventObserver {
    ...
    private static final SpeedometerMetric.Config RESCUED_EVENTS_PER_SECOND_CONFIG =
            new SpeedometerMetric.Config(INTERNAL_CATEGORY, "rescuedEv/sec")
                    .withDescription(
                            "number of events per second generated to prevent stale events")
                    .withFormat(FORMAT_16_2);
    private final SpeedometerMetric rescuedEventsPerSecond;

    private static final SpeedometerMetric.Config DUPLICATE_EVENTS_PER_SECOND_CONFIG =
            new SpeedometerMetric.Config(INTERNAL_CATEGORY, "dupEv/sec")
    ...
    private static final RunningAverageMetric.Config AVG_DUPLICATE_PERCENT_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "dupEv%")
    ...
    private static final SpeedometerMetric.Config TIME_FRAC_ADD_CONFIG =
            new SpeedometerMetric.Config(INTERNAL_CATEGORY, "timeFracAdd")
                    .withDescription(
                            "fraction of each second spent adding an event to the hashgraph and "
                                    + "finding consensus")
    ...
    private static final RunningAverageMetric.Config SHOULD_CREATE_EVENT_CONFIG =
            new RunningAverageMetric.Config(INTERNAL_CATEGORY, "shouldCreateEvent")
    ...
    private static final Counter.Config STALE_EVENTS_TOTAL_CONFIG =
            new Counter.Config(INTERNAL_CATEGORY, "staleEvTot")
                    .withDescription("total number of stale events ever");
    private final Counter staleEventsTotal;

    private static final SpeedometerMetric.Config STALE_EVENTS_PER_SECOND_CONFIG =
            new SpeedometerMetric.Config(INTERNAL_CATEGORY, "staleEv/sec")
    ...
    /**
     * Update a statistics accumulator whenever this node creates an event with an other-parent that
     * has no children. (The OP is "rescued".)
     */
