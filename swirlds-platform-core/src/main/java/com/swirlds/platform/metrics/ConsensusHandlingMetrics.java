	...
	public ConsensusHandlingMetrics(final Metrics metrics) {
		...
		consensusCycleTiming = new CycleTimingStat(
				...
				new CycleDefinition(
						INTERNAL_CATEGORY,
						"consRound",
						List.of(Pair.of("dataPropMillis/round", "average time to propagate consensus data to transactions"),
								Pair.of("handleMillis/round", "average time to handle a consensus round"),
								Pair.of("storeMillis/round", "average time to add consensus round events to signed state storage"),
								...
								Pair.of("forSigCleanMillis", "average time spent expiring signed state storage events")
						)
				)
		);
		newSignedStateCycleTiming = new CycleTimingStat(
				...
				new CycleDefinition(
						INTERNAL_CATEGORY,
						"newSS",
						List.of(Pair.of("getStateMicros", "average time to get the state to sign"),
								...
								Pair.of("newSSInstanceMicros", "average time spent creating the new signed state instance"),
								Pair.of("queueAdmitMicros", "average time spent admitting the signed state to the signing queue")
						)
				)
		);
