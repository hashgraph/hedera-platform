...
public class SyncMetrics {
    ...
    public SyncMetrics(final Metrics metrics) {
        ...
        multiTipsPerSync =
                new MaxStat(
                        ...
                        "the number of creators that have more than one tip at the start of each"
                                + " sync",
                        "%5d");
        gensWaitingForExpiry =
                new AverageStat(
                        metrics,
                        PLATFORM_CATEGORY,
    ...
    /**
     * Records the size of the known set during a sync. This is the most compute intensive part of
     * the sync, so this is useful information to validate sync performance.
     *
    ...
    /**
     * Called by {@link ShadowGraphSynchronizer} to update the {@code tips/sync} statistic with the
     * number of creators that have more than one {@code sendTip} in the current synchronization.
     *
     * @param multiTipCount the number of creators in the current synchronization that have more
     *     than one sending tip.
     */
    ...
    /**
     * Called by {@link ShadowGraphSynchronizer} to update the {@code tips/sync} statistic with the
     * number of {@code sendTips} in the current synchronization.
     *
    ...
    /**
     * Called by {@link ShadowGraph} to update the number of generations that should be expired but
     * can't be yet due to reservations.
     *
    ...
    /**
     * Called by {@link SyncManager#shouldAcceptSync()} when a sync is accepted or rejected to
     * maintain the ratio of rejected syncs to accepted syncs.
     *
     * @param syncRejected true is a sync was rejected, false otherwise
     */
