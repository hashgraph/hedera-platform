...
public class AddedEventMetrics implements EventAddedObserver {
    ...
    private static final SpeedometerMetric.Config EVENTS_CREATED_PER_SECOND_CONFIG =
            new SpeedometerMetric.Config(PLATFORM_CATEGORY, "cEvents/sec")
    ...
    private static final RunningAverageMetric.Config AVG_CREATED_RECEIVED_TIME_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "secC2R")
                    .withDescription(
                            "time from another member creating an event to receiving it and "
                                    + "verifying the signature (in seconds)")
                    .withFormat(FORMAT_10_3);
    private final RunningAverageMetric avgCreatedReceivedTime;

    private static final SpeedometerMetric.Config EVENTS_PER_SECOND_CONFIG =
            new SpeedometerMetric.Config(PLATFORM_CATEGORY, "events/sec")
                    .withDescription(
                            "number of unique events received per second (created by self and"
                                    + " others)")
                    .withFormat(FORMAT_16_2);
    ...
    private static final RunningAverageMetric.Config AVG_BYTES_PER_TRANSACTION_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "bytes/trans")
    ...
    private static final SpeedometerMetric.Config BYTES_PER_SECOND_TRANS_CONFIG =
            new SpeedometerMetric.Config(PLATFORM_CATEGORY, "bytes/sec_trans")
                    .withDescription(
                            "number of bytes in the transactions received per second " + DETAILS)
                    .withFormat(FORMAT_16_2);
    private final SpeedometerMetric bytesPerSecondTrans;

    private static final SpeedometerMetric.Config BYTES_PER_SECOND_SYS_CONFIG =
            new SpeedometerMetric.Config(INTERNAL_CATEGORY, "bytes/sec_sys")
                    .withDescription(
                            "number of bytes in the system transactions received per second "
                                    + DETAILS)
                    .withFormat(FORMAT_16_2);
    private final SpeedometerMetric bytesPerSecondSys;

    private static final SpeedometerMetric.Config TRANSACTIONS_PER_SECOND_CONFIG =
            new SpeedometerMetric.Config(PLATFORM_CATEGORY, "trans/sec")
    ...
    private static final SpeedometerMetric.Config TRANSACTIONS_PER_SECOND_SYS_CONFIG =
            new SpeedometerMetric.Config(INTERNAL_CATEGORY, "trans/sec_sys")
    ...
    private static final Counter.Config NUM_TRANS_CONFIG =
            new Counter.Config(INTERNAL_CATEGORY, "trans")
    ...
    public AddedEventMetrics(final NodeId selfId, final Metrics metrics) {
        ...
        averageOtherParentAgeDiff =
                new AverageStat(
                        metrics,
                        PLATFORM_CATEGORY,
                        "opAgeDiff",
                        "average age difference (in generations) between an event created by this"
                                + " node and its other parent",
                        FORMAT_5_3,
    ...
    public void eventAdded(final EventImpl event) {
        if (event.isCreatedBy(selfId)) {
            ...
            avgCreatedReceivedTime.update(
                    event.getTimeCreated().until(event.getTimeReceived(), ChronoUnit.NANOS)
