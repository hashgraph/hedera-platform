...
public interface ConsensusMetrics {
     ...
     *
     * @param numCoinRounds a number of coin rounds
     */
    void coinRounds(long numCoinRounds);

    /**
     * Update a statistics accumulator to receive the time when a gossiped event is the last famous
     * witness in its round.
     *
     * @param event an event
     */
    ...
    /**
     * Update a statistics accumulator with dot product time. This is used in the consensus impl to
     * track performance of strong seen ancestor searches in the parent round of a given event.
     *
    ...
    /**
     * Update a statistics accumulator with the number of witnesses that are strongly seen by events
     * added to the hashgraph whose parents have the same round created. If more than a strong
     * majority are seen, the round created is incremented.
     *
     * @param numSeen the number of witnesses strongly seen
     */
    void witnessesStronglySeen(final int numSeen);

    /**
     * Called each time an event is assigned a newer round created than it's parents based on the
     * number of witness in the parent round created that it strongly sees.
     */
