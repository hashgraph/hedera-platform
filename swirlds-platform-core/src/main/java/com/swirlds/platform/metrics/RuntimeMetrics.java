...
public final class RuntimeMetrics {
    ...
    private static final double WHOLE_PERCENT =
            100.0; // all of something is to be reported as 100.0%
    ...
    private static final RunningAverageMetric.Config MEM_FREE_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "memFree")
                    .withDescription(
    ...
    private static final RunningAverageMetric.Config MEM_TOT_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "memTot")
                    .withDescription("total bytes in the Java Virtual Machine")
                    .withFormat(FORMAT_16_0)
                    .withHalfLife(0.0);
    private final RunningAverageMetric memTot;

    private static final RunningAverageMetric.Config MEM_MAX_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "memMax")
                    .withDescription("maximum bytes that the JVM might use")
                    .withFormat(FORMAT_16_0)
                    .withHalfLife(0.0);
    private final RunningAverageMetric memMax;

    private static final RunningAverageMetric.Config DIRECT_MEM_IN_MB_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "directMemInMB")
    ...
    private static final RunningAverageMetric.Config DIRECT_MEM_PERCENT_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "directMemPercent")
                    .withDescription(
    ...
    private static final RunningAverageMetric.Config AVG_NUM_PROC_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "proc")
    ...
    private static final RunningAverageMetric.Config CPU_LOAD_SYS_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "cpuLoadSys")
                    .withDescription("the CPU load of the whole system")
                    .withFormat(FORMAT_1_4);
    private final RunningAverageMetric cpuLoadSys;

    private static final RunningAverageMetric.Config THREADS_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "threads")
                    .withDescription("the current number of live threads")
                    .withFormat(FORMAT_6_0);
    private final RunningAverageMetric threads;

    private static final FunctionGauge.Config<Long> DISKSPACE_FREE_CONFIG =
            new FunctionGauge.Config<>(
                            INTERNAL_CATEGORY,
                            "DiskspaceFree",
                            Long.class,
                            ROOT_DIRECTORY::getFreeSpace)
                    .withDescription("disk space being used right now")
                    .withFormat("%d");
    private static final FunctionGauge.Config<Long> DISKSPACE_WHOLE_CONFIG =
            new FunctionGauge.Config<>(
                            INTERNAL_CATEGORY,
                            "DiskspaceWhole",
                            Long.class,
                            ROOT_DIRECTORY::getTotalSpace)
                    .withDescription("total disk space available on node")
                    .withFormat("%d");
    private static final FunctionGauge.Config<Long> DISKSPACE_USED_CONFIG =
            new FunctionGauge.Config<>(
                            INTERNAL_CATEGORY,
                            "DiskspaceUsed",
                            Long.class,
        ...
        for (final Class<?> cls : RuntimeObjectRegistry.getTrackedClasses()) {
            ...
            metrics.getOrCreate(
                    new FunctionGauge.Config<>(
                                    ...
                                    Long.class,
                                    () ->
                                            RuntimeObjectRegistry.getOldestActiveObjectAge(
                                                            cls, Instant.now())
                                                    .toSeconds())
                            .withDescription(
    ...
    private void update() {
        ...
        // recording the value of -1 as (-1) / (1024 * 1024) makes it too close to 0; treat it as -1
        // megabytes
        // for visibility
        if (bytesUsed == -1) {
