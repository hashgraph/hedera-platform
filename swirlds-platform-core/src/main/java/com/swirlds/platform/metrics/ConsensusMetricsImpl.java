...
public class ConsensusMetricsImpl implements ConsensusMetrics {
	...
	private static final RunningAverageMetric.Config AVG_RECEIVED_FAMOUS_TIME_CONFIG =
			new RunningAverageMetric.Config(PLATFORM_CATEGORY, "secR2F")
					.withDescription("time from a round's first received event to all the famous witnesses being known (in seconds)")
					.withFormat(FORMAT_10_3);
	...
	private static final Counter.Config ROUND_INCREMENT_STRONGLY_SEEN_CONFIG = new Counter.Config(
			INTERNAL_CATEGORY,
			"roundIncEqParents"
	);
		...
		if (event.hasUserTransactions()) {
			if (selfId.equalsMain(event.getCreatorId())) { // set either created or received time to now
				avgCreatedConsensusTime
						.update(event.getTimeReceived().until(Instant.now(),
								ChronoUnit.NANOS) * NANOSECONDS_TO_SECONDS);
			} else {
				...
				avgCreatedReceivedConsensusTime
						.update(event.getTimeCreated().until(Instant.now(),
								ChronoUnit.NANOS) * NANOSECONDS_TO_SECONDS);
			}
		}
