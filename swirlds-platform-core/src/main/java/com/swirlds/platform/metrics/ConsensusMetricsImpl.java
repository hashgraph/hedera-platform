...
public class ConsensusMetricsImpl implements ConsensusMetrics {
    ...
    private static final RunningAverageMetric.Config AVG_FIRST_EVENT_IN_ROUND_RECEIVED_TIME_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "secR2nR")
                    .withDescription(
                            "time from first event received in one round, to first event received"
                                    + " in the next round (in seconds)")
                    .withFormat(FORMAT_10_3);
    private final RunningAverageMetric avgFirstEventInRoundReceivedTime;

    private static final RunningAverageMetric.Config NUM_COIN_ROUNDS_CONFIG =
            new RunningAverageMetric.Config(INTERNAL_CATEGORY, "coinR")
    ...
    private static final RunningAverageMetric.Config AVG_RECEIVED_FAMOUS_TIME_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "secR2F")
                    .withDescription(
                            "time from a round's first received event to all the famous witnesses"
                                    + " being known (in seconds)")
                    .withFormat(FORMAT_10_3);
    private final RunningAverageMetric avgReceivedFamousTime;

    private static final SpeedometerMetric.Config ROUNDS_PER_SECOND_CONFIG =
            new SpeedometerMetric.Config(PLATFORM_CATEGORY, "rounds/sec")
    ...
    private static final RunningAverageMetric.Config AVG_CREATED_RECEIVED_CONSENSUS_TIME_CONFIG =
            new RunningAverageMetric.Config(PLATFORM_CATEGORY, "secC2RC")
                    .withDescription(
                            "time from another member creating an event to it being received and"
                                    + " and knowing consensus for it (in seconds)")
                    .withFormat(FORMAT_10_3);
    ...
    private static final SpeedometerMetric.Config TIME_FRAC_DOT_CONFIG =
            new SpeedometerMetric.Config(INTERNAL_CATEGORY, "timeFracDot")
    ...
    /**
     * Time when this platform received the first event created by someone else in the most recent
     * round. This is used to calculate Statistics.avgFirstEventInRoundReceivedTime which is "time
     * for event, from receiving the first event in a round to the first event in the next round".
     */
    ...
    public ConsensusMetricsImpl(final NodeId selfId, final Metrics metrics) {
        ...
        avgFirstEventInRoundReceivedTime =
                metrics.getOrCreate(AVG_FIRST_EVENT_IN_ROUND_RECEIVED_TIME_CONFIG);
        ...
        avgCreatedReceivedConsensusTime =
                metrics.getOrCreate(AVG_CREATED_RECEIVED_CONSENSUS_TIME_CONFIG);
        ...
        witnessesSeen =
                new AverageAndMax(
                        ...
                        INTERNAL_CATEGORY,
                        "witnessesSeen",
                        "number of witnesses seen by an event added to the hashgraph when both"
                                + " parents have the same round created",
                        FORMAT_4_2);
        ...
        if (!selfId.equalsMain(event.getCreatorId())
            ...
            if (firstEventInLastRoundTime != null) {
                avgFirstEventInRoundReceivedTime.update(
                        firstEventInLastRoundTime.until(now, ChronoUnit.NANOS)
    ...
    public void lastFamousInRound(final EventImpl event) {
        if (!selfId.equalsMain(event.getCreatorId())) { // record this for events received
            avgReceivedFamousTime.update(
                    event.getTimeReceived().until(Instant.now(), ChronoUnit.NANOS)
                            * NANOSECONDS_TO_SECONDS);
        }
    }

    // this might not need to be a separate method
    // we could just update the stats in consensusReached(EventImpl event) when
    // event.lastInRoundReceived()==true
    ...
    public void consensusReached(final EventImpl event) {
        ...
        // Because of transThrottle, these statistics can end up being misleading, so we are only
        // tracking events that
        // have user transactions in them.
        if (event.hasUserTransactions()) {
            if (selfId.equalsMain(
                    event.getCreatorId())) { // set either created or received time to now
                ...
                avgCreatedReceivedConsensusTime.update(
                        event.getTimeCreated().until(Instant.now(), ChronoUnit.NANOS)
                                * NANOSECONDS_TO_SECONDS);
            }
        }
            ...
            if (selfId.equalsMain(event.getCreatorId())) {
                avgSelfCreatedTimestamp.update(
                        event.getTimeCreated()
                                        .until(event.getConsensusTimestamp(), ChronoUnit.NANOS)
                                * NANOSECONDS_TO_SECONDS);
            } else {
                avgOtherReceivedTimestamp.update(
                        event.getTimeReceived()
                                        .until(event.getConsensusTimestamp(), ChronoUnit.NANOS)
                                * NANOSECONDS_TO_SECONDS);
            }
        }
