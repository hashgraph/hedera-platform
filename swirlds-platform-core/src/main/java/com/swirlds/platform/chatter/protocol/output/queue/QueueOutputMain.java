...
/**
 * A {@link MessageOutput} that has a separate queue for each peer. When a message is supposed to be
 * sent, it adds it to each individual peer's queue
 *
...
public class QueueOutputMain<T extends SelfSerializable> implements MessageOutput<T> {
    ...
    public MessageProvider createPeerInstance(
        ...
        final QueueOutputPeer<T> queueOutputPeer =
                new QueueOutputPeer<>(queueCapacity, communicationState, sendCheck);
