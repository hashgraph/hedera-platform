...
/**
 * Prevents a {@link ChatterEvent} from being sent unless a variable amount of time has passed since
 * we have received this event. This excludes self events which should be sent immediately.
 *
...
public class VariableTimeDelay<E extends ChatterEvent> implements SendCheck<E> {
    ...
    /**
     * @param waitTimeSupplier A supplier of the amount of time to wait before sending an event. If
     *     the supplier returns null, the event is not sent
     * @param state the peer's state
     * @param now supplies the current time
     */
    public VariableTimeDelay(
            final Supplier<Duration> waitTimeSupplier,
            final PeerGossipState state,
