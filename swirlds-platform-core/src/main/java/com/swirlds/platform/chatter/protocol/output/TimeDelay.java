...
/**
 * Prevents a {@link ChatterEvent} from being sent unless a specified amount of time has passed
 * since we have received this event. This excludes self events which should be sent immediately.
 *
...
public class TimeDelay<E extends ChatterEvent> implements SendCheck<E> {
    ...
    /**
     * @param delayTime wait this much time from receiving an event to sending it
     * @param state the peer's state
     * @param now supplies the current time
     */
    public TimeDelay(
            final Duration delayTime, final PeerGossipState state, final Supplier<Instant> now) {
