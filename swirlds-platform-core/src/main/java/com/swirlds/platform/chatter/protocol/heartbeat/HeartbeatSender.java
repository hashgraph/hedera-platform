...
public class HeartbeatSender implements MessageProvider, MessageHandler<HeartbeatMessage> {
    ...
    public void handleMessage(final HeartbeatMessage message) {
        ...
        final HeartbeatSent respondedTrue =
                new HeartbeatSent(message.getHeartbeatId(), lastSent.time(), true);
    ...
    /**
     * Calculates the last known round-trip time of a heartbeat with this peer, or the time since
     * sending the last heartbeat without a response, whichever is greater.
     *
