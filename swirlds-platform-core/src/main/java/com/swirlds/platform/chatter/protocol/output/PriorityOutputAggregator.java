...
/**
 * Holds a list of message providers in order of priority. When a message is requested, it will
 * check the providers in priority order until it finds one that has messages to be sent.
 */
public class PriorityOutputAggregator implements MessageProvider {
    ...
    public PriorityOutputAggregator(
            final List<MessageProvider> providers, final PerSecondStat msgsPerSec) {
