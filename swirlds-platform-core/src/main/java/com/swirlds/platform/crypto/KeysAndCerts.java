...
/**
 * An instantiation of this class holds all the keys and CSPRNG state for one Platform object. No
 * other class should store any secret or private key/seed information.
 *
 * <p>The algorithms and key sizes used here are chosen in accordance with the IAD-NSA Commercial
 * National Security Algorithm (CNSA) Suite, and TLS 1.2, as implemented by the SUN and SunEC
 * security providers, using the JCE Unlimited Strength Jurisdiction files. The TLS suite used here
 * is called TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384. Java uses the NIST p-384 curve specified by
 * the CNSA for ECDH and ECDSA.
 *
 * <p>To aid in key recovery, this is designed so that all of the keys here are generated by
 * deterministic functions of a triplet (master key, swirld ID, member ID), which are passed in to
 * the constructor. When TLS key agreement needs a source of random numbers, it uses SecureRandom
 * with "SHA1PRNG", which calls the underlying operating system's /dev/random and /dev/urandom (or
 * Windows equivalent) to incorporate entropy gathered by the operating system. So the key pairs are
 * generated deterministically, but key agreement uses true random numbers.
 *
 * <p>At the time this class is first being written, neither TLS 1.3 nor Java 9 are available. As
 * they become available, it may become appropriate to switch from ECDSA/ECDHE to something based on
 * Ed25519, for increased speed, decreased bandwidth usage, and better safety (e.g., not needing
 * strong random numbers for key agreement). But when (or whether) that change happens will depend
 * on whether standards such as the CNSA Suite are updated. In the longer term, this may be upgraded
 * to a post quantum algorithm (or a hybrid using both pre-quantum and post-quantum). But again,
 * this will depend on a number of factors. The plan is to continue to follow the national standard
 * (currently CNSA Suit), changing the algorithms as the standard changes.
 */
public record KeysAndCerts(
     ...
     *
     * @param name The name to associate with the key. For example, if it is "alice", then the three
     *     key pairs will be named "s-alice", "e-alice", "a-alice" for signing, encrypting, and key
     *     agreement.
     * @param privateKeyStore read the 3 keyPairs (signing,agreement,encryption) from this store
     * @param publicStores all public certificates
     * @throws KeyStoreException if the supplied key store is not initialized
     * @throws UnrecoverableKeyException if a required key cannot be recovered (e.g., the given
     *     password is wrong).
     * @throws NoSuchAlgorithmException if the algorithm for recovering a required key cannot be
     *     found
     * @throws KeyLoadingException if a required certificate is missing or is not an instance of
     *     X509Certificate
     */
    ...
    private static KeyPair getKeyPair(
            final KeyStore privateKeyStore, final char[] password, final String storeName)
            throws KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException,
                    KeyLoadingException {
        ...
        throw new KeyLoadingException(
                String.format("Key '%s' is not an instance of PrivateKey!", storeName));
    }

    /**
     * Creates an instance holding all the keys and certificates. This also generates the key pairs
     * and certs and CSPRNG state. The key pairs are generated as a function of the seed. The seed
     * is the combination of the three parameters. The signing key pair is used to sign all 3 certs.
     *
     * @param name The name to associate with the key. For example, if it is "alice", then the three
     *     key pairs will be named "s-alice", "e-alice", "a-alice" for signing, encrypting, and key
     *     agreement.
     ...
     * @param memberId which identity is acting as a member in this swirld (because one human user
     *     might have several identities running in a given swirld)
     * @param publicStores all public certificates
     */
    public static KeysAndCerts generate(
        ...
        sigKeyGen =
                KeyPairGenerator.getInstance(
                        CryptoConstants.SIG_TYPE1, CryptoConstants.SIG_PROVIDER);
        encKeyGen =
                KeyPairGenerator.getInstance(
                        CryptoConstants.ENC_TYPE, CryptoConstants.ENC_PROVIDER);
        agrKeyGen =
                KeyPairGenerator.getInstance(
                        CryptoConstants.AGR_TYPE, CryptoConstants.AGR_PROVIDER);
        ...
        return new KeysAndCerts(
                sigKeyPair, agrKeyPair, encKeyPair, sigCert, agrCert, encCert, publicStores);
    }
}
