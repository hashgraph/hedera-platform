 ...
 *
 * @param agrTrustStore the trust store for all the sig certs (self-signed signing cert)
 * @param encTrustStore the trust store for all the agr certs (agreement cert, signed by signing
 *     key)
 * @param sigTrustStore the trust store for all the enc certs (encryption cert, signed by signing
 *     key)
 */
public record PublicStores(KeyStore sigTrustStore, KeyStore agrTrustStore, KeyStore encTrustStore) {
     ...
     *
     * @param allPublic key store with all certificates
     * @param names the names of all members
     * @return an instance
     * @throws KeyStoreException if there is no provider that supports {@link
     *     CryptoConstants#KEYSTORE_TYPE} or the key store provided has not been initialized
     * @throws KeyLoadingException if any of the certificates cannot be found
     */
    ...
    public void setCertificate(
            final KeyCertPurpose type, final X509Certificate certificate, final String name)
    ...
    /**
     * @param type the type of certificate requested
     * @param name the name of the member whose certificate is requested
     * @return a certificate stored in one of the stores
     * @throws KeyLoadingException if the certificate is missing or is not an instance of
     *     X509Certificate
     */
    public X509Certificate getCertificate(final KeyCertPurpose type, final String name)
        ...
        throw new KeyLoadingException(
                "Certificate is not an instance of X509Certificate", type, name);
    }

    /**
     * @param type the type of key requested
     * @param name the name of the member whose key is requested
     * @return this members public key
     * @throws KeyLoadingException if {@link #getCertificate(KeyCertPurpose, String)} throws
     */
    public PublicKey getPublicKey(final KeyCertPurpose type, final String name)
            throws KeyLoadingException {
