...
public class SyncManagerImpl implements SyncManager, FallenBehindManager {
    ...
    /**
     * When looking for a neighbor to call, this is the maximum number of neighbors we query before
     * just selecting one if a suitable neighbor is not found yet.
     */
    ...
    /**
     * This object is used for checking whether should initiate a sync and create an event for that
     * sync or not
     */
    ...
    private final AtomicInteger transThrottleInitialCalls =
            new AtomicInteger(TRANS_THROTTLE_INITIAL_CALLS_NUM);
    /**
     * a counter that keeps track of the number of consecutive syncs that happened without any
     * events being transferred
     */
     ...
     * @param eventCreationRules Contains a list of rules for checking whether this node should
     *     create an event or not
     * @param transThrottleSyncRules A list of rules for checking if this not should sync with other
     *     nodes
     * @param transThrottleSyncAndCreateRules Contains a list of rules for checking whether should
     *     initiate a sync and create an event for that sync or not
     * @param lastRoundSavedToDisk A binding of signedStateMgr.getLastRoundSavedToDisk().
     * @param lastCompletedRound A binding of signedStateMgr.getLastCompleteRound()).
     * @param transactionTracker tracks user transactions within events
     */
    SyncManagerImpl(
    ...
    public List<Long> getNeighborsToCall() {
        // if there is an indication we might have fallen behind, calling nodes to establish this
        // takes priority
        ...
        for (int i = 0; i < MAXIMUM_NEIGHBORS_TO_QUERY; i++) {
            ...
            // we try to call a neighbor in the bottom 1/3 by number of events created in the latest
            // round, if
            // we fail to find one after 10 tries, we just call the last neighbor we find
            if (criticalQuorum.isInCriticalQuorum(neighbor)
                    || i == MAXIMUM_NEIGHBORS_TO_QUERY - 1) {
                list.add(neighbor);
            }
        }
    ...
    /**
     * Ask the SyncManager whether we should stop calling other nodes because there are no more user
     * transactions
     *
    ...
    public boolean transThrottle() {
        ...
        // if all checks have passed, then we don't need to call anyone until one of the conditions
        // changes
        return false;
    }

    /**
     * Checks if according to transThrottle, we should initiate a sync and create an event for that
     * sync
     *
    ...
    public boolean transThrottleCallAndCreate() {
        ...
        // check 3: if we have non consensus user transaction in the hashgraph, initiate a sync.
        // This is the most common
        // scenario. We will only check the saved states if there are no non consensus transactions
        // in the hashgraph.
        ...
        if (settings.getState().getSaveStatePeriod() > 0) {
            // check 4: if we are saving states to disk, then we need to sync until we have saved a
            // state that has
            ...
            // check 5: if we are not saving states to disk, then we only need to collect enough
            // signatures for the
    ...
    public boolean shouldCreateEvent(
            ...
            final boolean oneNodeFallenBehind,
    ...
    /**
     * Called by {@link SyncUtils} after a successful sync to check whether it should create an
     * event or not
     *
    ...
    public boolean shouldCreateEvent(final SyncResult info) {
        ...
        // check 3: if neither node is part of the superMinority in the latest round, don't create
        // an event
        ...
        if (!transThrottleCallAndCreate()) {
            // once transThrottle says we should not create any more events, we should start keeping
            // track of the
        ...
        if (settings.getStaleEventPreventionThreshold() > 0
                && info.getEventsRead()
                        > settings.getStaleEventPreventionThreshold() * addressBook.getSize()) {
            // if we read too many events during this sync, we skip creating an event to reduce the
            // probability of
    ...
    public void successfulSync() {
        if (transThrottleInitialCalls.get() > 0) {
            // since this is not synchronized, it might make the transThrottleInitialCalls go below
            // 0, but that isn't an
