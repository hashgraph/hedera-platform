...
/**
 * Submits valid transactions received from the application to a consumer. Invalid transactions are
 * rejected.
 */
public class TransactionSubmitter {
     ...
     * @param isZeroStakeNode true is this node is a zero-stake node
     * @param addToTransactionPool a function that adds the transaction to the transaction pool, if
     *     room is available
    ...
    public boolean submitTransaction(final SwirldTransaction trans) {
        ...
        transactionMetrics.updateTransSubmitMicros(
                (long) ((System.nanoTime() - start) * NANOSECONDS_TO_MICROSECONDS));
