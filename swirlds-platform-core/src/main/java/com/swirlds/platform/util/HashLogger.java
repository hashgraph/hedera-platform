 ...
 *
 * <p>HashLogger will queue up tasks to generate and log hashes from the provided signed states on a
 * background thread. The implementation makes use of log4j and will make use of log4j configuration
...
public class HashLogger {
            ...
            logOutput.info(
                    ...
                    () ->
                            MESSAGE_FACTORY.newMessage(
                                    "*** [node-{}] Several rounds skipped. Round received {}."
                                            + " Previously received {}.",
                                    nodeId,
                                    currentRound,
                                    prevRound));
    ...
    public void logHashes(final SignedState signedState) {
        ...
        logQueue.offer(
                () -> log(signedState)); // NOSONAR: silently drop message if unable to queue.
    }
