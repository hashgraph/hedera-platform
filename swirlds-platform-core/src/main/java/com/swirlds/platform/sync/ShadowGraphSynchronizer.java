...
/**
 * The goal of the ShadowGraphSynchronizer is to compare graphs with a remote node, and update them
 * so both sides have the same events in the graph. This process is called a sync.
 *
 * <p>This instance can be called by multiple threads at the same time. To avoid accidental
 * concurrency issues, all the variables in this class are final. The ones that are used for storing
...
public class ShadowGraphSynchronizer {
    ...
    /**
     * provides the current consensus instance, a supplier is used because this instance will change
     * after a reconnect, so we have to make sure we always get the latest one
     */
    ...
    private boolean reserveSynchronize(final Connection conn)
            throws IOException, ParallelExecutionException, SyncException, InterruptedException {
        // accumulates time points for each step in the execution of a single gossip session, used
        // for stats
        ...
        try (final GenerationReservation reservation = shadowGraph.reserve()) {
            ...
            final Phase1Response theirGensTips =
                    readWriteParallel(
    ...
    private boolean fallenBehind(
            final Generations self, final Generations other, final Connection conn) {
    ...
    private List<EventImpl> createSendList(
        ...
        final Set<ShadowEvent> knownAncestors =
                shadowGraph.findAncestors(
                        knownSet,
                        SyncUtils.unknownNonAncient(knownSet, myGenerations, theirGenerations));

        // since knownAncestors is a lot bigger than knownSet, it is a lot cheaper to add knownSet
        // to knownAncestors
        // then vice versa
        ...
        final Set<ShadowEvent> sendSet =
                shadowGraph.findAncestors(unknownTips, knownAncestorsPredicate);
     ...
     *
     * <p>It is very important that the read task is executed by the caller thread. The reader
     * thread can always time out, if the writer thread gets blocked by a write method because the
     * buffer is full, the only way to unblock it is to close the connection. So the reader will
     * close the connection and unblock the writer if it times out or if anything goes wrong.
     *
     ...
     * @throws ParallelExecutionException thrown if anything goes wrong during these read write
     *     operations. the connection will be closed before this exception is thrown
     */
