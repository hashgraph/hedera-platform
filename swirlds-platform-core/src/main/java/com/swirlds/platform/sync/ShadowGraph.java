...
/**
 * A shadow graph is a lightweight replication of the hashgraph structure. It supports:
 *
 * <ul>
 *   <li>insertion of a shadow event
 *   <li>querying for a shadow event by hashgraph event base hash
 *   <li>querying for ancestors by shadow event
 *   <li>providing the current tips (shadow events with no self-child)
 *   <li>expiration of shadow events by generation
 *   <li>reservation of a generation of events to prevent event expiration
 * </ul>
 *
 * <p>The shadow graph is thread safe.
 */
public class ShadowGraph implements Clearable {
    ...
    /**
     * The set of all tips for the shadow graph. A tip is an event with no self child (could have
     * other children)
     */
    ...
    /**
     * Initializes the {@link ShadowGraph} with the given {@code events}. This method should be used
     * after reconnect or restart. {@code events} must be ordered by generation, smallest to
     * largest.
     *
     * <p>A minimum generation is necessary because events loaded from signed state a could have
     * generation gaps and are used in {@link com.swirlds.platform.Consensus}. {@link
     * com.swirlds.platform.Consensus} will eventually expire its smallest generation and that
     * generation must be present in the {@link ShadowGraph} or an exception is thrown, so we create
     * empty generations to match {@link com.swirlds.platform.Consensus}.
     *
     ...
     */
    public synchronized void initFromEvents(
            final List<EventImpl> events, final long minGeneration) {
        if (events == null || events.isEmpty()) {
            throw new IllegalArgumentException("events must not be null or empty");
        }

        // Set this to the oldest generation in the event list, so we can determine if parent events
        // are expired,
        ...
        for (EventImpl event : events) {
            // if an issue like this occurs, we still might be in a situation where we could
            // continue running, that's
        ...
        // Now that events are added, update (decrease) the oldest generation to match the
        // expireBelow value in case it
    ...
    /**
     * Reserves the events in generation {@code expireBelow}. A reservation prevents events in that
     * generation and later (higher) generations from being expired from the shadow graph.
     *
     ...
     * @return true if the hash matches the hash of a shadow event in the shadow graph, false
     *     otherwise
     */
    ...
    /**
     * Returns the ancestors of the provided {@code events} that pass the provided {@code predicate}
     * using a depth-first search. The provided {@code events} are not included in the return set.
     * Searching stops at nodes that have no parents, or nodes that do not pass the {@code
     * predicate}.
     *
     * <p>It is safe for this method not to be synchronized because:
     *
     * <ol>
     *   <li>this method does not modify any data
     *   <li>adding events to the the graph does not affect ancestors
     *   <li>checks for expired parent events are atomic
     * </ol>
     *
     * <p>Note: This method is always accessed after a call to a synchronized {@link ShadowGraph}
     * method, like {@link #getTips()}, which acts as a memory gate and causes the calling thread to
     * read the latest values for all variables from memory, including {@link ShadowEvent} links.
     *
     ...
     */
    public Set<ShadowEvent> findAncestors(
            final Iterable<ShadowEvent> events, final Predicate<ShadowEvent> predicate) {
    ...
    /**
     * Private method that searches for ancestors and takes a HashSet as input. This method exists
     * for efficiency, when looking for ancestors of multiple events, we want to append to the same
     * HashSet.
     *
    ...
    /**
     * Update the reservable generation and remove any events from the shadow graph that can and
     * should be expired.
     *
     * <p>Events that should be expired have a generation that is less than {@code expireBelow}.
     *
     * <p>Events that are allowed to be expired are events:
     *
     * <ol>
     *   <li>whose generation has zero reservations
     *   <li>whose generation is less than the smallest generation with a non-zero number of
     *       reservations
     * </ol>
     *
     * @param generation The generation below which all generations should be expired. For example,
     *     if {@code generation} is 100, events in generation 99 and below should be expired.
     */
    public synchronized void expireBelow(final long generation) {
        if (generation < expireBelow) {
            LOG.error(
                    ...
                    generation,
                    expireBelow);
            // The value of expireBelow must never decrease, so if we receive an invalid request
            // like this, ignore it
            return;
        }
    ...
    /**
     * Removes reservations that can and should be expired, starting with the oldest generation
     * reservation.
     *
     * @return the oldest generation with at least one reservation, or {@code -1} if there are no
     *     generations with at least one reservation.
     * @see ShadowGraph#expireBelow
     */
    private long pruneReservationList() {
        long oldestReservedGen = NO_GENERATION_RESERVED;

        // Iterate through the reservation list in ascending generation order, removing reservations
        // for generations
    ...
    /**
     * Returns a copy of the tips at the time of invocation. The returned list is not affected by
     * changes made to the tip set.
     *
    ...
    public synchronized boolean addEvent(final EventImpl e) throws ShadowGraphInsertionException {
        ...
        if (status == InsertableStatus.INSERTABLE) {
            ...
            // Every event received should be insertable, so throw an exception if that is not the
            // case
            if (status == InsertableStatus.EXPIRED_EVENT) {
                throw new ShadowGraphInsertionException(
                        String.format(
                                "`addEvent`: did not insert, status is %s for event %s,"
                                        + " oldestGeneration = %s",
    ...
    /**
     * Attach a shadow of a Hashgraph event to this graph. Only a shadow for which a parent hash
     * matches a hash in this@entry is inserted.
     *
     ...
     * @return An insertable status, indicating whether the event can be inserted, and if not, the
     *     reason it can not be inserted.
     */
    private InsertableStatus insertable(final EventImpl e) {
        ...
        // If e has an unexpired parent that is not already referenced by the shadow graph, then we
        // log an error. This
