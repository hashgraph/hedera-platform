...
public final class SyncComms {
    ...
    /**
     * The maximum time we will allow for phase 3. If phase 3 is not done within this time limit, we
     * will abort the sync
     */
    ...
    public static void rejectSync(final Connection conn, final int numberOfNodes)
            throws IOException {
        // respond with a nack
    ...
    public static Callable<Phase1Response> phase1Read(
        ...
        return () -> {
            // Caller thread requested a sync, so now caller thread reads if its request was
            // accepted.
            if (conn.isOutbound() && readInitByte) {
                try {
                    ...
                    final long inMs =
                            sentTime == null
                                    ? -1
                                    : sentTime.until(Instant.now(), ChronoUnit.MILLIS);
    ...
    public static Callable<List<Boolean>> phase2Read(
            final Connection conn, final int numberOfTips) {
        return () -> {
    ...
    public static Callable<Void> phase3Write(
        ...
        return () -> {
            ...
            // we have now finished reading and writing all the events of a sync. the remote node
            // may not have
            // finished reading and processing all the events this node has sent. so we write a byte
            // to tell the remote
        ...
        return () -> {
            ...
            try {
                ...
                while (true) {
                    ...
                    // if the peer continuously sends COMM_SYNC_ONGOING, or sends the data really
                    // slowly,
                    ...
                    switch (next) {
                        ...
                        case ByteConstants.COMM_EVENT_DONE -> {
                            ...
                            logger.info(
                                    ...
                                    conn.getDescription(),
                                    eventsRead);
                            // we are done reading event, tell the writer thread to send a
                            // COMM_SYNC_DONE
                            eventReadingDone.countDown();
                        }
                            // while we are waiting for the peer to tell us they are done, they
                            // might send COMM_SYNC_ONGOING
                        ...
                        default -> throw new SyncException(
                                conn,
                                String.format(
                                        "while reading events, received unexpected byte %02x",
                                        next));
                    }
                }
            } finally {
