 ...
 * </p>
 */
public final class SyncShadowGraphVerification {

	/**
	 * This class is not instantiable
	 */
	private SyncShadowGraphVerification() {
	 ...
	 * @return a {@link SyncShadowGraphVerificationStatus}
	 */
	public static SyncShadowGraphVerificationStatus verify(
	...
	public static SyncShadowGraphVerificationStatus verify(
		...
		for (final EventImpl e : events) {
			...
			final SyncShadowEvent s = sgm.shadow(e);

			final SyncShadowGraphVerificationStatus status = verify(s, sgm);
			if (status != SyncShadowGraphVerificationStatus.VERIFIED) {
				return status;
			}
		}

		return SyncShadowGraphVerificationStatus.VERIFIED;
	}
	 ...
	 * @return a {@link SyncShadowGraphVerificationStatus}
	 */
	private static SyncShadowGraphVerificationStatus verify(final SyncShadowEvent s, final SyncShadowGraphManager sgm) {
		...
		if (missingSelfParent(e, s)) {
			return SyncShadowGraphVerificationStatus.MISSING_SELF_PARENT;
		}
		...
		if (missingOtherParent(e, s)) {
			return SyncShadowGraphVerificationStatus.MISSING_OTHER_PARENT;
		}

		// The shadow graph should not contain events that the hashgraph no longer contains.
		if (expiredSelfParent(e, s)) {
			return SyncShadowGraphVerificationStatus.EXPIRED_SELF_PARENT;
		}

		// The shadow graph should not contain events that the hashgraph no longer contains.
		if (expiredOtherParent(e, s)) {
			return SyncShadowGraphVerificationStatus.EXPIRED_OTHER_PARENT;
		}

		// Internal structural failures: these should never happen.
		if (mismatchedSelfParent(e, s)) {
			return SyncShadowGraphVerificationStatus.MISMATCHED_SELF_PARENT;
		}

		if (mismatchedOtherParent(e, s)) {
			return SyncShadowGraphVerificationStatus.MISMATCHED_OTHER_PARENT;
		}

		if (missingSelfChildOfSelfParent(s)) {
			return SyncShadowGraphVerificationStatus.MISSING_INBOUND_SELF_CHILD_POINTER;
		}

		if (missingOtherChildOfOtherParent(s)) {
			return SyncShadowGraphVerificationStatus.MISSING_INBOUND_OTHER_CHILD_POINTER;
		}

		if (missingSelfParentOfSelfChild(e, s)) {
			return SyncShadowGraphVerificationStatus.MISSING_INBOUND_SELF_PARENT_POINTER;
		}

		if (mismatchedSelfParentOfSelfChild(e, s)) {
			return SyncShadowGraphVerificationStatus.MISMATCHED_INBOUND_SELF_PARENT_POINTER;
		}

		if (missingOtherParentOfOtherChild(e, s)) {
			return SyncShadowGraphVerificationStatus.MISSING_INBOUND_OTHER_PARENT_POINTER;
		}

		if (mismatchedOtherParentOfOtherChild(e, s)) {
			return SyncShadowGraphVerificationStatus.MISMATCHED_INBOUND_OTHER_PARENT_POINTER;
		}
		...
		if (isSelfParent && isTip) {
			return SyncShadowGraphVerificationStatus.INVALID_TIP;
		}

		if (!isSelfParent && !isTip) {
			return SyncShadowGraphVerificationStatus.MISSING_TIP;
		}
	 ...
	 * @param s
	 * 		the {@link SyncShadowEvent} that should reference it
	 * @return true iff {@code s} reference {@code e}
	 */
	private static boolean missing(final Event e, final SyncShadowEvent s) {
	 ...
	 */
	private static boolean expired(final Event e, final SyncShadowEvent s) {
		return e == null && s != null;
	}
	 ...
	 */
	private static boolean mismatched(final Event e, final SyncShadowEvent s) {
	 ...
	 * 		event of {@code s}
	 */
	private static boolean missingSelfParent(final Event e, final SyncShadowEvent s) {
	 ...
	 */
	private static boolean mismatchedSelfParent(final Event e, final SyncShadowEvent s) {
	 ...
	 * 		event of {@code s}
	 */
	private static boolean missingOtherParent(final Event e, final SyncShadowEvent s) {
	 ...
	 */
	private static boolean expiredSelfParent(final Event e, final SyncShadowEvent s) {
	 ...
	 */
	private static boolean expiredOtherParent(final Event e, final SyncShadowEvent s) {
	 ...
	 */
	private static boolean mismatchedOtherParent(final Event e, final SyncShadowEvent s) {
	 ...
	 */
	private static boolean missingSelfChildOfSelfParent(final SyncShadowEvent s) {
	 ...
	 */
	private static boolean missingOtherChildOfOtherParent(final SyncShadowEvent s) {
	 ...
	 */
	private static boolean mismatchedSelfParentOfSelfChild(final Event e, final SyncShadowEvent s) {
		for (final SyncShadowEvent sc : s.getSelfChildren()) {
	 ...
	 */
	private static boolean missingSelfParentOfSelfChild(final Event e, final SyncShadowEvent s) {
		for (final SyncShadowEvent sc : s.getSelfChildren()) {
	 ...
	 */
	private static boolean mismatchedOtherParentOfOtherChild(final Event e, final SyncShadowEvent s) {
		for (final SyncShadowEvent oc : s.getOtherChildren()) {
	 ...
	 */
	private static boolean missingOtherParentOfOtherChild(final Event e, final SyncShadowEvent s) {
		for (final SyncShadowEvent oc : s.getOtherChildren()) {
