...
public final class SyncUtils {
    ...
    /**
     * Returns a predicate that determines if a {@link ShadowEvent}'s generation is non-ancient for
     * the peer and greater than this node's minimum non-expired generation, and is not already
     * known.
     *
     * @param knownShadows the {@link ShadowEvent}s that are already known and should therefore be
    ...
    /**
     * Computes the number of creators that have more than one tip. If a single creator has more
     * than two tips, this method will only report once for each such creator. The execution time
     * cost for this method is O(T + N) where T is the number of tips including all forks and N is
     * the number of network nodes. There is some memory overhead, but it is fairly nominal in favor
     * of the time complexity savings.
     *
    ...
    public static int computeMultiTipCount(Iterable<ShadowEvent> tips) {
        ...
        // Make a single O(N) where N is the number of tips including all forks. Typically, N will
        // be equal to the
        // number of network nodes.
        for (final ShadowEvent tip : tips) {
            tipCountByCreator.compute(
                    tip.getEvent().getCreatorId(), (k, v) -> (v != null) ? (v + 1) : 1);
        }

        // Walk the entrySet() which is O(N) where N is the number network nodes. This is still more
        // efficient than a
        // O(N^2) loop.
        ...
        for (final Map.Entry<Long, Integer> entry : tipCountByCreator.entrySet()) {
            ...
            // This map is broken down by creator ID already as the key so this is guaranteed to be
            // a single increment
    ...
    static void sort(final List<EventImpl> sendList) {
        sendList.sort(
                (EventImpl e1, EventImpl e2) -> (int) (e1.getGeneration() - e2.getGeneration()));
    }
}
