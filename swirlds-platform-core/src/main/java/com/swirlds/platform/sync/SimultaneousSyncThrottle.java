...
/**
 * Controls simultaneous syncs: - prevents 2 simultaneous syncs from occurring with the same member
...
public class SimultaneousSyncThrottle {
    ...
    /**
     * Try to acquire the lock for syncing with the given node. The lock will be acquired if no
     * syncs are ongoing with that node.
     *
     * @param nodeId the ID of the node we wish to sync with
     * @param isOutbound is the sync initiated by this platform
     * @return an Autocloseable lock that provides information on whether the lock was acquired or
     *     not, and unlocks if previously locked on close()
     */
    ...
    /**
     * Waits for all sync locks to be released sequentially. It does not prevent these locks from
