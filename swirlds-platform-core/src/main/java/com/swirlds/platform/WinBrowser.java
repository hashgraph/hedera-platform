...
/**
 * The main browser window. It contains a tabbed pane, which contains the classes with names
 * WinTab*, some of which might themselves contain tabbed panes, whose tabs would be classes named
 * WinTab2*, and so on.
 *
 * <p>The classes WinBrowser, ScrollableJPanel, and UpdatableJPanel, each have an prePaint() method.
 * Once a second, a timer thread calls WinBrowser.prePaint, and then the call is passed down the
 * Component tree, so that every Component can do the slow calculations for what should appear on
 * the screen. Then the thread calls repaint(), to quickly re-render everything. For example, if
 * there is a long calculation necessary to find the text for a JTextArea, then the long calculation
 * is done inside prePaint(), and the fast rendering of the result is done in paintComponent(). This
 * prevents the GUI thread from hanging for too long when there are slow calculations being
 * performed.
 */
class WinBrowser extends JFrame {
    ...
    /**
     * A JPanel with an additional method prePaint(), which (possibly slowly) recalculates
     * everything, so the next repaint() will (quickly) render everything.
     */
    abstract static class PrePaintableJPanel extends JPanel {
        private static final long serialVersionUID = 1L;
        ...
        /**
         * Recalculate the contents of each Component, maybe slowly, so that the next repaint() will
         * trigger a fast render of everything. If this contains any other UpdatableJPanels, then it
         * must call their prePaint(), too.
         */
    ...
    static class ScrollableJPanel extends JScrollPane {
        ...
        /**
         * Wrap the given panel in scroll bars, and remember it so that calls to prePaint() can be
         * passed on to it.
         */
        ...
        /**
         * Recalculate the contents of each Component, maybe slowly, so that the next repaint() will
         * trigger a fast render of everything.
         */
        void prePaint() {
            if (contents != null) {
                contents.prePaint();
    ...
    /**
     * Perform a prePaint to recalculate the contents of each Component, maybe slowly, so that the
     * next repaint() will trigger a fast render of everything. Then perform a repaint(). This is
     * synchronized because it is called by a timer once a second, and is also called by the thread
     * that manages the mouse whenever a user changes a tab in this window or changes a tab in the
     * Network tab.
     */
    static synchronized void prePaintThenRepaint() {
        try {
            ...
            } else { // retry once a second until at least one member exists. Then choose the first
                // one.
                if (tabSwirlds != null) {
            ...
            if (win != null) {
                if (!win.didInit
                        ...
                        && tabSecurity != null) {
                    win.didInit = true;
                    // just once, do an init that does prePaint for everyone, so when we switch
                    // tabs, it
                    // appears
                    // instantly
                    tabSwirlds.prePaint();
    ...
    /**
     * This constructor creates the contents of the browser window, and creates a new thread to
     * continually update this window to reflect what is happening in the Browser.
     */
    public WinBrowser() {
        ActionListener repaintPeriodically =
                new ActionListener() {
                    public void actionPerformed(ActionEvent evt) {
                        // Do a (possibly slow) prePaint of components, like changing text in a
                        // JTextArea text.
                        // Then trigger a (fast) redrawing of the components, like rendering the
                        // JTextArea text.
                        prePaintThenRepaint();
                    }
                };

        // The following adjusts the size of everything slightly on MacOS (should use EmptyBorder
        // instead)
    ...
    /**
     * Add this to a window as a listener so that when the window closes, so does the entire
     * program, including the browser and all platforms.
     *
    ...
    /**
     * Instantiates and returns a JTextArea whose settings are suitable for use inside the browser
     * window's scroll area in a tab.
     */
