...
public final class Utilities {
     ...
     *
     * <p>A false is defined to be any string that, after trimming leading/trailing whitespace and
     * conversion to lowercase, is equal to null, or the empty string, or "off" or "0", or starts
     * with "f" or "n". All other strings are true.
     *
    ...
    /**
     * Do a deep clone of any serialiazable object that can reach only other serializable objects
     * through following references.
     *
    ...
    /**
     * Compare arrays lexicographically, with element 0 having the most influence. A null array is
     * considered less than a non-null array. This is the same as Java.Util.Arrays#compar
     *
     * @param sig1 first array
     * @param sig2 second array
    ...
    /**
     * Compare arrays lexicographically, with element 0 having the most influence, as if each array
     * was XORed with whitening before the comparison. The XOR doesn't actually happen, and the
    ...
    public static <T> void writeList(
            List<T> list, SerializableDataOutputStream stream, Serializer<T> serializer)
            throws IOException {
        if (list == null) {
            stream.writeInt(-1);
            return;
    ...
    public static <T> List<T> readList(
            SerializableDataInputStream stream,
    ...
    /**
     * Convert the given long to bytes, big endian, and put them into the array, starting at index
     * start
     *
     * @param bytes the array to hold the Long.BYTES bytes of result
     * @param n the long to convert to bytes
     * @param start the bytes are written to Long.BYTES elements of the array, starting with this
     *     index
     */
     ...
     * @param whole a long value, the whole being considered (such as the sum of the entire stake)
     ...
     *
     * @param ex
     * @return return true if it is a SocketException or is caused by SocketException; return false
     *     otherwise
     */
    ...
    public static boolean isRootCauseSuppliedType(
            final Throwable t, final Class<? extends Throwable> type) {
        if (t == null) {
            return false;
        }
        Throwable cause = t;
