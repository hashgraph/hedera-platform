...
class SyncCaller implements Runnable {
    ...
    /**
     * The platform instantiates this, and gives it the self ID number, plus other info that will be
     * useful to it. The platform can then create a thread to call run(). It will then repeatedly
     * initiate syncs with others.
     *
     ...
     * @param callerNumber 0 for the first caller thread created by this platform, 1 for the next,
     *     etc
     */
    public SyncCaller(
    ...
    /**
     * Chose another member at random, request a sync with them, and perform one sync if they accept
     * the request. A -1 is returned if the sync did not happen for some reason, such as the chosen
     * member wasn't connected, or had a communication error, or was already syncing by calling
     * self, or the lock was still held by the heartbeat thread, or some other reason.
     *
    ...
    private int callRequestSync() {
        ...
        try { // catch any exceptions, log them, and ignore them
            ...
            if (addressBook.getSize() == 1 && callerNumber == 0) {
                ...
                // This is like syncing with self, and then creating an event with otherParent being
                // self.
                ...
                platform.getEventTaskCreator()
                        .createEvent(
                                selfId.getId() /*selfId assumed to be main*/); // otherID (so self
                // will count as
                // the "other")
                Thread.sleep(50);
                platform.getSyncManager().successfulSync();
                // selfId assumed to be main
                return selfId.getIdAsInt(); // say that self just "synced" with self, and created an
                // event for it
            }
            ...
            // the array is sorted in ascending or from highest to lowest priority, so we go through
            // the array and
            // try to
            ...
            for (final Long aLong : nodeList) {
                ...
                try (final MaybeLocked lock =
                    ...
                    // Try to get both locks. If either is unavailable, then try the next node.
                    // Never block.
                    ...
                    try (final MaybeLockedResource<ConnectionManager> resource =
                            platform.getSharedConnectionLocks()
                                    .tryLockConnection(NodeId.createMain(otherId))) {
    ...
    private boolean doReconnect(final List<Long> reconnectNeighbors) {
        ...
        for (final Long neighborId : reconnectNeighbors) {
            ...
            try (final MaybeLockedResource<ConnectionManager> resource =
                    platform.getSharedConnectionLocks()
                            .tryLockConnection(NodeId.createMain(neighborId))) {
                if (!resource.isLockAcquired()) {
                    peerInfo.addPeerInfo(
                            neighborId, "failed to acquire lock, blocked by heartbeat thread");
                    continue;
                }
                final Connection conn = resource.getResource().getConnection();
                if (conn == null || !conn.connected()) {
                    ...
                    peerInfo.addPeerInfo(
                            neighborId, "peer unreachable, unable to establish connection");
                    continue;
                }
                if (!ReconnectUtils.isNodeReadyForReconnect(conn)) {
                    // The other node was unwilling to help this node to reconnect or the connection
                    // was broken
