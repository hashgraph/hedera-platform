...
public class DispatchBuilder implements Mutable, Startable {
     ...
     *
     * @param configuration dispatch configuration
     */
     ...
     *
     * @param owner the object (or the class of the object) that "owns" the observer. This
     *     information is used only for generating documentation, and does not affect the routing of
     *     dispatches. It is safe to pass "this" in a constructor for this parameter, as only the
     *     class if the object is used.
     * @param triggerClass the type of the trigger
     * @param observer the observer
     * @param <BASE_INTERFACE> the base functional interface for the trigger, e.g. {@link
     *     TriggerZero}, {@link TriggerOne}, etc.
     * @param <TRIGGER_CLASS> a specific trigger type, should inherit from the BASE_INTERFACE
     * @return this object
     ...
     * @param triggerClass the type of the trigger
     * @param observer the observer
     * @param comment a comment used to enhance the dispatch flowchart
     * @param <BASE_INTERFACE> the base functional interface for the trigger, e.g. {@link
     *     TriggerZero}, {@link TriggerOne}, etc.
     * @param <TRIGGER_CLASS> a specific trigger type, should inherit from the BASE_INTERFACE
     * @return this object
     ...
     * {@link #registerObserver(Object, Class, Trigger)}.
     *
        ...
        for (final Method method : object.getClass().getDeclaredMethods()) {
            ...
            if (annotation.value().length == 0) {
                throw new IllegalArgumentException(
                        "No triggers specified. At least one trigger type "
     ...
     *
     * @param object the object that is the observer
     * @param method the method that should be called when the dispatch is triggered
     * @param triggerClass the type of the trigger
     * @param comment a comment used to enhance the dispatch flowchart
     */
    private void registerAnnotatedClassMethod(
        ...
        try {
            ...
            throw new RuntimeException(
                    "unable to register observer " + object.getClass() + "." + method.getName(), e);
        }
    }
     ...
     *
     * @param owner the object (or the class of the object) that "owns" the dispatcher. This
     *     information is used only for generating documentation, and does not affect the routing of
     *     dispatches. It is safe to pass "this" in a constructor for this parameter, as only the
     *     class if the object is used.
     * @param triggerClass the type of the dispatch event
    ...
    public <BASE_INTERFACE extends Trigger<BASE_INTERFACE>, DISPATCHER_TYPE extends BASE_INTERFACE>
            BASE_INTERFACE getDispatcher(
                    final Object owner, final Class<DISPATCHER_TYPE> triggerClass) {
     ...
     *     TriggerZero}, {@link TriggerOne}, etc.
     * @param <DISPATCHER_TYPE> a specific dispatcher type, should inherit from the BASE_INTERFACE
     * @param comment a comment on how the dispatch is being used, used to enhance the dispatch
     *     flowchart
     * @return a dispatch method, not null even if no observers have been registered for this event
     */
    ...
    public <BASE_INTERFACE extends Trigger<BASE_INTERFACE>, DISPATCHER_TYPE extends BASE_INTERFACE>
            BASE_INTERFACE getDispatcher(
                    final Object owner,
                    final Class<DISPATCHER_TYPE> triggerClass,
                    final String comment) {
    ...
    /**
     * Get a list of observers for a given trigger type. If that dispatch type does not yet have a
     * list of observers then create one and insert it into the map of observer lists.
     *
     * @param triggerClass the type of trigger
     * @return a list of observers for the trigger type, calling this method more than once for the
     *     same trigger type always returns the same list instance
     */
