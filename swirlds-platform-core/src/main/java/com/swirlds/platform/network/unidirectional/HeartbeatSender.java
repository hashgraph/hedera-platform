...
public class HeartbeatSender implements InterruptableRunnable {
    ...
    public void run() throws InterruptedException {
        ...
        try (LockedResource<ConnectionManager> lc =
                sharedConnectionLocks.lockConnectIfNeeded(otherId)) {
        ...
        Thread.sleep(
                settings.sleepHeartbeatMillis()); // Slow down heartbeats to match the configured
        // interval
    }

    /**
     * Send out a heartbeat and wait to read the ACK response. The connection conn must be non-null,
     * and must have a valid connection. If the wait is too long, it will time out and disconnect.
     *
     * @param conn the connection (which must already be connected)
     */
