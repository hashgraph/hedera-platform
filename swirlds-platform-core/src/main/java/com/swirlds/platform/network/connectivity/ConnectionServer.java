...
/**
 * Listens on a server socket for incoming connections. All new connections are passed on to the
 * supplied handler.
 */
        ...
        while (!serverSocket.isClosed()) {
            try {
                final Socket clientSocket =
                        serverSocket.accept(); // listen, waiting until someone connects
                ...
                if (Thread.currentThread().isInterrupted()) {
                    // since accept() cannot be interrupted, we check the interrupted status on a
                    // timeout and throw
