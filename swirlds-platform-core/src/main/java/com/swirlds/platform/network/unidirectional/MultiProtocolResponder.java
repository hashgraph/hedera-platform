...
/**
 * Accepts incoming protocol requests and passes them on to the appropriate handler based on the
 * value of the initial byte
 */
public class MultiProtocolResponder implements NetworkProtocolResponder {
    ...
    public void protocolInitiated(final byte initialByte, final Connection connection)
        ...
        throw new NetworkProtocolException(
                String.format("No protocol registered for byte: %02x", initialByte));
    }
}
