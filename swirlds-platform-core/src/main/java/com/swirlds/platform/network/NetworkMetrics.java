...
public class NetworkMetrics {
	 ...
	 *
	 * @param platform
	 * 		the {@link AbstractPlatform} that the metrics should be linked with
	 * @throws IllegalArgumentException
	 * 		if {@code platform} is {@code null}
	 */
	public NetworkMetrics(final AbstractPlatform platform) {
		CommonUtils.throwArgNull(platform, "platform");
		this.selfId = platform.getSelfId();

		// 0 during unit tests
		final int abSize = platform.getAddressBook() == null ? 0 : platform.getAddressBook().getSize();

		final Metrics metrics = platform.getMetrics();
		avgPingMilliseconds = IntStream.range(0, abSize)
				.mapToObj(i -> metrics.getOrCreate(
						new RunningAverageMetric.Config(PING_CATEGORY, String.format("ping_ms_%02d", i))
								.withDescription(
				...
				.toList();
		avgBytePerSecSent = IntStream.range(0, abSize)
				.mapToObj(i -> metrics.getOrCreate(
