...
public class NegotiationProtocols {
    ...
    /**
     * @param protocols a list of protocols to negotiate in order of priority
     */
    public NegotiationProtocols(final List<Protocol> protocols) {
        if (protocols == null
                || protocols.isEmpty()
                || protocols.size() > NegotiatorBytes.MAX_NUMBER_OF_PROTOCOLS) {
            throw new IllegalArgumentException(
                    "the list of protocols supplied should be: non-null, non-empty and have a size"
                            + " lower than "
     ...
     * @throws NegotiationException if an invalid ID is supplied
     */
     ...
     * @throws IllegalStateException if no protocol was previously initiated
     */
    ...
    /**
     * If a negotiation exception occurred, we must notify any initiated protocol that the initiate
     * failed
     */
