...
/**
 * Exchanges software versions with the peer, either throws a {@link HandshakeException} or logs an
...
public class VersionCompareHandshake implements ProtocolRunnable {
    ...
    /**
     * Calls {@link #VersionCompareHandshake(SoftwareVersion, boolean)} with throwOnMismatch set to
     * true
     */
     ...
     * @param throwOnMismatch if set to true, the protocol will throw an exception on a version
     *     mismatch. if set to false, it will log an error and continue
     */
    ...
    public void runProtocol(final Connection connection)
        ...
        if (!(peerVersion instanceof SoftwareVersion sv) || version.compareTo(sv) != 0) {
            final String message =
                    String.format(
                            "Incompatible versions. Self version is '%s', peer version is '%s'",
                            version, peerVersion);
            if (throwOnMismatch) {
                throw new HandshakeException(message);
            } else {
                LOG.error(LogMarker.ERROR.getMarker(), message);
            }
        }
    }
}
