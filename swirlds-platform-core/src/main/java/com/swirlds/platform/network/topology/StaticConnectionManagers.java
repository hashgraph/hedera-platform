...
public class StaticConnectionManagers {
    ...
    public StaticConnectionManagers(
            final NetworkTopology topology, final OutboundConnectionCreator connectionCreator) {
        ...
        for (final NodeId neighbor : topology.getNeighbors()) {
            if (topology.shouldConnectToMe(neighbor)) {
                connectionManagers.put(
                        new ConnectionMapping(neighbor, false), new InboundConnectionManager());
            }
    ...
    /**
     * Called when a new connection is established by a peer. After startup, we don't expect this to
     * be called unless there are networking issues. The connection is passed on to the appropriate
     * connection manager if valid.
     *
