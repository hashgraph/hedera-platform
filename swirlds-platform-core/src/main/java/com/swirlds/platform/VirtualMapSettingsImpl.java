...
public class VirtualMapSettingsImpl extends SubSetting implements VirtualMapSettings {
    /**
     * If not set explicitly via {@code virtualMap.numHashThreads}, the number of hash threads
     * defaults to a calculation based on {@link VirtualMapSettings#getPercentHashThreads()} and
    ...
    public void setPercentHashThreads(final double percentHashThreads) {
        if (percentHashThreads < 0.0 || percentHashThreads > UNIT_FRACTION_PERCENT) {
            throw new IllegalArgumentException(
                    "Cannot configure percentHashThreads=" + percentHashThreads);
        }
                ...
                (numHashThreads == -1)
                        ? (int)
                                (Runtime.getRuntime().availableProcessors()
                                        * (getPercentHashThreads() / UNIT_FRACTION_PERCENT))
                        : numHashThreads;
    ...
    public void setPercentCleanerThreads(final double percentCleanerThreads) {
        if (percentCleanerThreads < 0.0 || percentCleanerThreads > UNIT_FRACTION_PERCENT) {
            throw new IllegalArgumentException(
                    "Cannot configure percentCleanerThreads=" + percentCleanerThreads);
        }
    ...
    public int getNumCleanerThreads() {
        ...
        final int threads =
                (numCleanerThreads == -1)
                        ? (int)
                                (numProcessors
                                        * (getPercentCleanerThreads() / UNIT_FRACTION_PERCENT))
                        : numCleanerThreads;
    ...
    public void setNumCleanerThreads(final int numCleanerThreads) {
        if (numCleanerThreads < 0) {
            throw new IllegalArgumentException(
                    "Cannot configure numCleanerThreads=" + numCleanerThreads);
        }
    ...
    public void setMaximumVirtualMapSize(final long maximumVirtualMapSize) {
        if (maximumVirtualMapSize < 1 || maximumVirtualMapSize > Integer.MAX_VALUE) {
            throw new IllegalArgumentException(
                    "Cannot configure maximumVirtualMapSize=" + maximumVirtualMapSize);
        }
    ...
    public void setVirtualMapWarningInterval(final long virtualMapWarningInterval) {
        if (virtualMapWarningInterval < 1
                || virtualMapWarningInterval > virtualMapWarningThreshold) {
            throw new IllegalArgumentException(
    ...
    /**
     * Set the preferred maximum number of virtual maps (in a fast copy family) that are waiting to
     * be flushed.
     */
    ...
    /**
     * Set the maximum amount of time that any virtual map fast copy will be delayed due to a flush
     * backlog.
     */
