...
/**
 * This class contains only static methods, used to encode and decode information into byte arrays
 * for use in the API.
 */
class APIUtils {
    ...
    /**
     * Given a number in this sequence, return the next number in the sequence. The sequence starts
     * at zero and goes through all positive and negative long values: {0, -1, 1, -2, 2, -3, 3 ...
     * -(1&lt;&lt;63)}. After the last number in the sequence, it wraps around and returns zero.
     *
     * @param n a number in the sequence
     ...
     *
     * <p>The varint format is described <a
     * href="https://developers.google.com/protocol-buffers/docs/encoding">here</a>.
     *
     * @param n the long to write
     * @param dos the stream to write to
     * @return the number of bytes that were written
     * @throws IOException if the stream throws this exception while writing to it
     */
     ...
     * @throws IOException if the stream throws this exception while reading from it, or if the
     *     number being read has an illegal format
     */
    ...
    /**
     * Write a duration in seconds, using as few bytes as possible, and return how many bytes were
     * written. The duration is rounded down to the nearest second before writing it.
     *
    ...
    /**
     * Write a duration in nanoseconds, using as few bytes as possible, and return how many bytes
     * were written. The full (64+32)-bit duration is written (seconds and nanoseconds), written as
     * a varint and an int. The int is 4 bytes, big endian, 2s complement.
     *
     ...
     * @throws IOException if the stream throws this exception while writing to it
     */
    public static int writeDurationNano(Duration duration, DataOutputStream dos)
            throws IOException {
    ...
    /**
     * Write the duration from reference to time, in seconds, using as few bytes as possible, and
     * return how many bytes were written. The duration is rounded down to the nearest second before
     * writing it.
     *
     * @param reference the starting time for calculating the duration
     * @param time the ending time for calculating the duration
     * @param dos the stream to write to
     * @return the number of bytes that were written
     * @throws IOException if the stream throws this exception while writing to it
     */
    public static int writeTimeSecRelative(Instant time, Instant reference, DataOutputStream dos)
            throws IOException {
        return writeDurationSec(Duration.between(reference, time), dos);
    }

    /**
     * Write the duration from reference to time, in nanoseconds, using as few bytes as possible,
     * and return how many bytes were written. The full (64+32) bit duration is written (seconds and
     * nanoseconds), written as a varint and an int.
     *
     * @param reference the starting time for calculating the duration
     * @param time the ending time for calculating the duration
     * @param dos the stream to write to
     ...
     * @throws IOException if the stream throws this exception while writing to it
     */
    public static int writeTimeNanoRelative(Instant time, Instant reference, DataOutputStream dos)
            throws IOException {
        return writeDurationNano(Duration.between(reference, time), dos);
    }

    /**
     * Write the time, in seconds, using as few bytes as possible, and return how many bytes were
     * written. The time is rounded down to the nearest second.
     *
    ...
    /**
     * Write the time, in nanoseconds, using as few bytes as possible, and return how many bytes
     * were written. The full (64+32) bit duration is written (seconds and nanoseconds), written as
     * a varint and an int. The int is 4 bytes, big endian, 2s complement.
     *
     * @param time the time to write
     ...
     * @param dis the stream to read from
     * @return the duration
     * @throws IOException if the stream throws this exception while reading from it
     */
    ...
    /**
     * Read a time that was written with writeTimeSecRelative using the same reference. The result
     * will be incorrect if the reference is different.
     *
     * @param reference the reference time for calculating this time
     * @param dis the stream to read from
     * @return the time
     * @throws IOException if the stream throws this exception while reading from it
     */
    public static Instant readTimeSecRelative(Instant reference, DataInputStream dis)
            throws IOException {
    ...
    /**
     * Read a time that was written with writeTimeNanoRelative using the same reference. The result
     * will be incorrect if the reference is different.
     *
     * @param reference the reference time for calculating this time
     * @param dis the stream to read from
     ...
     * @throws IOException if the stream throws this exception while reading from it
     */
    public static Instant readTimeNanoRelative(Instant reference, DataInputStream dis)
            throws IOException {
    ...
    /**
     * Write then read the given long as a varint. This is purely for testing and debugging. It
     * should never be called by production code.
     *
     * @param n the number to test (by writing then reading it).
     */
    ...
    /**
     * Test writing then reading 10 numbers starting with n. This is purely for testing and
     * debugging. It should never be called by production code.
     *
