        ...
        try {
            ...
            if (r0 == -1) {
                ...
                s +=
                        String.format(
                                "\n"
                                        + "%,10d = latest supermajority signed state round-decided"
                                        + " (deleted round +%,d)",
                                r0, r0 - r1);
            }
            ...
            s +=
                    Utilities.wrap(
                            70,
                            "\n\n"
                                + "After each round, there is a consensus state, which is the"
                                + " result of all the transactions so far, in their consensus"
                                + " order. Each member signs that state, and sends out their"
                                + " signature. \n\n"
                                + "The above shows how one member (shown at the top of this window)"
                                + " is collecting those signatures. It shows how many transactions"
                                + " have achieved consensus so far, and the latest round number"
                                + " that has its  events discarded, the latest that has collected"
                                + " signatures from members with at least 1/3 of the total stake,"
                                + " the latest that has its famous witnesses decided (which is the"
                                + " core of the hashgraph consensus algorithm), and the latest that"
                                + " has at least one known event. \n\n"
                                + "For each round, the table shows the round number, then the count"
                                + " of how many signatures are collected so far, then an indication"
                                + " of whether this represents everyone (___), or not everyone but"
