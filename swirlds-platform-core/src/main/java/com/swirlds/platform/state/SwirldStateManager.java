...
public interface SwirldStateManager
        extends FreezePeriodChecker,
                TransThrottleSyncAndCreateRule,
                Clearable,
                LoadableFromSignedState {

    /**
     * Invokes the pre-handle method. Called after the event has been verified but before {@link
     * #handlePreConsensusEvent(EventImpl)}.
     *
    ...
    /**
     * Provides a {@link Runnable} to execute while waiting for pre-consensus events (q1) to
     * process.
     *
     * @return the runnable
     */
    default InterruptableRunnable getPreConsensusWaitForWorkRunnable() {
        // tells QueueThread to execute its default method when there is nothing in the queue to
        // process
        return null;
    }
    ...
    /**
     * Determines if a pre-consensus event should be discarded or added to the pre-consensus queue
     * (q1) for processing.
     *
     * @param event the event to discard or not
    ...
    /**
     * Updates the state to a fast copy of itself and returns a reference to the previous state to
     * be used for signing. The reference count of the previous state returned by this is
     * incremented to prevent it from being garbage collected until it is put in a signed state, so
     * callers are responsible for decrementing the reference count when it is no longer needed.
     *
     * <p>Consensus event handling will block until this method returns. Pre-consensus event
     ...
     *
     * <p>Invoked only by the consensus handling thread, so there is no chance of the state being
     * modified by a concurrent thread.
     */
    void savedStateInFreezePeriod();

    /**
     * Return the current state of the app. It changes frequently, so this needs to be called
     * frequently. This method also guarantees that the state will not be deleted until {@link
     * #releaseCurrentSwirldState()} is invoked.
     *
    ...
    /**
     * Returns the consensus state. The consensus state could become immutable at any time.
     * Modifications must not be made to the returned state.
     */
    State getConsensusState();

    /**
     * Releases the state that was previously returned, so that another one can be obtained from
     * {@link #getCurrentSwirldState()}, and deletes it if it's not the current state being used.
     */
    ...
    /**
     * Submits a self transaction for any necessary processing separate from the transaction's
     * propagation to the network. A transaction must only be submitted here if it is also submitted
     * for network propagation in {@link EventTransactionPool}.
     *
    ...
    /**
     * Submits a self transaction (i.e. a transaction created by this node and put into a self
     * event). Implementations must submit this transaction for network propagation in {@link
    ...
    /**
     * Called during recovery. Updates the dual state status to clear any possible inconsistency
     * between freezeTime and lastFrozenTime.
     */
