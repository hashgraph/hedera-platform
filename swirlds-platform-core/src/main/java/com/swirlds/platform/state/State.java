...
/**
 * The root of the merkle tree holding the state of the Swirlds ledger. Contains three children: the
 * state used by the application; the state used by the platform; and the state used by both
...
public class State extends PartialNaryMerkleInternal implements MerkleInternal {
    ...
    private static class ChildIndices {
        /**
         * The state written and used by the application. It is the state resulting from all
         * transactions in consensus order from all events with received rounds up through the round
        ...
        switch (index) {
            ...
            case ChildIndices.PLATFORM_STATE:
                return childClassId == PlatformState.CLASS_ID
                        || childClassId == LegacyPlatformState.CLASS_ID;
