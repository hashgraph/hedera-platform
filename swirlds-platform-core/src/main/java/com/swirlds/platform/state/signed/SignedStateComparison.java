...
public final class SignedStateComparison {
    ...
    /**
     * Get a node at a given position within a tree. If there is no node in that position then
     * return null.
     */
    ...
    private static Predicate<MerkleInternal> buildDescendantFilter(
            final MerkleNode rootB, final boolean deep) {
     ...
     * @param deep if true then use deep comparison. This is only needed if there are internal node
     *     hashes that are incorrect, causing the hash mismatch to be invisible from the root of the
     *     tree. This may take significantly longer than a shallow comparison.
    ...
    public static void printMismatchedNodes(
            final Iterator<MismatchedNodes> nodeIterator, final int limit) {
        ...
        while (nodeIterator.hasNext()) {
            ...
            nodeIterator.next().appendNodeDescriptions(sb);
        }
