...
public class DualStateImpl extends PartialMerkleLeaf
    ...
    @Override
    public int hashCode() {
        return new HashCodeBuilder(17, 37).append(freezeTime).append(lastFrozenTime).toHashCode();
    }
    ...
    public boolean isInFreezePeriod(Instant consensusTime) {
        // if freezeTime is not set, or consensusTime is before freezeTime, we are not in a freeze
        // period
        // if lastFrozenTime is equal to or after freezeTime, which means the nodes have been frozen
        // once at/after the
