 ...
 * signatures. Unlike
 * {@link com.swirlds.common.system.state.notifications.NewSignedStateNotification NewSignedStateNotification}, this
 * is called on every single state that becomes signed, even if signing happens out of order. NOT called when a state
 * is read from disk or received from reconnect. State is guaranteed to hold a weak reservation until callback
 * is finished.
 */
public class StateHasEnoughSignaturesNotification extends AbstractNotification {
	 ...
	 *
	 * @param signedState
	 * 		the state that just became signed
	 * @param selfId
	 * 		the ID of this node
	 */
