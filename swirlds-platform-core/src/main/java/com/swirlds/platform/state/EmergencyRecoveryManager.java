...
public class EmergencyRecoveryManager {
    ...
    public EmergencyRecoveryManager(
            ...
            final Path emergencyRecoveryDir) {
    ...
    /**
     * Returns whether an emergency state is required to start the node. The state can be loaded
     * from disk or acquired via an emergency reconnect.
     *
    ...
    private EmergencyRecoveryFile readEmergencyRecoveryFile(final Path dir) {
        try {
            ...
            shutdownRequestedTrigger.dispatch(
                    "Emergency Recovery Error", EMERGENCY_RECOVERY_ERROR.getExitCode());
            return null;
        }
    }
}
