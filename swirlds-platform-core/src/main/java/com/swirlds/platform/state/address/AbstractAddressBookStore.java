...
public abstract class AbstractAddressBookStore implements AddressBookStore {
	...
	public void add(final AddressBook addressBook) {
		...
		if (getSize() == 0) {
			...
			if (round != getLatestRound() + 1) {
				LOG.warn(STARTUP.getMarker(), "gap in address books detected, " +
			...
			if (!isNextAddressBookValid(getLatest(), addressBook)) {
				LOG.error(EXCEPTION.getMarker(),
						"address book for round {} is invalid", addressBook.getRound());

				// carry forward the previous address book
				appendAddressBook(
						getAddressBookMap().get(getLatestRound()).copy().setRound(addressBook.getRound()).seal());
			} else {
	...
	public AddressBook get(final long round) {
		...
		final AddressBook addressBook = AddressBookSettingsFactory.get().isUpdateAddressBookOnlyAtUpgrade() ?
