...
public class AddressBookManagerImpl implements MutableAddressBookManager {
     ...
     *
     * @param initialStore the initial address book
     * @param numberOfRoundsToStore the total number of rounds to keep in memory
     */
    public AddressBookManagerImpl(
            final AddressBookStore initialStore, final int numberOfRoundsToStore) {

        // FUTURE WORK: fix skipped rounds so that we don't need to tolerate gaps

        if (initialStore.getSize() < numberOfRoundsToStore) {
            throw new IllegalArgumentException(
                    "The initial store has "
                            + initialStore.getSize()
                            + " rounds, but "
                            + numberOfRoundsToStore
                            + " rounds are needed to initialize the address book manager");
        }
     ...
     *
     * @param mostRecentAddressBook the most recent address book
     */
    @Override
    ...
    public AddressBook get(final long round) throws InterruptedException {
        try {
            ...
            // This should never throw an execution exception, but we have to catch it to satisfy
            // the compiler.
            throw new RuntimeException(
                    "Unable to get address book for round " + round + " due to exception", e);
        }
    }
