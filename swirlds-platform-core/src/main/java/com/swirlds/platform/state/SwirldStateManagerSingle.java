...
/**
 * Manages all interactions with the 3 state objects required by {@link SwirldState}.
 *
 * <p>Three threads modify states in this class: pre-consensus event handler, consensus event
 * handler, and thread-work (managed by this class). Transactions are submitted by a different
 * thread. Other threads can access parts of the states by calling {@link #getCurrentSwirldState()}
 * and {@link #getConsensusState()}. Sync threads access state to check if there is an active freeze
 * period. Careful attention must be paid to changes in this class regarding locking and
 * synchronization in this class and its utility classes.
 */
public class SwirldStateManagerSingle implements SwirldStateManager {
    ...
    /**
     * applies event in the forWork queue to stateWork then adds them to forNext. Marked as volatile
     * so its queue can be shuffled.
     */
    ...
    /**
     * Used to perform a "shuffle" of states and lists in order to update the current state with
     * consensus ordered transactions.
     */
    private final Shuffler shuffler;

    /**
     * Keeps the state returned to the user app to ensure that it doesn't get deleted. Must only be
     * accessed in synchronized blocks.
     */
    ...
    /**
     * Removes and returns a single transaction from the queue of transactions to be applied to
     * stateCurr.
     */
    ...
    public SwirldStateManagerSingle(
        ...
        shuffleBarrier =
                new CyclicBarrier(
                        THREAD_NUM_SWIRLD_STATE_SINGLE,
                        () -> {
                            try {
                                shuffler.shuffle();
     ...
     * {@inheritDoc}
     *
     * <p>Self events should not be passed here.
     */
    @Override
    ...
    private void updateEstimatedTime(final boolean isConsensus, final EventImpl event) {
        ...
        // time was estimated before, but we update it again here to estimate with the latest
        // information
        event.estimateTime(
                selfId,
    ...
    /**
     * Prevent self events from being added to the pre-consensus queue because self transactions are
     * processed through the {@link SwirldStateSingleTransactionPool} {@code transCurr} queue.
     *
    ...
    /**
     * If there is no event in the forWork event queue, shuffle it is it time, and handle self
     * transactions while waiting for an item to become available.
     */
    ...
    private boolean shuffleIfTimeToShuffle(final StateInfo stateInfo) {
        ...
        if (getMillisUntilNextShuffle() <= 0) {
            // wait until all 3 threads are waiting, then call shuffle(), then allow all 3 to
            // continue
    ...
    /**
     * Executed for each event in the threadWork queue. Handles transactions in the work queue
     * before applying the event to stateWork.
     *
     * <p>Unlike the methods to handle events in the pre-consensus and consensus queues, this method
     * does not check to see if it is time to shuffle. This is because threadQueue removes events
     * from the queue in bulk, then holds them in a buffer, passing them to this method one by one.
     * If threadWork enters a shuffle here, any events in the buffer would not be applied to
     * stateWork prior to the shuffle.
     *
     * <p>Because the first thing the shuffle does it drain the work transactions and threadWork
     * queue, it is ok to only check if it is time to shuffle when the threadWork queue is empty.
     * When threadCurr is waiting at the shuffle barrier, threadWork will continue draining its
     * queue. When it is empty (and threadCurr has stopped putting events in because it is waiting),
     * threadWork will execute the {@link #forWorkWaitForItem} runnable and see that it is time to
     * shuffle.
     *
    ...
    /**
     * Submits a background task that polls self transactions from the transCons queue, one for each
     * transaction in the round. Removing these transactions from transCons prevents them from being
     * sent to the new stateWork to handle after the next shuffle.
     *
    ...
    /**
     * Check if this event has a consensus time before or equal to the latest consensus event
    ...
    /**
     * Clears the work event queue, the transaction pool and releases the working and current
     * states.
     *
     * <p>NOTE: This will only clear current data, it will not prevent new data from being added
     * while clear is being called
     *
     * <p>This method also pauses thread work, for this instance to continue working, this thread
     * needs to be un-paused
     */
    @Override
    public void clear() {
    ...
    private static BlockingQueue<EventImpl> newQueue() {
        return new PriorityBlockingQueue<>(
                INITIAL_WORK_EVENT_QUEUE_CAPACITY, EventUtils::consensusPriorityComparator);
    }
    ...
    public synchronized void releaseCurrentSwirldState() {
        ...
        if (stateCurrReturned != null
                && (stateCurr == null || stateCurrReturned != stateCurr.getState())) {
    ...
    /**
     * Performs a "shuffle" of states, event queues, and transaction queues in order to update the
     * current state with consensus ordered transactions without requiring any one instance of the
     * state to handle any transactions more than once.
     */
    private class Shuffler {

        /**
         * do a shuffle, where stateCurr is discarded and replaced with stateWork, and stateWork is
         * replaced with a copy of stateCons, and transactionPool shuffles accordingly. This is
         * called by the shuffleBarrier while all 3 threads are waiting: thread-curr, thread-work,
         * thread-cons
         */
        public void shuffle() {
            ...
            // both queues feeding stateWork are now empty, and will stay empty (except for new
            // transactions
            // that won't hurt us because they will never get a chance to be handled by the old
            // stateCurr, and
            // they'll go into transactionPool queues for both forCurr and forWork). All 3 threads
            // are waiting now,
            ...
            // be a performance difference between this implementation and swapping pointers to
            // queues.
        ...
        /**
         * Deletes stateCurr if it is not the last one returned by {@link
         * SwirldStateManagerSingle#getCurrentSwirldState()}. If it is, then it will not delete now
         * and that state will be deleted on the next {@link
         * SwirldStateManagerSingle#releaseCurrentSwirldState()} invocation. Also replaces stateCurr
         * with stateWork.
         *
         * <p>Synchronize on the outer class because that is where threads not involved in the
         * shuffle could access stateCurr by calling {@link
         * SwirldStateManagerSingle#getCurrentSwirldState()}.
         */
