...
/**
 * Similar to an {@link java.util.concurrent.atomic.AtomicReference AtomicReference&lt;SignedState&gt;},
 * but with the additional functionality of holding a strong reservation on the contained state.
 */
public class SignedStateReference {
	 ...
	 *
	 * @param strong
	 * 		if true then take a strong reservation (i.e. a reservation that prevents archival), otherwise
	 * 		take a weak reservation
	 * @return an autocloseable wrapper with the state, when closed the state is released
	 */
	...
	public synchronized AutoCloseableWrapper<SignedState> get(final boolean strong) {
		...
		if (stateToReturn != null) {
			if (strong) {
				stateToReturn.reserveState();
			} else {
				stateToReturn.weakReserveState();
			}
		}

		return new AutoCloseableWrapper<>(stateToReturn, () -> {
			if (stateToReturn != null) {
				if (strong) {
					stateToReturn.releaseState();
				} else {
					stateToReturn.weakReleaseState();
				}
			}
		});
	}
