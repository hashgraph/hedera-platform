...
/**
 * An {@link AddressBookStore} that supports branching copies (i.e. when a copy is made, both the
 * new and the original remain mutable). This address book is compatible with {@link
 * com.swirlds.common.system.SwirldState SwirldState1}. Although technically compatible with {@link
 * com.swirlds.common.system.SwirldState SwirldState2}, it is less performant.
 */
public class BranchingAddressBookStore extends AbstractAddressBookStore
        implements ComposedMerkleLeaf {
    ...
    public void deserialize(final SerializableDataInputStream in, final int version)
        ...
        for (long round = earliestRound; round <= latestRound; round++) {
            addressBookMap.put(
                    round, Objects.requireNonNull(in.readSerializable(false, AddressBook::new)));
        }
    }
    ...
    protected void removeOldest() {
        ...
        final AddressBook addressBookToRemove =
                Objects.requireNonNull(
                        addressBookMap.remove(earliestRound),
                        "address book for round not found for round " + earliestRound);
