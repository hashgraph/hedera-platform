...
public final class SignedStateComparisonUtility {
    ...
    public static void loadClasses()
            throws SocketException, UnknownHostException, AppLoaderException,
    ...
    private static BiPredicate<MerkleNode, MerkleRoute> buildFilter(final MerkleNode rootB) {
        return (final MerkleNode nodeA, final MerkleRoute routeA) -> {
            ...
            final Hash hashA =
                    nodeA == null ? CryptoFactory.getInstance().getNullHash() : nodeA.getHash();
            final Hash hashB =
                    nodeB == null ? CryptoFactory.getInstance().getNullHash() : nodeB.getHash();

            if (hashA == null) {
                throw new IllegalStateException(
                        "Node "
                                + nodeA.getClass().getName()
                                + " at position "
                                + nodeA.getRoute()
                                + " is unhashed");
            }
    ...
    private static Predicate<MerkleInternal> buildDescendantFilter(final MerkleNode rootB) {
        return (final MerkleInternal nodeA) -> {
            ...
            if (nodeB == null || nodeB.isLeaf()) {
                // There is no point in iterating over descendants if nodeB is incapable of having
                // them
                return false;
            }
    ...
    /**
     * Create a lambda that will transform the comparison iterator into an iterator that returns
     * pairs of nodes that are different.
     */
    private static BiFunction<MerkleNode, MerkleRoute, MismatchedNodes> buildTransformer(
            final MerkleNode rootB) {
        return (final MerkleNode nodeA, final MerkleRoute routeA) -> {
            MerkleNode nodeB;
            try {
                nodeB = rootB.getNodeAtRoute(routeA);
    ...
    /**
     * Builds an iterator that walks over the parts of the merkle trees that do not match each
     * other. Nodes are visited in pre-ordered depth first order.
     *
    ...
    public static Iterator<MismatchedNodes> mismatchedNodeIterator(
            final MerkleNode rootA, final MerkleNode rootB) {
        ...
        // This iterator could be made more efficient, but since the differences in a state are
        // likely to be
        // small compared to the size of the state, such an optimization may not be worth the
        // effort.
     ...
     *
     * @param rootA a merkle tree, must be hashed
     * @param rootB a merkle tree, must be hashed
     * @param limit the maximum number of nodes to print
     */
    public static void printMismatchedNodes(
            final MerkleNode rootA, final MerkleNode rootB, final int limit) {
        ...
        if (iterator.hasNext()) {
            ...
            System.out.println(
                    "Maximum number of differences printed. "
                            + leftoverNodes
                            + " difference"
                            + (leftoverNodes == 1 ? "" : "s")
                            + " remain"
    ...
    /**
     * Must be run from SDK directory. Compares two states and writes the differences to standard
     * out.
     *
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length < 2) {
            System.out.println(
                    "Usage: SignedStateComparisonUtility <path_to_stateA> <path_to_stateB>\n");
            System.exit(1);
        }
        ...
        System.out.println(
                "State A hash = "
                        + stateA.getState().getHash()
                        + ", round = "
                        + stateA.getState().getPlatformState().getPlatformData().getRound());

        System.out.println("Loading state B from " + stateBPath);
        final SignedState stateB = loadSignedState(stateBPath);
        System.out.println(
                "State B hash = "
                        + stateB.getState().getHash()
                        + ", round = "
                        + stateB.getState().getPlatformState().getPlatformData().getRound());
