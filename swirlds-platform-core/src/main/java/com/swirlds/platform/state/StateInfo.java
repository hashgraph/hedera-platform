...
public class StateInfo implements Releasable {
    ...
    /**
     * lastCons is a consensus event whose transactions have already been handled by state, as have
     * all consensus events before it in the consensus order.
     */
     ...
     *
     * @param state the state to feed transactions to
     * @param lastCons the last consensus event, in consensus order
     */
     ...
     * AtomicReference#getAndUpdate(UnaryOperator)} to update the state, so the update function must
     * not have any side effects.
     *
     * @param updateFunction the side effect free update function to apply to the state
     */
