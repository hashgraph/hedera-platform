...
public class TransactionHandler {
    ...
    /**
     * Applies all transaction in an event to the {@link SwirldState1#preHandle(Transaction)} method
     * and handles any exceptions gracefully.
     *
     * @param event the event with transactions to apply
     * @param swirldState the swirld state to apply {@code event} to
     */
    ...
    /**
     * Applies a transaction to the {@link SwirldState1#preHandle(Transaction)} method and handles
    ...
    /**
     * Applies an event to {@link SwirldState1} as a pre-consensus event and handles any exceptions
     * gracefully.
     *
     * @param swirldState the swirld state to apply the transactions to
     * @param dualState the dual state associated with {@code swirldState}
     * @param event the event to apply
     */
    ...
    /**
     * Applies a consensus round to SwirldState, handles any exceptions gracefully, and updates
     * relevant statistics.
     *
            ...
            for (final EventImpl event : round.getConsensusEvents()) {
                ...
                if (event.getReachedConsTimestamp() != null) {
                    stats.consensusToHandleTime(
                            event.getReachedConsTimestamp().until(timeOfHandle, ChronoUnit.NANOS)
    ...
    /**
     * Applies transactions to a state. These transactions have no event context and have not
     * reached consensus.
     *
     * @param numTransSupplier a supplier for the number of transactions the {@code transSupplier}
     *     can provide
     * @param transSupplier the supplier of transactions to apply
     * @param swirldState the swirld state to apply the transactions to
     * @param dualState the dual state associated with {@code swirldState}
     */
    ...
    /**
     * Applies a single pre-consensus self transaction to {@link SwirldState1} and handles any
     * exceptions gracefully.
     *
     * @param swirldState the swirld state to apply the transactions to
     * @param dualState the dual state associated with {@code swirldState}
     * @param consTime the estimated consensus time of the transaction
     * @param transaction the transaction to apply
     */
