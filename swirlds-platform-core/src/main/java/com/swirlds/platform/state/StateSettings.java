...
public class StateSettings extends SubSetting {
     ...
     * unless the provided path begins with "/", in which case it will be interpreted as an absolute
     * path.
     */
    public String savedStateDirectory = "data/saved";

    /**
     * If true, clean out all data in the {@link #savedStateDirectory} except for the previously
     * saved state.
     */
    public boolean cleanSavedStateDirectory = false;

    /**
     * The number of states permitted to sit in the signed state file manager's queue of states
     * being written. If this queue backs up then some states may not be written to disk.
     */
     ...
     * so that we don't delete an old state while a new one is in the process of writing to disk.
     * set to 0 to not keep any states to disk.
     */
    ...
    /**
     * The minimum time, in seconds, that must transpire after dumping a state before another state
     * dump is permitted.
     */
    public long secondsBetweenISSDumps = Duration.ofHours(6).toSeconds();

    /**
     * Dump the state when there is an ISS even if this node is not in an ISS state. This feature is
     * for debugging purposes and should not be active in production systems.
     */
    public boolean dumpStateOnAnyISS = false;

    /**
     * Halt this node whenever any ISS in the network is detected. A halt causes the node to stop
    ...
    /**
     * If true, then halt this node if a catastrophic ISS is detected. A halt causes the node to
     * stop doing work, but does not shut down the JVM.
     */
    public boolean haltOnCatastrophicIss = false;

    /**
     * If true then a single background thread is used to do validation of signed state hashes.
     * Validation is on a best effort basis. If it takes too long to validate a state then new
     * states will be skipped.
     */
    ...
    /**
     * If there are problems with state lifecycle then write errors to the log at most once per this
     * period of time.
     */
    public int stateDeletionErrorLogFrequencySeconds = 60;

    /** When enabled, hashes for the nodes are logged per round. */
    public boolean enableHashStreamLogging =
