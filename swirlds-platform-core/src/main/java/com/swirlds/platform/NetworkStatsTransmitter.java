...
/**
 * This class is responsible for creating system transactions containing network statistics. The
 * system transactions created by this class are standard transactions (not priority transactions).
 */
public final class NetworkStatsTransmitter implements Startable, Stoppable {
    ...
    public void start() {
        if (basicConfig.enablePingTrans()) {
            executorService.scheduleAtFixedRate(
                    this::transmitStats, 0, basicConfig.pingTransFreq(), TimeUnit.SECONDS);
        }
    }
    ...
    private void transmitStats() {
        // Send a transaction giving the average ping time from self to all others (in
        // microseconds).
        // This data will eventually be used by chatter to optimize broadcast trees when they are
        // implemented.
