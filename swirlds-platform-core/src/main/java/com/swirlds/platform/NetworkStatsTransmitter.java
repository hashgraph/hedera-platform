...
public final class NetworkStatsTransmitter {
	 ...
	 */
	public static void transmitStats(final AbstractPlatform platform, final NetworkMetrics networkMetrics) {
		...
		if (SettingsCommon.enablePingTrans) {
			...
			for (int i = 0; i < n; i++) {
				avgPingMilliseconds[i] = (int)
						(networkMetrics.getAvgPingMilliseconds().get(i).get() * MILLISECONDS_TO_SECONDS);
			}
			final SystemTransaction systemTransaction = new SystemTransactionPing(avgPingMilliseconds);
			final boolean good = platform.createSystemTransaction(systemTransaction);
			if (!good) {
				LOG.error(EXCEPTION.getMarker(),
						"failed to create ping time system transaction)");
			}
		}
	}
}
