...
public final class OSEntropyChecker {
    ...
    /**
     * Perform entropy checks using the various sources of {@link java.security.SecureRandom} used
     * in the platform.
     *
     * @param sb the string builder to append the report to
     * @param osHealthCheckConfig config values for OS health checks
     * @return {@code true} if all the checks passed, {@code false} otherwise
     */
    public static boolean performEntropyChecks(
            final StringBuilder sb, final OSHealthCheckConfig osHealthCheckConfig) {
        return performEntropyChecks(
                sb,
                osHealthCheckConfig,
                List.of(
                        // Test this type because it is used by the platform, check that it does not
                        // hang
    ...
    private static boolean performEntropyCheck(
            final StringBuilder sb,
            final OSHealthCheckConfig osHealthConfig,
            final EntropySource entropySource) {
        final long maxGenNanos =
                TimeUnit.MILLISECONDS.toNanos(osHealthConfig.maxRandomNumberGenerationMillis());
        try {
     ...
     * @param entropyReport the entropy check report
     * @param maxGenNanos the maximum number of nanos the check may take before it is considered
     *     failed
