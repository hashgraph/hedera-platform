...
/**
 * Checks that the operating system is able to open a file and read a byte from it without throwing
 * an exception or timing out.
 */
public final class OSFileSystemCheck {
     ...
     *
     * @param fileToRead the file to read. Must contain at least 1 byte of data.
     * @return the file system check report
     * @throws InterruptedException if this thread is interrupted while waiting for the file read to
     *     complete
     */
     ...
     *
     * @param fileToRead the file to read. Must contain at least 1 byte of data.
     * @param timeoutMillis the maximum number of milliseconds to wait for the first byte of a file
     *     to be read
     * @return the file system check report
     * @throws InterruptedException if this thread is interrupted while waiting for the file read to
     *     complete
     */
    public static Report execute(final Path fileToRead, final long timeoutMillis)
        ...
        final OSHealthCheckUtils.SupplierResult<Byte> result =
     ...
     * @param readNanos the number of nanoseconds it took to open and read the first byte of a file,
     *     or {@code null} if the test failed
     * @param data the first byte of the provided file, or {@code null} if the file could not be
     *     read
     * @param exception if the check resulted in an exception, the exception that was thrown.
     *     Otherwise, {@code null}
     */
