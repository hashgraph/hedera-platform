...
public final class OSFileSystemChecker {
    ...
    public static boolean performFileSystemCheck(
        ...
        try {
            ...
            throw new RuntimeException(
                    "Thread interrupted while performing OS file system check", e);
        }
    }
     ...
     * @param maxFileReadMillis the maximum number of millis the file read may take before it is
     *     considered failed
    ...
    private static boolean appendReport(
            ...
            final OSFileSystemCheck.Report fileSystemReport,
            final long maxFileReadMillis) {
        if (fileSystemReport.code() == OSFileSystemCheck.TestResultCode.SUCCESS) {
            final double readMillis =
                    fileSystemReport.readNanos() * Units.NANOSECONDS_TO_MILLISECONDS;
            if (TimeUnit.NANOSECONDS.toMillis(fileSystemReport.readNanos()) > maxFileReadMillis) {
                ...
                addLine(
                        sb,
                        String.format(
                                "OS file read time was too slow. Maximum millis allowed: %d, Took:"
                                        + " %s",
            ...
            if (fileSystemReport.exception() == null) {
                addLine(
                        sb,
                        String.format("OS file check failed. Code: %s", fileSystemReport.code()));
            } else {
                addLine(
                        sb,
                        String.format(
