...
/**
 * Utility class that performs OS clock source speed checks and writes the report to a {@link
 * StringBuilder}.
 */
public final class OSClockSpeedSourceChecker {
     ...
     * @param clockReport the clock source speed check report
     * @param minClockCallsPerSec the minimum number of clock calls per second required for the
     *     check to pass
     * @return {@code true} if the check passed, {@code false} otherwise
     */
    private static boolean appendReport(
            final StringBuilder sb,
            OSClockSourceSpeedCheck.Report clockReport,
            final long minClockCallsPerSec) {
