...
/**
 * Checks that the operating system uses an entropy generator (by asking for a true random number
 * and ensuring the request does not hang) and measures how long it takes to generate a single
 * random number.
 */
public final class OSEntropyCheck {

    /**
     * The default number of milliseconds to wait for the random number generation process to
     ...
     * using the random number generation default algorithm and provider. The operation is aborted
     * if it exceeds the default timeout.
     *
     * @return the entropy check report
     * @throws InterruptedException if this thread is interrupted while waiting for the random
     *     number generation to complete
     */
    ...
    /**
     * Checks that the OS uses entropy and has collected enough of it to generate a random number
     * using the default random number generation algorithm and provider, waiting up to the number
     * of {@code timeoutMillis} for the operation to complete.
     *
     * @param timeoutMillis the number of milliseconds to wait for the random number to be generated
     *     before aborting
     * @return the entropy check report
     * @throws InterruptedException if this thread is interrupted while waiting for the random
     *     number generation to complete
    ...
    public static Report execute(final long timeoutMillis) throws InterruptedException {
        return execute(
                timeoutMillis,
                EntropySource.of(
                        DEFAULT_NUMBER_GENERATION_ALGORITHM, DEFAULT_NUMBER_GENERATION_PROVIDER));
    }

    /**
     * Checks that the OS uses entropy and has collected enough of it to generate a random number.
     *
     * @param timeoutMillis the maximum number of milliseconds to wait for the random number
     *     generation to complete
     * @param entropySource supplies the instance of {@link SecureRandom} to use for the check
     * @return the entropy check report
     * @throws InterruptedException if this thread is interrupted while waiting for the random
     *     number generation to complete
     */
    public static Report execute(final long timeoutMillis, final EntropySource entropySource)
        ...
        final OSHealthCheckUtils.SupplierResult<Long> result =
                timeSupplier(randomRequester, timeoutMillis);
     ...
     *
     * @param success if {@code true}, then the OS collected enough entropy to generate a single
     *     random number prior to the timeout elapsing. {@code false} otherwise.
     * @param entropySource the source of entropy used
     * @param elapsedNanos the number of nanos it took the OS to generate a single random long, or
     *     {@code null} if generation failed. This value measures the time it takes a newly
     *     initialized {@link SecureRandom} to generate a single random long. It is not
     *     representative of the average time to generate a random number.
     * @param randomLong the random long generated by the OS, or {@code null} if generation failed
     * @param exception if the check failed due to an exception, the exception thrown, or {@code
    ...
    public record Report(
        ...
        public static Report success(
                final String entropySource, final long elapsedNanos, final Long randomLong) {
