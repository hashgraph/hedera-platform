...
class WinTab2Stats extends PrePaintableJPanel {
    ...
    public WinTab2Stats() {
        metrics =
                WinBrowser.memberDisplayed.platform.getMetrics().getAll().stream()
        ...
        rm2cm = new int[numStats]; // position i in row major order is position rm2cm[i] in column
        // major
        cm2rm = new int[numStats]; // position j in column major order is position cm2rm[j] in row
        // major
    ...
    private class Chart extends JPanel {
        ...
        /**
         * translate an x value from the buffer to screen coordinates, clipping it to lie inside the
         * chart.
         *
        ...
        private int scaleX(double x) {
            ...
            return (int)
                    (Math.min(1, Math.max(0, (x - minXb) / (maxXb - minXb))) * (maxXs - minXs)
                            + minXs);
        }

        /**
         * translate a y value from the buffer to screen coordinates, clipping it to lie inside the
         * chart.
         *
        ...
        private int scaleY(double y) {
            ...
            return (int)
                    (maxYs
                            - Math.min(1, Math.max(0, (y - minYb) / (maxYb - minYb)))
                                    * (maxYs - minYs));
        }

        /**
         * all x axis labels reflect x values divided by this. This is 1 for seconds, 60 for
         * minutes, etc.
         */
        double xDivisor = 1;
        /**
         * the units for the x axis labels. If "seconds", then xDivisor is 1, if "minutes", then 60,
         * etc.
         */
         ...
         *
         * <p>This method "returns" xLabel and xLabelVal, by changing those class variables.
         */
        private void findYLabel() {
            ...
            double epsilon =
                    Math.pow(10, -1 - decimals); // if showing 2 decimal places, this is 0.001
        ...
        protected void paintComponent(Graphics g) {
            ...
            try {
                ...
                for (int i = 0; i < yLabel.length; i++) {
                    ...
                    g.drawString(
                            yLabel[i], minXs - stringWidth - 10, y + stringHeightNoDesc / 2 - 1);
                }
    ...
    /**
     * make visible the chart with the given index, and turn its box blue. This is also the index of
     * the statistic that it shows.
     *
    ...
    /**
     * make invisible the chart with the given index, and turn its box white. This is also the index
     * of the statistic that it shows.
     *
     * @param index the index of the chart, which is also the index of the statistic
     */
    ...
    public void prePaint() {
        ...
        instructions.setText(
                wrap(
                        50,
                        "The table shows various statistics about how the system is running. Click"
                            + " on a white square to plot the history of the statistic. Click on a"
                            + " highlighted square to hide that plot. \n\n"
                            + "Each plot shows history divided into periods. Within one period, the"
                            + " blue line goes through the average value during that period. The"
                            + " light gray box shows the min and max value during that period. The"
                            + " dark gray box shows one standard deviation above and below the"
                            + " mean."));
        ...
        for (int i = 0; i < numStats; i++) { // i goes through them in row major order
            ...
            if (metric.getStatsBuffered() == null
                    || metric.getStatsBuffered().getAllHistory() == null) {
                ...
                charts[j].setVisible(
                        false); // use "visible" as a flag to indicate it's in the container
                final int jj = j; // effectively final so the listener can use it
                statBoxes[i].addMouseListener(
                        new MouseAdapter() {
