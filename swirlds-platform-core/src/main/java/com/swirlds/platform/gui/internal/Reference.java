...
/**
 * Encapsulation of a Swirlds "reference", which is a 384-bit hash of a public key, file, swirld
 * name, or other entity. It supports converting between byte array and string, for several types of
 * string. Eventually, it may also allow converting between those and a QR code image. It is also
 * legal to instantiate with a 128-bit or 256-bit hash, but for some uses, 128 bits will not be
 * secure against birthday attacks, and 256 bits will not be secure against quantum computers. The
 * US CNSA Suite requires 384-bit hashes, and 256-bit AES keys.
 */
public class Reference {
     ...
     * thing being referenced (a key, a swirld name, a file, etc). A copy of data is made, so it is
     * OK to change the array after instantiating the Reference.
     *
     * <p>The stored information will be one byte longer than the data, found by first appending a
     * CRC8 checksum to the end, then XORing all the previous bytes with that checksum. Later, the
     * checksum can be checked by doing the XOR, then the checksum calculation, then comparing the
     * result to the last byte. When expressing a Reference in base 62 or as a list of words, the
    ...
    public Reference(byte[] data) {
        ...
        if (data == null || (data.length != 32 && data.length != 16 && data.length != 48)) {
            ...
            throw new InvalidParameterException(
                    "data.length() should be 16 or 32 or 48, not " + data.length);
        }
        this.data = new byte[data.length + 1];
        // store in scrambled form so changing one bit of reference changes the entire string
        // representation
        byte crc = crc8(data);
        this.data[data.length] = crc; // the crc goes at the end
        for (int i = 0; i < data.length; i++) {
            this.data[i] =
                    (byte) (data[i] ^ crc); // the checksum also is XORed with all previous bytes
        }
    }
    ...
    /**
     * Return the log base 2 of x. The answer is exactly correct (without roundoff error) when x is
     * an exact power of 2 that fits in a long.
     *
    ...
    /**
     * Pass to the constructor a string representing the data. If the string starts and ends in
     * &lt;&gt; and contains only letters and digits in between, then it is considered a base62
     * encoding. Otherwise, it is considered a list of words.
     *
    ...
    public Reference(String dataString) {
        if (dataString.matches("^<[a-zA-Z0-9]*>$")) { // base 62 encoding
            ...
            if (lenString != len128Bits && lenString != len256Bits && lenString != len384Bits) {
                ...
                throw new InvalidParameterException(
                        dataString
                                + " is not a proper encoding of 128, 256, or 384 bits because it"
                                + " has "
                                + lenString
                                + " digits instead of "
                                + len128Bits
    ...
    /**
     * Given a number in base fromRadix, with each digit being an integer in array number (number[0]
     * is most significant), return a new number in toRadix, where the array has toLength elements,
     * left padded with zeros if the number is too small, and chopping off the most significant
     * digits if it is too large.
     *
     ...
     * http://www.ece.cmu.edu/~koopman/roses/dsn04/koopman04_crc_poly_embedded.pdf Koopman says 0xA6
     * (1 0100110) is a good polynomial choice, which is x^8 + x^6 + x^3 + x^2 + x^1 . The following
     * code uses 0xB2 (1 0110010), which is 0xA6 with the bits reversed (after the first bit), which
     * is needed for this code.
     *
     * @param data the data to find checksum for (where the last byte does not affect the checksum)
     * @return the checksum
     */
    ...
    /**
     * Return a string representing the reference, made up of multiple lines of 4 words each, broken
     * into groups of 4 lines. The last group may have fewer lines, and its last line may have fewer
     * words.
     *
    ...
    public String toWords() {
        ...
        // uncomment the following instead of the above, to indent every other group instead of
        // skipping
    ...
    /**
     * Return a string representing the reference, made up of multiple lines of 4 words each, broken
     * into groups of 3 lines. The last group may have fewer lines, and its last line may have fewer
     * words. There are no actual line breaks unless they are included in the parameters passed in.
     *
     * @param prefix start of the entire string
     * @param wordSeparator between adjacent words in a line of 4 words
     * @param lineSeparator1 between lines in a group of 3 lines, for every other group, starting
     *     with first
     * @param lineSeparator2 between lines in a group of 3 lines, for every other group, starting
     *     with second
     * @param groupSeparator1 between groups of 4 lines, for every other group, starting with first
     * @param groupSeparator2 between groups of 4 lines, for every other group, starting with second
     ...
     * characters (including a leading zero, if necessary). Only use the array from index firstIndex
     * to index lastIndex. A lastIndex of -1 means the last element, -2 is the second to last, and
     * so on. If minIndex>maxIndex (after any wrapping of lastIndex), then return the null string.
     *
     * @param bytes an array of bytes, some of which are to be converted
     * @param firstIndex index of first element to convert
     * @param lastIndex index of last element to convert (or -1 for last -2 for 2nd to last, etc)
