...
/**
 * Metadata that is calculated based on a {@link AddressBook} that is used to aid in drawing a
 * hashgraph
 */
public class AddressBookMetadata {
    ...
    public AddressBookMetadata(final AddressBook addressBook, final boolean expand) {
        ...
        for (int x = 0; x < m * (m - 1) / 2 + 1; x++) {
            ...
            // columns
            col2mems[x][0] =
                    HashgraphGuiUtils.col2mem(
                            m, d1 + x); // find even m answer by asking for m+1 with skipped cols
            col2mems[x][1] =
    ...
    public int mems2col(final PlatformEvent e1, final PlatformEvent e2) {
        ...
        // there is no e1, so pick one of the e2 columns arbitrarily (next to 0 or 1). If there is
        // only 1
        // member, avoid the array out of bounds exception
        return mems2col[e2.getCreatorId() == 0 ? getNumColumns() == 1 ? 0 : 1 : 0][
                (int) e2.getCreatorId()];
    }
