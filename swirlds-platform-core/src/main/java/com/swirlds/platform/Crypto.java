...
public class Crypto {
     ...
     * @param cryptoThreadPool the thread pool that will be used for all operations that can be done
     *     in parallel, like signing and verifying
     */
    ...
    /**
     * Verify the given signature for the given data. This is submitted to the thread pool so that
     * it will be done in parallel with other signature verifications and key generation operations.
     * This method returns a Future immediately. If the signature is valid, then a get() method on
     * that Future will eventually return a Boolean which is true if the signature was valid. After
     * the thread does the validation, and before it returns, it will run doLast(true) if the
     * signature was valid, or doLast(false) if it wasn't.
     *
     * <p>This is flexible. It is OK to ignore the returned Future, and only have doLast handle the
     * result. It is also OK to pass in (Boolean b) for doLast, and handle the result of doing a
     * .get() on the Future. Or both mechanisms can be used.
     *
     ...
     * @param doLast a function that will be run after verification, and will be passed true if the
     *     signature is valid. To do nothing, pass in (Boolean b)
