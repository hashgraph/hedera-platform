...
public interface GraphGenerations {
    ...
    /**
     * @return The minimum judge generation number from the most recent fame-decided round, if there
     *     is one. Else this returns {@link #FIRST_GENERATION}.
     */
    ...
    /**
     * @return The minimum judge generation number from the oldest non-expired round, if we have
     *     expired any rounds. Else this returns {@link #FIRST_GENERATION}.
     */
    long getMinRoundGeneration();

    /**
     * Checks if we have any ancient rounds yet. For a short period after genesis, there will be no
     * ancient rounds. After that, this will always return true.
     *
    ...
    /**
     * Checks if the supplied event is ancient or not. An event is ancient if its generation is
     * smaller than the round generation of the oldest non-ancient round.
     *
