...
/**
 * This class is used for state recovery. Parse event stream files and playback event on given
 * SwirldState object
 *
 * <p>Running a different thread, parsing event files from given directory. Searching event whose
 * consensus timestamp following in the range of start timestamp (exclusive) and end timestamp
 * (inclusive), i.e., (startTimestamp, endTimestamp]
 */
public class StreamEventParser extends Thread {
    ...
    /**
     * Parse event stream file (.evts) version 5 and put parsed event objects into eventHandler
     *
     * @param file event stream file
     * @param eventHandler call back function for handling parsed event object
     * @param populateSettingsCommon should be true when this method is called from a utility
     *     program which may not read the settings.txt file and follow the normal initialization
     *     routines in the Browser class
    ...
    public static boolean parseEventStreamFile(
        ...
        if (populateSettingsCommon) {
            // Populate the SettingsCommon object with the defaults or configured values from the
            // Settings class.
            // This is necessary because this method may be called from a utility program which may
            // or may not
    ...
    /**
     * Parsing event stream files from a specific folder with a search timestamp, then playback
     * transactions inside those events
     */
     ...
     *
     * @param fileDir directory where event files are stored
     * @param eventHandler call back function for handling parsed event object
     */
                    ...
                    LOGGER.error(
                            ...
                            () ->
                                    new StreamParseErrorPayload(
                                            "Experienced error during parsing file "
                                                    + fullPathName));
                    return;
                }
            }
    ...
    /**
     * Processes a file in the file array: for a file which is not the last file, we check whether
     * we should skip it or not, and parse the file when needed; for the last file, we always parse
     * it.
     *
     * @param files a file array
     * @param index index of the file to be parsed
     * @param eventHandler call back function for handling parsed event object
     * @return whether there is error when parsing the file
     */
    private boolean processEventFile(
            final File[] files, final int index, final EventConsumer eventHandler) {
        boolean result = true;
        if (index < files.length - 1) {
            // if this is not the last file, we can compare timestamp from the next file with
            // startTimestamp
     ...
     * Parse event stream file (.evts) version 5 and put parsed event objects into eventHandler
     *
     * <p>If startTimestamp is null then return all parsed events
     *
     * @param file event stream file
     * @param eventHandler call back function for handling parsed event object
     * @return return false if experienced any error otherwise return true
    ...
    private boolean handleEvent(final Event event) {
        ...
        // we need to setConsensus to be true, otherwise we will got `ConsensusRoundHandler queue
        // has non consensus
        // event` error in ConsensusRoundHandler.applyConsensusEventToState() when handling this
        // event during state
        // recovery
        ...
        //
        // for event whose consensusTimestamp is before or equal to startTimestamp, we ignore such
        // event,
        // because this event should not be played back in swirdsState
        // we cannot write such events to event stream files, because we only have eventsRunningHash
        // loaded from signed
        // state. we must start to update eventsRunningHash for events whose consensus timestamp is
        // after the
        // loaded signed state, and then start to write event stream file at the first complete
        // window
        if (startTimestamp.isBefore(consensusTimestamp)
                && !consensusTimestamp.isAfter(endTimestamp)) {
            if (prevParsedEvent != null) {
                // this is not the first parsed event, push prevParsedEvent to queue
                addToQueue(prevParsedEvent);
            } else {
                // This is the first recovered event, also could be the first event within current
                // file
