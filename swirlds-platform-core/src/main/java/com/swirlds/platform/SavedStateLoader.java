...
public class SavedStateLoader {
     ...
     * @param savedStateFiles an array of saved state files to consider for loading, ordered from
     *     newest to oldest
    ...
    /**
     * Stores a signed state read from disk along with its original hash and it's recalculated hash.
     * These hashes could be different if a migration was performed.
     */
     ...
     * @throws SignedStateLoadingException if the node is configured to require a saved state to
     *     load but none are available
    ...
    /**
     * Returns the most recent, compatible emergency signed state to load into the system at
     * startup, if found.
     *
    ...
    private static SignedStateWithHashes readAndRehashState(final SavedStateInfo file)
        ...
        // When loading from disk, we should hash the state every time so that the first fast copy
        // will
