...
/**
 * A specialized implementation of {@link BlockingQueue} for the consensus queue (q2) that stores
 * consensus rounds to be handled. Despite the queue storing rounds, the capacity to store
 * additional rounds is determined by the total number of events in those rounds.
 *
 * <p>The implementation does not support multiple threads adding elements or multiple threads
 * removing elements, but it does support a single thread that adds elements that is different from
 * the thread removing elements.
 */
public class ConsensusQueue implements BlockingQueue<ConsensusRound> {
     ...
     *
     * @param consensusHandlingMetrics the stats object to record stats on
     * @param eventCapacity the maximum number of events allowed in all the rounds in the queue,
     *     unless there is a single round in the queue with more than this many events
     */
    ConsensusQueue(
            final ConsensusHandlingMetrics consensusHandlingMetrics, final int eventCapacity) {
        super();
    ...
    /**
     * Adds the round to the queue (q2) if there is room according to the event capacity. If there
     * is no room, wait until there is.
     *
     * @param consensusRound the round to add to the queue
     * @throws InterruptedException if this thread is interrupted while waiting or adding to the
     *     queue
     */
    @Override
    ...
    public synchronized boolean offer(
            final ConsensusRound consensusRound, final long timeout, final TimeUnit unit)
    ...
    /** {@inheritDoc} */
    @Override
    public ConsensusRound poll(final long timeout, final TimeUnit unit)
            throws InterruptedException {
     ...
     * memory or resource constraints) accept without blocking, or {@code Integer.MAX_VALUE} if
    ...
    /**
     * Determines if the {@code consensusRound} can be added to the queue based on the event
     * capacity. If the queue is currently empty, return true regardless of the number of events in
     * the round. If the queue is not empty, check if adding the round will exceed the maximum
     * number of events.
     *
     * @param consensusRound the round to add
    ...
    /**
     * Decrement the number of events in the queue by the number of events in {@code
     * consensusRound}. The handler of items in the queue is responsible for calling this method.
     *
     * @param consensusRound the round just removed from the queue
     */
