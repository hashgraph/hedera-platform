...
/**
 * Created by a Platform to manage the flow of pre-consensus events to SwirldState (1 instance or 3
 * depending on the SwirldState implemented). It contains a thread queue that contains a queue of
 * pre-consensus events (q1) and a SwirldStateManager which applies those events to the state
 */
public class PreConsensusEventHandler implements PreConsensusEventObserver, Clearable, Startable {
    ...
    public PreConsensusEventHandler(
        ...
        final BlockingQueue<EventImpl> queue =
    ...
    /**
     * Do pre-handle processing on the pre-consensus event and add it to the queue (q1) for
     * handling. Events that are null or empty are discarded immediately. All other events go
     * through pre-handle processing. Events that should not be handled pre-consensus according to
     * {@link SwirldStateManager#discardPreConsensusEvent(EventImpl)} are not added to the queue.
     */
    @Override
    public void preConsensusEvent(final EventImpl event) {
        ...
        try {
            ...
            event.estimateTime(
                    selfId,
                    metrics.getAvgSelfCreatedTimestamp(),
