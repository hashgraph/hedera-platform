...
/**
 * Keeps events that need to be stored in the signed state when it is created. It makes sure it has
 * last {@code staleRound} number of rounds created at all times.
 */
public class SignedStateEventStorage {
     ...
     *
     * @param minGenerationNonAncient generation below which all events are ancient
     */
    ...
    @Override
    public int hashCode() {
        return new HashCodeBuilder(17, 37).append(queue).append(latestRoundReceived).toHashCode();
    }
