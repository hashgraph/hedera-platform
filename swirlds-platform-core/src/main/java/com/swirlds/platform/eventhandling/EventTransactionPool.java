...
/**
 * Store a list of transactions created by self, both system and non-system, for wrapping in the
 * next event to be created.
 */
public class EventTransactionPool
        implements TransactionPool, TransactionSupplier, TransThrottleSyncAndCreateRule {
    ...
    /**
     * A list of high-priority transactions created by this node waiting to be put into a
     * self-event. Transactions in this queue are always inserted into an event before transactions
     * waiting in {@link #transEvent}.
     */
    ...
    /**
     * Get the next transaction that should be inserted into an event, or null if there is no
    ...
    private ConsensusTransactionImpl getNextTransaction(final int currentEventSize) {
        ...
        if (!priorityTransEvent.isEmpty()
                && priorityTransEvent.peek().getSerializedLength() <= maxSize) {
    ...
    /**
     * Removes as many transactions from the list waiting to be in an event that can fit (FIFO
     * ordering), and returns them as an array, along with a boolean indicating if the array of
     * transactions returned contains a freeze state signature transaction.
     */
    @Override
    ...
    /**
     * @return the number of transactions waiting to be put in an event, both user and state
     *     signature system transactions
     */
    ...
    /**
     * Add the given transaction to the list of transactions to be submitted to the network. If the
     * queue is full, it does nothing and returns false immediately.
     *
     * @param transaction The transaction. It must have been created by self.
     * @param priority if true, then this transaction will be submitted before other waiting
     *     transactions that are not marked with the priority flag. Use with moderation, adding too
     *     many priority transactions (i.e. thousands per second) may disrupt the ability of the
     *     platform to perform some core functionalities.
    ...
    public synchronized boolean submitTransaction(
        ...
        // Always submit system transactions. If it's not a system transaction, then only submit it
        // if we
