...
public class TemporaryFileSettingsImpl extends SubSetting implements TemporaryFileSettings {
     ...
     * {@inheritDoc}
     *
     * <p>Always use the saved state directory as the base for the temp dir so that hard linked
     * files used by virtual maps are kept on the same volume as the saved states.
     */
    @Override
    public String getTemporaryFilePath() {
        return Path.of(Settings.getInstance().getState().savedStateDirectory, temporaryFilePath)
                .toString();
    }
}
