...
class SocketFactoryTest {
    ...
    /**
     * Calls {@link #testSockets(SocketFactory, SocketFactory)} twice, to test both factories as
    ...
    /**
     * - establishes a connection using the provided factories - transfers some data - verifies the
     * transferred data is correct - closes the sockets
     *
    ...
    private static void testSockets(
            final SocketFactory serverFactory, final SocketFactory clientFactory) throws Throwable {
                        ...
                        () -> {
                            try {
                                ...
                                assertArrayEquals(
                                        DATA,
                                        bytes,
                                        "Data read from socket must be the same as the data"
                                                + " written");
                                s.close();
                            } catch (final IOException e) {
                                throw new RuntimeException(e);
                            } finally {
                                // for some reason, AutoClosable does not seem to close in time, and
                                // subsequent tests fail if used
                                try {
                                    serverSocket.close();
                                } catch (final IOException e) {
                                    throw new RuntimeException(e);
    ...
    /**
     * Tests the functionality {@link KeysAndCerts} are currently used for, signing and establishing
     * TLS connections.
     *
    ...
    void tlsFactoryTest(final AddressBook addressBook, final KeysAndCerts[] keysAndCerts)
            throws Throwable {
        ...
        testSocketsBoth(
                new TlsFactory(keysAndCerts[node1], NO_IP_TOS),
                new TlsFactory(keysAndCerts[node2], NO_IP_TOS));
        testSocketsBoth(
                new TlsFactory(keysAndCerts[node1], IP_TOS),
                new TlsFactory(keysAndCerts[node2], IP_TOS));
    }
