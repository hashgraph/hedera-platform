...
public class HashLoggerTest {
    ...
    public void setUp() {
        ...
        hashLogger = new HashLogger(NodeId.createMain(123), mockLogger);
        logged = new ArrayList<>();

        doAnswer(
                        invocation -> {
                            final MessageSupplier supplier =
                                    invocation.getArgument(1, MessageSupplier.class);
    ...
    public void loggingWithGapsAddsExtraWarning() {
        ...
        assertThat(logged.get(0)).contains("Round = 1");
        assertThat(logged.get(1)).contains("Round = 2");
        assertThat(logged.get(2))
                .contains("Several rounds skipped. Round received 5. Previously received 2.");
    ...
    public void loggerWithDefaultConstructorWorks() {
        assertDoesNotThrow(
                () -> {
                    hashLogger = new HashLogger(NodeId.createMain(123));
                    hashLogger.logHashes(createSignedState(1));
                    flush();
                });
    }
