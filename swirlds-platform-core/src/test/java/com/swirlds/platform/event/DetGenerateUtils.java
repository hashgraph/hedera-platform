...
public abstract class DetGenerateUtils {
    ...
    public static BaseEventHashedData generateBaseEventHashedData(final Random random) {
        return new BaseEventHashedData(
                ...
                generateTransactions(
                                DEFAULT_TRANSACTION_NUMBER, DEFAULT_TRANSACTION_MAX_SIZE, random)
    ...
    public static ConsensusData generateConsensusEventData(final Random random) {
        ...
        // isWitness & isFamous are no longer part of ConsensusEvent. random.nextBoolean() have been
        // left here so that
        ...
        data.setRoundReceived(
                nextLong(
                        random,
                        data.getRoundCreated()
                                + 1, // RoundReceived must be higher than RoundCreated
     ...
     *
     * @param number how many transaction to generate
     * @param maxSize maxiumyum payload size a transaction could have
     * @param random random seed generator
     * @return a list of transaction objects
     *     <p>Note: This routine is copied directly in
    ...
    public static List<Transaction> generateTransactions(
            final int number, final int maxSize, final Random random) {
    ...
    public static Instant generateRandomInstant(final Random random, final long maxEpoch) {
        return Instant.ofEpochSecond(
                nextLong(random, 0, maxEpoch - 1), nextLong(random, 0, 1_000_000_000));
    }
