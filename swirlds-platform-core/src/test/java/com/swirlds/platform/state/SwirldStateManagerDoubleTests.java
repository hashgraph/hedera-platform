...
public class SwirldStateManagerDoubleTests {
    ...
    void initialStateReferenceCount() {
        assertEquals(
                ...
                "The initial state is copied and should be referenced once as the previous"
                        + " immutable state.");
        assertEquals(
                1,
    ...
    void loadFromSignedStateRefCount() {
        ...
        assertEquals(
                ...
                "Loading from signed state should increment the reference count, because it is now"
                        + " referenced by the signed state and the previous immutable state in"
                        + " SwirldStateManagerDouble.");
        assertEquals(
                1,
                swirldStateManagerDouble.getConsensusState().getReservationCount(),
        ...
        assertEquals(
                1,
                ss1.getState().getReservationCount(),
                "The previous immutable state was replaced, so the old state's reference count"
    ...
    private static State newState() {
        ...
        assertEquals(
                0, state.getReservationCount(), "A brand new state should have no references.");
        return state;
    }
