...
class TransactionHandlerTest {
    ...
    private static final List<Supplier<Exception>> EXCEPTIONS =
            List.of(
        ...
        for (int i = 0; i < transactions.length; i++) {
            ...
            verify(
                            ...
                            times(numInvocations)
                                    .description(
                                            "preHandle() invoked incorrect number of times on"
                                                    + " transaction index "
                                                    + i))
    ...
    @Test
    @DisplayName(
            "handlePreConsensusEvent() invokes SwirldState.handlePreConsensusEvent() with the"
                    + " correct arguments")
    void testHandlePreConsensusEvent() {
        ...
        for (int i = 0; i < transactions.length; i++) {
            ...
            verify(
                            ...
                            times(numInvocations)
                                    .description(
                                            "handlePreConsensusEvent() invoked with incorrect"
                                                    + " arguments for transaction "
                                                    + i))
                    .handleTransaction(
                            eq(0L),
                            eq(event.getTimeCreated()),
                            any(Instant.class),
                            eq(transaction),
                            eq(dualState));
        }
    ...
    @Test
    @DisplayName(
            "handleRound() invokes SwirldState.handleConsensusRound() with the correct arguments")
    void testHandleRound() {
        ...
        verify(
                        swirldState1,
                        times(1).description(
                                        "handleConsensusRound() invoked with incorrect arguments"))
                .handleConsensusRound(round, dualState);
    }

    @Test
    @DisplayName(
            "handleTransactions() invokes SwirldState1.handleTransaction() with the correct"
                    + " arguments")
    void testHandleTransactions() {
        final Random r = RandomUtils.getRandomPrintSeed();
        final ConsensusTransaction[] transactions =
                TransactionUtils.incrementingMixedTransactions(r);
        ...
        for (int i = 0; i < transactions.length; i++) {
            ...
            verify(
                            ...
                            times(numInvocations)
                                    .description(
                                            "handleTransaction() invoked with incorrect arguments"
                                                    + " for transaction "
                                                    + i))
                    .handleTransaction(
                            eq(0L),
    ...
    void testHandleConsensusRoundThrows() {
        for (final Supplier<Exception> ex : EXCEPTIONS) {
            doThrow(ex.get())
                    .when(swirldState1)
                    .handleConsensusRound(any(Round.class), any(SwirldDualState.class));
