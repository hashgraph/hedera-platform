...
/**
 * This class is used for generating unit test method parameters, even though IntelliJ says it is
 * not used.
 */
public class CryptoArgsProvider {
    ...
    static Stream<Arguments> basicTestArgs() throws Exception {
        ...
        final KeysAndCerts[] loadedC =
                CryptoStatic.loadKeysAndCerts(
                        loadedAB, ResourceLoader.getFile("preGeneratedKeysAndCerts/"), PASSWORD);
        System.out.println("Key loading took " + Duration.between(start, Instant.now()).toMillis());
    ...
    private static AddressBook createAddressBook() {
        ...
        for (int i = 0; i < addresses.getSize(); i++) {
            addresses.add(
                    addresses.getAddress(i).copySetSelfName(memberName(i)).copySetOwnHost(true));
        }
