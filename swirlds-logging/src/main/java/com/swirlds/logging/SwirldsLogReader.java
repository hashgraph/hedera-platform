...
public abstract class SwirldsLogReader<T> {
    ...
    /**
     * Collect log entries that match a filter. The list returned by this method will be updated
     * with matching log entries as the log is read.
     *
     * <p>A maximum of 10,000 entries will be collected to prevent memory exhaustion.
     *
     * @param filter a filter that is applied to each log entry. If the predicate returns true then
     *     the entry is allowed to pass, otherwise it is rejected. If null then all entries are
     *     considered to pass the filter.
     ...
     *     the entry is allowed to pass, otherwise it is rejected. If null then all entries are
     *     considered to pass the filter.
     * @param limit the maximum number of entries to collect
     * @return a list that is updated as the log is read
     */
     ...
     *
     * @param filter a filter that is applied to each log entry. If the predicate returns true then
     *     the entry is allowed to pass, otherwise it is rejected.
     * @return a integer that holds the count of matched entries.
     */
    public AtomicInteger count(final Predicate<T> filter) {
        // Note: we are not using atomic because of thread safety. Rather, we just need an object
        // that represents
     ...
     *
     * @param filter the filter used to select log entries. If the predicate returns true then the
     *     entry is allowed to pass, otherwise it is rejected. If null then all entries are
     *     considered to pass the filter.
     * @param action a function that is called on each matching entry
     */
     ...
     *
     * @param filter the filter to use to select the next entry to return. If the predicate returns
     *     true then the entry is allowed to pass, otherwise it is rejected. If null then allow all
     *     entries to pass.
    ...
    /**
     * Read all entries from the log, adding them to the appropriate lists (as specified by {@link
     * SwirldsLogReader#collect(Predicate)} and calling the appropriate callbacks (as specified by
