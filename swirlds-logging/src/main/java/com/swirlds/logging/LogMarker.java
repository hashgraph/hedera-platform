...
public enum LogMarker {
    ...
    /**
     * log all exceptions, and serious problems. These should never happen there is a bug in the
     * code. In most cases, this should include a full stack trace of the exception.
     */
    EXCEPTION(LogMarkerType.ERROR),

    /**
     * exceptions that shouldn't happen during testing, but can happen in production if there is a
     * malicious node. This should be turned off in production so that a malicious node cannot
     * clutter the logs
     */
    TESTING_EXCEPTIONS(LogMarkerType.ERROR),

    /**
     * log the 4 sync exceptions (EOFException, SocketTimeoutException, SocketException,
     * IOException)
     */
    SOCKET_EXCEPTIONS(LogMarkerType.ERROR),

    /**
     * log socket exceptions related to connecting to a node, this is a separate marker to avoid
     * filling the log file in case one node is down, or it has not started yet
     */
    ...
    /**
     * logs explicitly related to beta mirror node operation and warnings. Enabling this marker may
     * generate large amounts of log output and is not recommended from production environments.
     */
    ...
    /**
     * logs containing entire event content (metadata, transactions, etc). In order to prevent DoS
     * and DDoS attacks from consuming excessive disk space we should only write event contents to
     * the log when this diagnostic marker is enabled.
     */
