...
public class IssResolvedPayload extends AbstractLogPayload {
     ...
     * @param round the round when the ISS was resolved
     * @param nodeId this node
     * @param otherId the node that is no longer in an ISS state
     */
    public IssResolvedPayload(
            final String message, final long round, final long nodeId, final long otherId) {
        super(message);
        this.round = round;
        this.nodeId = nodeId;
