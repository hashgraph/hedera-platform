...
public class MerkleDbStatistics {
    ...
    private static final String TOTAL_FILES_SIZE_PREFIX =
            "The total files size (in megabytes) of files ";
    private static final String SMALL_MERGE_PREFIX =
            "The time (in seconds) of the last Small Merge call ";
    private static final String MEDIUM_MERGE_PREFIX =
            "The time (in seconds) of the last Medium Merge call ";
    private static final String LARGE_MERGE_PREFIX =
            "The time (in seconds) of the last Large Merge call ";
    private static final String INTERNAL_HASHES_STORE_MIDDLE = "in the internal Hashes Store for ";
    private static final String LEAF_KEY_TO_PATH_STORE_MIDDLE =
            "in the Leaf Key To Path Store for ";
    private static final String LEAF_PATH_TO_HKV_STORE_MIDDLE =
            "in the Leaf Path to Hash/Key/Value Store for ";
    ...
    private static DoubleGauge buildDoubleGauge(
            final Metrics metrics, final String name, final String description) {
        return metrics.getOrCreate(
        ...
        internalHashesStoreFileCount =
                metrics.getOrCreate(
                        new IntegerGauge.Config(STAT_CATEGORY, "internalHashFileCount_" + label)
                                .withDescription(
                                        NUMBER_OF_FILES_PREFIX
                                                + INTERNAL_HASHES_STORE_MIDDLE
                                                + label
                                                + SUFFIX));
            ...
            leafKeyToPathStoreFileCount =
                    metrics.getOrCreate(
                            new IntegerGauge.Config(
                                            STAT_CATEGORY, "leafKeyToPathFileCount_" + label)
                                    .withDescription(
                                            NUMBER_OF_FILES_PREFIX
                                                    + LEAF_KEY_TO_PATH_STORE_MIDDLE
                                                    + label
                                                    + SUFFIX));
