 ...
 */
public interface KeySerializer<K extends VirtualKey<?>>
        extends BaseSerializer<K>, SelfSerializable {
    ...
    default KeyIndexType getIndexType() {
        return getSerializedSize() == Long.BYTES
                ? KeyIndexType.SEQUENTIAL_INCREMENTING_LONGS
                : KeyIndexType.GENERIC;
    }
    ...
    default int getTypicalSerializedSize() {
        if (isVariableSize()) {
            throw new IllegalStateException(
                    "Variable sized implementations have to override this method");
        }
     ...
     * Serialization format must be identical to {@link #serialize(Object,
     * SerializableDataOutputStream)}. Bytes written using this method will then be used for
     * deserialization with {@link #deserialize(ByteBuffer, long)} method.
     *
     * <p>This method returns the number of bytes written to the buffer. For fixed-sized keys the
     ...
     * #serialize(VirtualKey, ByteBuffer)} or {@link #serialize(Object,
     * SerializableDataOutputStream)} method.
     *
     * @param buffer The byte buffer to read from
