 ...
 * requested to create a new data source, or restore an existing data sources from snapshot, the
 * data source is hosted in the specified database. Full data source path is therefore databaseDir +
 * "/" + dataSource.label. To make sure there are no folder name conflicts between data sources with
 * the same label, e.g. on copy or snapshot, MerkleDb builders use different database directories,
 * usually managed using {@link TemporaryFileBuilder}.
 *
...
public class MerkleDbDataSourceBuilder<K extends VirtualKey<? super K>, V extends VirtualValue>
    ...
    /**
     * Default database folder used by this builder to create and restore data sources. If {@code
     * null}, the default {@link MerkleDb} database is used.
     */
     ...
     *
     * @param tableConfig Table configuration to use to create new data sources
     */
     ...
     *
     * @param databaseDir Default database folder. May be {@code null}
     * @param tableConfig Table configuration to use to create new data sources
     */
    public MerkleDbDataSourceBuilder(
            final Path databaseDir, final MerkleDbTableConfig<K, V> tableConfig) {
    ...
    public VirtualDataSource<K, V> build(
        ...
        // Creates a new data source in this builder's database dir or in the default MerkleDb
        // instance
    ...
    public VirtualDataSource<K, V> copy(final VirtualDataSource<K, V> snapshotMe) {
        if (!(snapshotMe instanceof MerkleDbDataSource<K, V> source)) {
            throw new IllegalArgumentException(
                    "The datasource must be compatible with the MerkleDb");
        }
    ...
    public VirtualDataSource<K, V> restore(final String label, final Path source) {
        try {
            // Restore to the default database. Assuming the default database hasn't been
            // initialized yet.
