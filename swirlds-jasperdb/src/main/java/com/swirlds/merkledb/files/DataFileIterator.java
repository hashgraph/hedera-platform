...
/**
 * Iterator class for iterating over data items in a DataFile. It is designed to be used in a
 * while(iter.next()){...} loop and you can then read the data items info for current item with
 * getDataItemsKey, getDataItemsDataLocation and getDataItemData.
 *
...
public final class DataFileIterator implements AutoCloseable {
    ...
    /**
     * Index of current data item this iterator is reading, zero being the first item, -1 being
     * before start
     */
    ...
    /**
     * Get the current dataItems data. This is a shared buffer and must NOT be leaked from the call
     * site or modified directly.
     *
     * @return ByteBuffer containing the key and value data. This will return null if the iterator
     *     has been closed, or if the iterator is in the before-first or after-last states.
     */
    ...
    public String toString() {
        return "DataFileIterator{"
                + "fileIndex="
                + metadata.getIndex()
                + ", currentDataItemHeader="
                + currentDataItemHeader
                + ", currentDataItemIndex="
                + currentDataItem
                + ", currentDataItemByteOffset="
                + currentDataItemFilePosition
                + ", fileName="
                + path.getFileName()
                + ", metadata="
                + metadata
    ...
    private ByteBuffer fillBuffer(int bytesToRead) throws IOException {
        ...
        // This only happens if we have advanced currentDataItemFilePosition and need to strip off
        // some bytes
        ...
        final int bytesRead =
                inputStream.read(dataItemBuffer.array(), offset, bytesToRead - offset);
        if (offset + bytesRead != bytesToRead) {
            throw new EOFException(
                    "Was trying to read a data item ["
                            + currentDataItem
                            + "] but ran out of data in the file ["
                            + path
                            + "].");
        }
    ...
    /**
     * Resizes the dataItemBuffer, or creates it if necessary, such that it is large enough to read
     * the bytes provided.
     *
     * @param bytesToRead Number of bytes to be able to fit into the buffer.
     */
