...
public final class VirtualInternalRecordSerializer
        implements DataItemSerializer<VirtualInternalRecord> {
    ...
    private static final int SERIALIZED_SIZE =
            Long.BYTES + DEFAULT_DIGEST.digestLength(); // path + hash
    ...
    @Override
    public VirtualInternalRecord deserialize(final ByteBuffer buffer, final long dataVersion)
            throws IOException {
        if (dataVersion != CURRENT_SERIALIZATION_VERSION) {
            throw new IllegalArgumentException(
                    "Cannot deserialize version "
                            + dataVersion
                            + ", current is "
    ...
    public int serialize(
            final VirtualInternalRecord data, final SerializableDataOutputStream outputStream)
            throws IOException {
        ...
        if (DEFAULT_DIGEST != digestType) {
            throw new IllegalArgumentException(
                    "Only "
                            + DEFAULT_DIGEST
                            + " digests allowed, but received hash with digest "
                            + digestType);
        }
