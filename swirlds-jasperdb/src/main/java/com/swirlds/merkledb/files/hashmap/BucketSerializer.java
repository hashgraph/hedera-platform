 ...
 */
public class BucketSerializer<K extends VirtualKey<? super K>>
        implements DataItemSerializer<Bucket<K>> {
    /**
    ...
    public BucketSerializer(final KeySerializer<K> keySerializer) {
        ...
        currentSerializationVersion =
                (keySerializer.getCurrentDataVersion()
                                << LOW_ORDER_BYTES_FOR_NON_KEY_SERIALIZATION_VERSION)
     ...
     */
    @Override
    public Bucket<K> deserialize(final ByteBuffer buffer, final long dataVersion)
            throws IOException {
        Bucket<K> bucket = getReusableEmptyBucket();
        bucket.putAllData(buffer);
        // split bucketSerializationVersion
        bucket.setKeySerializationVersion(
                (int) (dataVersion >> LOW_ORDER_BYTES_FOR_NON_KEY_SERIALIZATION_VERSION));
        return bucket;
    }
     ...
     */
    @Override
    public int serialize(final Bucket<K> bucket, final SerializableDataOutputStream outputStream)
            throws IOException {
        return bucket.writeToOutputStream(outputStream);
    }

    /**
     * Copy the serialized data item in dataItemData into the writingStream. Important if
     * serializedVersion is not the same as current serializedVersion then update the data to the
     ...
     * @return the number of bytes written, this could be the same as dataItemSize or bigger or
     *     smaller if serialization version has changed.
    ...
    public int copyItem(
        ...
        return DataItemSerializer.super.copyItem(
                serializedVersion, dataItemSize, dataItemData, writingStream);
    }
}
