...
/**
 * A set-like data structure that uses {@link HalfDiskHashMap} to maintain state on disk. This data
 * structure supports additions but not removals. It uses an in-memory bloom filter to reduce the
 * required number of disk reads.
 *
...
public class HalfDiskVirtualKeySet<K extends VirtualKey<? super K>> implements VirtualKeySet<K> {
    ...
    // FUTURE WORK: this data structure is mildly inefficient for this use case, as we just need a
    // set and not a map
    // that writes keys and values to the disk. If this component ever becomes a bottleneck,
    // consider writing a use case
     ...
     *
     * @param keySerializer a method that serializes keys
     * @param bloomFilterHashCount the number of hashes to include in the bloom filter
     * @param bloomFilterSize the size of the bloom filter, in bits
     * @param halfDiskHashMapSize the size of the half disk hash map
     * @param maxUnflushedElements the maximum number of elements to keep in memory before flushing
     *     to the half disk hash map
     */
    public HalfDiskVirtualKeySet(
        ...
        bloomFilter =
                new BloomFilter<>(
                        bloomFilterHashCount, new SelfSerializableBloomHasher<>(), bloomFilterSize);
    ...
    public boolean contains(final K key) {
        ...
        // This test always returns the ground truth. But it's expensive, so hopefully we don't get
        // here often.
        try {
