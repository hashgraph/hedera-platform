...
/**
 * An implementation of {@link VirtualDataSourceBuilder} for building {@link
 * VirtualDataSourceJasperDB} data sources. Some fields have default values that are read from
...
public class JasperDbBuilder<K extends VirtualKey<? super K>, V extends VirtualValue>
    ...
    /**
     * If the database should prefer to use the disk for its indexes. The default is false which
     * should be correct for most production use cases. Only set to true if you need to open a
     * database using the minimum RAM possible and can afford to take the performance hit.
     */
    private boolean preferDiskBasedIndexes = false;
    /**
     * The maximum number of unique keys we expect to be stored in this database. This is used for
     * calculating in memory index sizes.
     *
     * <p>The default value is loaded from settings.
     *
     ...
     * <p><b>must be specified</b>
     */
    private VirtualInternalRecordSerializer virtualInternalRecordSerializer =
     ...
     * <p><b>must be specified</b>
     */
    public JasperDbBuilder<K, V> virtualLeafRecordSerializer(
            final VirtualLeafRecordSerializer<K, V> serializer) {
     ...
     * <p><b>must be specified</b>
     */
    public JasperDbBuilder<K, V> virtualInternalRecordSerializer(
            final VirtualInternalRecordSerializer serializer) {
    ...
    /**
     * Specify the base directory in which the database directory will be created. The database will
     * be created as a sub-directory of this storage directory. By default, it is loaded from
     * settings.
     *
    ...
    /**
     * Specify the maximum number of unique keys we expect to be stored in this database. This is
     * used for calculating in memory index sizes.
     *
     * <p>The default value is loaded from settings.
     *
     * <p><b>IMPORTANT: This can only be set before a new database is created, changing on an
    ...
    /**
     * Specify the threshold where we switch from storing internal hashes in ram to storing them on
     * disk. If it is 0 then everything is on disk, if it is Long.MAX_VALUE then everything is in
     * ram. Any value in the middle is the path value at which we swap from ram to disk. This allows
     * a tree where the lower levels of the tree nodes hashes are in ram and the upper larger less
     * changing layers are on disk.
     *
     * <p>The default value is loaded from settings.
     *
     * <p><b>IMPORTANT: This can only be set before a new database is created, changing on an
    ...
    /**
     * Specify if the database should prefer to use the disk for its indexes. The default is false
     * which should be correct for most production use cases. Only set to true if you need to open a
     * database using the minimum RAM possible and can afford to take the performance hit.
     */
    ...
    /**
     * Created to enable tests. I really don't like doing things this way, but I couldn't find a
     * better way to test this because VirtualDataSourceJasperDB doesn't give access to these
     * variables once processed in the constructor (and really shouldn't anyway). So I needed to
     * intercept creation in a way that Mockito could interject.
     */
    ...
    public void deserialize(final SerializableDataInputStream in, final int version)
        ...
        virtualInternalRecordSerializer =
                in.readSerializable(false, VirtualInternalRecordSerializer::new);
    ...
    public boolean equals(final Object o) {
        ...
        return preferDiskBasedIndexes == that.preferDiskBasedIndexes
                ...
                && Objects.equals(
                        virtualInternalRecordSerializer, that.virtualInternalRecordSerializer)
