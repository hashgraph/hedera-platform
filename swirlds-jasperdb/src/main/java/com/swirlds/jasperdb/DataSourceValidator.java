...
// Note: This class is intended to be used with a human in the loop who is watching standard in and
// standard err.
...
public class DataSourceValidator<K extends VirtualKey<? super K>, V extends VirtualValue> {
    ...
    public boolean validate() {
        try {
            ...
            for (long path = 0; path < firstLeafPath; path++) {
                ...
                assertTrue(
                        internalRecord != null, "internal record for path [" + path + "] was null");
                assertTrue(
                        internalRecord.getPath() == path,
                        "internal record for path ["
                                + path
                                + "] had a bad path ["
                                + internalRecord.getPath()
                                + "]");
                assertTrue(
                        internalRecord.getHash() != null,
                        "internal record's hash for path [" + path + "] was null");
            ...
            for (long path = firstLeafPath; path <= lastLeafPath; path++) {
                ...
                assertTrue(
                        leaf.getHash() != null,
                        "leaf record's hash for path [" + path + "] was null");
                assertTrue(
                        leaf.getKey() != null,
                        "leaf record's key for path [" + path + "] was null");
            ...
            for (int i = 0; i < keys.size(); i++) {
                ...
                assertTrue(
                        leaf.getHash() != null,
                        "leaf record's hash for key [" + keys.get(i) + "] was null");
                assertTrue(
                        leaf.getKey() != null,
                        "leaf record's key for key [" + keys.get(i) + "] was null");
                assertTrue(
                        leaf.getKey().equals(keys.get(i)),
                        "leaf record's key for key ["
                                + keys.get(i)
                                + "] did not match, it was ["
                                + leaf.getKey()
                                + "]");
    ...
    private void printProgress(long position, long total) {
        ...
        assert newProgressPercentage >= 0
                : "newProgressPercentage [" + newProgressPercentage + "] is < 0";
