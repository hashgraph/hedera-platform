...
/**
 * A simple immutable POJO for keeping the min and max valid keys. These values are set atomically
 * and must be read atomically in other parts of the code, which this class facilitates.
 */
public final class KeyRange {
    /**
     * Keys are usually paths in our use cases, so we use INVALID_PATH = -1 here as nominal to
     ...
     *
     * @param minValidKey The minimum valid key in the range. Must be less than or equal to the
     *     {@code maxValidKey}.
     * @param maxValidKey The maximum valid key in the range. Must be greater than or equal to the
     *     {@code minValidKey}.
     */
    public KeyRange(final long minValidKey, final long maxValidKey) {
        if (maxValidKey < minValidKey) {
            throw new IllegalArgumentException(
                    "maxValidKey of "
                            + maxValidKey
                            + " must be less than minValidKey of "
                            + minValidKey);
        }
     ...
     *
     * @return The minimum key in the range.
     */
     ...
     *
     * @return The maximum key in the range.
     */
     ...
     *
     * @return True if the {@code key} is greater than or equal to the {@link #getMinValidKey()}}
     *     and less than or equal to the {@link #getMaxValidKey()}.
     */
