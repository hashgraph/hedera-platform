...
public final class DataFileReader<D>
        implements AutoCloseable, Comparable<DataFileReader<D>>, IndexedObject {
    ...
    /**
     * Flag for if this file can be included in merging, it defaults to false and has to be set to
     * true by file writing and merging after updating any indexes. But the file can still be read
     * from during that time.
     */
    private final AtomicBoolean fileAvailableForMerging = new AtomicBoolean(false);
    /**
     * The size of this file in bytes, cached as need it often, and it's constant as file is
     * immutable.
     */
     ...
     */
    public DataFileReader(final Path path, final DataItemSerializer<D> dataItemSerializer)
            throws IOException {
     ...
     *
     * @param newValue When true the file can be included in merging, false then it can not be
     *     included
     */
    ...
    public D readDataItem(final long dataLocation, final boolean deserialize) throws IOException {
        ...
        if (dataItemSerializer.isVariableSize()) {
            ...
            final ByteBuffer serializedHeader =
                    read(byteOffset, dataItemSerializer.getHeaderSize());
