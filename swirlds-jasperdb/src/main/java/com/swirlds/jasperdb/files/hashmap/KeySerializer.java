...
/**
 * Interface for serializers of hash map keys. This is very similar to a DataItemSerializer but only
 * serializes a key. The key can serialize to fixed number or variable number of bytes.
 *
 * <p>The reason that the VirtualKey's own serialization is not used here is because it is very
 * important that the data written for each key is as small as possible. For that reason
 * serialization is version once per file not once per key written.
 *
