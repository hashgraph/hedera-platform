...
/**
 * An implementation of {@link HashList} which makes use of an expanding, dynamic list of {@link
 * ByteBuffer}s for storing hashes. An instance of this class should <strong>only</strong> be used
 * for a homogenous set of hashes. A {@link Hash} normally serializes with both the hash bytes and a
 * hash type. When scaled to billions of hashes, this amounts to a lot of wasted space. This
 * implementation assumes a homogenous set of hashes and omits serializing the hash type, only
 * storing the hash bytes themselves.
 *
 * <p>This class improves upon the memory usage of a simple {@link Hash} array. In this class, each
 * hash is stored as exactly the number of hash bytes (48 for an SHA-384 hash). An array of {@link
 * Hash} objects would include java object overhead, amounting to about a 2x overhead.
 *
 * <pre>
...
public final class HashListByteBuffer implements HashList {
    ...
    /**
     * A suitable default value for the maximum number of hashes to store in a single instance of
     * {@link HashListByteBuffer}. This is to prevent a bug from creating off-heap stores that are
     * ridiculously large.
     */
     ...
     * If the value is small, then we will end up allocating a very large number of buffers. If the
     * value is large, then we will waste a lot of memory in the unfilled buffer.
     */
    private final int numHashesPerBuffer;

    /**
     * The amount of RAM needed to store one buffer of hashes. This will be computed based on the
     * number of bytes to store for the hash, and the {@link #numHashesPerBuffer}.
     */
    private final int memoryBufferSize;

    /**
     * The maximum number of hashes to be able to store in this data structure. This is used as a
     * safety measure to make sure no bug causes an out of memory issue by causing us to allocate
     * more buffers than the system can handle.
     */
    ...
    /**
     * Create a new off-heap {@link HashListByteBuffer} with default number of hashes per buffer and
     * max capacity.
     */
    ...
    public HashListByteBuffer(Path file) throws IOException {
        try (FileChannel fc = FileChannel.open(file, StandardOpenOption.READ)) {
            ...
            for (int i = 0; i < numOfBuffers; i++) {
                ByteBuffer buffer =
                        offHeap ? allocateDirect(memoryBufferSize) : allocate(memoryBufferSize);
     ...
     *
     * @param numHashesPerBuffer The number of hashes to store in each buffer. If the number of too
     *     large, then the last buffer will have wasted space. If the number is too small, then an
     *     excessive number of buffers will be created. Must be a positive number.
     * @param maxHashes The maximum number of hashes to store in the data structure. Must be a
     *     non-negative integer value.
     * @param offHeap Whether to store the buffer off the Java heap.
     */
    public HashListByteBuffer(
    ...
    public void put(final long index, final Hash hash) {
        ...
        maxIndexThatCanBeStored.updateAndGet(
                currentValue -> {
                    while (index > currentValue) { // need to expand
                        data.add(
                                offHeap
                                        ? allocateDirect(memoryBufferSize)
                                        : allocate(memoryBufferSize));
    ...
    public void writeToFile(Path file) throws IOException {
        ...
        try (final FileChannel fc =
            ...
            for (int i = 0; i < numOfBuffers; i++) {
                ByteBuffer buf =
                        data.get(i).slice(); // slice so we don't mess with state of stored buffer
                buf.position(0);
                if (i == (numOfBuffers - 1)) {
    ...
    /**
     * Get the ByteBuffer for a given index. Assumes the buffer is already created. For example, if
     * the {@code index} is 13, and the {@link #numHashesPerBuffer} is 10, then the 2nd buffer would
     * be returned.
     *
     * @param index the index we need the buffer for. This will never be out of range.
     * @return The ByteBuffer contain that index
     */
    private ByteBuffer getBuffer(final long index) {
        // This should never happen, because it is checked and validated by the callers to this
        // method
        ...
        // Create a new sub-buffer (slice). This is necessary for threading. In Java versions < 13,
        // you must
        // have a unique buffer for each thread, because each buffer has its own position and limit
        // state.
        // Once we have the buffer, compute the index within the buffer and the offset and then set
        // the
