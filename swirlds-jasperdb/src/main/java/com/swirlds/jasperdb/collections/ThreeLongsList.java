 ...
 *
 * <p>Allocated chunks are never released; but the existing chunks may be re-used by calling {@link
 * ThreeLongsList#clear()} and then storing more triples starting again at index 0.
 *
 * <p><b>Important:</b> This class is not thread-safe, and by default has a capacity of only 4k
 * triples (12k total longs).
 */
public class ThreeLongsList {
    ...
    /**
     * Construct a new ThreeLongsList with the given max capacity and default initial triples per
     * chunk.
     *
     * @param maxTriples the maximum number of triplets that can be stored
     */
     ...
     *
     * @param maxTriples the maximum number of triplets that can be stored
     * @throws IllegalArgumentException if triplesPerChunk is non-positive or greater than
     *     maxTriples
     */
    public ThreeLongsList(final int maxTriples, final int triplesPerChunk) {
        if (triplesPerChunk < 1 || maxTriples < triplesPerChunk) {
            throw new IllegalArgumentException(
                    "Cannot construct ThreeLongsList with "
                            + maxTriples
                            + " max triples and "
                            + triplesPerChunk
                            + " triples per chunk");
     ...
     * @throws IllegalStateException if the list size would exceed its max capacity
     */
    public void add(final long l1, final long l2, final long l3) {
        if (nextIndex == maxTriples) {
            throw new IllegalStateException(
                    "Cannot store any more triples, reached max capacity of " + maxTriples);
        }
     ...
     * @throws IndexOutOfBoundsException if index is negative or not less than nextIndex
     */
    public long[] get(final int index) {
        if (index < 0 || index >= nextIndex) {
            throw new IndexOutOfBoundsException(
                    "Index " + index + " unusable with " + nextIndex + " triples stored");
        }
    ...
    public void forEach(final ThreeLongFunction handler) {
        ...
        for (final long[] chunk : data) {
            for (int j = 0, k = 0; j < triplesPerChunk; j++, k += LONGS_PER_TRIPLE) {
                ...
                handler.process(
                        chunk[k], chunk[k + SECOND_LONG_OFFSET], chunk[k + THIRD_LONG_OFFSET]);
                index++;
            }
        }
    }
    ...
    /**
     * Gets the number of long triplets actually stored in this list (that is, the number of times
     * that {@link ThreeLongsList#add(long, long, long)} has been called since the last call to
     * {@link ThreeLongsList#clear()}.
     *
