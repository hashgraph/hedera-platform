...
/**
 * A wrapper for a LongList that has two modes, direct pass though or an overlaid cache buffer.
 * Important, any changes directly to the wrapped list while it is wrapped will cause this classes
...
public class LongListBufferedWrapper extends LongList implements Closeable {
    ...
    class Overlay {
        ...
        /**
         * Overlay cache, reference can be set to null if we are not currently using an overlay
         * cache.
         */
        private final AtomicReference<ConcurrentHashMap<Long, Long>> cachedChanges =
                new AtomicReference<>();
        /**
         * Indicates whether fresh changes should prefer being written directly to the
         * wrappedLongList, even if the cache exists. The only time when this is true that a change
         * will still update the overlay cache, is if the change is still in the cache and hasn't
         * been written down to the wrappedLongList yet.
         */
        volatile boolean skipCacheOnWrite = true;
        /**
         * Operation barrier - a mechanism to count operations between switching state and ensure
         * all current operations finish and all new operations see all changes preceding the
         * switch.
         */
     ...
     *
     * @param wrappedLongList The long list to wrap
     */
    ...
    /**
     * Get the list that we are wrapping. This is helpful when logging and tracking down errors as
     * it allows us to see if the data is different in base index.
     *
    ...
    public boolean putIfEqual(final long index, final long oldValue, final long newValue) {
        ...
        if (cache != null) {
            cache.compute(
                    ...
                    (k, v) -> {
                        if (overlay.skipCache() && v == null) {
                            // We're supposed to skip writing to the cache and go directly to the
                            // main index,
                            // because the overlay is being written to the main index and there is
                            // no entry
                            // in the overlay cache for this key, so we will just write directly to
                            // the
                            // main index.
                            valueWasSet.set(wrappedLongList.putIfEqual(index, oldValue, newValue));
                            return null;
                        } else {
                            // Either we are supposed to write to the overlay OR the overlay is
                            // being flushed
                            // to the main index but hasn't handled this key yet.
                            if (v == null) {
                                // There is no entry in the overlay. Check to see if there is a
                                // value in the main index.
                                final long indexValue =
                                        wrappedLongList.get(index, LongList.IMPERMISSIBLE_VALUE);
                                if (indexValue == LongList.IMPERMISSIBLE_VALUE) {
                                    // If there was not a value in the main index, then we have no
                                    // match.
    ...
    protected void writeLongsData(final FileChannel fc) {
        throw new UnsupportedOperationException(
                "LongListBufferedWrapper does not write longs data");
    }
    ...
    protected long lookupInChunk(final long chunkIndex, final long subIndex) {
        throw new UnsupportedOperationException(
                "LongListBufferedWrapper does not read from chunks");
    }
}
