...
import com.swirlds.jasperdb.files.FilesTestType;
...
class HalfDiskHashMapTest {
    ...
    // =================================================================================================================
    // Helper Methods
    private HalfDiskHashMap<VirtualLongKey> createNewTempMap(FilesTestType testType, int count)
            throws IOException {
        // create map
        HalfDiskHashMap<VirtualLongKey> map =
                new HalfDiskHashMap<>(
                        count,
    ...
    private static void checkData(
        ...
        for (int i = start; i < (start + count); i++) {
            ...
            assertEquals(
                    ...
                    result,
                    "Failed to read key="
                            + testType.createVirtualLongKey(i)
                            + " dataMultiplier="
                            + dataMultiplier);
        }
    ...
    void createDataAndCheck(FilesTestType testType) throws Exception {
        ...
        // create snapshot
        map.startSnapshot(tempSnapshotDir);
        map.middleSnapshot(tempSnapshotDir);
        map.endSnapshot(tempSnapshotDir);
        // open snapshot and check data
        HalfDiskHashMap<VirtualLongKey> mapFromSnapshot =
                new HalfDiskHashMap<>(
                        count,
                        (KeySerializer<VirtualLongKey>) testType.keySerializer,
                        tempSnapshotDir,
        ...
        assertEquals(
                -1,
                map.get(testType.createVirtualLongKey(5), -1),
