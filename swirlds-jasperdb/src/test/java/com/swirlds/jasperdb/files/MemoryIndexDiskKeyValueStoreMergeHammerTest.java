...
class MemoryIndexDiskKeyValueStoreMergeHammerTest {
    ...
    void testMerge(
        ...
        final var modifier =
                new FakeVirtualMap(
                        index, chanceOfAddElement, chanceOfDeleteElement, numIterationsPerFlush);
        ...
        while (endTime > System.currentTimeMillis()) {
            ...
            if (mergeFuture.isDone()) {
                assertDoesNotThrow(
                        (ThrowingSupplier<Void>) mergeFuture::get,
                        "Should not throw, something failed while merging.");
            }
    ...
    private static final class FakeVirtualMap extends Worker {
        ...
        public FakeVirtualMap(
                MemoryIndexDiskKeyValueStore<long[]> coll,
                int chanceOfAddElement,
                int chanceOfDeleteElement,
                int numIterationsPerFlush) {
            this.coll = coll;
         ...
         */
        private void doAdd(Map<Long, Long> cache) {
            final int numElements = getNumElements();
            if (numElements == 0) {
         ...
         */
        private void doUpdate(Map<Long, Long> cache) {
         ...
         */
        private void doDelete(Map<Long, Long> cache) {
            final long path = getRandomElement();
         ...
         */
        private void save(Map<Long, Long> cache) throws IOException {
            coll.startWriting();
            final List<Long> sortedKeys =
                    cache.keySet().stream().sorted().collect(Collectors.toList());
        ...
        private void validate() throws IOException {
            assertEquals(
                    expected.size(),
                    getNumElements(),
                    "Expected map is incongruent with first and last paths");

            for (final var key : expected.keySet()) {
                assertEquals(
                        expected.get(key), coll.get(key)[1], "Missing key in DB that we expected!");
            }
        }
         ...
         * @return the value
         */
        private long find(Map<Long, Long> cache, long path) {
        ...
        private long newRandomValue() {
            ...
            if (value == DELETED) {
                throw new AssertionError(
                        "You should have bought a lotto ticket because today was your lucky day."
