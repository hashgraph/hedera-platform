...
class BucketTest {
    ...
    void toStringAsExpectedForBucket() {
        final ExampleLongKeyFixedSize.Serializer keySerializer =
        ...
        assertEquals(
                emptyBucketRepr, bucket.toString(), "Empty bucket should represent as expected");
        ...
        final String nonEmptyBucketRepr =
                "Bucket{bucketIndex=0, entryCount=1, size=32\n"
        ...
        assertEquals(
                nonEmptyBucketRepr, bucket.toString(), "Non-empty bucket represent as expected");
    }

    private void checkKey(Bucket<VirtualLongKey> bucket, VirtualLongKey key) {
        var findResult =
                assertDoesNotThrow(
                        () -> bucket.findValue(key.hashCode(), key, -1),
