...
import com.swirlds.jasperdb.JasperDbTestUtils;
...
class HashToolsTest {
    ...
    void constructsExpectedBuffer() {
        final Hash hash = JasperDbTestUtils.hash(123);

        final ByteBuffer buffer = HashTools.hashToByteBuffer(hash);

        assertArrayEquals(
                hash.getValue(), buffer.array(), "Hash digest should match created buffer");
    }
    ...
    void putsDigestAsExpected() {
        ...
        assertArrayEquals(
                hash.getValue(), buffer.array(), "Hash digest should matched filled buffer");
    }
