...
class SmartContractKeyHashingTest {
    ...
    void smartContractKeyTest(final long mapSize) {
        // for smart contracts we have two data items combined, the first is a long for the contract
        // address and the
        // second is an uint256 for contract storage index. Contract addresses will generally be
        // sequential numbers. The
        // storage indexes could be sequential or sequential at random offsets, so we will test both
        // starting with
        // sequential.
        ...
        for (int i = 0; i < mapSize; i++) {
            if (keyValueCount >= contractSize.keyValueCount) {
                ...
                contractSize =
                        randomNum < HUGE_CONTRACT_PERCENT
                                ? ContractSize.Huge
                                : randomNum < BIG_CONTRACT_PERCENT
                                        ? ContractSize.Big
                                        : ContractSize.Small;
