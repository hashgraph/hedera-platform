...
/**
 * Singleton factory for loading and registering {@link MerkleMap} statistics. This is the primary
 * entry point for all {@link SwirldMain} implementations that wish to track {@link MerkleMap}
 * statistics.
 */
public final class MerkleMapMetrics {
    ...
    private static final RunningAverageMetric.Config MM_GFM_MICRO_SEC_CONFIG =
            new RunningAverageMetric.Config(MM_CATEGORY, "mmGfmMicroSec")
                    .withDescription(
                            "avg time taken to execute the MerkleMap getForModify method (in"
                                    + " microseconds)");
    ...
    private static final RunningAverageMetric.Config MM_REPLACE_MICRO_SEC_CONFIG =
            new RunningAverageMetric.Config(MM_CATEGORY, "mmReplaceMicroSec")
                    .withDescription(
                            "avg time taken to execute the MerkleMap replace method (in"
                                    + " microseconds)");
