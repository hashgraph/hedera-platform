 ...
 *
 * <p>This data structure utilizes an internal {@link FCHashMap} to provide O(1) read access (but
 * only to non-archived copies of the map). It uses a copy-on-write algorithm to provide O(1) fast
 * copies, with write operations costing O(log n) where n is the number of entries in the map.
 *
 * <p>This data structure does not support null keys or null values.
 *
 * <p>If this data structure is used inside a SwirldState it should be archived when
 * SwirldState.archive() is invoked, otherwise a much larger memory footprint than necessary may
 * result.
 *
 * @param <K> the type of the key. Must be effectively immutable. That is, after insertion into a
 *     map, no operation on this key should be capable of changing the behavior of its {@link
 *     Object#hashCode()} or {@link Object#equals(Object)} methods. It is STRONGLY recommended that
 *     this type not implement {@link MerkleNode}. Although a merkle key will technically "work", it
 *     is quite inefficient from a memory perspective.
 * @param <V> value that implements {@link MerkleNode} and {@link Keyed}. Can be an internal node or
 *     a leaf. If this value is an internal node the key will need to be stored inside a descendant
 *     leaf node.
 */
...
public class MerkleMap<K, V extends MerkleNode & Keyed<K>> extends PartialBinaryMerkleInternal
    ...
    /**
     * Used to track the lifespan of this merkle map. The record is released when the map is
     * destroyed.
     */
    ...
    /**
     * If you attempt to enter methods in this class with a debugger then it can cause deadlock. Set
     * this to {@code true} to disable locks for testing.
     *
     * <p>IMPORTANT: never commit this file without reverting the value of this variable to {@code
     * false}.
     */
     ...
     *
     * <p>Note: if you attempt to enter this class with a debugger then it can cause deadlock.
     ...
     * which is more suitable than {@link #size} when the number of keys is greater than the maximum
     * value of an {@code int}.
     *
     * <p>This operation takes O(1) time
     *
     ...
     * {@inheritDoc}
     *
     * <p>Creates an immutable fast copy of this MerkleMap.
     *
    ...
    /**
     * This method updates the {@link FCHashMap} based index. Called when entries are fast copied by
     * the {@link MerkleBinaryTree}.
     *
     ...
     *
     * <p>Calling this method allows for the garbage collector to prune the size of the data
     * structure. It also allows for newer copies to perform read operations more quickly by
     * reducing the number of recent modifications.
     *
     * <p>A MerkleMap copy is not required to call this method before being deleted.
     *
     * @throws ArchivedException if this copy of the map has been archived
     */
    @Override
            ...
            try {
                if (!isImmutable()) {
                    throw new ArchivedException(
                            "A mutable FCMap may not have fast read access revoked.");
                }
                index.release();
                index = null;
            } finally {
     ...
     * {@inheritDoc}
     *
     * <p>Important: this method is not synchronized, and is unsafe to call concurrently with
     * another thread that may archive this object. For thread safe access, acquire a read lock on
     * the map first.
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * <p>Removes the mapping for the specified key from this map if present.
     *
     * <p>This operation takes O(log n) time
     *
     ...
     *
     * <p>This data structure does not support null values, so if null is returned for a key then it
     * can be inferred that the map does not contain an entry for that specific key.
     *
     * <p>This operation takes O(1) time for both the mutable copy and for all immutable copies.
     *
     * <p>The value returned by this method should not be directly modified. If a value requires
     * modification, call {@link #getForModify(Object)} and modify the value returned by that method
     * instead.
     *
     * <p>The value returned by this method should not be directly modified. If a value requires
     * modification, call {@link #getForModify(Object)} and modify the value returned by that method
    ...
    /**
     * Get the value associated with a given key. Value is safe to directly modify. If given key is
     * not in the map then null is returned.
     *
     * <p>In a prior implementation of this method it was necessary to re-insert the modified value
     * back into the tree via the replace() method. In the current implementation this is no longer
     * required. Replacing a value returned by this method has no negative side effects, although it
     * will have minor performance overhead and should be avoided if possible.
     *
     * @param key the key that will be used to look up the value
     * @return an object that is safe to directly modify, or null if the requested key is not in the
     *     map
     */
    ...
    /**
     * The implementation of replace without locks. This allows for the replace operation to be
     * performed while a lock is already held by the outer context.
     */
    private V replaceInternal(final K key, final V value) {
        ...
        if (value.getReservationCount() != 0) {
            throw new IllegalArgumentException(
                    "Value is already in a tree, can not insert into map");
        }

        // Once fast copies are managed by a utility, these manual hash invalidations will no longer
        // be necessary.
        invalidateHash();
     ...
     *
     * <p>This operation takes O(lg n) time where <i>n</i> is the current number of keys.
     *
     * @param key key with which the specified value is to be associated and a previous value is
     ...
     * @return the previous value associated with {@code key}, or {@code null} if the key was not
    ...
    public void rebuild() {
        ...
        getTree()
                ...
                .setDescendantFilter(
                        node ->
                                node.getDepth() < splitDepth
                                        && node.getClassId() == MerkleTreeInternalNode.CLASS_ID)
                .forEachRemaining(
                        node -> {
        ...
        // Process all entries near the top of the tree. Don't bother fanning this work out to
        // threads.
            ...
            for (final Future<?> future : futures) {
                try {
                    ...
                    throw new RuntimeException(
                            "interrupted while attempting to rebuild MerkleMap, this is"
                                    + " unrecoverable");
