...
/**
 * A merkle data structure that implements a balanced binary tree. Utilizes a copy-on-write
 * algorithm for O(1) fast copies and O(log n) updates.
 *
 * @param <T> Type for leaves
 */
public class MerkleBinaryTree<T extends MerkleNode> extends PartialBinaryMerkleInternal
    ...
    /**
     * Keeps track of the right most leaf in order to replace a deleted leaf with it. Once the
     * replacement is done, a new right most leaf must be assigned.
     */
        ...
        switch (index) {
            ...
            default:
                throw new IllegalChildIndexException(
                        getMinimumChildCount(), getMaximumChildCount(), index);
        }
    }
    ...
    /** Creates an instance of {@link MerkleBinaryTree} {@link SerializableHashable} */
     ...
     *
     * @param leafToDelete Leaf to be removed from the tree
     * @param updateCache a function that is used to register changes that may need to be tracked by
     *     a cache in the outer scope
     */
     ...
     *
     * @param path a path of nodes that have been replaced down to the parent of the right most leaf
     * @param parentOfRightMostLeaf the parent of the right most leaf, pre-computed by the caller
     * @param updateCache a function that is used to register changes that may need to be tracked by
     *     a cache in the outer scope
     */
    private void deleteRightMostLeafSibling(
        ...
        final T copy =
                copyTreeToLocation(
                        grandparentOfRightMostLeaf, indexOfParentInGrandparent, rightMostLeaf);
        updateCache.accept(copy);
    }
    ...
    private void deleteMiddleLeaf(
        ...
        final int indexOfChildToDelete =
                findChildPositionInParent(parentOfNodeToDelete, leafToDelete);

        // Move the right most leaf to replace the leaf being deleted
        final T copy =
                copyTreeToLocation(parentOfNodeToDelete, indexOfChildToDelete, rightMostLeaf);
        updateCache.accept(copy);

        // Move the sibling of the rightmost leaf to replace its parent.
        // Since we already have copied the rightmost leaf, it is safe to simply delete the original
        // tree.
     ...
     * Deletes a leaf.
     *
     * @param leafToDelete leaf to delete from the tree
     * @param updateCache a function that is used to register changes that may need to be tracked by
     *     a cache in the outer scope
     */
    ...
    private void setRightMostLeaf() {
        if (getSize().getValue() <= SIMPLE_TREE_SIZE) {
            ...
            final MerkleTreeInternalNode node =
                    findFirstLeafAtCompleteSubtree(
                            leftMostOneBit, getSize().getValue() - 1, this.getRoot());
     ...
     *
     * @param oldLeaf Old leaf to replace
     * @param newLeaf New leaf replacing
     */
    public void update(final T oldLeaf, final T newLeaf) {
        ...
        if (oldLeaf == null || newLeaf == null) {
            ...
            // Replace a path down to the parent of the old leaf. Don't bother replacing the old
            // leaf itself.
     ...
     *
     * @param entry New entry to be inserted into the tree
     * @param updateCache a function that is used to register changes that may need to be tracked by
     *     a cache in the outer scope
     */
    public void insert(final T entry, final Consumer<T> updateCache) {
        ...
        if (getSize().getValue() == 0) {
            ...
            // Take an artificial reference to the node to push down to prevent it from being
            // destroyed too soon.
            nodeToPush.reserve();
            ...
            final int childIndexOfNodeToPush =
                    findChildPositionInParent(parentOfNodeToPush, nodeToPush);
    ...
    /**
     * Walks node by node from the specified node to the first leaf in the last complete layer
     *
     * @param leftMostOneBit Left most bit
     * @param size Number of leaves
     * @param internalNode Initial node to start path
    ...
    /**
     * Finds a leaf by iterating over the leaves. This takes O(n), where n is the number of leaves
     * and worst case requires traversing the whole tree.
     *
     * @param condition Condition to find the Leaf
    ...
    /**
     * Returns the right most leaf of the tree. This method is available in order to test
     * correctness of mutating methods.
     *
    ...
    /**
     * Whenever an entry in this tree is copied by an external source the new copy must be
     * registered. This registration is required to maintain internal data structures.
     *
     * @param original the original entry
     * @param copy the copy of the original entry
     */
     ...
     *
     * @param node the node the iterator might want to return
    ...
    /**
     * This method is used by the iterator to ignore all nodes that may sit below entries if the
     * entries are internal nodes
     *
     * @param node the node the iterator might want to return the descendants of
