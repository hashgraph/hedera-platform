...
public class BrokenBuilderMerkleDb extends BrokenBuilder {

    private static final long CLASS_ID = 0x5a79654cd0f96dd0L;

    public BrokenBuilderMerkleDb() {}

    public BrokenBuilderMerkleDb(VirtualDataSourceBuilder<TestKey, TestValue> delegate) {
        super(delegate);
    }
    ...
    public BreakableDataSource build(final String label, final boolean withDbCompactionEnabled) {
        return new BreakableDataSourceMerkleDb(
                this, delegate.build(label, withDbCompactionEnabled));
    }
    ...
    public BreakableDataSource copy(final VirtualDataSource<TestKey, TestValue> snapshotMe) {
        ...
        return new BreakableDataSourceMerkleDb(this, delegate.copy(breakableSnapshot.delegate));
    }

    @Override
    public BreakableDataSource restore(final String label, final Path from) {
        return new BreakableDataSourceMerkleDb(this, delegate.restore(label, from));
    }
}
