...
public class VirtualMapMerkleDbReconnectTestBase extends VirtualMapReconnectTestBase {
        ...
        ConstructableRegistry.getInstance()
                .registerConstructable(
                        new ClassConstructorPair(
                                TestKeySerializerMerkleDb.class, TestKeySerializerMerkleDb::new));
        ConstructableRegistry.getInstance()
                .registerConstructable(
                        new ClassConstructorPair(
                                TestValueSerializerMerkleDb.class,
                                TestValueSerializerMerkleDb::new));
    }

    @Override
    protected VirtualDataSourceBuilder<TestKey, TestValue> createBuilder() throws IOException {
        // The tests create maps with identical names. They would conflict with each other in the
        // default
        ...
        final MerkleDbTableConfig<TestKey, TestValue> tableConfig =
    ...
    @Override
    protected BrokenBuilder createBrokenBuilder(
            final VirtualDataSourceBuilder<TestKey, TestValue> delegate) {
    ...
    public static void setupJPDB() throws ConstructableRegistryException {
        ConstructableRegistry.getInstance()
                .registerConstructable(
                        new ClassConstructorPair(
                                BrokenBuilderMerkleDb.class, BrokenBuilderMerkleDb::new));
    }
}
