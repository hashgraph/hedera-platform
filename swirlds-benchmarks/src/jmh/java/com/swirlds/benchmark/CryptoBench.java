...
public class CryptoBench extends VirtualMapBench {
    ...
    private static final Logger LOG = LogManager.getLogger(CryptoBench.class);
    ...
    private static final BenchmarkKey FIXED_KEY1 = new BenchmarkKey(FIXED_KEY_ID1);
    private static final BenchmarkKey FIXED_KEY2 = new BenchmarkKey(FIXED_KEY_ID2);
    ...
    /**
     * Emulates crypto transfer. Reads a batch of "account" pairs and updates them by transferring a
     * random amount from one to another. Single-threaded.
     */
    @Benchmark
    public void transferSerial() throws Exception {
        ...
        // Initialize fixed accounts
        if (virtualMap.get(FIXED_KEY1) == null) {
            virtualMap.put(FIXED_KEY1, new BenchmarkValue(0));
        }
        if (virtualMap.get(FIXED_KEY2) == null) {
            virtualMap.put(FIXED_KEY2, new BenchmarkValue(0));
        }
        ...
        for (int i = 1; i <= numFiles; ++i) {
            ...
            for (int j = 0; j < numRecords; ++j) {
                ...
                // Model fees
                value1 = virtualMap.getForModify(FIXED_KEY1);
                value1.update(l -> l + 1);
                value2 = virtualMap.getForModify(FIXED_KEY2);
                value2.update(l -> l + 1);
    ...
    /**
     * Emulates crypto transfer. Fetches a batch of "accounts" in parallel, updates the "accounts"
     * in order by transferring a random amount from one to another.
     */
    @Benchmark
                ...
                Executors.newCachedThreadPool(
                        new ThreadConfiguration()
                                ...
                                .setExceptionHandler(
                                        (t, ex) ->
                                                LOG.error(
                                                        "Uncaught exception during prefetching",
                                                        ex))
                                .buildFactory());
        ...
        for (int i = 1; i <= numFiles; ++i) {
            ...
            for (int thread = 0; thread < numThreads; ++thread) {
                ...
                prefetchPool.execute(
                        () -> {
                            try {
                                final BlockingQueue<Optional<BenchmarkValue>> queue =
                                        queues.get(idx);
                                for (int j = idx * KEYS_PER_RECORD;
                                        j < keys.length;
                                        j += numThreads * KEYS_PER_RECORD) {
                                    queue.put(
                                            Optional.ofNullable(
                                                    currentMap.get(new BenchmarkKey(keys[j]))));
                                    queue.put(
                                            Optional.ofNullable(
                                                    currentMap.get(new BenchmarkKey(keys[j + 1]))));
                                }
            ...
            for (int j = 0; j < numRecords; ++j) {
                ...
                // Model fees
                value1 = currentMap.getForModify(FIXED_KEY1);
                value1.update(l -> l + 1);
                value2 = currentMap.getForModify(FIXED_KEY2);
                value2.update(l -> l + 1);
