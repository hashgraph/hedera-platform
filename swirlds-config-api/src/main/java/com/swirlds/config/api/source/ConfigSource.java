...
public interface ConfigSource {
    ...
    /**
     * The default ordinal that is used if an implementation does not provide an ordinal by its own.
     * See {@link ConfigSource#getOrdinal()}
     */
     ...
     * @throws NoSuchElementException if the property with the given name is not defined in the
     *     source
     */
    String getValue(String propertyName) throws NoSuchElementException;

    /**
     * Returns the ordinal. The ordinal is used to define a priority order of all config sources
     * while the config source with the highest ordinal has the highest priority. A config source
     * will overwrite values of properties that are already defined by a config source with a lower
     * ordinal.
     *
     * @return the ordinal
     */
