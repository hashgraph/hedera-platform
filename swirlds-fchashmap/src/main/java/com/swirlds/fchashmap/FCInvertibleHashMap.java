...
/**
 * A fast copyable hashmap that is also able to generate inverse mappings between a value and the
 * set of all keys that map to that value. Supports pagination.
 *
 * <p>This data structure does not support null values.
 *
 * @param <K> the type of the key. This key must be effectively immutable -- that is, no changes to
 *     the key after its insertion may have any effect on its {@link Object#equals(Object)} or
 *     {@link Object#hashCode()}.
 * @param <V> the type of the value. This key must be effectively immutable -- that is, no changes
 *     to the value after its insertion may have any effect on its {@link Object#equals(Object)} or
 *     {@link Object#hashCode()}.
 */
public class FCInvertibleHashMap<K, V> extends AbstractMap<K, V> implements FastCopyable {
     ...
     *
     * <p>This iterator should not used after this copy of the map is updated in any way. This
     * iterator may exhibit undefined behavior if this copy of the map is updated between the
     * creation of this iterator and the use of this iterator.
     *
    ...
    /**
     * Given a value, return an iterator that iterates over a paginated set of keys that map to the
     * value. Iteration starts at a given index and continues until the end of the key set.
     *
     * <p>This iterator should not used after this copy of the map is updated in any way. This
     * iterator may exhibit undefined behavior if this copy of the map is updated between the
     * creation of this iterator and the use of this iterator.
     *
     * @param value the value in question
     * @param startIndex the index of the first key to return (inclusive). An index of 0 starts
     *     iteration at the first key in the set.
     ...
     * @param startIndex the index of the first key to return (inclusive). An index of 0 starts
     *     iteration at the first key in the set.
     * @param endIndex the index bounding the last key to return (exclusive). An index of N will
    ...
    /**
     * Return a count of the number of keys that reference a given value, i.e. the number of times
     * that a value appears in the map.
     *
     * @param value the value in question
