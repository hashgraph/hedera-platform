...
/**
 * A map that with {@link java.util.HashMap HashMap} like O(1) performance that provides {@link
 * FastCopyable} semantics.
 *
 * <p>All operations are thread safe if performed simultaneously on different copies of the map.
 *
 * <p>It is safe to read and write simultaneously to the mutable copy of the map with multiple
 * threads as long as the read operations are not performed concurrently with write operations on
 * the same key. {@link #size} may return incorrect results if executed concurrently with an
 * operation that modifies the size.
 *
 * <p>It is not thread safe to perform read/write operations on a copy of this this map while that
 * copy is being released.
 *
...
public class FCHashMap<K, V> extends AbstractMap<K, V> implements FastCopyable {
    ...
    /**
     * Monotonically increasing version number that is incremented every time copy() is called on
     * the mutable copy.
     */
    ...
    /**
     * All copies of the map that have not yet been garbage collected. New copies are added to the
     * end, old copies are removed from the beginning.
     */
    ...
    /**
     * Exposed for testing. Get the total number of copies that have not been fully garbage
     * collected (including copies not eligible for garbage collection).
     *
    ...
    /**
     * Use this to clean up resources held by this copy. Failure to call delete on a copy before it
     * is garbage collected will result in a memory leak.
     *
     * <p>Not thread safe. Must not be called at the same time another thread is attempting to read
     * from this copy.
     */
    @Override
     ...
     *
     * @param key the key that requires garbage collection
     * @param version the version of the map that has been released
     */
    private void cleanOldMutations(final K key, final long version) {
        data.compute(
                key,
                (final K k, final Mutation<V> mutationHead) -> {
     ...
     *
     * @param key the key that points to a list of mutations that require garbage collection
     * @param version that version that, when deleted, will require garbage collection to be done
     */
     ...
     * @return The mutation that corresponds to this version. May be null if the key is not in the
     *     map at this version.
     */
    ...
    private V mutate(final K key, final V value) {
        ...
        data.compute(
                key,
                (final K k, final Mutation<V> mutationHead) -> {
                    originalValueReference.setValue(
                            mutationHead == null ? null : mutationHead.getValue());
                    ...
                    if (value == null && mutation.getPrevious() == null) {
                        // If the only remaining mutation is a deletion then it is safe to remove
        ...
        if (requiresGarbageCollection.getValue()) {
            // Set up future garbage collection. This is called iff the mutate operation caused an
            // additional
            // mutation to be added to the list. Once all copies of the map before the current
            // version
            // are released, no mutations before the current mutations will be reachable. So request
            // a garbage collection operation on this list of mutations when the version right
            // before
    ...
    /**
     * Directly inject a value into the map. This method designed for rapid initialization of an
     * FCHashMap, and should only be used on maps being initialized. Should not be called after the
     * map has been copied or modified by any method other than this method..
     *
     * <p>This method does not update the size of the data structure. After all injections have been
     * completed, call {@link #initialResize()}.
     *
     * <p>It is thread safe to use this method concurrently.
     */
    public void initialInjection(final K key, final V value) {
        data.put(key, new Mutation<>(version, value, null));
    }

    /**
     * This method MUST be called if the FCHashMap has been initialized using {@link
     * #initialInjection(Object, Object)}. If called, must be called before any copies of the map
     * are made or any modifications are made by any method other than {@link
     * #initialInjection(Object, Object)}.
     */
    ...
    /**
     * Not thread safe on an immutable copy of the map if it is possible that another thread may
     * have deleted the map copy. Map deletion and reads against the map must be externally
     * synchronized. The function hasBeenDeleted() can be used to check to see if the copy has been
     * deleted.
     */
    @SuppressWarnings("unchecked")
    @Override
    ...
    /**
     * A value from an {@link FCHashMap} that is safe to modify. Return type of {@link
     * #getForModify(Object)}.
     *
     * @param value the value from the FCHashMap, or null if no value exists
     * @param original the original value that was copied. Equal to value if there was no copying
     *     performed
     * @param <V> the type of the value
     */
        ...
        final Mutation<V> mutation =
                data.compute(
                        ...
                        (final K k, final Mutation<V> mutationHead) -> {
                            ...
                            if (mutationHead.getVersion() == version
                                    || mutationHead.getValue() == null) {
                                return mutationHead;
                            }

                            return new Mutation<>(
                                    version,
                                    ((FastCopyable) mutationHead.getValue()).copy(),
                                    mutationHead);
                        });
