...
/**
 * This class implements a fast copyable one-to-many relation map. This class is analogous to a
 * {@code Map<K, Set<V>>} but with the ability to make fast copies.
 *
 * @param <K> the type of the key. Type must be effectively immutable -- that is, once inserted into
 *     this data structure its equals() and hashcode() must always yield the same results.
 * @param <V> the type of the value. Type must be effectively immutable -- that is, once inserted
 *     into this data structure its equals() and hashcode() must always yield the same results.
 */
public class FCOneToManyRelation<K, V> implements FastCopyable {
     ...
     *
     * <p>This data structure is only required for operations that update the mutable copy of the
     * this object (i.e. gap filling). When this FCOneToManyRelation is copied and becomes
     * immutable, this map is set to null.
     */
     ...
     * Copy constructor.
     *
     * @param that the object to copy. This object is mutable, but that object becomes immutable.
     */
     ...
     *
     * <p>Has no effect if the key and value already has an association.
     *
     * @param key the key will be associated with the value
     * @param value the value that will be associated with the key
     * @return true if a new association is made, false if the key and value are already associated
     */
    ...
    /**
     * Break an association between a key and a value. Has no effect if the key is not associated
     * with the value.
     *
     * @param key the key may be associated with a value
     * @param value the value that the key references
     * @return true the association is broken, false if the key and value were not associated in the
     *     first place
     */
     ...
     *
     * @param association the association in question
     ...
     *
     * <p>This iterator should not used after this data structure is updated in any way. This
     * iterator may exhibit undefined behavior if this copy of the data structure is updated between
     ...
     *
     * @param key the key in question
     * @param startIndex the index of the first value to return (inclusive). An index of 0 starts
     *     iteration at the first value in the set.
     * @return an iterator for the key set starting at the given index
     * @throws IndexOutOfBoundsException if the requested start index is invalid
     */
     ...
     * @param startIndex the index of the first value to return (inclusive). An index of 0 starts
     *     iteration at the first value in the set.
     * @param endIndex the index bounding the last value to return (exclusive). An index of N will
     *     cause the set to include the last element if there are N elements.
     * @return an iterator for the value set for the over the requested indices
     * @throws IndexOutOfBoundsException if the requested indices are invalid
     */
     ...
     *
     * @param key the key in question
     * @param startIndex the index of the first key to return (inclusive). An index of 0 starts at
     ...
     *     the first value associated with the key.
     * @param endIndex the index bounding the last value to return (exclusive). An index of N will
     *     cause the list to include the last element if there are N elements.
     * @return a list of associated values
     */
