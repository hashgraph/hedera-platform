...
/**
 * Test a *HUGE* billion-leaf tree. This test is separated out from {@link VirtualHasherTest} so we
 * can easily ignore it or include it depending on the build context. It takes several minutes to
 * complete.
 */
class VirtualHasherHugeTest extends VirtualHasherTestBase {
    ...
    void hugeTree() {
        ...
        // Since we're hashing every internal node, we can generate new ones with null hashes. None
        // are ever
        // asked for twice.
        final LongFunction<VirtualInternalRecord> internalGetter = VirtualInternalRecord::new;

        // Go ahead and hash. I'm just going to check that the root hash produces *something*. I'm
        // not worried
        // in this test as to the validity of this root hash since correctness is validated heavily
        // in other
        // tests. In this test, I just want to be sure that we complete, and that we don't run out
        // of memory.
