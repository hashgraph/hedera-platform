...
/**
 * Defines a data source, used with {@code VirtualMap}, to implement a virtual tree. Both in-memory
 * and on-disk data sources may be written. When constructing a {@code VirtualMap}, create a
 * concrete data source implementation.
 *
 * <p>The {@link VirtualDataSource} defines methods for getting the root node, and for looking up a
 * leaf node by key.
 *
 * <p>The nodes returned by the methods on this interface represent the *LATEST* state on disk. Once
 * retrieved, the nodes can be fast-copied for later versions, and persisted to disk via *archive*.
 *
 * <p>Each datasource instance works for a single type of K and V
 *
 * <p><strong>YOU MUST NEVER ASK FOR SOMETHING YOU HAVE NOT PREVIOUSLY WRITTEN.</strong> If you do,
 * you will get very strange exceptions. This is deemed acceptable because guarding against it would
 * require obnoxious performance degradation.
 *
 * @param <K> The key for a leaf node.
 * @param <V> The type of leaf node.
 */
...
public interface VirtualDataSource<K extends VirtualKey<? super K>, V extends VirtualValue> {
     ...
     *
     * @throws IOException If there was a problem closing the data source
     */
     ...
     *
     * <p><strong>YOU MUST NEVER ASK FOR SOMETHING YOU HAVE NOT PREVIOUSLY WRITTEN.</strong>
     *
     ...
     *
     * @param path The path for an internal node
     ...
     *
     * @param path The path for an internal node
     * @param deserialize When set to false, a Java object is not deserialized but the OS still
     *     loads it and the OS file cache is populated. This may speed up subsequent reads
     * @return The internal node's record if one was stored for the given path; {@code null} if not
     *     stored or deserialization is not requested
     * @throws IOException If there was a problem reading the internal record
     */
    VirtualInternalRecord loadInternalRecord(final long path, final boolean deserialize)
            throws IOException;

    /**
     * Reads the internal record at {@code path} into OS file cache, but don't deserialize it into
     * Java heap. This gives us a free cache at the OS level without storing anything into Java
     * heap.
     *
     * @param path The path for an internal node
     * @throws IOException If there was a problem reading the internal record
     ...
     *
     * <p>NOTE: Called during the hashing phase ONLY. Never called on non-existent nodes.
     *
     * @param path the path to the leaf
     * @return leaf's hash or null if no leaf hash is stored for the given path
     * @throws IOException If there was a problem loading the leaf's hash from data source
     */
    Hash loadLeafHash(final long path) throws IOException;

    /**
     * Write a snapshot of the current state of the database at this moment in time. This will need
     * to be called between calls to saveRecords to have a reliable state. This will block till the
     * snapshot is completely created.
     *
    ...
    /**
     * Switch this database instance to use the statistics registered in another database. Required
     * due to statistics restriction that prevents stats from being registered after initial boot up
     * process.
     *
     * @param that the database with statistics to copy
     */
    ...
    /**
     * Build an empty {@link VirtualKeySet}. This key set should be compatible with data in this
     * data source, but should otherwise have no direct connection to the data in this data source.
     *
