...
/**
 * Base class for {@link VirtualInternalRecord} and {@link VirtualLeafRecord}. Records are data
 * stored in the {@link VirtualDataSource} and in the {@code VirtualNodeCache}. These records are
 * mutable. Since the cache maintains versions across rounds (copies), it is necessary to create new
 * copies of the VirtualRecord in each round in which it is mutated.
 */
public abstract class VirtualRecord implements Hashable {
     ...
     *
     * @param path Must be non-negative, or {@link Path#INVALID_PATH}.
     */
