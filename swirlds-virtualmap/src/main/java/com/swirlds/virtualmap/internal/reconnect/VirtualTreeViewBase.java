...
public abstract class VirtualTreeViewBase<K extends VirtualKey<? super K>, V extends VirtualValue>
    ...
    /**
     * The root node that is involved in reconnect. This would be the saved state for the teacher,
     * and the new root node into which things are being serialized for the learner.
     */
    protected final VirtualRootNode<K, V> root;

    /**
     * The state representing the tree being reconnected. For the teacher, this corresponds to the
     * saved state. For the learner, this is the state of the tree being serialized into.
     */
    protected final StateAccessor reconnectState;

    /**
     * The state representing the original, unmodified tree on the learner. For simplicity, on the
     * teacher, this is the same as {@link #reconnectState}. For the learner, it is the state of the
     * detached, unmodified tree.
     */
     ...
     * @param originalState The original state of a learner. Cannot be null.
     * @param reconnectState The state of the trees being reconnected. Cannot be null.
     */
    ...
    public MerkleNode getMerkleRoot(final Long node) {
        // NOTE: It is not clear what this "node" is. Original path? New path? It seems to be both
        // depending on the
    ...
    public boolean isInternal(final Long node, final boolean isOriginal) {
        ...
        // Based on isOriginal I can know whether the node is out of the original state or the
        // reconnect state.
