...
/**
 * A POJO containing the information needed by a future thread to be able to create a hash for some
 * leaf or internal node. The class itself is somewhat intentionally messy. To cut down on garbage
 * collection and object allocation for this very hot code path, we create these objects and then
 * reuse them over and over. Sometimes they need to represent a hash job for a leaf, and sometimes
 * for an internal node, so they have references to both types of records. The {@code left} and
 * {@code right} hashes are null for leaf types, but present for internals. All jobs have a path,
 * and the {@code hash} keeps track of the final result.
 *
...
final class HashJob<K extends VirtualKey<? super K>, V extends VirtualValue> implements Hashable {
    ...
    void hash(final HashBuilder builder) {
        if (leaf != null) {
            ...
            final long classId =
                    path == ROOT_PATH ? VirtualRootNode.CLASS_ID : VirtualInternalNode.CLASS_ID;
