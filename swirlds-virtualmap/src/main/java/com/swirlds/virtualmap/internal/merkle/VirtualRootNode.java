...
/**
 * An {@link AbstractMerkleInternal} that represents the root node of a virtual tree. This root node
 * always exists, even if no leaves exist.
 *
 * <p><hr>
 *
 * <p><strong>Hashing</strong>
 *
 * <p>This class implements a multithreaded 'leaf-first' hashing algorithm that visits only those
 * nodes required for hashing and visits them at most exactly once. The set of leaves modified
 * during the round (so-called "dirty" leaves) and sorts them such that the first leaf in the list
 * is the leaf with the largest path, and the last leaf has the lowest path. (A path in our system
 * is an incrementing number where root is 0, root's left child is 1, root's right child is 2,
 * root's left-most grand child is 3, and so forth, rank by rank from left to right). This algorithm
 * is <strong>not</strong> depth-first or breadth-first and requires a path-to-node lookup map to
 * work if the nodes do not already have a reference to the parent. Since our leaves are "realized"
 * on demand, most of them will not have references to their parent. However, as the graph builds up
 * over time, many of the parent nodes in the lower ranks (those closer to root) will have strong
 * references to their parents, obviating the need to look up those nodes in the map.
 *
 * <p>This class is responsible for hashing itself. This class has its own thread pool and worker
 * threads which it uses for implementing the algorithm. If the entire in-memory Merkle tree is
 * modified to use the same algorithm, then this code can be moved or eliminated.
 *
 * @param <K> The key
 * @param <V> The value
 */
...
public final class VirtualRootNode<K extends VirtualKey<? super K>, V extends VirtualValue>
    ...
    /**
     * This version number should be used to handle compatibility issues that may arise from any
     * future changes
     */
     ...
     * leaves to the hashing system. The size of this variable will depend on the incoming rate of
     * leaves vs. the speed of hashing.
     */
     ...
     * cancel the reconnect with an exception. If we cannot make space after this many seconds, then
     * it means a single round of hashing has exceeded this time threshold.
     */
    private static final int MAX_RECONNECT_HASHING_BUFFER_TIMEOUT = 60;

    /**
     * Placeholder (since this is such a hotspot) to hold the results from {@link
     * VirtualMapSettingsFactory#get()} rather than calling that method more than once during the
     * lifecycle of a {@link VirtualRootNode} instance.
     */
    private final VirtualMapSettings settings = VirtualMapSettingsFactory.get();

    /**
     * The maximum size() we have reached, where we have (already) recorded a warning message about
     * how little space is left before this {@link VirtualRootNode} hits the size limit. We retain
     * this information because if we later delete some nodes and then add some more, we don't want
     * to trigger a duplicate warning.
     */
     ...
     * during reconnect to create a new data source based on a snapshot directory, or in various
     * other scenarios.
     */
    private VirtualDataSourceBuilder<K, V> dataSourceBuilder;

    /**
     * Provides access to the {@link VirtualDataSource} for tree data. All instances of {@link
     * VirtualRootNode} in the "family" (i.e. that are copies going back to some first progenitor)
     * share the exact same dataSource instance.
     */
    private VirtualDataSource<K, V> dataSource;

    /**
     * A cache for {@link VirtualRecord}s. This cache is very specific for this use case. The
     * elements in the cache are those nodes that were modified by this root node, or any copy of
     * this node, that have not yet been written to disk. This cache is used for two purposes.
     * First, we avoid writing to disk until the round is completed and hashed as both a performance
     * enhancement and, more critically, to avoid having to make the filesystem fast-copyable.
     * Second, since modifications are not written to disk, we must cache them here to return
     * correct and consistent results to callers of the map-like APIs.
     *
     * <p>Deleted leaves are represented with records that have the "deleted" flag set.
     *
     * <p>Since this is fast-copyable and shared across all copies of a {@link VirtualRootNode}, it
     * contains the changed leaves over history. Since we always flush from oldest to newest, we
     * know for certain that anything here is at least as new as, or newer than, what is on disk. So
     * we check it first whenever we need a leaf. This allows us to keep the disk simple and not
     * fast-copyable.
     */
    private VirtualNodeCache<K, V> cache;

    /**
     * An interface through which the {@link VirtualRootNode} can access persistent virtual map
     * state, such as the first leaf path, last leaf path, name, and other state. We do not access
     * the {@link VirtualMapState} directly because the {@link VirtualMapState} instance may change
     ...
     * and the data source. By encapsulating this logic in a RecordAccessor, we make it convenient
     * to access records using a combination of different caches, states, and data sources, which
     * becomes important for reconnect and other uses. This should never be null except for a brief
     * window during initialization / reconnect / serialization.
     */
    ...
    /**
     * The {@link VirtualPipeline}, shared across all copies of a given {@link VirtualRootNode},
     * maintains the lifecycle of the nodes, making sure they are merged or flushed or hashed in
     * order and according to the defined lifecycle rules. This class makes calls to the pipeline,
     * and the pipeline calls back methods defined in this class.
     */
    private VirtualPipeline pipeline;

    /**
     * If true, then this copy of {@link VirtualRootNode} should eventually be flushed to disk. A
     * heuristic is used to determine which copy is flushed.
     */
    ...
    /**
     * Specifies whether this current copy has been flushed. This will only be true if {@link
     * #shouldBeFlushed} is true, and it has been flushed.
     */
    private final AtomicBoolean flushed = new AtomicBoolean(false);

    /**
     * Specifies whether this current copy hsa been merged. This will only be true if {@link
     * #shouldBeFlushed} is false, and it has been merged.
     */
    ...
    /**
     * Created at the beginning of reconnect as a <strong>learner</strong>, this iterator allows for
     * other threads to feed its leaf records to be used during hashing.
     */
    ...
    /**
     * Required by the {@link com.swirlds.common.constructable.RuntimeConstructable} contract. This
     * can <strong>only</strong> be called as part of serialization, not for normal use.
     */
     ...
     * Create an instance.
     *
     * @param dataSourceBuilder The datasource builder.
     * @param enforce Whether to enforce the data source and state being non-null.
     */
    private VirtualRootNode(
            final VirtualDataSourceBuilder<K, V> dataSourceBuilder, final boolean enforce) {
        this.fastCopyVersion = 0;
        this.cache = new VirtualNodeCache<>();
        this.hasher = new VirtualHasher<>();
        this.shouldBeFlushed = false;
        this.dataSourceBuilder =
                enforce ? Objects.requireNonNull(dataSourceBuilder) : dataSourceBuilder;
    }
    ...
    private VirtualRootNode(VirtualRootNode<K, V> source) {
        ...
        if (this.pipeline.isTerminated()) {
            throw new IllegalStateException(
                    "A fast-copy was made of a VirtualRootNode with a terminated pipeline!");
        }

        // These three will be set in postInit. This is very unfortunate, but stems from the
        // way serialization / deserialization are implemented which requires partially constructed
        // objects.
        this.state = null;
        this.shouldBeFlushed = false;
        this.records = null;
    ...
    public void postInit(final StateAccessor state) {
        ...
        if (statistics == null) {
            // Only create statistics instance if we don't yet have statistics. During a reconnect
            // operation.
    ...
    public <T extends MerkleNode> T getChild(final int index) {
        ...
        if (path < state.getFirstLeafPath()) {
            ...
            if (leafRecord == null) {
                throw new IllegalStateException(
                        "Invalid null record for child index "
                                + index
                                + " (path = "
                                + path
                                + "). First leaf path = "
            ...
            // The index is out of bounds. Maybe we have a root node with one leaf and somebody has
            // asked
    ...
    protected void setChildInternal(final int index, final MerkleNode child) {
        throw new UnsupportedOperationException(
                "You cannot set the child of a VirtualRootNode directly with this API");
    }
    ...
    /**
     * Gets the value associated with the given key such that any changes to the value will be used
     * in calculating hashes and eventually saved to disk. If the value is actually never modified,
     * some work will be wasted computing hashes and saving data that has not actually changed.
     *
    ...
    /**
     * Gets the value associated with the given key. The returned value *WILL BE* immutable. To
     * modify the value, use call {@link #getForModify(VirtualKey)}.
     *
     * @param key The key. This must not be null.
    ...
    /**
     * Puts the key/value pair into the map. The key must not be null, but the value may be null.
     * The previous value, if it existed, is returned. If the entry was already in the map, the
     * value is replaced. If the mapping was not in the map, then a new entry is made.
     *
     * @param key the key, cannot be null.
    ...
    /**
     * Replace the given key with the given value. Only has an effect if the key already exists in
     * the map. Returns the value on success. Throws an IllegalStateException if the key doesn't
     * exist in the map.
     *
     * @param key The key. Cannot be null.
     * @param value The value. May be null.
     ...
     * @throws IllegalStateException if an attempt is made to replace a value that didn't already
     *     exist
     */
    ...
    /**
     * Removes the key/value pair denoted by the given key from the map. Has no effect if the key
     * didn't exist.
     *
     * @param key The key to remove. Cannot be null.
     * @return The removed value. May return null if there was no value to remove or if the value
     *     was null.
     */
    public V remove(final K key) {
        ...
        if (leafToDeletePath != lastLeafPath) {
            ...
            markDirty(lastLeaf);
            // NOTE: at this point, if leafToDelete was in the cache at some "path" index, it isn't
            // anymore!
        ...
        if (lastLeafParent == ROOT_PATH) {
            if (firstLeafPath == lastLeafPath) {
                ...
                // We removed the second to last leaf, so the first & last leaf paths are now the
                // same.
    ...
    public void onShutdown(final boolean immediately) {
        ...
        if (immediately) {
            // If immediate shutdown is required then let the hasher know it is being stopped. If
            // shutdown
    ...
    public void merge() {
        ...
        if (!(isDestroyed() || isDetached())) {
            throw new IllegalStateException(
                    "merge is legal only after this node is destroyed or detached");
        }
        if (!isImmutable()) {
    ...
    private void flush(
        ...
        try {
            ...
            final Stream<VirtualLeafRecord<K, V>> sortedDirtyLeaves =
                    cacheToFlush.dirtyLeaves(
                            stateToUse.getFirstLeafPath(), stateToUse.getLastLeafPath());
    ...
    public void computeHash() {
        ...
        final VirtualHashListener<K, V> hashListener =
        ...
        if (virtualHash == null) {
            final VirtualInternalRecord rootRecord =
                    state.size() == 0 ? null : records.findInternalRecord(0);
            if (rootRecord != null) {
    ...
    public void setupWithOriginalNode(final MerkleNode originalNode) {
        assert originalNode instanceof VirtualRootNode
                : "The original node was not a VirtualRootNode!";
        ...
        // shutdown background compaction on original data source as it is no longer needed to be
        // running as all data
        // in that data source is only there as a starting point for reconnect now. So compacting it
        // further is not
        ...
        assert originalMap.isHashed()
                : "The system should have made sure this was hashed by this point!";
        ...
        // Initial timeout is intentionally very long, timeout is reduced once we receive the first
        // leaf in the tree.
        reconnectIterator =
                new ConcurrentBlockingIterator<>(
    ...
    public void prepareReconnectHashing(final long firstLeafPath, final long lastLeafPath) {
        ...
        new ThreadConfiguration()
                ...
                .setRunnable(
                        () ->
                                reconnectHashingFuture.complete(
                                        hasher.hash(
                                                path ->
                                                        reconnectRecords.findLeafRecord(
                                                                path, false),
    ...
    public void endLearnerReconnect() {
        try {
            ...
            final var message =
                    "VirtualMap@" + getRoute() + " failed to get hash during learner reconnect";
    ...
    /**
     * Adds a new leaf with the given key and value. The precondition to calling this method is that
     * the key DOES NOT have a corresponding leaf already either in the cached leaves or in the data
     * source.
     *
    ...
    private void add(final K key, final V value) {
        ...
        if (lastLeafPath == INVALID_PATH) {
            ...
            final long firstLeafPath = state.getFirstLeafPath();
            final long nextFirstLeafPath =
                    isFarRight(firstLeafPath)
                            ? getPathForRankAndIndex((byte) (getRank(firstLeafPath) + 1), 0)
                            : getPathForRankAndIndex(
                                    getRank(firstLeafPath), getIndexInRank(firstLeafPath) + 1);
            ...
            // Create a new internal node that is in the position of the old leaf and attach it to
            // the parent
        ...
        markDirty(newLeaf);
        super.setHash(
                null); // Make sure VirtualMap has an invalid hash, so it will be recomputed later
    }

    /**
     * An internal helper method that replaces the value for the given key and returns true, or
