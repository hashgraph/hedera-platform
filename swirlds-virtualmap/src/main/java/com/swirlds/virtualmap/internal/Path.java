...
/**
 * A utility class with static methods for working with paths in the virtual tree. A path in the
 * virtual tree is efficiently represented as a single long. This works because internal nodes on
 * the virtual tree always have the same number of children, and that number is always a power of
 * two. In the current implementation, the virtual tree is strictly binary. Encoding the path as a
 * long is critical to the overall performance of the system.
 */
public final class Path {

    /**
     * A special constant that represents the Path of a root node. It isn't necessary for this
     * constant to be used, but it makes the code a little more readable.
     */
    public static final long ROOT_PATH = 0L;

    /**
     * A special constant that represents the first left child of the root not. It isn't necessary,
     * but helps clarify the code a little.
     */
    ...
    /**
     * The maximum "rank" in the tree. The "rank" is the level. A rank of 0 is for the root, the
     * first rank (rank 1) contains the left and right children of root, and so on. Since the path
     * is a long, and all longs are signed the max rank is 62. With some effort we may be able to
     * support negative longs, but this number of ranks is already so incredibly large and beyond
     * our scope, it should be OK.
     */
    ...
    /**
     * The leading bit (before being shifted left (rank - 1) times) used by the getIndexInRank()
     * method.
     */
    ...
    /**
     * Given some path, get the rank of that path. For example, if the path is 0, then the rank is
     * 0. If the path is 1 or 2, then the rank is 1, and so forth.
     *
     * @param path The path
     * @return The rank of the path. If the path is {@link #INVALID_PATH}, then the rank will be -1.
     */
    ...
    /**
     * Gets the zero-based index of the Path within its rank, where {@code 0} is the left-most node
     * at that rank, and the number increases going towards the right.
     *
    ...
    /**
     * Gets the Path representing a node located at the given rank and the given index within that
     * rank. Throws an IllegalArgumentException if the index is greater than the max possible for
     * that rank.
     *
     * @param rank A non-negative rank for the node
     * @param index The non-negative index into the rank at which to find the node
     * @return A Path representing the path to the node found at the given rank and index. Never
     *     returns null.
     */
    public static long getPathForRankAndIndex(final int rank, final long index) {
        ...
        if (index > (maxForRank - 1)) {
            throw new IllegalArgumentException(
                    "The index ["
                            + index
                            + "] is too large for the number of items at this rank. maxForRank ="
                            + maxForRank);
        }
    ...
    public static boolean isFarRight(long path) {
        // It turns out, all 0's followed by all 1's followed by a single 0 is always the far right
        // node.
        // Given a valid far right like 0b0000000_00111110, -1L << (64 - numLeadingZeros) will
        // produce a complimentary
        // mask for the high leading 0's, such as 0b11111111_11000000. Xor on the path with 0x1 will
        // end up
        // flipping the low bit, so it becomes 0b00000000_00111111. By or'ing the two together, we
        // should
        // get all 1's, which is represented as -1L.
        assert path != INVALID_PATH;
        final int numLeadingZeros = Long.numberOfLeadingZeros(path);
        return path == 0
                || ((path ^ 0x1) | (-1L << (MAX_NUMBER_OF_LEADING_ZEROS - numLeadingZeros))) == -1L;
    }
     ...
     *
     * @param parentPath The parent path. Must be valid.
     * @param childIndex The child index. Must be 0 or 1.
    ...
    /**
     * Computes the {@link com.swirlds.common.merkle.route.MerkleRoute} by iterating over every node
     * from the root of the Virtual Tree up to the {@code targetPath}, and determining if the path
     * is left or right (0 or 1).
     *
     * @param targetPath path for which we want its route
     * @return list of integers representing the {@link com.swirlds.common.merkle.route.MerkleRoute}
     *     of this path.
     */
    ...
    /**
     * Gets the next step along the path from thisPath to targetPath. For example, if thisPath = 0
     * and targetPath = 3, then this method will return 1.
     *
     * @param thisPath The start path. Must not be INVALID_PATH.
     * @param targetPath The path we're trying to get to. Must not be INVALID_PATH and must be &gt;
     *     thisPath.
    ...
    public static long getNextStep(final long thisPath, final long targetPath) {
        ...
        // If the targetPath is within the left side of the sub-tree, then return the left child
        // path,
        // otherwise return the right child path.
        return (targetPath - leftMostPath < midPoint)
                ? getLeftChildPath(thisPath)
                : getRightChildPath(thisPath);
    }
