...
public final class VirtualTeacherTreeView<K extends VirtualKey<? super K>, V extends VirtualValue>
    ...
    private final ConcurrentNodeStatusTracker nodeStatusTracker =
            new ConcurrentNodeStatusTracker(Long.MAX_VALUE);
    ...
    public void registerResponseForNode(final Long node, final boolean learnerHasNode) {
        final ConcurrentNodeStatusTracker.Status status =
                learnerHasNode
    ...
    public void serializeInternal(final SerializableDataOutputStream out, final Long internal)
        ...
        // We don't need to really serialize anything here, except the learner needs a long. So
        // we'll send a long.
        ...
        if (parent > ROOT_PATH || (parent == ROOT_PATH && reconnectState.getLastLeafPath() > 1)) {
            ...
            if (leftHash == null && rightHash == null) {
                throw new MerkleSynchronizationException(
                        "Both children had null hashes at paths "
                                + leftPath
                                + " and "
                                + rightPath
                                + " for parent "
