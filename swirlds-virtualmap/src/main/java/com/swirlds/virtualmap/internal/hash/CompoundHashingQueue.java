...
/**
 * A {@link HashingQueue} implementation that is backed by (and delegates to) two other queues.
 * There is one specific case in the {@link VirtualHasher} where this is useful and avoids array
 * copies.
 *
 * <p>This implementation is peculiar in that all add modifications ({@link #addHashJob(int)},
 * {@link #appendHashJob()}) only make use of queue1. In our use case, this works out because both
 * queue1 and queue2 are sized the same, and both are individually large enough to contain all data
 * in the "stop level" rank and therefore also large enough for all ranks from there to the root
 * rank. Thus, we can cheat on the implementation. And we do.
 *
 * @param <K> The key
 * @param <V> The value
 */
