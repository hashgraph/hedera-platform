...
public final class VirtualInternalNode<K extends VirtualKey<? super K>, V extends VirtualValue>
    ...
    /**
     * The {@link VirtualMap} associated with this node. Nodes cannot be moved from one map to
     * another.
     */
    ...
    /**
     * Locates and returns an internal node based on the given path. A new instance is returned each
     * time.
     *
     * @param path The path of the node to find. If INVALID_PATH, null is returned.
        ...
        if (rec == null) {
            try {
                ...
                throw new RuntimeException(
                        "Failed to read a internal record from the data source", ex);
            }
        }
    ...
    /**
     * @param path The path. Must not be null and must be a valid path
     * @return The leaf, or null if there is not one.
     * @throws RuntimeException If we fail to access the data store, then a catastrophic error
     *     occurred and a RuntimeException is thrown.
     */
    private VirtualLeafNode<K, V> getLeafNode(final long path) {
        ...
        if (rec == null) {
            try {
                ...
                // This should absolutely be impossible. We already checked to make sure the path
                // falls
                ...
                if (rec == null) {
                    throw new IllegalStateException(
                            "Attempted to read from disk but couldn't find the leaf.");
                }
            } catch (final IOException ex) {
