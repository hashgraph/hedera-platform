 ...
 * This implementation uses {@link Long} as the representation of a node and corresponds directly to
...
public final class VirtualLearnerTreeView<K extends VirtualKey<? super K>, V extends VirtualValue>
     ...
     * {@link ExpectedLesson} data. If the value is too large, we use some more memory than needed,
     * if it is too small, we put pressure on the GC.
     */
    ...
    /**
     * As part of tracking {@link ExpectedLesson}s, this keeps track of the "nodeAlreadyPresent"
     * boolean.
     */
    ...
    public Hash getNodeHash(final Long originalChild) {
        // This method is only called on the Learner. The path given is the _ORIGINAL_ child. Each
        // call to this
        // method will be made only for the original state from the original tree.

        // If the originalChild is null, then it means we're outside the range of valid nodes, and
        // we will
        // return a NULL_HASH.
        ...
        // Get the original record (which may be in cache or on disk) to get the hash. We should
        // look at optimizing
        // this in the future, so we don't read the whole record if we don't have to (for example,
        // we could use
        ...
        if (node == null) {
            throw new MerkleSynchronizationException(
                    "Could not find node for path " + originalChild);
        }
        ...
        if (hash == null) {
            throw new MerkleSynchronizationException(
                    "Node found, but hash was null. path=" + originalChild);
        }
        return hash;
    }
