...
/**
 * Contains state for a {@link VirtualMap}. This state is stored in memory in the merkle tree as the
 * first (left) child of the VFCMap / {@link VirtualMap}.
 */
public class VirtualMapState extends PartialMerkleLeaf implements MerkleLeaf {
    ...
    /**
     * The label for the virtual tree. Needed to differentiate between different VirtualMaps (for
     * stats).
     */
     ...
     *
     * @param source The map state to copy. Cannot be null.
     */
     ...
     *
     * @param path The new path. Can be {@link Path#INVALID_PATH}, or positive. Cannot be 0 or any
    ...
    public void setFirstLeafPath(final long path) {
        if (path < 1 && path != Path.INVALID_PATH) {
            throw new IllegalArgumentException(
                    "The path must be positive, or INVALID_PATH, but was " + path);
        }
        if (path > lastLeafPath) {
            throw new IllegalArgumentException(
                    "The firstLeafPath must be less than or equal to the lastLeafPath");
        }
        firstLeafPath = path;
    }
