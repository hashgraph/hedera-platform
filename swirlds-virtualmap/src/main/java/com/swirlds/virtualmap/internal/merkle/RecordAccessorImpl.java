...
/**
 * Implementation of {@link RecordAccessor} which, given a state, cache, and data source, provides
            ...
            try {
                ...
                if (rec != null && copy) {
                    assert rec.getKey().equals(key)
                            : "The key we found from the DB does not match the one we were looking"
                                    + " for! key="
                                    + key;
                    cache.putLeaf(rec);
                }
                ...
                throw new UncheckedIOException(
                        "Failed to read a leaf record from the data source by key", ex);
            }
        }
    ...
    public VirtualLeafRecord<K, V> findLeafRecord(final long path, final boolean copy) {
        ...
        if (rec == null) {
            try {
                ...
                throw new UncheckedIOException(
                        "Failed to read a leaf record from the data source by path", ex);
            }
        }
