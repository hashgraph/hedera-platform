...
/**
 * A concurrent FIFO queue-like data structure of ever-increasing Long values implemented by an
 * array of BitSets that together support more than {@value Integer#MAX_VALUE} bits. This data
 * structure supports a single writer thread and a single reader thread.
 */
final class ConcurrentBitSetQueue {
    ...
    /**
     * Value to keep track of the latest element added to guarantee a strictly increasing insertion.
     * No negative values are allowed, so the minimum value accepted is 0. This value is only
     * require during insertion, hence it is not required to be volatile.
     */
    private long previousBitIndex;

    /**
     * Value to keep track of the next element to be removed. We keep track of the first element
     * inserted to speed up finding the first element in the queue.
     */
    ...
    /**
     * Adds the given long value to the queue. Only strictly increasing long values are supported,
     * i.e., each value added must be strictly greater than the one before it.
     *
     * @param value a positive bit index
     * @throws IllegalArgumentException If the bitIndex is not strictly greater than the previous
     *     one inserted
     */
    void add(final long value) {
        if (value <= previousBitIndex) {
            throw new IllegalArgumentException(
                    "Each value added must be strictly greater than the one before it");
        }
