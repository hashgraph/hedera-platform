...
/**
 * Unlike some data structures like MerkleMap that rebuild all metadata after a reconnect, a virtual
 * map's metadata is very large and is prohibitive to rebuild in its entirety. This means that the
 * virtual map's metadata must be modified as new data is streamed from the teacher. This class is
 * responsible for removing metadata entries for data that his no longer present in the tree after a
 * reconnect.
 *
...
public class ReconnectNodeRemover<K extends VirtualKey<? super K>, V extends VirtualValue> {
    ...
    /**
     * The path of most recent node handled. Since nodes are handled in path order, this value will
     * only increase over time.
     */
    private final AtomicLong handledPath = new AtomicLong(-1);

    /**
     * Describes a new node that was received during a reconnect. Key will be null if the node is an
     * internal.
     *
     * @param path the path of the node
     * @param isLeaf true if the node is a leaf node
     * @param key if the node is a leaf node then this is the node's key, otherwise null
     * @param <K> the type of the key
     */
     ...
     *     reconnect
     * @param oldFirstLeafPath the original first leaf path from before the reconnect
    ...
    /**
     * Set the first/last leaf path for the tree as it will be after reconnect is completed.
     * Expected to be called before the first node is passed to this object.
     *
     * @param newFirstLeafPath the first leaf path after reconnect completes
     * @param newLastLeafPath the last leaf path after reconnect completes
     */
     ...
     * formally occupied by a leaf node then this method will ensure that the old leaf node is
     * properly deleted.
     *
     * @param path the path of the node
     */
    ...
    /**
     * Register the receipt of a new leaf node. If the leaf node is in the position that was
     * formally occupied by a leaf node then this method will ensure that the old leaf node is
     * properly deleted. If the leaf node is in the position that was formally occupied by an
     * internal node then this method will ensure all leaves in the subtree are properly deleted.
     *
     * @param path the path to the node
     * @param newKey the key of the new leaf node
     */
     ...
     *
     * @param requiredPath the minimum path that must be processed before the stream is returned
     * @return a stream of keys to be deleted. Only the key and path in these records are populated,
     *     all other data is uninitialized.
     */
    public Stream<VirtualLeafRecord<K, V>> getRecordsToDelete(final long requiredPath) {
        ...
        final Stream<VirtualLeafRecord<K, V>> stream =
                keysToBeRemoved.entrySet().stream()
                        .map(
                                (final Map.Entry<K, Long> entry) ->
     ...
     *
     * @param receivedNode a node that was received during a reconnect
     */
    ...
    private void handleLeaf(final long path, final ReplacedNodeType replacedType, final K key) {
        ...
        // If we think we need to remove a key but find out that the leaf has actually just been
        // moved,
     ...
     *
     * @param path the path ot the node
     */
    private void handleInternal(final long path, final ReplacedNodeType replacedType) {
        if (path < 0 || (path >= newFirstLeafPath && newFirstLeafPath != -1)) {
            throw new IllegalStateException(
                    "Expected internal node path between 0 and "
                            + newFirstLeafPath
                            + ", got "
                            + path);
        }
     ...
     *
     * @param path the path of the leaf being removed
     */
