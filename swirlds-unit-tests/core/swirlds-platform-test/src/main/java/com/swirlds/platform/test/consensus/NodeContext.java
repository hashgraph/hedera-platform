...
public class NodeContext {
    ...
    private static final StateSettings SETTINGS = new StateSettings();
     ...
     *
     * @param random source of randomness for the {@link EventEmitter}
     * @param eventEmitter the emitter of events
     */
    public static NodeContext genesisContext(
            final RandomGenerator random, final EventEmitter<?> eventEmitter) {
        final EventEmitter<?> eventEmitterCopy = eventEmitter.cleanCopy(random.nextLong());
        final SignedStateEventsAndGenerations eventsAndGenerations =
                new SignedStateEventsAndGenerations(SETTINGS);
    ...
    /**
     * Simulates a restart on a node by creating a new ConsensusImpl with events and generations
     * stored in SignedStateEventsAndGenerations
     */
    public void restart(final Path stateDir) throws ConstructableRegistryException, IOException {
        ...
        // We create a copy of the signed state like in a restart or reconnect to closely mimic a
        ...
        // now that we have used the data we need to create a signed state, we can clear events and
        // generations
        ...
        for (final Address address : collectingEmitter.getGraphGenerator().getAddressBook()) {
            final EventSource<?> source =
                    collectingEmitter.getGraphGenerator().getSource((int) address.getId());
            final List<IndexedEvent> eventsByCreator =
                    Arrays.stream(indexedEvents)
    ...
    /**
     * Applies a number of events to this {@link NodeContext}'s consensus and keeps track of which
     * sequence each event was created in and reached consensus in.
     *
     * @param sequenceNum the 1 based sequence number these events are applied in
     * @param numberOfEvents the number of events to generate and apply to consensus
     */
    public void applyEventsToConsensus(final int sequenceNum, final long numberOfEvents) {
        ...
        final List<IndexedEvent> newConsensus =
                intake.getConsensusRounds().stream()
