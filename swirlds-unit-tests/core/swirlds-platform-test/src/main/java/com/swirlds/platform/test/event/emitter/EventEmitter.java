 ...
 *
 * @param <T> the type of class implementing this interface
 */
public interface EventEmitter<T extends EventEmitter<T>> {
     ...
     *
     * @param numberOfEvents The number of events to get.
     */
     ...
     * Set a checkpoint.
     *
     * <p>Two emitters with the same graph generator and with the same checkpoint are guaranteed to
     * emit the same events (in potentially different orders) when the number of events emitted are
     * equal to the checkpoint. For events emitted after the checkpoint has been reached, no special
     * guarantees are provided.
     *
     * <p>It is ok to specify more than one checkpoint during the lifetime of a generator.
     */
    void setCheckpoint(long checkpoint);

    /**
     * Reset this emitter to its original state. Does not undo settings changes, just the events
     * that have been generated and emitted.
     */
    void reset();
    ...
    /**
     * Get an exact copy of this event emitter in its current state. The events emitted by this new
     * emitter will be equivalent to the events returned by this emitter, but these events will be
     * distinct objects that are not inter-connected in any way.
     *
     * <p>Note: if this emitter has emitted a large number of events, this method may be expensive.
     * The copied emitter needs to skip all events already emitted.
     */
    default T copy() {
     ...
     *
     * @param numberToSkip The number of events to skip.
     */
    ...
    /**
     * Creates a clean copy of the underlying {@link GraphGenerator} with the supplied seed, forcing
     * it to create a different graph.
     *
     * @param seed the new seed to use
     */
