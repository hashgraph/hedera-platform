...
public class SimulatedNode {
    ...
    public SimulatedNode(
        ...
        this.chatter =
                builder.getChatterFactory()
                        .build(selfId, nodeIds, this::registerEvent, currentTime::get);
    ...
    /**
     * Simulate one step in the simulation. Send and receive a bunch of messages, and possibly
     * generate an event.
     *
    ...
    private void maybeGenerateEvent(final Instant now) {
        final double averageEventsPerTimeStep =
                averageEventsPerSecond * (timeStep.getNano() * NANOSECONDS_TO_SECONDS);
            ...
            final int size =
                    (int)
                            Math.max(
                                    1,
                                    random.nextGaussian(
                                            averageEventSize, eventSizeStandardDeviation));
            final SimulatedEvent event =
                    new SimulatedEvent(random, selfId, roundProvider.get(), size);
    ...
    private void sendMessages(final Instant now) {
        ...
        while (network.isOutgoingCapacityAvailable(selfId) && !destinations.isEmpty()) {
            ...
            final SimulatedMessage message =
                    new SimulatedMessage(selfId, payload.destination(), payload.payload());
            if (debugEnabled) {
                System.out.println(
                        now.toEpochMilli()
                                + ": node "
