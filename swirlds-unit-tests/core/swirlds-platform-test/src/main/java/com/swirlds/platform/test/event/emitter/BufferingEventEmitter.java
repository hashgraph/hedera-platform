...
/**
 * A base event emitter class that buffers events created by the {@link GraphGenerator}. Buffering
 * events allows subclasses to emit events in an order different from the order they are generated.
 *
 * @param <T> the type of class extending this class
 */
public abstract class BufferingEventEmitter<T extends BufferingEventEmitter<T>>
    ...
    /**
     * A list of queues, each containing buffered events from an event source. There is one queue
     * per source. The queue at index 0 corresponds to the source with node ID 0, and so on. Events
     * are strongly ordered within an individual queue.
     */
    ...
    /**
     * Generates 0 or more events that are internally buffered. Events will be generated until there
     * is at least one buffered event from the given node ID or until the buffer fills up.
     */
     ...
     *
     * <ul>
     *   <li>Events can not be emitted if all of their ancestors have not yet been emitted.
     *   <li>Events can not be emitted if their generator index is not less than the current active
     *       checkpoint.
     * </ul>
     */
