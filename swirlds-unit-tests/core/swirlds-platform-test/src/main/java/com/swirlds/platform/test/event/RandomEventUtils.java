...
public class RandomEventUtils {
    ...
    public static final Instant DEFAULT_FIRST_EVENT_TIME_CREATED =
            Instant.ofEpochMilli(1588771316678L);
    ...
    public static <T extends EventImpl> T randomEvent(
        ...
        return randomEvent(
                random, creatorId, selfParent, otherParent, DEFAULT_FAKE_HASH, DEFAULT_REAL_EVENT);
    }
    ...
    public static <T extends EventImpl> T randomEvent(
        ...
        return randomEvent(
                ...
                TransactionUtils.randomSwirldTransactions(
                        random,
                        DEFAULT_MIN_TRANSACTION_NUMBER
                                + random.nextInt(DEFAULT_MAX_TRANSACTION_NUMBER)),
                selfParent,
                otherParent,
                fakeHash,
                realEvent);
    ...
    public static <T extends EventImpl> T randomEvent(
        ...
        final BaseEventUnhashedData unhashedData =
                new BaseEventUnhashedData(
                        otherParent != null ? otherParent.getCreatorId() : -1, sig);

        return constructor.construct(hashedData, unhashedData, selfParent, otherParent);
    }

    /**
     * Similar to randomEvent, but the timestamp used for the event's creation timestamp is provided
     * by an argument.
     */
    public static IndexedEvent randomEventWithTimestamp(
        ...
        final BaseEventHashedData hashedData =
                randomEventHashedDataWithTimestamp(
                        random,
                        creatorId,
                        timestamp,
                        transactions,
                ...
                new BaseEventHashedData(
                        ...
                        selfParent != null
                                ? selfParent
                                        .getTimeCreated()
                                        .plusMillis(
                                                1 + random.nextInt(DEFAULT_MAX_NEXT_EVENT_MILLIS))
                                : firstTimeCreated,
                        transactions);
    ...
    /**
     * Similar to randomEventHashedData but where the timestamp provided to this method is the
     * timestamp used as the creation timestamp for the event.
     */
