...
public class GossipSimulation {
    ...
    public void simulate(final Duration time) {
        ...
        final double realtimeSeconds =
                Duration.between(start, finish).toNanos() * NANOSECONDS_TO_SECONDS;
        final double temporalCompression = secondsSimulated / realtimeSeconds;
        System.out.println(
                "Finished simulating "
                        + commaSeparatedValue(time.toSeconds())
                        + " seconds, temporal compression = "
    ...
    private void reportTime() {
        if (timeReportPeriod != null) {
            ...
            if (timeSinceLastReport.compareTo(timeReportPeriod) > 0) {
                ...
                System.out.println(
                        "Simulation time: "
                                + roundDecimal(simulationSeconds, 2)
                                + " seconds, tc = "
                                + roundDecimal(temporalCompression, 2));
