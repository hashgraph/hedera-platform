...
public class SimulatedMessage {
    ...
    public SimulatedMessage(
            final long source, final long destination, final SelfSerializable payload) {
        this.source = source;
    ...
    /**
     * In order to make sure there is no funny business where nodes pass objects directly between
     * each other, copy the payload via serialization. Measure the size of a message at the same
     * time.
     *
     * @param payload the payload to copy
     * @return a pair with the size and the copy
     */
    private static Pair<Integer /* size */, SelfSerializable> copyBySerialization(
            final SelfSerializable payload) {
