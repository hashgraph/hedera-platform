...
public class ConsensusWithShadowGraph extends ConsensusImpl {
    ...
    public ConsensusWithShadowGraph(
            final AddressBook addressBook, final BiConsumer<Long, Long> minGenConsumer) {
        super(ConsensusUtils.NOOP_CONSENSUS_METRICS, minGenConsumer, addressBook);
    ...
    public ConsensusWithShadowGraph(final AddressBook addressBook, final SignedState signedState) {
        super(ConsensusUtils.NOOP_CONSENSUS_METRICS, NOOP_MINGEN, addressBook, signedState);
        shadowGraph = new ShadowGraph(mock(SyncMetrics.class));
        // consensus will not return old events, so these are the ones we need to add to shadowgraph
        shadowGraph.initFromEvents(
                EventUtils.prepareForShadowGraph(signedState.getEvents()), getMinRoundGeneration());
    }
    ...
    public List<EventImpl> addEvent(EventImpl event, AddressBook addressBook) {
        try {
            ...
            fail(
                    String.format(
                            "Failed to insert event %s into the shadow graph",
                            event.toShortString()),
                    e);
        }
