...
public class EventTracker {
    ...
    /**
     * Records the number of times that an event has been distributed to N nodes. For example,
     * distributionCounts.get(3) returns the number of events that have been distributed to at least
     * 3 nodes.
     */
    private final Map<Integer, Long> distributionCounts = new HashMap<>();

    /**
     * Records the number of times that an event has failed to be distributed to N nodes. For
     * example, distributionFailureCounts.get(3) returns the number of events that were distributed
     * to fewer than 3 nodes.
     */
    private final Map<Integer, Long> distributionFailureCounts = new HashMap<>();

    /**
     * Records the total time required to deliver events to N nodes. Times are summed, when the
     * simulation is complete the average can be computed.
     */
    ...
    public void registerNewEvent(
            final ChatterEventDescriptor descriptor, final Instant creationTime) {
        final TrackedEvent prev =
                events.put(descriptor, new TrackedEvent(descriptor.getCreator(), creationTime));
    ...
    public void registerEvent(
            final ChatterEventDescriptor descriptor, final long nodeId, final Instant receiveTime) {
        ...
        if (trackedEvent == null) {
            ...
            if (debugEnabled) {
                System.out.println(
                        "event " + descriptor.getHash() + " registered after purge threshold");
            }
            return;
        }
     ...
     *
     * @param olderThanRound all events associated with rounds strictly older than this will be
     *     erased
     */
    ...
    private void captureStatistics(
            final ChatterEventDescriptor descriptor, final TrackedEvent event) {
        ...
        for (int i = 0; i < nodeCount; i++) {
            ...
            if (times.size() > i) {
                ...
                final long originalCount =
                        distributionCounts.getOrDefault(nodesThatReceivedEvent, 0L);
                distributionCounts.put(nodesThatReceivedEvent, originalCount + 1);

                final Duration time = times.get(i);

                final Duration originalTime =
                        distributionSums.getOrDefault(nodesThatReceivedEvent, Duration.ZERO);
                distributionSums.put(nodesThatReceivedEvent, originalTime.plus(time));
            } else {
                // the event was not distributed to this number of nodes
                final long originalCount =
                        distributionFailureCounts.getOrDefault(nodesThatReceivedEvent, 0L);
    ...
    private void printNumberOfEventsDelivered() {
        final TextTable table =
                new TextTable(
                        "NUMBER OF EVENTS DELIVERED TO AT LEAST N NODES",
                        "Number Of Nodes",
                        "Event Count",
                        "Fraction",
                        "Threshold",
        ...
        for (int i = 2; i <= nodeCount; i++) {
            ...
            final long totalCount =
                    distributionCounts.getOrDefault(i, 0L)
                            + distributionFailureCounts.getOrDefault(i, 0L);
            final long count = distributionCounts.getOrDefault(i, 0L);
            final double fraction = ((double) count) / totalCount * 100;

            table.addRow(
                    i,
                    commaSeparatedValue(count),
    ...
    private void printNumberOfEventsNotDelivered() {
        final TextTable table =
                new TextTable(
                        "NUMBER OF EVENTS DELIVERED TO FEWER THAN N NODES",
                        "Number Of Nodes",
                        "Event Count",
                        "Fraction",
                        "Threshold",
    ...
    private void printAverageDeliveryTime() {
        final TextTable table =
                new TextTable(
                        "AVERAGE TIME TO DELIVER AN EVENT TO N NODES",
                        "Number Of Nodes",
                        "Time",
                        "Threshold",
                        "Significance");
        ...
        for (int i = 2; i <= nodeCount; i++) {
            ...
            table.addRow(
                    i,
                    commaSeparatedValue(averageTime.toMillis()) + " ms",
                    threshold,
                    significance);
        }
