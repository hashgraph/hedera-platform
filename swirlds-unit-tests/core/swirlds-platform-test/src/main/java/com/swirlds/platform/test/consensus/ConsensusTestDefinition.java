...
/**
 * A type that has the necessary information for executing a single consensus test based on a seed
...
public class ConsensusTestDefinition {
    ...
    /**
     * A function that creates a graph generator with an address book with the provided node stake
     * values
     */
    ...
    /**
     * The node stakes generated for each node using the current seed. {@code null} until {@link
     * #setSeed(long)} is called.
     */
    private List<Long> nodeStakes;

    /**
     * The event emitters used to emit events. They wrap a graph generator that creates a graph
     * using sources with the calculated node stakes. {@code null} until {@link #setSeed(long)} is
     * called. There are two, one for each consensus instance.
     */
    ...
    /**
     * The test sequences generated using the current seed. {@code null} until {@link
     * #setSeed(long)} is called.
     */
     ...
     * @param stakeGenerator used to generate a stake value for each node in the network
     * @param eventsPerPhase the number of events per phase/test sequence
     */
    public ConsensusTestDefinition(
        ...
        // There is overlap between this code and EventSourceFactory and EventGeneratorFactory. It
        // should be
        // consolidated at some point.
        graphGeneratorProvider =
                stakes -> {
                    final List<EventSource<?>> eventSources = newStandardEventSources(stakes);
                    // Maybe abstract this so that this code block is reusable by all consensus
                    // tests
    ...
    /**
     * Creates and returns a new set of {@link TestSequence} objects using the provided {@code
     * seed}. {@link #setTestSequenceGenerator(Function)} must be called prior to calling this
     * method.
     */
    ...
    /**
     * Sets the function that provides the {@link EventEmitter} for the first instance of consensus
     * in this test.
     */
    ...
    /**
     * Sets the function that provides the {@link EventEmitter} for the second instance of consensus
     * in this test.
     */
    ...
    /**
     * Generates new values for {@link #nodeStakes}, {@link #node1EventEmitter}, {@link
     * #node2EventEmitter}, and {@link #testSequences} using the provided {@code seed}.
     *
    ...
    public void setSeed(final long seed) {
        ...
        final GraphGenerator<?> graphGenerator =
                graphGeneratorProvider.getGraphGenerator(nodeStakes);
        node1EventEmitter =
                node1EventEmitterGenerator.getEventEmitter(graphGenerator.cleanCopy(), seed);
        node2EventEmitter =
                node2EventEmitterGenerator.getEventEmitter(graphGenerator.cleanCopy(), seed);
