...
public class SimulatedNetwork {
    ...
    /**
     * Messages that need to be sent but have not yet been put "on the wire". These messages will be
     * sent in a later time step.
     */
    ...
    /**
     * Maximum number of messages in incoming buffer, more than this number will cause connection to
     * throttle.
     */
    ...
    public SimulatedNetwork(final GossipSimulationBuilder builder) {
        ...
        for (long source = 0; source < nodeCount; source++) {
            outgoingBandwidth.put(
                    source,
                    (long)
                            (builder.getOutgoingBandwidth(source)
                                    * timeStep.toNanos()
                                    * NANOSECONDS_TO_SECONDS));
            incomingBandwidth.put(
                    source,
                    (long)
                            (builder.getIncomingBandwidth(source)
                                    * timeStep.toNanos()
                                    * NANOSECONDS_TO_SECONDS));
            for (long destination = 0; destination < nodeCount; destination++) {
                if (source != destination) {
                    final SourceDestinationPair pair =
                            new SourceDestinationPair(source, destination);
        ...
        statistics =
                new SimulatedNetworkStatistics(
                        nodeCount, timeStep, incomingBandwidth, outgoingBandwidth);
    ...
    /**
     * Check if there is outgoing capacity that is available for a particular node in this time
     * step.
     *
     ...
     * @return true if there is a connection that allows messages to flow from the source to the
     *     destination
     */
    ...
    /**
     * Check if a destination is currently available for messages to be sent to in the current time
     * step.
     *
    ...
    public void send(final SimulatedMessage message) {
        ...
        if (remainingBandwidth >= message.getBytesToSend()) {
            ...
            messagesInTransit.get(new SourceDestinationPair(source, destination)).add(message);
        } else {
            ...
            for (long destination = 0; destination < nodeCount; destination++) {
                ...
                while (!messageQueue.isEmpty()) {
                    ...
                    final Duration elapsedTime =
                            Duration.between(message.getTransmissionTime(), now);
    ...
    /**
     * Called when one time step has finished and the next is starting. Attempt to send any messages
     * in the queue that were not able to be sent in the previous step.
     */
            ...
            for (long destination = 0; destination < nodeCount; destination++) {
                ...
                while (!messageQueue.isEmpty()) {
                    ...
                    if (remainingBandwidth >= message.getBytesToSend()) {
                        // The entire message can be sent this round
                        message.setTransmissionTime(now);
                        outgoingBandwidthUsed.put(
                                source, utilizedBandwidth + message.getBytesToSend());
                        message.setBytesToSend(0); // all bytes have been sent
