...
import static com.swirlds.platform.test.chatter.simulator.GossipSimulationUtils.commaSeparatedValue;
import static com.swirlds.platform.test.chatter.simulator.GossipSimulationUtils.formatByteQuantity;
...
public class SimulatedNetworkStatistics {
    ...
    private final Map<Long /* node ID */, Long /* num messages */> messagesSentByEachNode =
            new HashMap<>();
    ...
    private final Map<Long /* node ID */, Long /* num messages */> messagesReceivedByEachNode =
            new HashMap<>();

    /** Records the number of bytes received by each node. Does not include dropped messages. */
    private final Map<Long /* node ID */, Long /* bytes */> bytesReceivedByEachNode =
            new HashMap<>();

    /** Records the number of messages sent, sorted by type of message. */
    private final Map<Class<?> /* message type */, Long /* num messages */> messagesSentByType =
            new HashMap<>();

    /** Records the number of bytes sent, sorted by type of message. */
    private final Map<Class<?> /* message type */, Long /* bytes */> bytesSentByType =
            new HashMap<>();

    /** The number of messages received and not dropped, sorted by message type. */
    private final Map<Class<?> /* message type */, Long /* num messages */> messagesReceivedByType =
            new HashMap<>();

    /** Number of bytes received and not dropped, sorted by message type. */
    private final Map<Class<?> /* message type */, Long /* bytes */> bytesReceivedByType =
            new HashMap<>();

    /** Number of messages dropped, sorted by message type. */
    private final Map<Class<?> /* message type */, Long /* num messages */> messagesDroppedByType =
            new HashMap<>();

    /** Number of bytes dropped, sorted by message type. */
    private final Map<Class<?> /* message type */, Long /* bytes */> bytesDroppedByType =
            new HashMap<>();

    /** Sum of all available incoming bandwidth over the course of the simulation. */
    private final Map<Long /* node ID */, Long /* bytes */> incomingBandwidthCapacity =
            new HashMap<>();

    /** Sum of all incoming bandwidth used over the course of the simulation. */
    private final Map<Long /* node ID */, Long /* bytes */> outgoingBandwidthCapacity =
            new HashMap<>();
    ...
    public void simulateOneStep() {
        ...
        for (long nodeId = 0; nodeId < nodeCount; nodeId++) {
            final long previousIncomingBandwidthCapacity =
                    incomingBandwidthCapacity.getOrDefault(nodeId, 0L);
            incomingBandwidthCapacity.put(
                    nodeId, previousIncomingBandwidthCapacity + incomingBandwidth.get(nodeId));

            final long previousOutgoingBandwidthCapacity =
                    outgoingBandwidthCapacity.getOrDefault(nodeId, 0L);
            outgoingBandwidthCapacity.put(
                    nodeId, previousOutgoingBandwidthCapacity + outgoingBandwidth.get(nodeId));
        }
    }
    ...
    private void printNetworkSummary() {
        final TextTable table =
                new TextTable("NETWORK SUMMARY", "", "", "Utilization", "Fraction", "Rate");
        ...
        table.addRow(
                "Total messages sent",
                commaSeparatedValue(totalMessagesSent) + " messages",
                "",
                "",
                commaSeparatedValue(messagesSentPerSecond) + " messages/second");
        ...
        table.addRow(
                "Total bytes sent",
                formatByteQuantity(totalBytesSent),
                roundDecimal(sentUtilization, 2) + " %",
                "",
                formatByteQuantity(bytesSentPerSecond) + "/second");
        ...
        final int messagesReceivedPerSecond =
                (int) (totalMessagesReceived / totalSimulationSeconds);
        table.addRow(
                "Total messages received",
                commaSeparatedValue(totalMessagesReceived) + " messages",
                "",
                roundDecimal(percentReceived, 2) + " % of messages sent",
                commaSeparatedValue(messagesReceivedPerSecond) + " messages/second");
        ...
        final double receivedUtilization =
                ((double) totalBytesReceived) / totalReceiveCapacity * 100;
        table.addRow(
                "Total bytes received",
                formatByteQuantity(totalBytesReceived),
                roundDecimal(receivedUtilization, 2) + " %",
                roundDecimal(percentBytesReceived, 2) + " % of bytes sent",
                formatByteQuantity(bytesReceivedPerSecond) + "/second");
    ...
    private void printMessagesSentByEachNode() {
        ...
        for (long nodeId = 0; nodeId < nodeCount; nodeId++) {
            ...
            table.addRow(
                    nodeId,
                    commaSeparatedValue(messagesSentByNode) + " messages",
                    roundDecimal(fractionOfWhole, 2) + " %",
                    commaSeparatedValue(messagesPerSecond) + " messages/second");
        }
    ...
    private void printBytesSentByEachNode() {
        ...
        for (long nodeId = 0; nodeId < nodeCount; nodeId++) {
            ...
            final double utilization =
                    ((double) bytesSent) / outgoingBandwidthCapacity.get(nodeId) * 100;
            final long bytesPerSecond = (long) (bytesSent / totalSimulationSeconds);

            table.addRow(
                    nodeId,
                    formatByteQuantity(bytesSent),
                    roundDecimal(fractionOfWhole, 2) + " %",
                    roundDecimal(utilization, 2) + " %",
                    formatByteQuantity(bytesPerSecond) + "/s");
        }
    ...
    private void printMessagesSentByType() {
        ...
        for (final Class<?> type : messagesSentByType.keySet()) {
            ...
            table.addRow(
                    type.getSimpleName(),
                    commaSeparatedValue(messagesSent),
                    roundDecimal(fraction, 2) + " %",
                    commaSeparatedValue(rate) + " messages/second");
        }
    ...
    private void printBytesSentByType() {
        ...
        for (final Class<?> type : bytesSentByType.keySet()) {
            ...
            table.addRow(
                    ...
                    formatByteQuantity(bytesSent),
                    roundDecimal(fraction, 2) + " %",
                    formatByteQuantity(rate) + "/second");
        }
        ...
        for (long nodeId = 0; nodeId < nodeCount; nodeId++) {
            ...
            final double fraction =
                    totalMessagesReceived == 0
                            ? 0
                            : ((double) messagesReceived) / totalMessagesReceived * 100;
            final long rate = (long) (messagesReceived / totalSimulationSeconds);

            table.addRow(
                    nodeId,
                    commaSeparatedValue(messagesReceived) + " messages",
                    roundDecimal(fraction, 2) + " %",
                    commaSeparatedValue(rate) + " messages/second");
        }
        ...
        for (long nodeId = 0; nodeId < nodeCount; nodeId++) {
            ...
            final double fraction =
                    totalBytesReceived == 0
                            ? 0
                            : ((double) bytesReceived) / totalBytesReceived * 100;
            final double utilization =
                    ((double) bytesReceived) / incomingBandwidthCapacity.get(nodeId) * 100;
            final long rate = (long) (bytesReceived / totalSimulationSeconds);

            table.addRow(
                    nodeId,
                    formatByteQuantity(bytesReceived),
                    roundDecimal(fraction, 2) + " %",
                    roundDecimal(utilization, 2) + " %",
                    formatByteQuantity(rate) + "/second");
        }
