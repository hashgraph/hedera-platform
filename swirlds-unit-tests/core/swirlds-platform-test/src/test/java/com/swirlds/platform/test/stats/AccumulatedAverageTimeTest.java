...
class AccumulatedAverageTimeTest {
    ...
    void test() {
        ...
        final AccumulatedAverageTime metric =
                new AccumulatedAverageTime(metrics, "category", "name", "description");
        Assertions.assertEquals(0d, metric.get(), "if no data is added, value should be 0");
        metric.add(Duration.ofMillis(10).toNanos());
        Assertions.assertEquals(
                10d, metric.get(), "one value was added, so that should be the average");
