...
class VariableTimeDelayTest {
    ...
    void testVariableDelay() {
        ...
        assertEquals(
                SendAction.WAIT, eventTimeDelay.shouldSend(event), "Event should not be sent yet");

        now.set(event.getTimeReceived().plus(Duration.ofMillis(90)));
        assertEquals(
                SendAction.WAIT,
                eventTimeDelay.shouldSend(event),
                "Event should still not be sent yet");
        ...
        assertEquals(
                SendAction.SEND, eventTimeDelay.shouldSend(event), "Event should be sent yet now");
    }
}
