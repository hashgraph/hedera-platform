...
public class NetworkTestChatter implements SimulatedChatter {
    ...
    public NetworkTestChatter(
            final long selfId, final int bytesPerSecond, final int generateEveryMs) {
        this.selfId = selfId;
    ...
    public GossipPayload generatePayload(
        ...
        if (lastPayload == null
                || Duration.between(lastPayload, now).toMillis() >= generateEveryMs) {
            ...
            return new GossipPayload(
                    new SimulatedEvent(new Random(), selfId, destination, payloadSizeInBytes),
                    destination);
        }
        return null;
    }
