...
public class OtherParentMatrixFactory {
    ...
    /**
     * Creates a balanced, fully connected other parent affinity matrix so that each node is equally
     * weighted against the others to be selected as the other parent for an event unless it is its
     * own event.
     *
    ...
    public static List<List<Double>> createBalancedOtherParentMatrix(final int numNodes) {
        return createOtherParentMatrix(
                numNodes,
                (creator, otherParent) -> {
    ...
    /**
     * Creates an other parent affinity matrix that forces the other parent of an event to be {@code
     * nextOtherParent} unless the creator is the {@code nextOtherParent}.
     *
     * @param numNodes
     * @param nextOtherParent
     * @return
     */
    public static List<List<Double>> createForcedOtherParentMatrix(
            final int numNodes, final int nextOtherParent) {
        return createOtherParentMatrix(
                numNodes,
                (creator, otherParent) -> {
                    if (creator == otherParent) {
    ...
    /**
     * Creates an other parent affinity matrix that creates cliques of nodes. Each clique syncs
     * within itself frequently but with outsiders rarely.
     *
     * @param numNodes the number of nodes in the entire network
     * @param cliques a map from node id to clique id
     * @return
     */
    ...
    /**
     * Creates an other parent affinity matrix that forces a partition. One partition will contain
     * the {@code nodeIDsInThisPartition}, and the other partition will contain all the other nodes.
     * Each creator may only select an other parent that is in it's own partition.
     *
     * @param numNodes
        ...
        return createOtherParentMatrix(
                ...
                (creator, otherParent) -> {
                    if (creator == otherParent) {
                        ...
                        if ((nodeIDsInThisPartition.contains(creator)
    ...
    /**
     * Creates an other parent affinity matrix that shuns a particular node as an other parent.
     * Events will never select the {@code shunnedNode} as an other parent.
     *
     * @param numNodes
     * @param shunnedNode
     * @return
     ...
     *
     * @param numSources the number of sources in the graph
     * @param function the function that determines the weight of a particular other parent being
     *     selected for a creator's new event
     * @return
     */
