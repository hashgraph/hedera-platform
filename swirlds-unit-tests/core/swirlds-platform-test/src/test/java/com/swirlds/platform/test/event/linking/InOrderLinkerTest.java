...
class InOrderLinkerTest {
    ...
    void testMissingParents(
            final int selfParentGen, final int otherParentGen, final boolean expectedValidity) {
        final GossipEvent event = mockEvent(selfParentGen, otherParentGen);
        final EventLinker linker = new InOrderLinker(new ParentFinder(h -> null), l -> null);
