...
/**
 * A class which generates a TSV text grid with the results given. A header and a footer are also
 * included.
 *
 * @param <X> the type of the index for the x-coordinate
 * @param <Y> the type of the index for the y-coordinate
 */
public class GridGraph<X extends Comparable<X>, Y extends Comparable<Y>>
     ...
     * @param yAxisLabel the y-axis label
     * @param valueLabel the value label
     * @param formatSpec the string format specifier for value entries
     */
    protected GridGraph(
            final String graphName,
            final String xAxisLabel,
     ...
     * @return true iff the value at the given coordinates was inserted (i.e., was not already
     *     present)
     */
    ...
    public void render(final PrintWriter writer) {
        ...
        final List<Y> yValues =
                matrix.keySet().stream().distinct().sorted().collect(Collectors.toList());
        ...
        for (final Y colHeader : yValues) {
            final Map<X, GridGraphResult<X, Y>> row =
                    matrix.getOrDefault(colHeader, new HashMap<>());
    ...
    public boolean equals(final Object o) {
        ...
        return new EqualsBuilder().append(getGraphName(), gridGraph.getGraphName()).isEquals();
    }
    ...
    public int compareTo(final GridGraph<X, Y> o) {
        ...
        return new CompareToBuilder().append(this.getGraphName(), o.getGraphName()).build();
    }
