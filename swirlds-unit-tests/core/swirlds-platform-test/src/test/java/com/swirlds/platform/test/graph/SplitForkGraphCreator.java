...
/**
 * This class manipulates the event generator to force the creation of a split fork, where each node
 * has one branch of a fork. Neither node knows that there is a fork until they sync.
 *
 * <p>Graphs will have {@link SyncTestParams#getNumCommonEvents()} events that do not fork, then the
 * split fork occurs. Events generated after the {@link SyncTestParams#getNumCommonEvents()} will
 * not select the creator with the split fork as an other parent to prevent more split forks from
 * occurring. The creator with the split fork may select any other creator's event as an other
 * parent.
 */
public class SplitForkGraphCreator {
    ...
    private static void forceNextOtherParent(
        ...
        final List<List<Double>> balancedMatrix =
                createBalancedOtherParentMatrix(params.getNumNetworkNodes());

        final List<List<Double>> forcedOtherParentMatrix =
                createForcedOtherParentMatrix(numSources, nextOtherParent);
                ...
                .setOtherParentAffinity(
                        (Random r, long eventIndex, List<List<Double>> previousValue) -> {
                            if (eventIndex < params.getNumCommonEvents()) {
                                ...
                                // After the fork, shun the creator that forked so that other
                                // creators dont use it and
                                // therefore create
