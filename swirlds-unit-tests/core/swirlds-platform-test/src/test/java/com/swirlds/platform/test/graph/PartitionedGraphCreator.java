...
/**
 * This class manipulates the event generator to force a partition in the graphs. Nodes in partition
 * A only create events with other parents that are also in partition A. Nodes in partition B only
 * create events with other parents that are also in partition B.
 *
 * <p>Graphs will have {@link SyncTestParams#getNumCommonEvents()} events that are in a fully
 * connected graph, then the partition occurs. Events generated after the {@link
 * SyncTestParams#getNumCommonEvents()} will conform to the defined partition parameters.
 */
public class PartitionedGraphCreator {
    ...
    public static void setupPartitionForNode(
        ...
        final List<List<Double>> fullyConnectedMatrix =
                createBalancedOtherParentMatrix(params.getNumNetworkNodes());

        final List<List<Double>> partitionedOtherMatrix =
                createPartitionedOtherParentAffinityMatrix(
                        params.getNumNetworkNodes(), nodesInPartition);

        // Setup node sources to select creators and other parents in it's own partition after the
        // common
        // events are generated. Nodes not in this partition must not create any events so that the
        // caller and listener
