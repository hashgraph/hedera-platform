...
class SystemTransactionHandlerTest {
    ...
    void testHandleExceptions() {
        ...
        final SystemTransactionHandler handler = new SystemTransactionHandlerImpl(dispatchBuilder);

        dispatchBuilder.registerObserver(
                PreConsensusStateSignatureTrigger.class,
                (PreConsensusStateSignatureTrigger)
                        (round, signerId, hash, signature) -> {
                            throw new IllegalStateException("this is intentionally thrown");
                        });

        dispatchBuilder.registerObserver(
                PostConsensusStateSignatureTrigger.class,
                (PostConsensusStateSignatureTrigger)
                        (round, signerId, hash, signature) -> {
                            throw new IllegalStateException("this is intentionally thrown");
                        });
        ...
        final ConsensusRound round =
                new ConsensusRound(
                        List.of(eventWithThisNodeSig, eventWithOtherNodeSig),
    ...
    void testHandleConsensus() {
        ...
        dispatchBuilder.registerObserver(
                PreConsensusStateSignatureTrigger.class,
                (PreConsensusStateSignatureTrigger)
                        (round, signerId, hash, signature) -> preConsensusCount.getAndIncrement());

        final AtomicInteger postConsensusCount = new AtomicInteger(0);
        dispatchBuilder.registerObserver(
                PostConsensusStateSignatureTrigger.class,
                (PostConsensusStateSignatureTrigger)
                        (round, signerId, hash, signature) -> postConsensusCount.getAndIncrement());
    ...
    private EventImpl createSigEvent(
        ...
        final ConsensusTransactionImpl nodeSig =
                createSigTransaction(SYS_TRANS_STATE_SIG, round, signature, hash);
    ...
    private static EventImpl newEvent(
            final long creatorId, final ConsensusTransactionImpl[] transactions) {
