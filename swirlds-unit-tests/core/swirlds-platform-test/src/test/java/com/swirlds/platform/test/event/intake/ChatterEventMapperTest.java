...
class ChatterEventMapperTest {
    ...
    void test() {
        ...
        final GossipEvent cr1gen1 =
                GossipEventBuilder.builder().setCreatorId(creator1).setGeneration(1).buildEvent();
        mapper.mapEvent(cr1gen1);
        Assertions.assertEquals(
                cr1gen1,
                mapper.getMostRecentEvent(creator1),
                "the event just added should be the most recent");
        Assertions.assertNull(
                mapper.getMostRecentEvent(creator2), "creator1 should not affect creator2");

        final GossipEvent cr1gen10 =
                GossipEventBuilder.builder().setCreatorId(creator1).setGeneration(10).buildEvent();
        ...
        final GossipEvent cr1gen5 =
                GossipEventBuilder.builder().setCreatorId(creator1).setGeneration(5).buildEvent();
        mapper.mapEvent(cr1gen5);
        Assertions.assertEquals(
                cr1gen10,
                mapper.getMostRecentEvent(creator1),
                "the event just added should NOT be the most recent");
        Assertions.assertNull(
                mapper.getMostRecentEvent(creator2), "creator1 should not affect creator2");

        mapper.clear();
        Assertions.assertNull(
                mapper.getMostRecentEvent(creator1), "after a clear, there should be no events");
        Assertions.assertNull(
                mapper.getMostRecentEvent(creator2), "after a clear, there should be no events");
    }
}
