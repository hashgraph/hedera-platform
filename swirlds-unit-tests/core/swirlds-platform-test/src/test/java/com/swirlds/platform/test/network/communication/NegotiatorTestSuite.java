...
public class NegotiatorTestSuite {
    ...
    private final ReadWriteFakeConnection connection =
            new ReadWriteFakeConnection(input, outputCheck);
    private final Negotiator negotiator =
            new Negotiator(
                    new NegotiationProtocols(List.of(protocol1, protocol2, protocol3)),
                    connection,
                    SLEEP_MS);

    /**
     * @param bytes the input to provide to the negotiator
     */
    ...
    /**
     * @param bytes the expected output coming out of the negotiator
     */
     ...
     * @param expectedRuns the number of runs expected
     */
    public void assertProtocolRan(final int protocol, final int expectedRuns) {
        Assertions.assertEquals(
                ...
                String.format(
                        "the protocol with the Id %d was expected to run %d times",
                        protocol, expectedRuns));
    }

    /**
     * Assert that each of the protocols ran the specified number of times
     *
     * @param protocol1runs expected runs for protocol 1
     * @param protocol2runs expected runs for protocol 2
     * @param protocol3runs expected runs for protocol 3
     */
    public void assertProtocolRuns(
            final int protocol1runs, final int protocol2runs, final int protocol3runs) {
