...
class HeartbeatTest {
    ...
    private final HeartbeatSendReceive heartbeat =
            new HeartbeatSendReceive(
                    peerId, pingChecker::checkPing, heartbeatInterval, clock::nanoTime);
    ...
    void sendHeartbeat() {
        ...
        Assertions.assertNull(
                heartbeat.getMessage(), "no messages expected until the interval runs out");
        ...
        clock.advanceClock(responseTime);
        ...
        Assertions.assertEquals(
                responseTime,
                Duration.ofNanos(heartbeat.getLastRoundTripNanos()),
                "round trip time is not correct");
    }
    ...
    void getLastRoundTripNanosBeforeResponseReceived() {
        ...
        // We have sent the first heartbeat request, but not gotten a response yet, so this should
        // return null
        ...
        // We have sent the first heartbeat request and a response, so there should be a round trip
        // time available
        Assertions.assertNotNull(heartbeat.getLastRoundTripNanos(), "round trip time expected");
        ...
        clock.advanceClock(responseWaitTime);

        // We have sent a second heartbeat, not gotten a response yet, and we've waited for a
        // response longer than the
        // duration of the last ping time, so the round trip time should be the time since the
        // second heartbeat was sent
        Assertions.assertNotNull(heartbeat.getLastRoundTripNanos(), "round trip time expected");
        Assertions.assertEquals(
    ...
    /**
     * Sends a heartbeat after 1 second, does not receive a response. After the first heartbeat
     * times out, another request is sent
     */
    @Test
    void heartbeatTimeout() {
        ...
        clock.advanceClock(HeartbeatSender.HEARTBEAT_TIMEOUT.minusMillis(1));
        Assertions.assertNull(
                heartbeat.getMessage(),
                "until the timeout runs out, we should not send any more heartbeats");
        clock.advanceClock(Duration.ofSeconds(1));
        final HeartbeatMessage request = (HeartbeatMessage) heartbeat.getMessage();
