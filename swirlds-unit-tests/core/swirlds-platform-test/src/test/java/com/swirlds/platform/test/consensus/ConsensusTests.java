...
class ConsensusTests {
     ...
     * in different topological orders, and check if they match. 10 iterations takes about 13
     * minutes.
     */
    ...
    void reconnectSimulation(final ConsensusTestParams params) {
        ConsensusTestDefinitions.reconnectSimulation(
                testDirectory,
                params.numNodes(),
                params.stakeGenerator(),
                NUM_ITER,
                params.seeds());
    }

    @MethodSource(
            "com.swirlds.platform.test.consensus.ConsensusTestArgs#reconnectSimulationWithShadowGraph")
    @Tag(TestTypeTags.FUNCTIONAL)
    @Tag(TestComponentTags.PLATFORM)
    @Tag(TestComponentTags.CONSENSUS)
    ...
    void staleEvent(final ConsensusTestParams params) {
        ConsensusTestDefinitions.staleEvent(
    ...
    void forkingTests(final ConsensusTestParams params) {
        ConsensusTestDefinitions.forkingTests(
                params.numNodes(), params.stakeGenerator(), NUM_ITER, params.seeds());
    }
    ...
    void forkingTestsOldVersion(final int numNodes) {
        // This seed used to cause a bug in the consensus algorithm. It was supposedly fixed by
        // branch
        // 02617-D-reconnect-connect-recalculation but it still fails. Ticket
        // swirlds/swirlds-platform/issues/4995
    ...
    void partitionTests(final ConsensusTestParams params) {
        ConsensusTestDefinitions.partitionTests(
                params.numNodes(), params.stakeGenerator(), NUM_ITER, params.seeds());
    }
    ...
    void cliqueTests(final ConsensusTestParams params) {
        ConsensusTestDefinitions.cliqueTests(
                params.numNodes(), params.stakeGenerator(), NUM_ITER, params.seeds());
    }
