...
public class EventFlowTestUtils {
    ...
    /**
     * Creates a {@link StandardEventEmitter} with custom event sources that generate transactions
     * with incrementing long values such that no transaction is the same as another across all
     * sources.
     *
    ...
    public static StandardEventEmitter createEventEmitter(
        ...
        // Create standard event sources that generate events with incrementing transactions instead
        // of random
        // transactions
        final Supplier<EventSource<?>> eventSourceSupplier =
                () ->
                        new StandardEventSource(
                                false,
                                (r) ->
                                        TransactionUtils.incrementingMixedTransactions(
                                                r,
                                                TX_PER_EVENT_AVG,
                                                TX_PER_EVENT_STD_DEV,
                                                systemTransactionRatio));

        final EventEmitterFactory eventGeneratorFactory = new EventEmitterFactory(random, numNodes);
        eventGeneratorFactory
                .getSourceFactory()
                .addCustomSource((nodeIndex) -> true, eventSourceSupplier);
