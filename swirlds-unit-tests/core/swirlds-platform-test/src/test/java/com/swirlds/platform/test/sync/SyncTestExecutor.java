...
/**
 * This class executes a single sync between two {@link SyncNode} instances. It defines the high
 * level structure and order of operations common to each sync test. Custom logic is injected via
 * lambdas as necessary.
 */
public class SyncTestExecutor {
    ...
    public SyncTestExecutor(final SyncTestParams params) {
        ...
        factoryConfig = (f) -> {};
        callerExecutorSupplier = () -> new CachedPoolParallelExecutor("sync-node");
        listenerExecutorSupplier = () -> new CachedPoolParallelExecutor("sync-node");
        callerSupplier =
                (factory) ->
                        new SyncNode(
    ...
    /**
     * Creates and initializes the caller and listener nodes. At the completion of this method, the
     * caller and listener are ready to begin graph generation.
     */
    private void initialize() throws IOException {
        ...
        final EventEmitterFactory factory =
                new EventEmitterFactory(random, params.getNumNetworkNodes());
     ...
     *
     * <ol>
     *   <li>Initial Graph Creation
     *   <li>Graph Customization
     *   <li>Caller Graph Additions
     *   <li>Listener Graph Additions
     * </ol>
     *
    ...
    private BiConsumer<SyncNode, SyncNode> getDefaultGenerationDefinitions() {
        return (caller, listener) -> {
            ...
            when(caller.getConsensus().getMinGenerationNonAncient())
                    .thenReturn(callerMinNonAncientGen);
            ...
            when(listener.getConsensus().getMinGenerationNonAncient())
                    .thenReturn(listenerMinNonAncientGen);
        };
    }
    ...
    private void sync() throws Exception {
        ...
        System.out.printf(
                ...
                caller.getReceivedEvents().size() + listener.getReceivedEvents().size(),
    ...
    public void setListenerExecutorSupplier(
            final Supplier<ParallelExecutor> listenerExecutorSupplier) {
    ...
    /**
     * Sets a custom generation definition function that defines what generations the caller and
     * listener will use in the sync.
     *
     * @param generationDefinitions
     */
    public void setGenerationDefinitions(
            final BiConsumer<SyncNode, SyncNode> generationDefinitions) {
        this.generationDefinitions = generationDefinitions;
    }

    /**
     * Sets a custom supplier for the caller node.
     *
     * <p>This method should be used if the caller {@link SyncNode} requires custom initialization
     * such as a custom {@link ParallelExecutor} or an {@link EventEmitter} other than a {@link
     * ShuffledEventEmitter}.
     *
     * <p>Please note that is an {@link EventEmitter} other than {@link ShuffledEventEmitter} is
     * used to create this node, the {@link SyncTestExecutor#setInitialGraphCreation(BiConsumer)}
     * must be called with a compatible {@link BiConsumer}.
     *
     ...
     *
     * <p>This method should be used if the listener {@link SyncNode} requires custom initialization
     * such as a custom {@link ParallelExecutor} or an {@link EventEmitter} other than a {@link
     * ShuffledEventEmitter}.
     *
     * <p>Please note that is an {@link EventEmitter} other than {@link ShuffledEventEmitter} is
     ...
     * must be called with a compatible {@link BiConsumer} .
     */
    public void setListenerSupplier(
            final Function<EventEmitterFactory, SyncNode> listenerSupplier) {
        this.listenerSupplier = listenerSupplier;
    }

    /**
     * Defines the custom initialization function for creating the initial shadow graph for the
     * nodes.
     *
     * <p>Note that several validator functions require that {@link
     * SyncNode#setSaveGeneratedEvents(boolean)} be called with {@code true} during this step.
     *
    ...
    /**
     * Defines the custom initialization function to be performed after common initialization of the
     * caller and listener nodes but before graph creation.
     *
     * <p>For example, this could be used to turn on saving events for one or both of the node prior
     * to any event creation.
     *
    ...
    /**
     * Defines a graph customization function to be performed after initial graph creation, but
     * before {@link SyncTestParams#getNumCallerEvents()} and {@link
     * SyncTestParams#getNumListenerEvents()} events are added to the caller and listener shadow
     * graphs.
     *
     * <p>For example, this method could be used to setup the sync nodes to partition for the
     * remaining events.
     *
    ...
    /**
     * Defines the custom pre-sync configuration function to be performed after shadow graph
     * creation, but before sync.
     *
     * @param customPreSyncConfiguration
     */
    public void setCustomPreSyncConfiguration(
            final BiConsumer<SyncNode, SyncNode> customPreSyncConfiguration) {
        this.customPreSyncConfiguration = customPreSyncConfiguration;
    }

    /**
     * Defines a custom predicate that determines if caller events generated after the common events
     * should be added to the caller's shadow graph.
     *
    ...
    /**
     * Defines a custom predicate that determines if listener events generated after the common
     * events should be added to the caller's shadow graph.
     *
