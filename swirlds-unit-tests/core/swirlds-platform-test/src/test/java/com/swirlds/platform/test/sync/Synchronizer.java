...
public class Synchronizer {
    ...
    // The parallel executor used to execute the caller's and listener's synchronize at the same
    // time
    private final ParallelExecutor parallelExecutor;

    public Synchronizer() {
        parallelExecutor = new SyncPhaseParallelExecutor(null, null, false);
    }

    /**
     * Performs synchronization between the caller and listener nodes.
     *
     * <p>The {@link ShadowGraphSynchronizer#synchronize(Connection)} method is invoked on each node
     * in parallel using the {@link ParallelExecutor}.
     *
     * @throws Exception is there is any exception during connection setup ornode synchronization
     */
    public void synchronize(final SyncNode caller, final SyncNode listener) throws Exception {
        ...
        parallelExecutor.doParallel(
                () -> {
                    try {
                        caller.getSynchronizer().synchronize(caller.getConnection());
                ...
                () -> {
                    if (listener.isSendRecInitBytes()) {
                        // Read the COMM_SYNC_REQUEST byte on the listener prior to calling
                        // synchronize to match the
