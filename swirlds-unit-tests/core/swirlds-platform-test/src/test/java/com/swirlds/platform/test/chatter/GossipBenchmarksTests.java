...
class GossipBenchmarksTests {
    ...
    private static List<Long> getConnections(
            final GossipSimulationBuilder builder, final long source) {
     ...
     *
     * <p>Note that no node will have fewer than the requested number of connections, but some nodes
     * may have more. This happens if it is impossible to remove a connection from a node without
     * reducing another below the requested number.
     *
     * @param builder builds the simulation
     * @param desiredNumberOfConnections the desired number of connections per each node
     */
    private static void limitConnections(
            final Random random,
            final GossipSimulationBuilder builder,
            final int desiredNumberOfConnections) {
        ...
        for (long source = 0; source < builder.getNodeCount(); source++) {
            ...
            while (connectedNodes.size() > desiredNumberOfConnections && iterator.hasNext()) {
                ...
                final int otherNodeConnectionCount = getConnections(builder, destination).size();
