...
class EventTaskCreatorTest {
    ...
    void testCreateEvent() throws InterruptedException {
        ...
        assertEquals(
                ...
                "otherId should be same in the task as the one passed in");

        reset(eventQueueThread);

        // with mirror node enabled
        when(setting.isEnableBetaMirror()).thenReturn(true);
    ...
    void testAddEvent() throws InterruptedException {
        GossipEvent task =
                new GossipEvent(mock(BaseEventHashedData.class), mock(BaseEventUnhashedData.class));
    ...
    void testEventRescue() throws InterruptedException {
        ...
        when(eventMapper.getMostRecentEvent(eventToRescue.getCreatorId()))
                .thenReturn(eventToRescue);
    ...
    void testSyncDone_ShouldCreateEvent() throws InterruptedException {
        ...
        when(setting.isEnableBetaMirror()).thenReturn(false);
