...
/**
 * Each test specifies the input to feed into the negotiator and the output that is expected to be
 * returned. Each test is repeated, and they all run on the same negotiator. The purpose of this is
 * to test if the negotiator is returning to the initial state after the previous test, which it
 * should.
 */
class NegotiatorStateTransitionTests {
    ...
    void bothKeepalive()
            throws NetworkProtocolException, NegotiationException, IOException,
    ...
    @DisplayName(
            "Send a keepalive, expect the negotiator to initiate protocol 2, and run it when"
                    + " accepted")
    @RepeatedTest(REPETITIONS)
    ...
    void simInitiateRun()
        ...
        suite.getProtocol(protocolInitiated)
    ...
    @RepeatedTest(REPETITIONS)
    void simInitiateDontRun()
            throws NetworkProtocolException, NegotiationException, IOException,
                    InterruptedException {
        final int protocolInitiated = PROTOCOL_1;
        suite.getProtocol(protocolInitiated)
                .setShouldInitiate(true)
                .setAcceptOnSimultaneousInitiate(false);
        suite.input(protocolInitiated);
        suite.expectedOutput(protocolInitiated);
        suite.execute();
        suite.assertProtocolRuns(0, 0, 0);
    ...
    @DisplayName(
            "Two different protocols are initiated simultaneously, test initiated a lower priority"
                    + " protocol")
    @RepeatedTest(REPETITIONS)
    ...
    @DisplayName(
            "Two different protocols are initiated simultaneously,  test initiated a higher"
                    + " priority protocol")
    @RepeatedTest(REPETITIONS)
