...
public class SystemTransactionTracker implements SystemTransactionHandler, Failable {
    ...
    public synchronized void handlePreConsensusSystemTransactions(final EventImpl event) {
        ...
        for (final Iterator<SystemTransaction> iter = event.systemTransactionIterator();
                iter.hasNext(); ) {
        ...
        for (final Iterator<ConsensusEvent> eventIt = round.eventIterator(); eventIt.hasNext(); ) {
            ...
            for (final Iterator<SystemTransaction> iter = event.systemTransactionIterator();
                ...
                if (!consensusTransactions.add(trans)) {
                    addFailure(
                            String.format(
                                    "encountered duplicate consensus system transaction in round"
                                        + " %s, event %s via handleSystemTransactions(EventImpl)",
