...
class NegotiatorPair {
    ...
    public NegotiatorPair(
            final TestProtocol protocol, final Pair<Connection, Connection> connections) {
    ...
    public void assertTimesRan(final int times) {
        Assertions.assertEquals(
                times,
                n1.getProtocol().getTimesRan(),
                String.format("protocol 1 was expected to run %d times", times));
        Assertions.assertEquals(
                times,
                n2.getProtocol().getTimesRan(),
                String.format("protocol 2 was expected to run %d times", times));
    }

    public void assertHandshakeRan() {
        Assertions.assertEquals(
                1, n1.getHandshakeRunNumber(), "handshake is expected to run exactly once");
        Assertions.assertEquals(
                1, n2.getHandshakeRunNumber(), "handshake is expected to run exactly once");
    }
