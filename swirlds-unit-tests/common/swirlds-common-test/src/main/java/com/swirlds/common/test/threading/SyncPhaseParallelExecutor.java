...
public class SyncPhaseParallelExecutor implements ParallelExecutor {
    ...
    public <T> T doParallel(
            final Callable<T> task1, final Callable<Void> task2, final Runnable onThrow)
            throws ParallelExecutionException {
        return doParallel(task1, task2);
    }

    /**
     * Execute two tasks in parallel, then executes a {@link Runnable} after {@link
     * SyncPhaseParallelExecutor#phase}.
     *
     * <p>This method assumes that this instance of {@link SyncPhaseParallelExecutor} is used for
     * both the caller and the listener. The caller and listener threads can enter the {@link
     * SyncPhaseParallelExecutor#doParallel(Callable, Callable)} method at the same time, each with
     * their 2 tasks, resulting in 4 tasks executing at once.
     *
    ...
    public <T> T doParallel(final Callable<T> task1, final Callable<Void> task2)
        ...
        // Both task1 and task2 are complete. However, either the caller's 2 tasks or the listener's
        // 2 tasks will
        // complete first. The first of the two (caller or listener) must wait for the other to
        // complete so that the
        // runnable to execute after a certain phase executes before the first thread that completes
        // moves on to the
        // next phase.
        ...
        try {
            ...
            synchronized (threadStatus) {
                if (waitForSecondThread
                        && threadStatus.get() == ThreadStatus.WAITING_FOR_FIRST_THREAD) {
                    ...
                    // once the second thread wakes me up, we're done, so we reset the status to the
                    // initial value
