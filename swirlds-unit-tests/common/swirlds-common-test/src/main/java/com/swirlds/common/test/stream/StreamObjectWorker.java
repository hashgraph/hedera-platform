...
public class StreamObjectWorker {
	...
	public StreamObjectWorker(int totalNum, int intervalMs,
			String dirPath, int logPeriodMs, Hash initialHash,
			boolean startWriteAtCompleteWindow,
			Instant firstTimestamp,
			Signer signer) throws NoSuchAlgorithmException {

		// writes objects to files
		TimestampStreamFileWriter<ObjectForTestStream> fileWriter = new TimestampStreamFileWriter<>(
				dirPath,
				logPeriodMs,
				signer,
	...
	public StreamObjectWorker(int totalNum, int intervalMs,
			Hash initialHash,
			Instant firstTimestamp,
			SerializableDataOutputStream stream) throws IOException {
	...
	private void initialize(int totalNum, int intervalMs, Hash initialHash,
			Instant firstTimestamp) {
		...
		HashCalculatorForStream<ObjectForTestStream> hashCalculator = new HashCalculatorForStream<>(
				runningHashCalculator, cryptography);
