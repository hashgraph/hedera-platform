...
public class NoOpMetrics implements Metrics {
    ...
    private final Map<String /* category */, Map<String /* name */, Metric>> metrics =
            new HashMap<>();
    ...
    public synchronized Collection<Metric> getAll() {
        // Not very efficient, but the no-op metrics doesn't do snapshotting, so this should rarely
        // (if ever) be called.
    ...
    public synchronized <T extends Metric> T getOrCreate(final MetricConfig<T, ?> config) {
        ...
        final Map<String, Metric> metricsInCategory =
                metrics.computeIfAbsent(category, k -> new HashMap<>());
