 ...
 *
 * @param <S> the type of the merkle node state
 * @param <M> the type of the benchmark metadata
 */
public class Benchmark<S extends MerkleNode, M extends BenchmarkMetadata> {
     ...
     *
     * @param initialState the initial merkle state
     * @param metadata the metadata object
     * @param mutableStateOperations a list of operations to perform on the mutable state
     * @param immutableStateOperations a list of operations to perform on the immutable state
     * @param immutableThreadCount the number of threads running immutable operations
     * @param roundPeriod the average time between copies of the state
     * @param statesInMemory the number of state copies to keep in memory (excluding mutable copy)
     * @param spinUpTime the amount of time to wait before starting to collect statistics
     * @param testDuration the amount of time to run the test (after spinUpTime)
     */
    Benchmark(
    ...
    public void printSettings() {
        ...
        System.out.println(
                "States in memory: " + statesInMemory + " immutable states + 1 mutable state");
    ...
    public Collection<BenchmarkStatistic> run() throws InterruptedException {
        ...
        final Thread mutableThread =
                new Thread(
                        buildOperationRunnable(
                                copyOperationList(mutableStateOperations), mutableStateManager));
        final List<Thread> immutableThreads = new LinkedList<>();
        for (int i = 0; i < immutableThreadCount; i++) {
            final Thread immutableThread =
                    new Thread(
                            buildOperationRunnable(
                                    copyOperationList(immutableStateOperations),
        ...
        System.out.println(
                "Waiting for "
                        + spinUpTime.toSeconds()
                        + " seconds to allow the system to reach a steady state");
    ...
    private List<BenchmarkOperation<S, M>> copyOperationList(
            final List<BenchmarkOperation<S, M>> operations) {
    ...
    private void registerStatisticForOperation(final BenchmarkOperation<S, M> operation) {
        if (statistics.containsKey(operation.getName())) {
            throw new IllegalArgumentException(
                    "Operation names must be unique, but \""
                            + operation.getName()
                            + "\" has already been used");
        }
        statistics.put(
                operation.getName(), new BenchmarkStatistic(operation.getName(), testDuration));
    }
    ...
    private BenchmarkStatistic getStatisticsForOperation(final BenchmarkOperation<S, M> operation) {
        if (!statistics.containsKey(operation.getName())) {
            throw new IllegalArgumentException(
                    "No statistics registered for operation " + operation.getName());
        }
     ...
     *
     * @param operations a list of possible operations
     * @param totalWeight the sum of the weights of all operations in the list
     */
    ...
    private void runWithStatistics(
            final RunnableWithException operation, BenchmarkStatistic statistic) {
        try {
    ...
    private void copyThreadRunnable() {
        while (alive) {
            ...
            S originalState;
            try (final AutoCloseableWrapper<S> originalStateWrapper =
                    mutableStateManager.getState()) {
                originalState = originalStateWrapper.get();
                runWithStatistics(
                        () -> mutableStateManager.setState((S) originalState.copy()),
                        copyStatistics);
            }
    ...
    private Runnable buildOperationRunnable(
        ...
        return () -> {
            while (alive) {
                final BenchmarkOperation<S, M> operation =
                        chooseRandomOperation(operations, operationsWeight);
