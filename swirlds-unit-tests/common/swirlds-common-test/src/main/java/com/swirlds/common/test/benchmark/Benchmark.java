...
public class Benchmark<S extends MerkleNode, M extends BenchmarkMetadata> {
	...
	public void printSettings() {
		...
		System.out.println("Immutable operations (" + immutableThreadCount +
				" thread" + (immutableThreadCount == 1 ? "" : "s") + ")");
		for (final BenchmarkOperation<S, M> operation : immutableStateOperations) {
			System.out.println("    - " + operation);
		}
	...
	public Collection<BenchmarkStatistic> run() throws InterruptedException {
		...
		final Thread mutableThread = new Thread(
				buildOperationRunnable(copyOperationList(mutableStateOperations), mutableStateManager));
		...
		System.out.println("Waiting for " + spinUpTime.toSeconds()
	...
	private void registerStatisticForOperation(final BenchmarkOperation<S, M> operation) {
		if (statistics.containsKey(operation.getName())) {
			throw new IllegalArgumentException("Operation names must be unique, but \"" + operation.getName()
	...
	private void hashingThreadRunnable() {
		try {
			while (alive) {
				...
				runWithStatistics(() -> {
					final Future<Hash> future = CryptoFactory.getInstance().digestTreeAsync(stateToHash);
					future.get();
				}, hashStatistics);
			}
	...
	private Runnable buildOperationRunnable(
			final List<BenchmarkOperation<S, M>> operations,
			final StateManager<S> stateManager) {
