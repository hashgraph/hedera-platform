...
/**
 * Similar to a {@link RandomAccessHashSet} but with hotspots (i.e. elements that are chosen more
 * oftan than others).
 *
 * @param <T> the type of the object held by the set
 */
public class HotspotHashSet<T> implements RandomAccessSet<T> {
     ...
     *
     * @param defaultWeight the weight that determines the frequency that elements that are not in a
     *     hotspot are returned If the set contains 1,000 elements and has a single hotspot with 100
     *     elements then 900 elements will be in the default set. If the default weight is 1.0 and a
     *     hotspot of 2.0, then elements from the hotspot will be returned twice as often as
     *     elements in the default set.
     * @param hotspots an array of zero or more hotspot configurations
     */
    ...
    private void addSetToArray(
            final Object[] array, final int startIndex, final RandomAccessHashSet<T> set) {
