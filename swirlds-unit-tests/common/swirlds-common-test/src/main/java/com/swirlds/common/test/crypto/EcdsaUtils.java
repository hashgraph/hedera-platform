...
public final class EcdsaUtils {
    ...
    public static final String WELL_KNOWN_DIGEST =
            "744c77a7af70b3a522009f0a963384eccfa77662a594d6e0247dfba095eb48d5";
    ...
    public static KeyPair genEcdsaSecp256k1KeyPair()
        ...
        final KeyPairGenerator generator =
                KeyPairGenerator.getInstance(
                        ECDSA_SECP256K1.keyAlgorithm(), ECDSA_SECP256K1.provider());
        generator.initialize(ecSpec, new SecureRandom());
        return generator.generateKeyPair();
    }

    /**
     * Signs the well-known digest with the given ECDSA(secp256k1) private key and returns the raw
     * signature; that is, the 64-byte concatenation of the signature's {@code r} value followed by
     * its {@code s} value as unsigned big-endians.
     *
    ...
    public static byte[] signWellKnownDigestWithEcdsaSecp256k1(final PrivateKey privateKey) {
        return signDigestWithEcdsaSecp256k1(
                privateKey, WELL_KNOWN_DIGEST.getBytes(StandardCharsets.UTF_8));
    }

    /**
     * Signs the given digest with the given ECDSA(secp256k1) private key and returns the raw
     * signature; that is, the 64-byte concatenation of the signature's {@code r} value followed by
     * its {@code s} value in unsigned big-endian form.
     *
     * @param privateKey the private key to use
     * @return the raw signature of the given digest
     */
    public static byte[] signDigestWithEcdsaSecp256k1(
            final PrivateKey privateKey, final byte[] digest) {
        try {
     ...
     *
     * @param cryptoPubKey the public key to encode
     ...
     *
     * @param derSig the ASN.1 DER-encoded signature
    ...
    private static byte[] rawEcdsaSigFromAsn1Der(final byte[] derSig) {
        ...
        System.arraycopy(
                ...
                rStart,
                rawBytes,
                BIG_ENDIAN_SIZE - finalRLen,
                Math.min(BIG_ENDIAN_SIZE, origRLen));
