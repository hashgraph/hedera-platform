...
public final class EcdsaUtils {
	 ...
	 */
	public static KeyPair genEcdsaSecp256k1KeyPair() throws NoSuchAlgorithmException,
			InvalidAlgorithmParameterException, NoSuchProviderException {
		final ECGenParameterSpec ecSpec = new ECGenParameterSpec(ECDSA_SECP256K1.ellipticalCurve());
		final KeyPairGenerator generator = KeyPairGenerator.getInstance(ECDSA_SECP256K1.keyAlgorithm(),
				ECDSA_SECP256K1.provider());
	...
	public static byte[] signDigestWithEcdsaSecp256k1(final PrivateKey privateKey, final byte[] digest) {
		...
		} catch (final NoSuchAlgorithmException | InvalidKeyException | SignatureException | NoSuchProviderException fatal) {
	...
	private static byte[] rawEcdsaSigFromAsn1Der(final byte[] derSig) {
		...
		System.arraycopy(derSig, rStart, rawBytes,
				BIG_ENDIAN_SIZE - finalRLen, Math.min(BIG_ENDIAN_SIZE, origRLen));
		...
		System.arraycopy(derSig, sStart, rawBytes, BIG_ENDIAN_SIZE + (BIG_ENDIAN_SIZE - finalSLen),
