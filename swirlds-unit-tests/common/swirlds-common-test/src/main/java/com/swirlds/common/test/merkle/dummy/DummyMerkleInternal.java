...
public class DummyMerkleInternal extends PartialNaryMerkleInternal
        implements DummyMerkleNode, MerkleInternal {
    ...
    private static BiFunction<DummyMerkleInternal, Integer, MerkleNode> migrationMapper =
            (node, version) -> node;
     ...
     *
     * @param migrationMapper a method that is used to migrate a dummy merkle internal
     */
    public static void setMigrationMapper(
            final BiFunction<DummyMerkleInternal, Integer, MerkleNode> migrationMapper) {
            ...
            if (child instanceof DummyMerkleNode && child.isInternal()) {
                ...
                if (!internalChild.isInitialized()) {
                    throw new RuntimeException(
                            "Children must be initialized before their parents.");
                }
            }
        }
    ...
    /**
     * When this node migrates, it will always replace itself with a DummyMerkleInternal containing
     * a single leaf in position 0, and that leaf will equal this node.
     */
    @Override
