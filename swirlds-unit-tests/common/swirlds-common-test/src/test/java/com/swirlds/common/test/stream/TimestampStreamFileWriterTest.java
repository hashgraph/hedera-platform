...
class TimestampStreamFileWriterTest {
     ...
     *
     * @param windowSizeMs the window size
     * @param startAtCompleteWindow true if only complete files should be written
     * @param objectsToAdd a sequence of objects to add
     */
    ...
    void shouldStartNewFileCompleteWindowTest() {
        ...
        verifyNewFileSequence(
                ...
                new NextStreamObject(
                        new ObjectForTestStream(
                                id++, firstPeriodStart.plusMillis(windowSizeMs / 2)),
                        false,
    ...
    void shouldStartNewFileAlignedCompleteWindowTest() {
        ...
        verifyNewFileSequence(
                ...
                new NextStreamObject(
                        new ObjectForTestStream(
                                id++, firstPeriodStart.plusMillis(windowSizeMs / 2), ++alignment),
                        false,
                        "same period but different alignment, should not start first file"),
                new NextStreamObject(
                        new ObjectForTestStream(
                                id++, firstPeriodStart.plusMillis(windowSizeMs), alignment),
                        false,
                        "new period but same alignment, should not start first file"),
                new NextStreamObject(
                        new ObjectForTestStream(
                                id++, firstPeriodStart.plusMillis(windowSizeMs + 1), ++alignment),
                        true,
                ...
                new NextStreamObject(
                        new ObjectForTestStream(
                                id++, firstPeriodStart.plusMillis(2 * windowSizeMs), alignment),
                        false,
                        "new period but same alignment, should not start new file"),
                new NextStreamObject(
                        new ObjectForTestStream(
                                id, firstPeriodStart.plusMillis(3 * windowSizeMs), ++alignment),
                        true,
    ...
    /**
     * @param object an object that will be added to the stream
     * @param shouldStartNewFile if this object should cause the creation of a new file
     * @param errorMessage a message to be printed if file creation does not follow expectations
     */
    private record NextStreamObject(
            ObjectForTestStream object, boolean shouldStartNewFile, String errorMessage) {}
}
