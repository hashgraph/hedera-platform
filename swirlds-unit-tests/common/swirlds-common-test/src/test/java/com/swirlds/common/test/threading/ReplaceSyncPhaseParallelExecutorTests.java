...
class ReplaceSyncPhaseParallelExecutorTests {
	...
	void testReplacePhase(final int phaseNum, final int taskNum) throws ParallelExecutionException {
		...
		final ReplaceSyncPhaseParallelExecutor executor = new ReplaceSyncPhaseParallelExecutor(phaseNum, taskNum,
		...
		for (final PhaseTask phaseTask : PhaseTask.values()) {
			if (phaseTask.matches(phaseNum, taskNum)) {
				assertEquals(0, phaseTask.getTask().get(),
						format("Phase %s, Task %s should have been replaced with the replacement task.", phaseNum,
								taskNum));
			} else {
				assertEquals(1, phaseTask.getTask().get(),
						format("Phase %s, Task %s should not have been replaced with the replacement task.", phaseNum,
								taskNum));
			}
		}
