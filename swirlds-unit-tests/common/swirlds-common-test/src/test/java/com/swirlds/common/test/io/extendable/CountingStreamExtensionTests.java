/*
 * Copyright (C) 2016-2020 Hedera Hashgraph, LLC
 *
    ...
    void inputStreamSanityTest() throws IOException {
        StreamSanityChecks.inputStreamSanityCheck(
                (final InputStream base) ->
                        new ExtendableInputStream(base, new CountingStreamExtension()));

        StreamSanityChecks.inputStreamSanityCheck(
                (final InputStream base) ->
                        new ExtendableInputStream(base, new CountingStreamExtension(false)));
    }
    ...
    void outputStreamSanityTest() throws IOException {
        StreamSanityChecks.outputStreamSanityCheck(
                (final OutputStream base) ->
                        new ExtendableOutputStream(base, new CountingStreamExtension()));

        StreamSanityChecks.outputStreamSanityCheck(
                (final OutputStream base) ->
                        new ExtendableOutputStream(base, new CountingStreamExtension(false)));
    }
