...
class ThrottleStreamExtensionTests {
	...
	void inputStreamSanityTest() throws IOException {
		StreamSanityChecks.inputStreamSanityCheck((final InputStream base) ->
				new ExtendableInputStream(base,
						new ThrottleStreamExtension(1024 * 1024)));
	}
	...
	void inputStreamTest(final int timeIncrementMs) throws IOException {
		...
		final InputStream in = extendInputStream(byteIn,
				new ThrottleStreamExtension(bytesPerSecond, Duration.ofMillis(timeIncrementMs)), counter);
		...
		assertTrue(ratio > 0.8 && ratio < 1.2,
				"read at " + ratio + " times the expected rate");
	}
	...
	void outputStreamTest(final int timeIncrementMs) throws IOException {
		...
		final OutputStream out = extendOutputStream(byteOut,
				new ThrottleStreamExtension(bytesPerSecond, Duration.ofMillis(timeIncrementMs)), counter);
