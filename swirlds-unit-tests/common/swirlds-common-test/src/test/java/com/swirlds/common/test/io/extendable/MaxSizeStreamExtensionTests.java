...
class MaxSizeStreamExtensionTests {
        ...
        StreamSanityChecks.inputStreamSanityCheck(
                (final InputStream base) ->
                        new ExtendableInputStream(
                                base, new MaxSizeStreamExtension(Long.MAX_VALUE)));

        StreamSanityChecks.inputStreamSanityCheck(
                (final InputStream base) ->
                        new ExtendableInputStream(
                                base, new MaxSizeStreamExtension(Long.MAX_VALUE, false)));
    }
    ...
    void inputStreamTest() throws IOException {
        ...
        extension.reset();
        assertThrows(
                IOException.class, () -> in.read(new byte[1025], 0, 1025), "too many bytes read");
        ...
        assertThrows(
                IOException.class,
                () -> in.readNBytes(new byte[1025], 0, 1025),
                "too many bytes read");
    ...
    void outputStreamTest() throws IOException {
        ...
        assertThrows(
                IOException.class,
                () -> out.write(new byte[1025], 0, 1025),
                "too many bytes written");
