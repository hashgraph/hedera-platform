...
/**
 * Contains static methods for doing sanity checks on streams. Ensures that streams conform to basic
 * expected behavior.
 */
public final class StreamSanityChecks {
     ...
     * @param expectedBytes the bytes that are expected to come out of the stream
     * @param index the index of the next byte to be read
     * @return the new index
     */
    private static int testRead(final InputStream in, final byte[] expectedBytes, final int index)
            throws IOException {
    ...
    private static int testReadIntoArray(
            final Random random, final InputStream in, final byte[] expectedBytes, final int index)
            throws IOException {
        ...
        if (bytesActuallyRead == -1) {
            ...
            assertEquals(
                    expectedBytesRead,
                    bytesActuallyRead,
                    "expected number of bytes should have been read");
     ...
     *
     * @param streamBuilder a method that takes an output stream and wraps it in another stream
     */
    public static void inputStreamSanityCheck(
            final Function<InputStream, InputStream> streamBuilder) throws IOException {
        ...
        for (int iteration = 0; iteration < iterations; iteration++) {
            ...
            // Until there is no data remaining, randomly read from the stream using the different
            // methods available
            ...
            assertEquals(
                    0, in.readNBytes(new byte[10], 0, 10), "stream should have no bytes remaining");

            in.close();
        }
    }

    public static void outputStreamSanityCheck(
            final Function<OutputStream, OutputStream> streamBuilder) throws IOException {
        ...
        for (int iteration = 0; iteration < iterations; iteration++) {
            ...
            // Until there is no data remaining, randomly write into using the different methods
            // available
            while (index < dataSize) {
                if (random.nextBoolean()) {
                ...
                if (random.nextBoolean()) {
                    final int bytesToWrite =
                            Math.min(
                                    dataSize - index,
                                    (int) (random.nextDouble() * (dataSize / 10)));
