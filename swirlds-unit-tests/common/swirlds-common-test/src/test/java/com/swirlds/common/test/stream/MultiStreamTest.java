...
class MultiStreamTest {
	...
	void addNullNextStreamsTest() {
		...
		assertTrue(exception.getMessage().contains(NOT_ENOUGH_NEXT_STREAMS),
	...
	void addNullNextStreamTest() {
		...
		assertTrue(exception.getMessage().contains(NEXT_STREAM_NULL),
				"nextStream should not be null");
	}
}
