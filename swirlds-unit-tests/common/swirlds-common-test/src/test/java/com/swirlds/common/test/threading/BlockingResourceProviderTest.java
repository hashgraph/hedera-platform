...
class BlockingResourceProviderTest {
     ...
     *
     * <ul>
     *   <li>Starts up a number of providers and a single consumer
     *   <li>The consumer waits for a resource and consumes it until the number of expected
     *       resources is met
     *   <li>The providers fight over the permit and sometimes a resource and sometimes release the
     *       permit
     * </ul>
     *
     ...
     *
     * <ul>
     *   <li>Before we provide a resource, we check if the previous one has already been consumed
     *   <li>At the end, we check that each resource has been consumed exactly once
     *   <li>That no exceptions have been thrown
     *   <li>That all threads finished gracefully
     * </ul>
     */
    @Test
    @Tag(TIME_CONSUMING)
    void testProvidersTakeTurns() throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < numThreads; i++) {
            concurrentTesting.addRunnable(
                    new Provider(provider, lastProvided, consumed, i, numResources));
        }
        concurrentTesting.addRunnable(
                () -> {
                    int numConsumed = 0;
        ...
        Assertions.assertEquals(
                numThreads * numResources,
                consumed.size(),
                "each resource was supposed to be consumed exactly once");
    }
            ...
            while (seq < numResources) {
                ...
                if (last != null && !consumed.containsKey(last)) {
                    throw new RuntimeException(
                            String.format(
                                    "last (%s) resource provided has not been consumed. current:"
                                            + " %s",
                                    last, resource));
                }
