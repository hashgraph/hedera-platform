...
class StatsBufferTest {
    ...
    void recordOneValueTest() {
        ...
        assertEquals(
                1,
                statsBuffer.numBins(),
                "numBins should be 1 after recording a value when current time is not in the first"
                        + " half life");

        assertEquals(
                value,
                statsBuffer.yMax(),
                "when only one value is recorded, yMax should be equal to that value");
        assertEquals(
                statsBuffer.yMax(),
                statsBuffer.yMax(0),
        ...
        assertEquals(
                value,
                statsBuffer.yMin(),
                "when only one value is recorded, yMin should be equal to that value");
        assertEquals(
                statsBuffer.yMin(),
                statsBuffer.yMin(0),
        ...
        assertEquals(
                0, statsBuffer.yStd(0), "when only one value is recorded, yStd(0) should be 0");
        assertEquals(
                statsBuffer.yStd(0),
    ...
    void recordMultipleValueTest() {
        ...
        assertEquals(
                MAX_BINS / 2,
                statsBuffer.numBins(),
                "when numBins is MAX_BINS - 1, record one more value will make numBins be MAX_BINS"
                        + " / 2");

        assertEquals(
                maxValue, statsBuffer.yMax(), "yMax should be the maximum value recorded so far");

        assertEquals(
                minValue, statsBuffer.yMin(), "yMin should be the minimum value recorded so far");
    }
}
