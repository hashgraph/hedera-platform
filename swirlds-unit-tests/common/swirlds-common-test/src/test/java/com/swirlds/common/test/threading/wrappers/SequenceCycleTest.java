...
class SequenceCycleTest {
    ...
    void test() throws InterruptedException, ExecutionException, TimeoutException {
        ...
        final InterruptableConsumer<Void> consumer =
                v -> {
                    waiting.countDown();
                    consumerWait.await();
                };
        ...
        final Future<Void> consumerFuture =
                executor.submit((ThrowingRunnable) () -> cycle.accept(null));

        // wait until the consumer starts blocking
        Assertions.assertTrue(
                waiting.await(WAIT_TIME, WAIT_TIME_UNIT), "the consumer did not start the cycle");

        // once we are sure the consumer started the cycle, we start waiting for it to end
        final Future<Void> waitFuture =
                executor.submit((ThrowingRunnable) cycle::waitForCurrentSequenceEnd);
