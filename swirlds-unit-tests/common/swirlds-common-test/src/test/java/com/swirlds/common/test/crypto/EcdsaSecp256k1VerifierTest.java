...
class EcdsaSecp256k1VerifierTest {
    ...
    void encodesAsn1DerAsExpected(
            final String hexedR, final String hexedS, final String hexedDerSig) {
        ...
        assertArrayEquals(
                derSig, EcdsaSecp256k1Verifier.asn1DerEncode(rawSig), "derSigs should match");
    }
    ...
    void verifySignatureVerification(int count) throws Exception {
        for (int i = 0; i < count; i++) {
            ...
            final byte[] signature =
                    EcdsaUtils.signWellKnownDigestWithEcdsaSecp256k1(pair.getPrivate());
            final byte[] rawPubKey =
                    EcdsaUtils.asRawEcdsaSecp256k1Key((ECPublicKey) pair.getPublic());

            final boolean isValid =
                    subject.verify(
                            signature,
                            EcdsaUtils.WELL_KNOWN_DIGEST.getBytes(StandardCharsets.UTF_8),
                            rawPubKey);
