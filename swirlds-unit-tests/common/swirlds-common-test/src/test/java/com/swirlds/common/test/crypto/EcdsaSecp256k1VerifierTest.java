...
class EcdsaSecp256k1VerifierTest {
	...
	void minifiesPositiveBigEndianAsExpected(final String hexedUnsignedV, final String hexedAns) {
		...
		assertArrayEquals(ans, EcdsaSecp256k1Verifier.minifiedPositiveBigEndian(v),
	...
	void verifySignatureVerification(int count) throws Exception {
		for (int i = 0; i < count; i++) {
			...
			final boolean isValid = subject.verify(
					signature, EcdsaUtils.WELL_KNOWN_DIGEST.getBytes(StandardCharsets.UTF_8), rawPubKey);
