...
public class MapKey implements SelfSerializable, Comparable<MapKey>, FastCopyable {
    ...
    @JsonProperty private long shardId;

    @JsonProperty private long realmId;

    @JsonProperty private long accountId;

    private static final Comparator<MapKey> CANONICAL_ORDERING =
            Comparator.comparing(MapKey::getShardId)
    ...
    @Override
    public int hashCode() {
        return new HashCodeBuilder().append(shardId).append(realmId).append(accountId).toHashCode();
    }
    ...
    @Override
    public void deserialize(SerializableDataInputStream inputStream, int version)
            throws IOException {
