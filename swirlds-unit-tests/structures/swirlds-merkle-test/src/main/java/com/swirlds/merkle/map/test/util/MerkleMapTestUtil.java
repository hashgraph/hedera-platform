...
public final class MerkleMapTestUtil {
    ...
    private static final File LOG_PATH =
            canonicalFile(".", "src", "test", "resources", "log4j2-test.xml");

    /**
     * Given a sequence of directory and file names, such as {".","sdk","test","..","config.txt"},
     * convert it to a File in canonical form, such as /full/path/sdk/config.txt by assuming it
     * starts in the current working directory (which is the same as
     ...
     * /full/path/sdk/data/config.txt by assuming it starts in the current working directory (which
     * is the same as System.getProperty("user.dir")).
     *
     * @param names the sequence of names
    ...
    public static void insertIntoMap(
            final int startIndex, final int endIndex, final Map<Key, Value> fcm) {
