...
public class TransactionRecord extends AbstractSerializableHashable
        implements FastCopyable, Serializable {
    ...
    public TransactionRecord(
            final long index, final long balance, final byte[] content, final long expirationTime) {
        this.index = index;
    ...
    private TransactionRecord(final TransactionRecord sourceTransaction) {
        ...
        if (sourceTransaction.content != null) {
            this.content =
                    Arrays.copyOf(sourceTransaction.content, sourceTransaction.content.length);
        }
