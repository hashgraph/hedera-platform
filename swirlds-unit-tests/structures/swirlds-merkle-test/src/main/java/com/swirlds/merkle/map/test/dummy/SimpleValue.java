...
public class SimpleValue extends PartialMerkleLeaf implements MerkleLeaf {
    ...
    SimpleValue(
            final long balance, final long sendThresholdValue, final long receiveThresholdValue) {
    ...
    public static SimpleValue deserialize(final DataInputStream inputStream) throws IOException {
        ...
        if (valueVersion != VALUE_VERSION) {
            throw new IOException(
                    String.format(
                            "Invalid value version. Expected %d. Received %d",
                            EXTENDED_VALUE_TYPE, valueVersion));
        }
    ...
    protected static SimpleValue deserializeValue(final DataInputStream inputStream)
            throws IOException {
