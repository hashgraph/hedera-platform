...
public class SaveExpectedMapHandler {
    private static final Logger log = LogManager.getLogger(SaveExpectedMapHandler.class);
     ...
     *
     * @param map ExpectedMap to be serialized
     * @param directory location where the file should be serialized
     * @param returnJsonString it should only be true for unit test; because for big regression test
     *     such as FCM1M, it would throw `NegativeArraySizeException` because StringBuilder's
     *     capacity overflow
    ...
    public static String serialize(
        ...
        log.info(MARKER, "Serializing ExpectedMap {}", jsonZipFileName);
        try (final FileOutputStream fos = new FileOutputStream(jsonZip);
            ...
            log.error(
                    MARKER,
                    String.format(
                            "Error occurred while serializing ExpectedMap: %s", jsonZipFileName),
                    e);
        }
    ...
    public static Map<MapKey, ExpectedValue> deserialize(final File sourceFile) {
        ...
        if (jsonFile == null) {
            log.error(MARKER, "No JSON file found in the zip file: {}", sourceFile);
            return newMap;
        }
        ...
        try (FileInputStream fileInputStream = new FileInputStream(jsonFile)) {
            newMap =
                    objectMapper.readValue(
                            fileInputStream, new TypeReference<Map<MapKey, ExpectedValue>>() {});
        } catch (IOException e) {
            log.error(MARKER, "Error occurred while reading: {}", sourceFile, e);
        }
    ...
    public static String createExpectedMapName(final long nodeId, final Instant consensusTime) {
        return String.format(
                JSON_FILE_NAME_TEMPLATE,
                nodeId,
                consensusTime.toEpochMilli(),
    ...
    private static File unzipExpectedMap(final File sourceFile) {
        try (ZipInputStream zis = new ZipInputStream(new FileInputStream(sourceFile))) {
            ...
            log.error(MARKER, "Error occurred while unzipping: {}", sourceFile, e);
        }
    ...
    private static File extractFile(
            final ZipInputStream zis, final File directory, final String fileName) {
        ...
        try (FileOutputStream fos = new FileOutputStream(newFile)) {
            ...
            log.error(MARKER, "Error occurred while extracting file", e);
        }
