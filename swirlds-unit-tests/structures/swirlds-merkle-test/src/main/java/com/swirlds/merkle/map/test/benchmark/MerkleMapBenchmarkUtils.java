...
public final class MerkleMapBenchmarkUtils {
     ...
     * @param dataSize the size of the auxiliary data for the account
     * @param benchmarkMetadata metadata for the benchmark
     * @param size the number of accounts to create
     * @param accountFactory an object which can create accounts of the proper type
     */
    public static <A extends BenchmarkAccount> MerkleMap<BenchmarkKey, A> generateInitialState(
        ...
        for (final BenchmarkKey key : benchmarkMetadata.getNodeFeeKeys()) {
            map.put(
                    key,
                    accountFactory.buildAccount(
                            random.nextLong(), randomByteArray(random, dataSize)));
        }
