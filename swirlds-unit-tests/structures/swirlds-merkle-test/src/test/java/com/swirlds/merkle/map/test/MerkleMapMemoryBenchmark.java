...
/**
 * This tool provides a set of benchmarks that can be used to profile the memory usage of a
 * MerkleMap. The results of these benchmarks are intended to be interpreted using jprofiler.
 */
...
class MerkleMapMemoryBenchmark {
    ...
    static void setUp() throws ConstructableRegistryException {
        ConstructableRegistry.registerConstructables("com.swirlds.MerkleMap");
        ConstructableRegistry.registerConstructables("com.swirlds.merkletree");
    ...
    private MerkleMap<SerializableLong, KeyedMerkleLong<SerializableLong>> buildMap(
            final Random random, final int size) throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < size; i++) {
            map.put(
                    new SerializableLong(random.nextInt()),
                    new KeyedMerkleLong<>(random.nextInt()));
        }
