...
class MerkleMapMemoryTests {
    ...
    /**
     * Creates the specified number of accounts and measures the initial, ending, and {@code N - 2}
     * maxMemSamples.
     *
     * @param numberOfAccounts the total of number accounts to create
     * @param numberOfCopies the total number of fast copies to make
     * @param maxKeepCopies the total number of fast copies to be retained in memory
     * @param maxMemSamples the total number of memory samples to take throughout the test
     */
    @ParameterizedTest
    ...
    void testCreationMemoryUsage(
            int numberOfAccounts, int numberOfCopies, int maxKeepCopies, int maxMemSamples) {
        buildAndExecute(null, 0, numberOfAccounts, numberOfCopies, maxKeepCopies, maxMemSamples);
    }

    /**
     * Creates the specified number of accounts, modifies the accounts starting at the specified
     * offset, and measures the initial, ending, and {@code N - 2} maxMemSamples.
     *
     * @param numberOfAccounts the total of number accounts to create
     * @param numberOfCopies the total number of fast copies to make
     * @param maxKeepCopies the total number of fast copies to be retained in memory
     * @param maxMemSamples the total number of memory samples to take throughout the test
     * @param opOffset the offset at which to begin executing the modifications relative to the
    ...
    private void buildAndExecute(
        ...
        assertEquals(
                ...
                copyQueue.size(),
                String.format(
                        "Failed to maintain the correct number of fast-copies, expected %d but had"
                                + " %d copies at the test conclusion.",
    ...
    private void reportEstimatedUsage() {
        ...
        System.out.printf(
                "Total Estimated MerkleMap Usage: %,d (%s)%n",
                deltaUsed, FileUtils.byteCountToDisplaySize(deltaUsed));
    }
}
