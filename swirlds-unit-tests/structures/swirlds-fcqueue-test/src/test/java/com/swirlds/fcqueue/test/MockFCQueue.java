...
/**
 * A thread-safe fast-copyable queue, each of whose elements is fast-copyable. Elements must always
 * be inserted at the tail and removed from the head. It is not allowed to insert nulls. This is
 * fast copyable. A fast copy of a queue can be either immutable or mutable. A mutable fast copy can
 * only be created from a mutable queue, which would then become immutable after creating this
 * mutable fast copy.
 *
 ...
 * takes O(n) time.
 *
 * <p>The MockFCQueue objects can be thought of as being organized into "queue groups". A fast copy
 * of a queue creates another queue in the same queue group. But instantiating a queue with "new"
 * and the constructor creates a new queue group.
 *
 * <p>All write operations are synchronized within a queue group. So it is possible to write to two
 * different queue groups at the same time, but writing to different queues in the same queue group
 * will be done serially. Reads via getters are also serialized within a thread group, but it is ok
 * for multiple iterators to be running in multiple threads at the same time within that thread
...
public class MockFCQueue<E extends FastCopyable & SerializableHashable> extends FCQueue<E> {
     ...
     *     added to this queue. This will happen if the fast-copyable object o has an IOException
    ...
    public boolean add(final E o) {
        synchronized (original) {
            ...
            if (HASH_ALGORITHM == FCQHashAlgorithm.SUM_HASH) {
                // This is treated as a "set", not "list", so changing the order does not change the
                // hash. The
                // hash is
                // the sum of the hashes of the elements, modulo 2^384.
                //
                // Note, for applications like Hedera, for the queue of receipts or queue of
                // records, each
                // element of
                // the queue will have a unique  timestamp, and they will always be sorted by those
                // timestamps. So
                // the
                // hash of the set is equivalent to the hash of a list. But if it is ever required
                // to have a hash
                // of a
                ...
                // if the queue contains {a,b,c,d}, where "a" is the head and "d" is the tail, then
                // define:
                //
                ...
                //    Which implies these:
                //
                //    hash64({a,b,c,d}) = hash64({a,b,c}) * 3 + d                   mod 2^^64
                // //add(d)
                //    hash64({b,c,d})   = hash64({a,b,c,d}) - a * 3^^3              mod 2^^64
                // //remove()
                //    deletes a
                //
                // so we add an element by multiplying by 3 and adding the new element's hash,
                // and we remove an element by subtracting that element times 3 to the power of the
                // resulting size.
                //
                // This is all easy to do for a 64-bit hash by keeping track of 3^^size modulo
                // 2^^64, and
                // multiplying
                // it by 3 every time the size increments, and multiplying by the inverse of 3 each
                // time it
                // decrements.
                // The multiplicative inverse of 3 modulo 2^^64 is 0xaaaaaaaaaaaaaaab (that's 15 a
                // digits then a b).
                //
                // It would be much slower to use modulo 2^^384, but we don't have to do that. We
                // can treat the
                // 48-byte hash as a sequence of 6 numbers, each of which is an unsigned 64 bit
                // integer.  We do this
                // rolling hash on each of the 6 numbers independently. Then it ends up being simple
                // and fast
                ...
                throw new UnsupportedOperationException(
                        "Hash algorithm " + HASH_ALGORITHM + " is not supported");
    ...
    public E remove() {
        synchronized (original) {
            ...
            oldHead.decRefCount(); // this will garbage collect the old head, if no copies point to
            // it
            size--;
