...
public final class ConfigReflectionUtils {
    ...
    /**
     * Returns the generic type of the class or throws an {@link IllegalArgumentException} if the
     * given class has not exactly one generic type
     *
     * @param parameterizedType the class
     * @return the generic type of the class
     */
    static Type getSingleGenericTypeArgument(final ParameterizedType parameterizedType) {
     ...
     */
    static boolean isPublic(final Class<?> type) {
        return Modifier.isPublic(type.getModifiers());
    }

    /**
     * Returns the config property name for a property of a config data object (see {@link
     * ConfigData})
     *
     ...
     */
    public static String getPropertyNameForConfigDataProperty(
            final String prefix, final RecordComponent component) {
        return Optional.ofNullable(component.getAnnotation(ConfigProperty.class))
                .map(
                        propertyAnnotation -> {
                            if (!propertyAnnotation.value().isBlank()) {
                                return getPropertyNameForConfigDataProperty(
                                        prefix, propertyAnnotation.value());
                            } else {
                                return getPropertyNameForConfigDataProperty(
     ...
     */
    static String getPropertyNameForConfigDataProperty(final String prefix, final String name) {
        if (prefix.isBlank()) {
            return name;
        }
        return prefix + "." + name;
     ...
     */
    public static <T extends Record> String getNamePrefixForConfigDataRecord(final Class<T> type) {
    ...
    public static <A extends Annotation, V>
        ...
        return configuration.getConfigDataTypes().stream()
                ...
                .map(
                        component ->
                                (AnnotatedProperty<A, V>)
                                        createData(
                                                constraintAnnotationType, configuration, component))
    ...
    private static <A extends Annotation, V> AnnotatedProperty<A, V> createData(
        ...
        try {
            ...
            final Class<? extends Record> recordType =
                    (Class<? extends Record>) component.getDeclaringRecord();
            ...
            final V propertyValue = (V) component.getAccessor().invoke(recordInstance);
            final String propertyName =
                    getPropertyNameForConfigDataProperty(propertyNamePrefix, component);
            final Class<V> propertyType = (Class<V>) component.getType();
            return new AnnotatedProperty<>(
                    annotation, component, propertyName, propertyValue, propertyType);
        } catch (final Exception e) {
            throw new IllegalArgumentException(
                    "Can not get the needed metadata for the given type", e);
        }
    }
