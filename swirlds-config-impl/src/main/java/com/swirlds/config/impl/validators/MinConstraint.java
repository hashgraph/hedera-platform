...
/**
 * An implementation of {@link ConfigPropertyConstraint} that will result in a violation if the
 * property value is lower than a defined minimum value.
 *
    ...
    public ConfigViolation check(final PropertyMetadata<T> metadata) {
        ...
        if (ConfigNumberUtils.compare(convertedValue, metadata.getValueType(), min) < 0) {
            final String message =
                    "Value of Property '" + metadata.getName() + "' must be >= '" + min + "'";
            return DefaultConfigViolation.of(metadata, message);
        } else {
            return null;
        }
