...
class ConverterService implements ConfigLifecycle {
    ...
    private static final ConfigConverter<BigDecimal> BIG_DECIMAL_CONVERTER =
            new BigDecimalConverter();

    private static final ConfigConverter<BigInteger> BIG_INTEGER_CONVERTER =
    ...
    private static final ConfigConverter<ZonedDateTime> ZONED_DATE_TIME_CONVERTER =
            new ZonedDateTimeConverter();

    private static final ConfigConverter<Duration> DURATION_CONVERTER = new DurationConverter();

    private static final ConfigConverter<ChronoUnit> CHRONO_UNIT_CONVERTER =
            new ChronoUnitConverter();

    ConverterService() {
        this.converters = new HashMap<>();
    }

    private <T, C extends ConfigConverter<T>> Class<T> getConverterType(
        ...
        return Arrays.stream(converterClass.getGenericInterfaces())
                ...
                .filter(
                        parameterizedType ->
                                Objects.equals(
                                        ConfigConverter.class, parameterizedType.getRawType()))
                        ...
                        typeArguments -> {
                            if (typeArguments.length != 1) {
                                throw new IllegalStateException(
                                        "Can not extract generic type for converter "
                                                + converterClass);
                            }
    ...
    <T> T convert(final String value, final Class<T> targetClass) {
        ...
        if (converter == null) {
            throw new IllegalArgumentException(
                    "No converter defined for type '" + targetClass + "'");
        }
        try {
    ...
    private <T> void add(final Class<T> converterType, final ConfigConverter<T> converter) {
        ...
        if (converters.containsKey(converterType)) {
            throw new IllegalStateException(
                    "Converter for type '" + converterType + "' already registered");
        }
