...
/**
 * This class is an implementation of the {@link ConfigurationBuilder} interface that is based on
 * the given config implementation.
 */
final class ConfigurationBuilderImpl implements ConfigurationBuilder {
    ...
    private <T extends Record> void addConfigDataType(final Class<T> type) {
        ...
        if (initialized.get()) {
            throw new IllegalStateException(
                    "ConfigDataType can not be added to initialized config");
        }
